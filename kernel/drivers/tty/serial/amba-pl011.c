...
#define uart_port_6085  4

//ttyAMA5, Uart5, for qsc6085 modem
#define UAP_PORT_NUMBER_SUSPEND_FLAG_ENABLE_GPIO_CONTROL  4 //for qsc6085
#define UART1_AUTOSUSPEND_DELAY_MS  500
...
#else
	.ifls			= UART011_IFLS_RX2_8|UART011_IFLS_TX4_8,
#endif
	.fifosize		= 64,
...
#define HI6620_DMA_SLAVE_BURST_SIZE 16
...
static void pl011_dma_probe_initcall(struct uart_amba_port *uap)
{
	...
	struct dma_slave_config tx_conf = {
		...
		.dst_maxburst = HI6620_DMA_SLAVE_BURST_SIZE,
	};
	struct dma_chan *chan;
	dma_cap_mask_t mask;
	...
	if (plat->dma_rx_param) {
		struct dma_slave_config rx_conf = {
			...
			.src_maxburst = HI6620_DMA_SLAVE_BURST_SIZE,
		};
...
static void pl011_dma_rx_irq(struct uart_amba_port *uap)
{
	...
	if (dmastat != DMA_PAUSED)
		dev_dbg(uap->port.dev, "unable to pause DMA transfer again.\n");
...
skip_rx:
	...
	dev_notice(uap->port.dev, "using dma. TX:%d RX:%d\n", uap->using_tx_dma, uap->using_rx_dma);
}
...
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
	struct amba_pl011_data *plat = port->dev->platform_data;
	unsigned int cr;
	int retval;


	dev_notice(port->dev, "pl011 startup.\n");
	...
	if (retval) {
		dev_err(port->dev,
			"clk_enable failed with error: %d\n", retval);
		goto out;
	}
	...
	if(!uap->no_rtim)
		...
		uap->im = UART011_OEIM | UART011_BEIM | UART011_PEIM | UART011_FEIM;
...
static void pl011_shutdown(struct uart_port *port)
{
    ...
    dev_notice(port->dev, "pl011_shutdown.\n");
...
#define B375_PORT_NAME "amba-uart.5"
...
#if 0 /* GPS can not work, modify it, need father study - dq  */
...
static int pl011_ioctl(struct uart_port *port, unsigned int cmd, unsigned long param)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;

	if (cmd == TIOCLOSERTIM) {
		if (param == 1) {
			dev_info(uap->port.dev, "set to NO-RTIM mode\n");
			uap->no_rtim = true;
			uap->im &= ~UART011_RTIM;
			writew(uap->im, uap->port.membase + UART011_IMSC);
		}
		else {
			dev_info(uap->port.dev, "exit NO-RTIM mode\n");
			uap->no_rtim = false;
		}
		return 0;
	}
...
static int pl011_probe(struct amba_device *dev, const struct amba_id *id)
{
 ...
 out:
 	/* FIXME: should disable pclk which enabled by amba bus */
	if (!ret)
...
static inline void amba_pl011_port_runtime_disable_oneshot(struct uart_amba_port *uap)
{
    if(uap->runtime_suspend_wq) {
        ...
        printk(KERN_ERR "amba_pl011 runtime disable: workqueue is NULL\n");
    }
}
...
static inline void amba_pl011_port_runtime_enable_oneshot(struct uart_amba_port *uap)
{
    if(uap->runtime_suspend_wq) {
        ...
        printk(KERN_ERR "pl011 runtime enable: workqueue is NULL\n");
    }
}
...
static struct amba_driver pl011_driver = {
	...
	.probe		= pl011_probe,
	.remove		= pl011_remove,
#ifndef CONFIG_ENABLE_UART_SLEEP_CONTROL
#ifdef CONFIG_PM
	.suspend	= pl011_suspend,
	.resume		= pl011_resume,
#endif
