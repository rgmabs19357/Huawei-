...
static pMEMNODE create_node(uint size, uint base, pMEMNODE pNode)
{
    ...
    /*add by daipeihua*/
    mntn_print_log(EN_ID_MNTN_BLVIDEO_DRV_MEMALLOC_MEMHEAP, EN_VIDEO_LOG_LEVLE_DEBUG,
                   "hisi_memheap: NewNode->m_BusAddr = 0x%x NewNode->m_Size = %u", base, size);
