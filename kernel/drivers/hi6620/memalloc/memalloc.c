			   ...
			   unsigned int cmd, unsigned long arg)
{
	...
	case MEMALLOC_IOCXGETBUFFER:{
			...
			if (get_cma_type()) {
				...
				cma_pages = dma_alloc_from_contiguous(NULL, (PAGE_ALIGN(memparams.size)>>PAGE_SHIFT), 0);
				ba = page_to_phys(cma_pages);
				//printk(KERN_ERR "[wuhuihui] memalloc: phys:0x%x:size:0x%x size_a:0x%x\n", ba, memparams.size, PAGE_ALIGN(memparams.size));
			} else {
	...
	case MEMALLOC_IOCSFREEBUFFER:{
			...
			if (get_cma_type()) {
				...
				dma_release_from_contiguous(NULL, phys_to_page(memparams.busAddress), (PAGE_ALIGN(memparams.size)>>PAGE_SHIFT));
				//printk(KERN_ERR "[wuhuihui] memalloc: phys:0x%x size:0x%x size_a:0x%x\n", memparams.busAddress, memparams.size, PAGE_ALIGN(memparams.size));
			} else {
...
int __init memalloc_init(void)
{
	...
	if (!get_cma_type()) {
    	...
    	/*add by daipeihua*/
    	printk(KERN_INFO "memalloc: hisi_reserved_codec_phymem = 0x%lx codec cfg_size = %ud",
