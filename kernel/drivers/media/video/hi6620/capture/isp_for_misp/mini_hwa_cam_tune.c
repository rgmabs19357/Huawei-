	...
	switch(v4l2_param->id)
	{
		case V4L2_CID_FLASH_STATUS:
		{
			if (mini_this_ispdata->is_using_mini_isp)
				v4l2_param->value = mini_this_ispdata->mini_isp_flash_flow;
			else
				v4l2_param->value = mini_this_ispdata->flash_flow;
			break;
		}
...
int mini_hwa_v4l2_ioctl_s_ctrls(struct file *file, void *fh, struct v4l2_control *v4l2_param)
{
	...
	switch(v4l2_param->id)
	{
		...
		case V4L2_CID_SET_OVER_EXPOSURE:
		{
			ret = mini_isp_set_over_exposure(v4l2_param->value);
			break;
		}
		case V4L2_CID_SET_SCENE_MODE:
		{
			ret = mini_isp_set_scene_mode(v4l2_param->value);
			break;
		}
		case V4L2_CID_SET_MOTION:
		{
			ret = mini_isp_set_motion(v4l2_param->value);
			break;
		}
		case V4L2_CID_SET_MULTIDENOISE:
		{
			ret = mini_isp_set_multidenoise(v4l2_param->value);
			break;
		}
		case V4L2_CID_SET_FAST_SNAPSHOT_MODE:
		{
			if(mini_this_ispdata->sensor->sensor_index== CAMERA_SENSOR_PRIMARY)
				ret = camera_agent_set_fast_shot_mode(mini_this_ispdata->sensor->sensor_index,v4l2_param->value);
			break;
		}
		case V4L2_CID_SET_CAPTURE_MODE:
		{
			if(mini_this_ispdata->sensor->sensor_index== CAMERA_SENSOR_PRIMARY)
				ret = camera_agent_set_capture_mode(mini_this_ispdata->sensor->sensor_index,v4l2_param->value);
			break;
		}
		default:
			ret = -1;
...
int mini_hwa_v4l2_ioctl_g_ext_ctrls(struct v4l2_ext_control **controls, __u32 cmd_id, __u32 cid_idx)
{
	...
	switch(cmd_id)
	{
		case V4L2_CID_GET_MULTI_ISP_REG:
		{
			...
			data_buf = kmalloc(sizeof(mini_hwq_multi_reg_data), GFP_KERNEL);
			if (NULL == data_buf) {
				ret = -ENOMEM;
				print_error("fail to kmalloc buffer");
				goto out;
			}
			seq_data = (mini_hwq_multi_reg_data *)data_buf;
			if(copy_from_user(seq_data, seq_data_temp, sizeof(mini_hwq_multi_reg_data))){
			...
			if(copy_to_user(seq_data_temp, data_buf, sizeof(mini_hwq_multi_reg_data))){
		...
		case V4L2_CID_GET_BRACKET_ISO_VALUE:
		{
			...
			data_buf = kmalloc(sizeof(int) * 3, GFP_KERNEL);
			if (NULL == data_buf) {
				ret = -ENOMEM;
				print_error("fail to kmalloc buffer");
				goto out;
			...
			if(CAMERA_USE_K3ISP == mini_this_ispdata->sensor->isp_location){
				ret = mini_hwa_get_bracket_iso((int*)data_buf);
			}
		...
		case V4L2_CID_GET_BRACKET_EXP_VALUE:
		{
			...
			if(CAMERA_USE_K3ISP == mini_this_ispdata->sensor->isp_location){
				ret = mini_hwa_get_bracket_exp((int*)data_buf);
			}
		...
		case V4L2_CID_GET_EXTRA_COFF:
		{
			...
			extra_data = (extra_coff *)data_buf;
		...
		case V4L2_CID_GET_HDR_MOVIE_SUPPORTED:
		{
			...
			if(CAMERA_USE_K3ISP == mini_this_ispdata->sensor->isp_location){
				if (mini_this_ispdata->sensor->sensor_hdr_movie.support_hdr_movie) {
					int temp = mini_this_ispdata->sensor->sensor_hdr_movie.support_hdr_movie();
					if(copy_to_user(support_hdr_movie, &temp, sizeof(int))){
						ret = -EINVAL;
						print_error("V4L2_CID_GET_HDR_MOVIE_SUPPORTED copy_to_user  failed");
						goto out;
					}
				}
			}
		...
		case V4L2_CID_GET_HDR_MOVIE_CONSTANT_PARAM:
		{
			...
			{
				ret = -EFAULT;
				print_error("NULL input pointer!");
				goto out;
			}
			data_buf = kmalloc(sizeof(hdr_ae_constant_param), GFP_KERNEL);
			if (NULL == data_buf) {
				ret = -ENOMEM;
				print_error("fail to allocate buffer for ext controls");
				goto out;
			}
			//ispv1_get_hdr_movie_ae_init_param((hdr_ae_constant_param *)data_buf);
			if(copy_to_user(result, data_buf, sizeof(hdr_ae_constant_param))){
				print_error("%s V4L2_CID_GET_HDR_MOVIE_CONSTANT_PARAM mem error!", __FUNCTION__);
			}
			break;
		}
		...
		case V4L2_CID_GET_HDR_MOVIE_VOLATILE_PARAM:
		{
			...
			data_buf = kmalloc(sizeof(hdr_ae_volatile_param), GFP_KERNEL);
			if (NULL == data_buf) {
				ret = -ENOMEM;
				print_error("fail to allocate buffer for ext controls");
				goto out;
			...
			//ispv1_get_hdr_movie_ae_running_param((hdr_ae_volatile_param *)data_buf);
			if(copy_to_user(result, data_buf, sizeof(hdr_ae_constant_param))){
				print_error("%s V4L2_CID_GET_HDR_MOVIE_VOLATILE_PARAM mem error!", __FUNCTION__);
			}
			break;
		}
		case  V4L2_CID_GET_HDR_STATS_INFO:
		{
			void __user *stats_data = ((*controls)[cid_idx]).string;
			if(stats_data != NULL)
			{
				data_buf = kmalloc(sizeof(stats_info), GFP_KERNEL);
				if (NULL == data_buf) {
					ret = -ENOMEM;
					print_error("fail to allocate buffer for ext controls");
					goto out;
				}
				((stats_info*)data_buf)->stats_h = mini_this_ispdata->sensor->sensor_hdr_movie.stats_data.stats_h;
				((stats_info*)data_buf)->stats_v = mini_this_ispdata->sensor->sensor_hdr_movie.stats_data.stats_v;;
				if(copy_to_user(stats_data, data_buf, sizeof(stats_info))){
					print_error("%s V4L2_CID_GET_HDR_STATS_INFO mem error!", __FUNCTION__);
				}
			}
			break;
		}
		case V4L2_CID_GET_AWB_OTP_INFO:
		{
			...
			data_buf = kmalloc(sizeof(mini_hw_awb_otp), GFP_KERNEL);
			if (NULL == data_buf) {
				ret = -ENOMEM;
				print_error("fail to allocate buffer for ext controls");
				goto out;
			...
			awb_otp = data_buf;
			if(AWB_OTP_SUPPORT == support_awb_otp){
			...
			if(copy_to_user((((*controls)[cid_idx]).string), awb_otp, sizeof(mini_hw_awb_otp))){
		...
		case V4L2_CID_GET_MINI_ISP_DEBUG_INFO:
		{
			...
			if(NULL == mini_this_ispdata->meta_data) {
...
int mini_hwa_v4l2_ioctl_s_ext_ctrls(struct v4l2_ext_control **controls, __u32 cmd_id, __u32 cid_idx)
{
	...
	switch(cmd_id)
	{
		...
		case V4L2_CID_SET_MINI_ISP_FACE_INFO:
		{
			...
			face_info->width = mini_this_ispdata->sensor->frmsize_list[mini_this_ispdata->sensor->preview_frmsize_index].width;
			face_info->height = mini_this_ispdata->sensor->frmsize_list[mini_this_ispdata->sensor->preview_frmsize_index].height;
...
int mini_hwa_get_bracket_iso(int *iso)
{
	...
	for (index=0; index<MAX_BRACKET_COUNT; index++) {
		temp_iso = ispdata->hdr_iso[index];
		temp_iso = (temp_iso!=0) ? temp_iso : 100;
		iso[index] = temp_iso;
	}

	print_info("%s iso [%d] [%d] [%d]",__func__,iso[0],iso[1],iso[2]);
...
int mini_hwa_get_bracket_exp(int *exp)
{
	...
	mini_k3_isp_data *ispdata = mini_this_ispdata;
	int index = 0;
	int denominator_expo_time = 1000, temp_expo = 0;

	for (index=0; index<MAX_BRACKET_COUNT; index++) {
		temp_expo = ispdata->hdr_expo[index];
		temp_expo = (temp_expo!=0) ? temp_expo : 10000;
		denominator_expo_time = (1000000/temp_expo);
		exp[index] = denominator_expo_time;
	}

	print_info("%s exp [%d] [%d] [%d]",__func__,exp[0],exp[1],exp[2]);
...
hwa_hwscope_ctl mini_hwa_hwscope;
...
int mini_hwa_set_hwscope(void *data)
{
	...
	else if(hwscope_data->mode == HW_SCOPE_OFF)
	{
		...
		mini_hwa_hwscope.mode = HW_SCOPE_OFF;
	}
	else
	{
		print_error("%s mode=%d error", __func__, hwscope_data->mode);
	}
	mini_hwa_hwscope.border = hwscope_data->border;
...
void mini_hwa_calc_zoom_with_crop(camera_state state, scale_strategy_t scale_strategy, u32 *in_width, u32 *in_height)
{
	u32 scale_width;
	u32 scale_height;
	u32 temp_width;
	...
	out_width = attr->out_width * 10 / (mini_this_ispdata->zoom + 10) + mini_hwa_hwscope.border * 2;
	out_width = (out_width + 16) & ~31;
	out_height = attr->out_height * 10 / (mini_this_ispdata->zoom + 10) + mini_hwa_hwscope.border * 2;
	/*
	ov Kevin:
	Format NV12 or NV21
		a = ceil (width/256)
...
{
	bool ret = false;
	mini_isp_meta_data *temp = NULL;
	mini_aec_data_t ae_data;
	u32 gain;
	u32 target_y;
	if((false == mini_this_ispdata->is_using_mini_isp) || NULL == mini_this_ispdata->meta_data)
		return ret;
	temp = mini_this_ispdata->meta_data;
	memset(&ae_data,0x00,sizeof(mini_aec_data_t));

	gain = temp->data.ae_info.gain;
	gain <<=4;
	gain = gain/100;

	target_y = MINI_ISP_TARGET_Y;

	ae_data.lum = temp->data.ae_info.avg_raw_lum;
	ae_data.gain = gain;

	if (mini_ae_is_need_flash(sensor, &ae_data, target_y) == true) {
		ret = true;
	}

	print_info("%s(), LCD need flash return: %s, lum[%u] gain[%u]", __func__, ret?"true":"false",ae_data.lum,ae_data.gain);
	return ret;
}
...
int mini_hwa_check_lcd_compensation_supported(void)
{
	...
	print_info("%s(), LCD compensation supported: 0", __func__);
	return 0;
}
...
int mini_hwa_check_lcd_compensation_needed(void)
{
	...
	if(true == hwa_is_lcd_need_flash(mini_this_ispdata->sensor))
		return 1;
...
int mini_hwa_set_af_dest_code(u32 dest_code)
{
	...
	if (mini_this_ispdata->sensor->af_enable)
	{
		...
		if(dest_code>255)
			code = 255;
		ret = camera_agent_set_AFlevel(mini_this_ispdata->sensor->sensor_index,code);
	}
	else
...
int misp_g_motion = 0;
int misp_g_multidenoise = 0;

int mini_isp_set_over_exposure(int exposure)
{
	print_debug("enter %s, exposure=%d", __func__, exposure);
	camera_agent_set_ae_over_state((u8)exposure);
	return 0;
}

int mini_isp_set_scene_mode(int scene)
{
	print_debug("enter %s, scene=%d", __func__, scene);
	camera_agent_set_scene_mode(mini_this_ispdata->sensor->sensor_index, scene);
	return 0;
}

int mini_isp_set_motion(int motion)
{
	print_info("enter %s, motion=%d", __func__, motion);
	misp_g_motion = motion;
	return 0;
}

int mini_isp_set_multidenoise(int denoise)
{
	print_info("enter %s, fdn=%d", __func__, denoise);
	misp_g_multidenoise = denoise;
	return 0;
}
/********************************** END **********************************************/
