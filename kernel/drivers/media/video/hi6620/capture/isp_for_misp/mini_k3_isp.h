...
typedef struct _mini_isp_meta_data {
	void (*exit)(void **);
	void (*parse)(void *, void *, camera_state);

	meta_data_st data;
	meta_data_st temp_buf[2];
}mini_isp_meta_data;
...
typedef struct _k3_isp_data {
	...
	bool				hdr_shoot_enable;
	u32					hdr_frame_index;
	struct work_struct	hdr_process_work;
	u32 				hdr_expo[MAX_BRACKET_COUNT];
	u32					hdr_iso[MAX_BRACKET_COUNT];
	camera_scene		scene;
	int					ev;
	...
	struct workqueue_struct	*mini_isp_work_queue;
	struct work_struct 		mini_isp_work;
	...
	u32				mini_isp_flash_enerage;
	camera_flash_flow	mini_isp_flash_flow;
	camera_flash_state	mini_isp_flash_state;
	int					flash_off_drop_frame_num;
...
typedef struct _mini_isp_hw_controller {
	...
	int (*isp_set_zoom) (mini_zoom_t *zoom, zoom_quality_t quality);
