...
static bool global_motion = false;
static bool local_motion = false;
...
int mini_ispv1_focus_init(void)
{
	...
	size = MAX_PREVIEW_WIDTH * MAX_PREVIEW_HEIGHT *
		DEFAULT_AF_WIDTH_PERCENT * DEFAULT_AF_HEIGHT_PERCENT / 100 / 100 * 9;
	...
	mini_afae_ctrl->next_frame = vmalloc(size / 9);
	if (mini_afae_ctrl->next_frame == NULL) {
		print_error("malloc is failed in %s function at line#%d\n", __func__, __LINE__);
		kfree(mini_afae_ctrl->map_table);
		mini_afae_ctrl->map_table = NULL;
	...
	if (!af_start_work_queue) {
		...
		vfree(mini_afae_ctrl->stat_data);
		mini_afae_ctrl->stat_data = NULL;
		vfree(mini_afae_ctrl->next_frame);
		mini_afae_ctrl->next_frame = NULL;
		kfree(mini_afae_ctrl);
		mini_afae_ctrl = NULL;
		return -ENOMEM;
	}
	...
	camera_agent_set_PreviewAFLock(mini_this_ispdata->sensor->sensor_index,1);
...
int mini_ispv1_focus_exit(void)
{
	...
	if (mini_afae_ctrl->next_frame) {
		vfree(mini_afae_ctrl->next_frame);
		mini_afae_ctrl->next_frame = NULL;
	}

	mini_afae_ctrl->not_first_frame = false;
...
int mini_ispv1_auto_focus(int flag)
{
	int ret = 0;
	int i = 0;
	mini_camera_flashlight *flashlight;
	bool need_s1_lock = false;
	static bool set_s0_after_s1_lock = false;
	...
	if (flag == FOCUS_CAF_FORCE_AF) {
		if (mini_isp_check_caf_need_trigger_af()) {
			need_s1_lock = true;
			set_s0_after_s1_lock = true;
		} else {
			need_s1_lock = false;
			camera_agent_set_PreviewAFLock(mini_this_ispdata->sensor->sensor_index,1);
		}
	} else if (flag == FOCUS_START) {
		if (get_focus_mode() == CAMERA_FOCUS_AUTO
			...
			camera_agent_set_PreviewAFLock(mini_this_ispdata->sensor->sensor_index,0);
		}
	} else if (flag == FOCUS_AF_RESUME_CAF) {
		set_s0_after_s1_lock = false;
	}
	...
	if (need_s1_lock) {
		...
		print_info("FLASH S1_3A_UNLOCKED_BY_AF");
		camera_agent_set_s1_AF_enable(mini_this_ispdata->sensor->sensor_index, 1);
		if (flag == FOCUS_CAF_FORCE_AF)
			camera_agent_set_s1_AE_enable(mini_this_ispdata->sensor->sensor_index, 0);
		else
			camera_agent_set_s1_AE_enable(mini_this_ispdata->sensor->sensor_index, 1);
		...
		mini_this_ispdata->s1_3a_state = S1_3A_UNLOCKED_BY_AF;
...
static int ispv1_focus_adjust_yuvrect(mini_camera_rect_s *yuv, u32 preview_width, u32 preview_height, u32 zoom)
{
	...
	ratio = (ratio - ISP_ZOOM_BASE_RATIO) * (this_focus_area->max_zoom_ratio - ISP_ZOOM_BASE_RATIO);
...
static int ispv1_face_area_adjust_yuvrect(mini_camera_rect_s *yuv, u32 preview_width, u32 preview_height)
{

	/* check if edge is not valid, set to default. */
	if ((yuv->left > preview_width) || (yuv->top > preview_height) ||
		(yuv->width > preview_width) || (yuv->height > preview_height)) {
		ispv1_focus_get_default_yuvrect(yuv, preview_width, preview_height);
	} else if (((yuv->left + yuv->width) > preview_width) || ((yuv->top + yuv->height) > preview_height)) {
...
int mini_ispv1_set_focus_mode(camera_focus focus_mode)
{
	...
	if(focus_mode==CAMERA_FOCUS_CONTINUOUS_PICTURE)
	{
		...
		camera_agent_set_PreviewAELock(mini_this_ispdata->sensor->sensor_index,0);
		camera_agent_set_PreviewAWBLock(mini_this_ispdata->sensor->sensor_index,0);
		mini_this_ispdata->mini_isp_flash_flow = FLASH_DONE;		//zhoutian
	}
...
int mini_ispv1_set_sharpness_zoom(u32 zoom)
{
	...
	if(zoom == 0)
	{
		SETREG8(REG_ISP_SHARPNESS, max_sharpness);
		SETREG8(REG_ISP_SHARPNESS-1, max_sharpness>>8);
	}
	else
...
static int ispv1_set_focus_area_done(mini_focus_area_s *area, u32 zoom)
{
	...
	memcpy(&yuv_orig, &yuv_in_full, sizeof(mini_camera_rect_s));
	...
	ret = mini_k3_isp_yuvrect_to_rawrect(&yuv_orig, &raw_rect);
		if (ret) {
			print_error("%s, line %d: error", __func__, __LINE__);
			return ret;
		}
...
int mini_ispv1_set_ae_statwin(mini_pic_attr_t *pic_attr, mini_coordinate_s *center, METERING_STATWIN_MODE statwin_mode, u32 zoom)
{
	...
	if (mini_k3_isp_yuvrect_to_rawrect(&yuv, &raw)) {
		print_error("%s:line %d error", __func__, __LINE__);
		return -1;
	}
	...
	print_debug("focus AE %s new stat raw win[left %d,top %d, right %d, bottom %d]",
...
#define BLOCKSIZE 			32
#define MAXBLOCK  			16
#define LOCAL_MOTION_THRESH 90
#define FRAME_COUNT_THRESH  1
#define CONTRAST_THRESH_FOR_SMOOTH_AREA 100
...
static void ispv1_k3focus_run(void)
{
	...
	if (mini_afae_ctrl->zoom > 0) {
		rect_used = rect_large;
	}
...
static bool enlarge_focus_window(mini_camera_rect_s *rect, int width, int height, int zoom)
{
	...
	if (zoom == 0) {
		...
		new_height = NORMAL_RECT_SIZE;
		new_width = NORMAL_RECT_SIZE;
	}
...
int mini_ispv1_face_yuvrect_to_rawrect(void* face_area_s)
{
	...
	/* adjust yuv rect. */
	ispv1_face_area_adjust_yuvrect(&yuv_in_full, preview_width, preview_height);
...
static int mini_ispv1_gyro_status_collect(mini_focus_frame_stat_s *curr_data, mini_focus_frame_stat_s *mean_data)
{
	int loopi;
	u32 variance[5] = {0, 0, 0, 0, 0};
	u32 contrast_diff = 0;
...
static u16 mini_ispv1_calc_gyro_stat_diff(mini_focus_frame_stat_s *pre_data, mini_focus_frame_stat_s *cur_data)
{
	u16 ret_val = 0;
	mini_focus_frame_stat_s diff;
	mini_effect_params *effect = get_effect_ptr();
 ...
 */
static bool mini_ispv1_check_gyro_caf_need_trigger(mini_focus_frame_stat_s *compare_data, mini_focus_frame_stat_s *mean_data, mini_focus_frame_stat_s cur_data)
{
	u8 unpeace = 0;
	static u16 unpeace_ms;
	u16 force_start = get_caf_forcestart();
	...
	if (force_start == 0) {
		/* if diff too much, should force focus start. */
		diff_val = mini_ispv1_calc_gyro_stat_diff(compare_data, mean_data);
		force_start = diff_val;
		set_caf_forcestart(force_start);
	} else if ((force_start & CAF_FORCESTART_FORCE) != 0) {
		unpeace_ms = 0;
	...
	if (force_start && unpeace == 0) {
			if ((this_afae_ctrl->focus_stat_frames > this_caf_trigger->stat_skip_frame)
				...
				diff_val = mini_ispv1_calc_gyro_stat_diff(compare_data, &cur_data);
				if (diff_val != 0)
					trigger = true;
			}
			unpeace_ms = 0;
...
mini_camera_rect_s set_motion_detection_area(void)
{
	u32 preview_width = mini_this_ispdata->pic_attr[STATE_PREVIEW].out_width;
	u32 preview_height = mini_this_ispdata->pic_attr[STATE_PREVIEW].out_height;
	mini_camera_rect_s rect = {0};

	rect.height = DEFAULT_AF_HEIGHT_PERCENT * preview_height / 100;
	rect.width = DEFAULT_AF_WIDTH_PERCENT * preview_width / 100;
	rect.left = (preview_width - rect.width) >> 1;
	rect.top = (preview_height - rect.height) >> 1;

	return rect;
}

int mini_ispv1_compare_mean_var(u32 mean_var_pre[],u32 mean_var_cur[],int blocknum)
{
    int i,ret;
    u32 sum,sumA,sumB;    
    u32 max_pre, max_cur;
    ...
    for(i = 0;i < blocknum;i++){  
        if (mean_var_pre[i] > max_pre)
            max_pre = mean_var_pre[i];
        
        if (mean_var_cur[i] > max_cur)
            max_cur = mean_var_cur[i];  
    }
    
    if ((max_pre > 10000)||(max_cur > 10000)){
        scale = 10;
    } else if((max_pre > 1000)||(max_cur > 1000)){
		scale = 7;
    } else{
		scale = 4;
    }  
    ...
    for (i = 0;i < blocknum;i++){
       	sum += (mean_var_pre[i]>>scale) * (mean_var_cur[i]>>scale);
		sumA += (mean_var_pre[i]>>scale) * (mean_var_pre[i]>>scale);
		sumB += (mean_var_cur[i]>>scale) * (mean_var_cur[i]>>scale);
    }

    if(sumA&&sumB){
        
		ret = (sum*100)/sumA*sum/sumB;
    }
    
    return ret;
}

void mini_ispv1_cal_mean_var(u8 *inputimg, int height, int width, mini_camera_rect_s rect, int *mean,int *var)
{
    int i,j;
    u32 sum = 0;
    int pos;
    ...
    for (i = 0; i < rect.height; i++){ 
		...
		pos = width*(rect.top + i) + rect.left;
    ...
    *mean = sum / (rect.width * rect.height);
    ...
    for (i = 0; i < rect.height; i++){
		...
		for (j = 0; j < rect.width; j++){
		     ...
		     sum +=(inputimg[pos] - *mean)*(inputimg[pos] - *mean);
             pos++;
		}
    }
    
    *var = sum / (rect.width * rect.height - 1);

}

u32 mean_var_pre[MAXBLOCK * MAXBLOCK];
u32 mean_var_cur[MAXBLOCK * MAXBLOCK];

int ispv1_scene_change_detect(u8 *inputimg, mini_camera_rect_s *focusrect)
{
    int blocknumw,blocknumh;
    int i,j;
    mini_camera_rect_s rect;
    int mean,var;
    int ret;
    int sim;
    int blocknum ;
    ...
    blocknumw = (int)(focusrect->width/BLOCKSIZE) > MAXBLOCK? MAXBLOCK : (int)(focusrect->width/BLOCKSIZE);
    blocknumh = (int)(focusrect->height / BLOCKSIZE)> MAXBLOCK? MAXBLOCK : (int)(focusrect->height/BLOCKSIZE);

    rect.height = BLOCKSIZE;
    rect.width = BLOCKSIZE;
    
    if (mini_afae_ctrl->not_first_frame){
		for(i = 0; i< blocknumh; i++) {
            		for (j = 0; j<blocknumw; j++){
                    	rect.top = i * BLOCKSIZE;
                    	rect.left = j * BLOCKSIZE;                    
                    	mini_ispv1_cal_mean_var(inputimg,focusrect->height,focusrect->width,rect,&mean,&var);
				    mean_var_cur[i * blocknumw + j] = mean*var;
                    	blocknum++;
			}
    }
	sim = mini_ispv1_compare_mean_var(mean_var_pre,mean_var_cur,blocknum);
	memcpy(mean_var_pre, mean_var_cur,blocknum*sizeof(u32));
	return sim;	
    } else {
		for(i = 0; i < blocknumh; i++){
	            for(j = 0; j < blocknumw; j++){                
                    ...
                    rect.left = j * BLOCKSIZE;                    
                    mini_ispv1_cal_mean_var(inputimg,focusrect->height,focusrect->width,rect,&mean,&var);
                    mean_var_pre[i * blocknumw + j] = mean*var;
		}
	}
	mini_afae_ctrl->not_first_frame = true;
	return ret ;
    }    
}
...
void misp_gyro_info(void)
{
	...
	int local_ret;
	mini_camera_rect_s rect = set_motion_detection_area();
	bool copy_ret = false;
	/* Calculate the contrast in motion detection area */
	int contrast = 0;
	...
	memcpy(&curr_data.xyz, &mini_mXYZ, sizeof(mini_axis_triple));
	ret = mini_ispv1_gyro_status_collect(&curr_data,&mean_data);
	if (ret != 0)
		return;
	result = mini_ispv1_check_gyro_caf_need_trigger(&mini_afae_ctrl->compare_data, &mean_data, curr_data);
	copy_ret = ispv1_copy_preview_data(mini_afae_ctrl->next_frame, &rect);
	if (false == copy_ret) {
		return;
	}


	contrast = ispv1_focus_calc_edge(mini_afae_ctrl->next_frame, &rect, 0, false);
	if (contrast > CONTRAST_THRESH_FOR_SMOOTH_AREA) {
		local_ret = ispv1_scene_change_detect(mini_afae_ctrl->next_frame, &rect);
	} else {
		local_ret = -1;
	}
	...
	if (local_ret >= LOCAL_MOTION_THRESH || local_ret < 0) {
		afae_ctrl_ptr->local_static_frame++;
		afae_ctrl_ptr->local_moving_frame = 0;
		if (afae_ctrl_ptr->local_static_frame > FRAME_COUNT_THRESH) {
			local_motion = false;
			print_debug("%s, local motion detection result: static.", __func__);
		}
	} else {
		afae_ctrl_ptr->local_static_frame = 0;
		afae_ctrl_ptr->local_moving_frame++;
		if (afae_ctrl_ptr->local_moving_frame > FRAME_COUNT_THRESH) {
			local_motion = true;
			print_debug("%s, local motion detection result: moving.", __func__);
		}
	}
	camera_agent_set_Motion_Detected(global_motion, local_motion);
}

bool mini_isp_is_phone_moved(void)
{
	return (global_motion || local_motion);
}
