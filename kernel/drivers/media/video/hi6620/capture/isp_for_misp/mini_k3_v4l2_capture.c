				...
				struct v4l2_control *v4l2_param)
{
	...
	case V4L2_CID_SET_ZOOM_CENTER:
		{
			...
			ret = mini_k3_isp_set_zoom_center(cam->running[STATE_PREVIEW], v4l2_param->value);
			break;
		}
	...
	case  V4L2_CID_SET_3A_MODE:
        ...
        case V4L2_CID_SET_SENSORMODE:
        {
            if (v4l2_ctl.sensor->sensor_index == CAMERA_SENSOR_PRIMARY) {
                //primary camera have only one mode so it can set sensor id and mode id here
                camera_agent_set_sensormode(v4l2_ctl.sensor->sensor_index , 0, 0);
                v4l2_ctl.sensor->needSetmode = false;
                v4l2_ctl.sensor->preview_frmsize_index = 0;
            }
            break;
        }
    /* end */
				     ...
				     struct v4l2_ext_controls *arg)
{
	...
	for (cid_idx = 0; cid_idx < ext_ctls->count; cid_idx++) {
		...
		case V4L2_CID_FOCUS_RESULT:
		{
            ...
            data_buf = kmalloc(sizeof(mini_focus_result_s), GFP_KERNEL);
            if (!data_buf) {
            	ret = -ENOMEM;
            	print_error("fail to allocate buffer");
            	goto out;
            }
			mini_k3_isp_get_focus_result((mini_focus_result_s *)data_buf);
            if(copy_to_user(result, data_buf, sizeof(mini_focus_result_s))){
		...
		case V4L2_CID_GET_FOCUS_RECT:
		{
            ...
            data_buf = kmalloc(sizeof(mini_camera_rect_s), GFP_KERNEL);
            if (!data_buf) {
            	ret = -ENOMEM;
            	print_error("fail to allocate buffer");
            	goto out;
            }
			mini_k3_isp_get_focus_rect((mini_camera_rect_s *)data_buf);
            if(copy_to_user(rect, data_buf, sizeof(mini_camera_rect_s))){
		...
		case V4L2_CID_GET_YUV_CROP_RECT:
		{
            ...
            data_buf = kmalloc(sizeof(mini_crop_rect_s), GFP_KERNEL);
            if (!data_buf) {
            	ret = -ENOMEM;
            	print_error("fail to allocate buffer");
            	goto out;
            }
			mini_k3_isp_get_yuv_crop_rect((mini_crop_rect_s *)data_buf);
            if(copy_to_user(rect, data_buf, sizeof(mini_crop_rect_s))){
...
static void k3_v4l2_set_sensor(sensor_index_t sensor_index, char *sensor_name)
{
	...
	int index = 0;
	mini_camera_sensor *temp_sensor_array[CAMERA_SENSOR_NUM_MAX];
	...
	do {
		temp_sensor = mini_get_camera_sensor_from_array(sensor_index);
		if (NULL == temp_sensor) {
			print_error("fail to get camera [%d]", sensor_index);
			memcpy(mini_get_camera_sensor_array(sensor_index), temp_sensor_array, sizeof(temp_sensor_array));
			return;
		}
		...
		if (NULL == ret) {
			...
			mini_clean_camera_sensor(sensor_index);
			mini_register_camera_sensor(sensor_index, temp_sensor);
			mini_set_camera_sensor(sensor_index, temp_sensor);
		}
	} while (NULL == ret);
}
				     ...
				     struct v4l2_ext_controls *arg)
{
	...
	for (cid_idx = 0; cid_idx < ext_ctls->count; cid_idx++) {
		...
		case V4L2_CID_BUFFER_OFFSET:
			{
				if (controls[cid_idx].value == 0) {
					...
					for (i = 0; i < MAX_FRAME_NR; i++) {
						if (cam->buffer_arr[state].buffers[i].statviraddr) {
							iounmap(cam->buffer_arr[state].buffers[i].statviraddr);
							cam->buffer_arr[state].buffers[i].statviraddr = NULL;
						}
					}
		...
		case V4L2_CID_STATBUFFER_OFFSET:
			{
				...
				if (controls[cid_idx].value) {
					cam->buffer_arr[state].buffers[vbuffer.index].statphyaddr = controls[cid_idx].value;
					statviraddr = ioremap_nocache((phys_addr_t)controls[cid_idx].value, sizeof(meta_data_st) * 2);
					if (statviraddr) {
						print_info("ioremap_nocache statviraddr = %#x", statviraddr);
						cam->buffer_arr[state].buffers[vbuffer.index].statviraddr = statviraddr;
					} else {
						print_error("statphyaddr ioremap_nocache error");
						cam->buffer_arr[state].buffers[vbuffer.index].statviraddr = NULL;
					}
				} else {
					print_error("fatal error statphyaddr is null");
					ret = -EINVAL;
					goto end;
				}
				break;
		...
		case V4L2_CID_FOCUS_AREA_INFO:
        {
            ...
            data_buf = kmalloc(sizeof(mini_focus_area_s), GFP_KERNEL);
            if (!data_buf) {
            	ret = -ENOMEM;
            	print_error("fail to allocate buffer");
            	goto end;
            }
            if(copy_from_user(data_buf, area, sizeof(mini_focus_area_s))){
                print_error("set V4L2_CID_FOCUS_AREA_INFO mem error! %s", __FUNCTION__);
            }
			func_ret = mini_k3_isp_set_focus_area((mini_focus_area_s *)data_buf);
			if (func_ret != 0) {
				ret = -EINVAL;
			}
			goto end;
		...
		case V4L2_CID_BRACKET_INFO:
        {
			...
			func_ret = mini_k3_isp_set_bracket_info((int *)data_buf);
			if (func_ret != 0) {
				ret = -EINVAL;
			}
			goto end;
		...
		case V4L2_CID_METERING_AREA_INFO:
        {
            ...
            data_buf = kmalloc(sizeof(mini_metering_area_s), GFP_KERNEL);
            if (!data_buf) {
            	ret = -ENOMEM;
            	print_error("fail to allocate buffer");
            	goto end;
            }
            if(copy_from_user(data_buf, metering_area, sizeof(mini_metering_area_s))){
                print_error("set V4L2_CID_METERING_AREA_INFO mem error! %s", __FUNCTION__);
            }
			func_ret = mini_k3_isp_set_metering_area((mini_metering_area_s *)data_buf);
			if (func_ret != 0) {
				ret = -EINVAL;
			}
			goto end;
...
//			sensor_name = (char *)(controls[cid_idx].string);
		...
		case V4L2_CID_SET_GSENSOR_STAT:
        {
			...
			data_buf = kmalloc(sizeof(mini_axis_triple), GFP_KERNEL);
			if (!data_buf) {
				ret = -ENOMEM;
				print_error("fail to allocate buffer");
				goto end;
			}
			if(copy_from_user(data_buf, xyz, sizeof(mini_axis_triple))){
                print_error("set V4L2_CID_SET_GSENSOR_STAT mem error! %s", __FUNCTION__);
            }
			func_ret = mini_k3_isp_set_gsensor_stat((mini_axis_triple *)data_buf);
			if (func_ret != 0) {
				ret = -EINVAL;
			}
			goto end;
...
int k3_v4l2_set_camera(mini_v4l2_ctl_struct *cam, int new_sensor)
{
	...
	if (CAMERA_SENSOR_PRIMARY == new_sensor) {
		if (NULL == *pri_sensor) {
			...
			ret = mini_k3_isp_set_camera(*pri_sensor, *sec_sensor, CAMERA_SENSOR_KNOWN);
		...
		if (NULL == *sec_sensor) {
			...
			ret = mini_k3_isp_set_camera(*sec_sensor, *pri_sensor, CAMERA_SENSOR_KNOWN);
