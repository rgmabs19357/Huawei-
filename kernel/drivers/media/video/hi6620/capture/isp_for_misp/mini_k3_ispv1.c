...
#elif defined(CONFIG_MACH_HI6620OEM)
...
static int ispv1_set_zoom(mini_zoom_t *zoom, zoom_quality_t quality);
static int ispv1_offline_cmd(mini_pic_attr_t *pic_attr, u32 source_addr, u32 target_addr);
static int ispv1_capture_cmd(mini_pic_attr_t *pic_attr, u32 phyaddr, u32 statphyaddr, u32 count, int *ev, mini_camera_sensor *sensor, bool flash_on, camera_scene scene);
...
static void ispv1_enable_irq(void)
{
	SETREG8(REG_ISP_INT_EN,
		MASK_EOF_INT_ENABLE |MASK_SOF_INT_ENABLE | MASK_CMDSET_INT_ENABLE | MASK_MAC_INT_ENABLE);
...
static int ispv1_set_zoom(mini_zoom_t *zoom, zoom_quality_t quality)
{
	...
	{
		ratio = isp_zoom_to_ratio(zoom->zoom, mini_this_ispdata->video_stab);
		SETREG8(COMMAND_REG1, quality | ZOOM_CENTER_CHANGED_DISABLE);
		SETREG16(COMMAND_REG2, ratio);
		SETREG16(COMMAND_REG4, ratio);

		print_info("mirror:%s: zoom = 0x%x,zoom.user_center_xy=%d", __func__, zoom->zoom,zoom->user_center_xy);
	}
	else
	{
		...
		SETREG16(COMMAND_REG2, ratio);
		SETREG16(COMMAND_REG4, ratio);
		SETREG16(REG_ISP_ZOOM_CENTER_X,zoom->center_x);
		SETREG16(REG_ISP_ZOOM_CENTER_Y,zoom->center_y);
		print_info("mirror:%s: zoom = 0x%x,zoom.center_x=%d,zoom.center_y=%d", __func__, zoom->zoom,zoom->center_x,zoom->center_y);
	}
 ...
 **************************************************************************
 */
static int ispv1_capture_cmd(mini_pic_attr_t *pic_attr, u32 phyaddr, u32 statphyaddr,u32 count, int *ev,
	mini_camera_sensor *sensor, bool flash_on, camera_scene scene)
{
	...
	{
		sharpness = effect->sharpness[0].sharpness_capture;
		SETREG8(REG_ISP_SHARPNESS, sharpness);
		SETREG8(REG_ISP_SHARPNESS-1, sharpness>>8);
	}
	/*
	...
	print_info("%s, in_width=%d, in_height=%d, out_width=%d, out_height=%d,"
			" out_stride=%d, phyaddr=%#x, statphy=%#x, count=%d, bypass=%d, sensor setting:%d",
			__func__, pic_attr->in_width, pic_attr->in_height,
			pic_attr->out_width, pic_attr->out_height, pic_attr->out_stride,
			phyaddr, statphyaddr,count, is_bypass_isp, frame_index);
	...
	*/
	...
	/* 6. Tune config */
	mini_ov_set_wb_value(mini_this_ispdata->meta_data, STATE_CAPTURE);
...
#else
	reg5 |= ISP_SENSOR_INT_DISABLE | ISP_MACINT_SET_ENABLE | ISP_EXPO_RATIO_ENABLE;
#endif
	...
	sensor->stream_on(STATE_CAPTURE);
	if (ispdata->hdr_shoot_enable)
		schedule_work(&ispdata->hdr_process_work);
                ...
                u32             target_addr)
{
    ...
    if ((mini_this_ispdata->zoom == 14))
    {
        if ((pic_attr->out_width == 3264)
         ...
         && (mini_this_ispdata->pic_attr[STATE_PREVIEW].out_width == 720)
...
//    spin_unlock_irqrestore(&mini_isp_hw_data.irq_status_lock, lock_flags);
    ...
    /* set max drops we can tolerate while processing image with preview.
    when processing slice raw data occures drop,firmware will do the process again */
    SETREG8(ISP_PROC_IMAGE_MAC_DROP_THRESHHOLD,PROC_IMAGE_MAC_DROP_THRESHHOLD);
			     ...
			     mini_camera_sensor *sensor, bool cold_boot, camera_scene scene)
{
	...
	if(mini_this_ispdata->zoom==0)
	{
		sharpness = effect->sharpness[0].sharpness_preview;
		SETREG8(REG_ISP_SHARPNESS, sharpness);
		SETREG8(REG_ISP_SHARPNESS-1, sharpness>>8);
	}
#if 0
...
#else
	if (cold_boot == true)
		reg5 = ISP_SENSOR_INT_DISABLE | ISP_MACINT_SET_DISABLE | ISP_EXPO_RATIO_ENABLE;
	else
		reg5 = ISP_SENSOR_INT_DISABLE | ISP_MACINT_SET_DISABLE | ISP_EXPO_RATIO_ENABLE;
#endif
...
static int ispv1_start_capture(mini_pic_attr_t *pic_attr, mini_camera_sensor *sensor, int *ev, bool flash_on, camera_scene scene,	mini_buffer_arr_t *buf_arr)
{
	...
	if (CAMERA_SHOOT_BURST == ispdata->shoot_mode)
	...
	if (count < 5) {
		ispdata->hdr_shoot_enable = false;
	} else {
		ispdata->hdr_shoot_enable = true;
		ispdata->hdr_frame_index = MAX_HDR_FRAME_INDEX;
		memset(ispdata->hdr_iso, 0, sizeof(ispdata->hdr_iso));
		memset(ispdata->hdr_expo, 0, sizeof(ispdata->hdr_expo));
	}
	count = 1;
    ...
    SETREG8(REG_ISP_INT_EN, MASK_SOF_INT_ENABLE | MASK_CMDSET_INT_ENABLE | MASK_MAC_INT_ENABLE);/* Enable CMD_SET MAC_INT int */

	#ifndef READ_BACK_RAW
	ret = ispv1_capture_cmd(pic_attr, frame->phyaddr, frame->statphyaddr, count, ev, sensor, flash_on, scene);
	if (0 != ret) {
...
static int ispv1_stop_preview(void)
{
	...
	mini_isp_hw_data_t *dev_hw_data = &mini_isp_hw_data;
	long timeout, ret;
	...
	if (sensor->stream_off != NULL) {
		/* wait sof signal */
		timeout = msecs_to_jiffies(120);
		dev_hw_data->wait_sof_status = WAIT_SOF_NONE;
		ret = wait_event_interruptible_timeout(dev_hw_data->wait_sof_queue,\
							(dev_hw_data->wait_sof_status == WAIT_SOF_READY),\
							timeout);

		if (ret <= 0)
			print_warn("%s wait sof signal timeout", __func__);
		/* stream off cmd */
...
static int ispv1_stop_capture(void)
{
	...
	if (ispdata->hdr_shoot_enable)
		flush_work(&ispdata->hdr_process_work);
	...
	if (sensor->stream_off != NULL) {
		...
		if (ret <= 0)
			print_warn("%s wait sof signal timeout", __func__);
		/* stream off cmd */
		sensor->stream_off(STATE_CAPTURE);
	}
	ispv1_disable_irq();
	mini_burst_capture = false;
	print_info("enter %s", __func__);
...
static int ispv1_poweron(void)
{
    ...
    if (0 == mini_get_cam_pwdn_status(DEBUGFS_PWDN_ISP)) {
        ...
        if (clk_set_parent(mini_isp_hw_data.isp_gated_clk, mini_isp_hw_data.dsppll_clk)) {
    		print_error("Error: %s, set isp clk src failed", __func__);
    		return -EFAULT;
        }
#endif

    	if (mini_isp_hw_data.isp_clk == NULL || clk_enable(mini_isp_hw_data.isp_clk)) {
    	...
    	if (chip_id == CS_CHIP_ID) {
    		...
    		if (mini_isp_hw_data.ispmipi_clk == NULL || clk_enable(mini_isp_hw_data.ispmipi_clk)) {
    		...
    		if (mini_isp_hw_data.isp_aclk == NULL || clk_enable(mini_isp_hw_data.isp_aclk)) {
    			print_error("Error: %s, isp aclk enable failed", __func__);
    			return -EFAULT;
    		}

    		if (mini_isp_hw_data.isp_csi_clk == NULL || clk_enable(mini_isp_hw_data.isp_csi_clk)) {
    			print_error("Error: %s, isp APB MUX clk enable failed", __func__);
    			return -EFAULT;
    		}

    		if (mini_isp_hw_data.isp_hclk == NULL || clk_enable(mini_isp_hw_data.isp_hclk)) {
...
static void ispv1_poweroff(void)
{
    ...
    if (0 != mini_get_cam_dbgfs_pwdncfg()) {
    	...
    	if (mini_isp_hw_data.isp_clk) {
    		clk_disable(mini_isp_hw_data.isp_clk);
    	}

    	chip_id = get_chipid();
    	if (chip_id == CS_CHIP_ID) {
    		if (mini_isp_hw_data.ispmipi_clk) {
    			clk_disable(mini_isp_hw_data.ispmipi_clk);
    		}

    		if (mini_isp_hw_data.isp_aclk) {
    			clk_disable(mini_isp_hw_data.isp_aclk);
    		}

    		if (mini_isp_hw_data.isp_csi_clk) {
    			clk_disable(mini_isp_hw_data.isp_csi_clk);
    		}

    		if (mini_isp_hw_data.isp_hclk) {
    			clk_disable(mini_isp_hw_data.isp_hclk);
    		}
    	}
...
static int ispv1_frame_done_handler(camera_state state)
{
	...
	if (!list_empty(work_q)) {
		...
		if (ispdata->is_using_mini_isp && ispdata->meta_data && ispdata->meta_data->parse) {
			ispdata->meta_data->parse(frame->statviraddr, ispdata->meta_data, state);
			/* save capture info for EXIF */
			if (state == STATE_CAPTURE) {
				if (ispdata->hdr_shoot_enable) {
					frame_index = ispdata->meta_data->data.common_info.frame_index;
					hdr_index = ispdata->hdr_frame_index;
					if ((frame_index >= hdr_index) && (frame_index < (hdr_index + MAX_BRACKET_COUNT))) {
						ispdata->hdr_iso[frame_index - hdr_index] = ispdata->meta_data->data.ae_info.iso;
						ispdata->hdr_expo[frame_index - hdr_index] = ispdata->meta_data->data.ae_info.expo_time;
						if (ispdata->bracket_ev[frame_index - hdr_index] == 0) {
							ispdata->capture_iso = ispdata->meta_data->data.ae_info.iso;
							ispdata->capture_expo_time = ispdata->meta_data->data.ae_info.expo_time;
						}
					}
				} else {
					ispdata->capture_iso = ispdata->meta_data->data.ae_info.iso;
	...
	/* better to use: ispdata->meta_data->data.common_info.frame_index */
	if (state == STATE_CAPTURE) {
		ispdata->flash_off_drop_frame_num = 0;
		if (ispdata->hdr_shoot_enable) {
			/* handle hdr */
			frame_index = ispdata->meta_data->data.common_info.frame_index;
			if (frame_index < ispdata->hdr_frame_index) {
				mini_del_from_queue(frame, CAMERA_FLAG_DONEQ);
				mini_add_to_queue(frame, ready_q, CAMERA_FLAG_READYQ);
				print_info("discard hdr capture frame:%d", mini_isp_hw_data.frame_count);
				goto OUT;
			}
		}else {
			/* handle burset & oneshot */
			if (mini_this_ispdata->sensor->sensor_index == CAMERA_SENSOR_SECONDARY && mini_isp_hw_data.frame_count < misp_skip_cap_frame) {
				mini_del_from_queue(frame, CAMERA_FLAG_DONEQ);
				mini_add_to_queue(frame, ready_q, CAMERA_FLAG_READYQ);
		...
		if (ispdata->flash_off_drop_frame_num) {
			ispdata->flash_off_drop_frame_num--;
			mini_del_from_queue(frame, CAMERA_FLAG_DONEQ);
			mini_add_to_queue(frame, ready_q, CAMERA_FLAG_READYQ);
			print_info("FLASH discard one preview frame after flash turn off");
			goto OUT;
		}
	}
...
static DECLARE_WORK(flash_work, mini_isp_flash_handler);
...
static void isr_do_tasklet(unsigned long data)
{
	...
	if (mini_isp_hw_data.irq_val.irq_status & MASK_SOF_INT_ENABLE) {
		...
		if (mini_isp_hw_data.wait_sof_status == WAIT_SOF_NONE) {
			mini_isp_hw_data.wait_sof_status = WAIT_SOF_READY;
			wake_up_interruptible(&mini_isp_hw_data.wait_sof_queue);
		}
 #ifdef SUPPORT_ZSL_FLASH
     zsl_sof_start_flash();
 #endif
	...
	if (mini_isp_hw_data.irq_val.mac_irqstatus_l & ISP_WRITE_DONE_INT0) {
		...
		if(mini_this_ispdata->is_using_mini_isp) {
			queue_work(mini_this_ispdata->mini_isp_work_queue, &mini_this_ispdata->mini_isp_work);
			if (mini_isp_hw_data.frame_count && STATE_CAPTURE == cur_state
				&& mini_this_ispdata->flash_on) {
...
static int ispv1_hw_init(struct platform_device *pdev, mini_data_queue_t* data_queue)
{
    ...
    static struct clk *isp_gated_clk;
	...
	chip_id = get_chipid();
	if (chip_id == CS_CHIP_ID) {
		camera_mipi_clk = clk_get(NULL, CLK_ISP_REF);
		...
		camera_aclk = clk_get(NULL, CLK_ISP_AXI);
		...
		camera_csi_clk = clk_get(NULL, CLK_CSI_APB_MUX);
		...
		camera_hclk = clk_get(NULL, CLK_ISP_CFG_CLK);
		...
		dsppll_clk = clk_get(NULL, CLK_DSPPLL);/*1200Mhz*/
        ...
        /* ISP GATED CLK controlled */
		isp_gated_clk = clk_get(NULL, CLK_PERIPLL_ISP_GATED);
		if (IS_ERR_OR_NULL(isp_gated_clk)) {
			print_error("failed to get isp gated clock");
			ret = -ENXIO;
			goto fail;
		}
		mini_isp_hw_data.isp_gated_clk = isp_gated_clk;
	}

	/* get isp regulator */
	camera_inter_ldo = regulator_get(&pdev->dev, ISP_VDD);
	...
	init_waitqueue_head(&mini_isp_hw_data.wait_sof_queue);
...
fail:
	...
	if (chip_id == CS_CHIP_ID) {
		...
		if (mini_isp_hw_data.isp_gated_clk) {
			clk_put(mini_isp_hw_data.isp_gated_clk);
			mini_isp_hw_data.isp_gated_clk = NULL;
		}
	}
