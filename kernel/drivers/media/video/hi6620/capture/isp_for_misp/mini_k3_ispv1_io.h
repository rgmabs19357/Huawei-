...
typedef struct _mini_isp_io_hw {
	...
	struct iomux_block *block_mcam;
	struct iomux_block *block_scam;
	struct block_config *block_conf_mcam;
	struct block_config *block_conf_scam;
#endif
	struct work_struct	mipi_csi_exception_work;
	u8	mipi_csi_exception_already_report;
} mini_ispio_hw_t;
