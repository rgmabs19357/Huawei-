...
#include <hsad/config_mgr.h>  //add by zhoutian for mini-ISP
...
void mini_k3_isp_set_flash_state_in_exif(camera_flash_state state)
{
	flash_exif = state;
}
...
void mini_k3_isp_check_flash_level(camera_flash_state state)
{
	...
	bool s1_abort_flg = false;
	if (isp_data.is_using_mini_isp) {
		if (NULL == flashlight) {
			isp_data.s1_3a_state = S1_3A_LOCKED;
			isp_data.flash_on = false;
			isp_data.mini_isp_flash_flow = FLASH_DONE;
			isp_data.mini_isp_flash_period = FLASH_IDLE;
			flash_exif = FLASH_OFF;
			return;
		}
		if (FLASH_ON == state) {
			mini_isp_check_s1_abort_done();
			camera_agent_set_flash_mode(mini_isp_check_pre_flash_mode());
			print_info("FLASH S1_3A_UNLOCKED_BY_CAPTURE");
			isp_data.s1_3a_state = S1_3A_UNLOCKED_BY_CAPTURE;
			camera_agent_set_s1_AF_enable(isp_data.sensor->sensor_index, 0);
			camera_agent_set_s1_AE_enable(isp_data.sensor->sensor_index, 0);
			camera_agent_s1_lock(isp_data.sensor->sensor_index);
			isp_data.mini_isp_flash_flow = FLASH_DONE;
			flash_exif = FLASH_OFF;
		} else {
			mutex_lock(&isp_data.mini_isp_flash_period_lock);
			if (isp_data.s1_3a_state != S1_3A_LOCKED) {
				// HAL timeout > 2s
				if (isp_data.sensor->sensor_index == CAMERA_SENSOR_PRIMARY)
					print_error("FLASH pre timeout! FLASH_IDLE");
				else
					print_error("Front camera s1_lock timeout! FLASH_IDLE");
				camera_agent_s1_lock_abort();
				s1_abort_flg = true;
				isp_data.s1_3a_state = S1_3A_LOCKED;
				isp_data.mini_isp_flash_period = FLASH_IDLE;
				// use lowest shot flash enerage
				isp_data.mini_isp_flash_enerage = 0;
			}
 ...
 **************************************************************************
 */
int mini_k3_isp_set_camera(mini_camera_sensor *open_sensor, mini_camera_sensor *close_sensor, sensor_known_t known_sensor)
{
	int ret = 0;
	int try = 0;
	...
	if (CAMERA_SENSOR_KNOWN == known_sensor) {
		...
		while(try++ < LOADFW_TRY_TIMES) {
			if(!misp_load_fw()) {
				break;
			}
		}
		if(try >= LOADFW_TRY_TIMES) {
		     ...
		     enum v4l2_buf_type buf_type, camera_state state,mini_buffer_arr_t *buf_arr)
{
	...
	if (CAMERA_ZSL_OFF == mini_k3_isp_get_zsl_state()
		&& mini_hwa_hwscope.mode == HW_SCOPE_ON
		&& state == STATE_CAPTURE) {
		mini_hwa_calc_zoom_with_crop(state, YUV_SCALE_FIRST, &isp_data.pic_attr[state].in_width,
			&isp_data.pic_attr[state].in_height);
	}
	isp_data.zoom_ext.zoom = isp_data.zoom;
	isp_data.zoom_ext.user_center_xy = (isp_data.pic_attr[STATE_PREVIEW].out_width/2 << 16) | (isp_data.pic_attr[STATE_PREVIEW].out_height/2);
	isp_data.zoom_ext.user_center_x = isp_data.pic_attr[STATE_PREVIEW].out_width/2;
	isp_data.zoom_ext.user_center_y = isp_data.pic_attr[STATE_PREVIEW].out_height/2;
	isp_data.zoom_ext.user_full_view_center_x = isp_data.pic_attr[STATE_PREVIEW].out_width/2;
	isp_data.zoom_ext.user_full_view_center_y = isp_data.pic_attr[STATE_PREVIEW].out_height/2;
	isp_data.zoom_ext.center_x = isp_data.pic_attr[STATE_PREVIEW].in_width / 2;
	isp_data.zoom_ext.center_y = isp_data.pic_attr[STATE_PREVIEW].in_height / 2;
		...
		if ((isp_data.sensor->sensor_index == CAMERA_SENSOR_PRIMARY) && (flashlight != NULL)) {
			...
			if (isp_data.is_using_mini_isp) {
				if (isp_data.mini_isp_flash_period == MAIN_FLASH
					&& isp_data.mini_isp_flash_state == FLASH_OFF && isp_data.flash_on) {
					flash_level = isp_data.mini_isp_flash_enerage;
					if (flash_level > 9) {
						print_error("MAIN_FLASH LUM_LEVEL%d too large! limit to LUM_LEVEL9", flash_level);
						flash_level = 9;
					}
					flashlight->turn_on(FLASH_MODE, flash_level);
					print_info("turn_on MAIN_FLASH, LUM_LEVEL%d", flash_level);
					isp_data.mini_isp_flash_state = FLASH_ON;
					isp_data.mini_isp_flash_flow = FLASH_CAPFLASH_START;
				}
			} else {
...
int mini_k3_isp_stream_off(camera_state state)
{
	...
	if (state == STATE_PREVIEW) {
		...
		/*< modify by zhoutian for mini-ISP flash begin */
		if (isp_data.is_using_mini_isp) {
			if (true == isp_data.flash_on && FLASH_ON == isp_data.mini_isp_flash_state && flashlight) {
				flashlight->turn_off();
				isp_data.flash_off_drop_frame_num = 2;
				isp_data.mini_isp_flash_state = FLASH_OFF;
				isp_data.mini_isp_flash_flow = FLASH_PREFLASH_DONE;
		...
		if ((isp_data.sensor->sensor_index == CAMERA_SENSOR_PRIMARY) && (flashlight != NULL)) {
			/*< modify by zhoutian for mini-ISP flash begin */
			if (isp_data.is_using_mini_isp) {
				if (isp_data.flash_on && flashlight) {
					flashlight->turn_off();
					isp_data.flash_on = false;
					isp_data.mini_isp_flash_state = FLASH_OFF;
					isp_data.mini_isp_flash_flow = FLASH_DONE;
	...
	flush_workqueue(isp_data.mini_isp_work_queue);
...
int mini_k3_isp_start_process(struct v4l2_pix_format *pixfmt, u8 mode,mini_buffer_arr_t *ipp_buf_arr)
{
    ...
    if (CAMERA_ZSL_ON == mini_k3_isp_get_zsl_state()){
        ...
        if(mini_hwa_hwscope.mode == HW_SCOPE_ON){
            memcpy(&attr_backup, &isp_data.pic_attr[STATE_CAPTURE], sizeof(attr_backup));
            mini_hwa_calc_zoom_with_crop(STATE_CAPTURE,
                                    YUV_SCALE_FIRST,
    ...
    int ret = isp_hw_ctl->start_process(&isp_data.pic_attr[STATE_CAPTURE], mode,ipp_buf_arr);
    if (CAMERA_ZSL_ON == mini_k3_isp_get_zsl_state()){
        if(mini_hwa_hwscope.mode == HW_SCOPE_ON){
...
{
	int ret = 0;
	mini_k3_isp_data *ispdata = &isp_data; 
	print_debug("enter %s", __func__);
	...
	if(ispdata->is_using_mini_isp) {  //add by zhoutian for mini-ISP
		ret = mini_isp_meta_data_init(&ispdata->meta_data);
		if (0 != ret) {
			print_error("%s mini_isp_meta_data_init failed", __func__);
			goto fail;
		}
		ret = mini_isp_handler_init(ispdata);
		if (0 != ret) {
			print_error("%s mini_isp_handler_init failed", __func__);
			goto fail;
		}
	}
...
int mini_k3_isp_exit(void *par)
{
	...
	if(isp_data.is_using_mini_isp) {
		flush_workqueue(isp_data.mini_isp_work_queue);
		mini_isp_handler_exit(&isp_data);
		if(isp_data.meta_data && isp_data.meta_data->exit)
...
int mini_k3_isp_set_awb_lock( int awb_lock)
{
	...
	camera_agent_set_PreviewAWBLock(isp_data.sensor->sensor_index,(awb_mode_t)awb_lock);
...
{
//  if (zoom != isp_data.zoom_ext.zoom)
//  {
		mini_k3_isp_set_zoom_and_center(preview_running,
									zoom,
									isp_data.zoom_ext.user_center_x,
									isp_data.zoom_ext.user_center_y);
//  }
}

/*
 **************************************************************************
 * FunctionName: mini_k3_isp_set_zoom_center;
 * Description : NA;
 * Input       : NA;
 * Output      : NA;
 * ReturnValue : 0:success  other:failed;
 ...
 **************************************************************************
*/
int mini_k3_isp_set_zoom_center(char preview_running, u32 user_center_xy)
{
	u32 user_center_x = user_center_xy >> 16;
	u32 user_center_y = user_center_xy & 0x0000ffff;

	if ((user_center_x > isp_data.pic_attr[STATE_PREVIEW].out_width)
		|| (user_center_y > isp_data.pic_attr[STATE_PREVIEW].out_height))
	{
		print_warn("overflow.user_center_x=%d,user_center_y=%d.",user_center_x,user_center_y);
		return 0;
	}

	if (user_center_xy != isp_data.zoom_ext.user_center_xy)
	{
		#if 0
		mini_k3_isp_set_zoom_and_center(preview_running,
					isp_data.zoom_ext.zoom,
					(user_center_xy >> 16),
					(user_center_xy & 0x0000ffff));
		#endif

		isp_data.zoom_ext.user_center_xy = user_center_xy;
		isp_data.zoom_ext.user_center_x = user_center_x;
		isp_data.zoom_ext.user_center_y = user_center_y;
		print_info("isp_data.zoom_ext.user_center_xy=%d,isp_data.zoom_ext.user_center_x=%d,isp_data.zoom_ext.user_center_y=%d.",
				isp_data.zoom_ext.user_center_xy,
				isp_data.zoom_ext.user_center_x,
				isp_data.zoom_ext.user_center_y);
...
int mini_k3_isp_set_zoom_and_center(char preview_running, u32 zoom, u32 user_center_x, u32 user_center_y)
{
	int ret = 0;
	mini_zoom_t  zoom_org;
	mini_zoom_t  zoom_new;
	mini_camera_rect_s zoom_new_center_yuv;
	mini_camera_rect_s zoom_new_center_raw;
	u32 scale;
	u32 user_full_view_center_x;
	u32 user_full_view_center_y;
	u32 center_x_min;
	u32 center_y_min;
	u32 center_x_max;
	u32 center_y_max;
	u32 width_before_scale;
	u32 height_before_scale;

	u32 user_full_view_center_x_min;
	u32 user_full_view_center_y_min;
	u32 user_full_view_center_x_max;
	...
	print_debug("mirror:zoom=%d,user_center_x=%d,user_center_y=%d",zoom,user_center_x,user_center_y);

	if ((user_center_x !=0) || (user_center_y !=0))
	{
		print_info("mirror: enter mini_k3_isp_set_zoom_and_center zoom mode");
		/* change zoom center from yuv output view to isp input view */
		zoom_org.zoom = isp_data.zoom_ext.zoom;
		zoom_org.user_full_view_center_x = isp_data.zoom_ext.user_full_view_center_x;
		zoom_org.user_full_view_center_y = isp_data.zoom_ext.user_full_view_center_y;
		zoom_org.center_x = isp_data.zoom_ext.center_x;
		zoom_org.center_y = isp_data.zoom_ext.center_y;
		print_debug("mirror:zoom_org.zoom=%d,zoom_org.user_full_view_center_x=%d,zoom_org.user_full_view_center_y=%d,zoom_org.center_x=%d,zoom_org.center_y=%d",
					zoom_org.zoom,
					zoom_org.user_full_view_center_x,
					zoom_org.user_full_view_center_y,
					zoom_org.center_x,
					zoom_org.center_y);

		/* get min and max value */
		user_full_view_center_x = zoom_org.user_full_view_center_x + user_center_x*10/(zoom_org.zoom+10)-((isp_data.pic_attr[STATE_PREVIEW].out_width / 2)*10)/(zoom_org.zoom+10);
		user_full_view_center_y = zoom_org.user_full_view_center_y + user_center_y*10/(zoom_org.zoom+10)-((isp_data.pic_attr[STATE_PREVIEW].out_height/ 2)*10)/(zoom_org.zoom+10);

		print_debug("mirror:before:zoom_new.user_full_view_center_x=%d,zoom_new.user_full_view_center_y=%d",
				user_full_view_center_x,
				user_full_view_center_y);

		/* check min and max value */
		user_full_view_width_before_scale=((isp_data.pic_attr[STATE_PREVIEW].out_width* 10)/(zoom+10))/2;
		user_full_view_height_before_scale=((isp_data.pic_attr[STATE_PREVIEW].out_height* 10)/(zoom+10))/2;

		print_debug("mirror:user_full_view_width_before_scale=%d,user_full_view_height_before_scale=%d",
				user_full_view_width_before_scale,
				user_full_view_height_before_scale);

		user_full_view_center_x_min=user_full_view_width_before_scale;
		user_full_view_center_y_min=user_full_view_height_before_scale;
		user_full_view_center_x_max=isp_data.pic_attr[STATE_PREVIEW].out_width - user_full_view_width_before_scale;
		user_full_view_center_y_max=isp_data.pic_attr[STATE_PREVIEW].out_height - user_full_view_height_before_scale;

		if (user_full_view_center_x < user_full_view_center_x_min) user_full_view_center_x = user_full_view_center_x_min;
		if (user_full_view_center_x > user_full_view_center_x_max) user_full_view_center_x = user_full_view_center_x_max;
		if (user_full_view_center_y < user_full_view_center_y_min) user_full_view_center_y = user_full_view_center_y_min;
		if (user_full_view_center_y > user_full_view_center_y_max) user_full_view_center_y = user_full_view_center_y_max;
		print_debug("mirror:user_full_view_center_x_min=%d,user_full_view_center_x_max=%d,user_full_view_center_y_min=%d,user_full_view_center_y_max=%d",
						user_full_view_center_x_min,
						user_full_view_center_x_max,
						user_full_view_center_y_min,
						user_full_view_center_y_max);

		print_debug("mirror:after:zoom_new.user_full_view_center_x=%d,zoom_new.user_full_view_center_y=%d",
				user_full_view_center_x,
				user_full_view_center_y);

		zoom_new_center_yuv.left    = 0;
		zoom_new_center_yuv.top     = 0;
		//zoom_new_center_yuv.width   = user_center_x;
		//zoom_new_center_yuv.height  = user_center_y;
		zoom_new_center_yuv.width   = user_full_view_center_x;
		zoom_new_center_yuv.height  = user_full_view_center_y;

		//print_info("mirror:user_center_x=%d,user_center_x=%d",user_center_x,user_center_y);
		mini_k3_isp_yuvrect_to_rawrect2(&zoom_new_center_yuv, &zoom_new_center_raw);
		print_debug("mirror:zoom_new_center_raw.width=%d,zoom_new_center_raw.height=%d",
						zoom_new_center_raw.width,
						zoom_new_center_raw.height);

		print_debug("mirror:isp_data.pic_attr[STATE_PREVIEW].crop_width_for_view_angle=%d,isp_data.pic_attr[STATE_PREVIEW].crop_height_for_view_angle=%d",
						isp_data.pic_attr[STATE_PREVIEW].crop_width_for_view_angle,
						isp_data.pic_attr[STATE_PREVIEW].crop_height_for_view_angle);

		/* get new center in isp input view */
		zoom_new.zoom = zoom;
		zoom_new.user_center_x = (isp_data.pic_attr[STATE_PREVIEW].out_width / 2);//set to zoom view center
		zoom_new.user_center_y = (isp_data.pic_attr[STATE_PREVIEW].out_height / 2);//set to zoom view center
		zoom_new.user_center_xy = ((zoom_new.user_center_x << 16) | zoom_new.user_center_y);
		zoom_new.user_full_view_center_x = user_full_view_center_x;
		zoom_new.user_full_view_center_y = user_full_view_center_y;
		zoom_new.center_x = isp_data.pic_attr[STATE_PREVIEW].in_width / 2 - isp_data.pic_attr[STATE_PREVIEW].crop_width_for_view_angle / 2 + zoom_new_center_raw.width;
		zoom_new.center_y = isp_data.pic_attr[STATE_PREVIEW].in_height / 2 - isp_data.pic_attr[STATE_PREVIEW].crop_height_for_view_angle / 2 + zoom_new_center_raw.height;

		print_debug("mirror:zoom_new.zoom=%d,zoom_new.user_full_view_center_x=%d,zoom_new.user_full_view_center_y=%d,zoom_new.user_center_x=%d,zoom_new.user_center_y=%d,zoom_new.center_x=%d,zoom_new.center_y=%d",
						zoom_new.zoom,
						zoom_new.user_full_view_center_x,
						zoom_new.user_full_view_center_y,
						zoom_new.user_center_x,
						zoom_new.user_center_y,
						zoom_new.center_x,
						zoom_new.center_y);

		/* zoom center overflow check*/
		//scale = k3_isp_get_init_scale();

		width_before_scale=((isp_data.pic_attr[STATE_PREVIEW].crop_width_for_view_angle * 10)/(zoom_new.zoom+10))/2;
		height_before_scale=((isp_data.pic_attr[STATE_PREVIEW].crop_height_for_view_angle * 10)/(zoom_new.zoom+10))/2;

		print_debug("mirror:width_before_scale=%d,height_before_scale=%d",
						width_before_scale,
						height_before_scale);

		//center_x_min=width_before_scale;
		//center_y_min=height_before_scale;
		//center_x_max=isp_data.pic_attr[STATE_PREVIEW].in_width- width_before_scale;
		//center_y_max=isp_data.pic_attr[STATE_PREVIEW].in_height- height_before_scale;

		center_x_min=width_before_scale+4;
		center_y_min=height_before_scale+4;
		center_x_max=isp_data.pic_attr[STATE_PREVIEW].in_width- width_before_scale-4;
		center_y_max=isp_data.pic_attr[STATE_PREVIEW].in_height- height_before_scale-4;

		if (zoom_new.center_x < center_x_min) zoom_new.center_x = center_x_min;
		if (zoom_new.center_x > center_x_max) zoom_new.center_x = center_x_max;
		if (zoom_new.center_y < center_y_min) zoom_new.center_y = center_y_min;
		if (zoom_new.center_y > center_y_max) zoom_new.center_y = center_y_max;
		print_debug("mirror:center_x_min=%d,center_x_max=%d,center_y_min=%d,center_y_max=%d",
						center_x_min,
						center_x_max,
						center_y_min,
						center_y_max);
		...
		isp_data.zoom_ext.user_center_xy = zoom_new.user_center_xy;
		isp_data.zoom_ext.user_center_x = zoom_new.user_center_x;
		isp_data.zoom_ext.user_center_y = zoom_new.user_center_y;
		isp_data.zoom_ext.user_full_view_center_x = zoom_new.user_full_view_center_x;
		isp_data.zoom_ext.user_full_view_center_y = zoom_new.user_full_view_center_y;
		isp_data.zoom_ext.center_x = zoom_new.center_x;
		isp_data.zoom_ext.center_y = zoom_new.center_y;

		print_debug("mirror:isp_data.zoom_ext.zoom=%d,isp_data.zoom_ext.center_x=%d,isp_data.zoom_ext.center_y=%d",isp_data.zoom_ext.zoom,isp_data.zoom_ext.center_x,isp_data.zoom_ext.center_y);
	}
...
int mini_k3_isp_set_ae_lock(int mode)
{
	...
	ret = camera_agent_set_PreviewAELock(isp_data.sensor->sensor_index,(aecagc_mode_t)mode);
...
static void k3_isp_set_default(void)
{
	...
	isp_data.zoom_ext.user_center_xy      = 0;
	isp_data.sensor				= NULL;
	...
	isp_data.hdr_shoot_enable = false;
	isp_data.hdr_frame_index  = 0;
	INIT_WORK(&isp_data.hdr_process_work, mini_isp_hdr_process);
	...
	isp_data.mini_isp_work_queue = NULL;
	...
	isp_data.mini_isp_flash_state = FLASH_OFF;
	isp_data.mini_isp_flash_flow = FLASH_DONE;
	isp_data.flash_off_drop_frame_num = 0;
