...
int hwa_set_cap_iso_ratio_value(int ratio)
{
	...
	if (1 == ratio) // reset digital gain to 1x
	{
		if (sensor->set_gain)
		{
			sensor->set_gain(0);
...
void hwa_calc_zoom_with_crop(camera_state state, scale_strategy_t scale_strategy, u32 *in_width, u32 *in_height)
{
	...
	int out_width = attr->out_width * 10 / (this_ispdata->zoom + 10) + hwa_hwscope.border * 2;
	out_width = (out_width + 16) & ~31;
	int out_height = attr->out_height * 10 / (this_ispdata->zoom + 10) + hwa_hwscope.border * 2;
	/*
	ov Kevin:
	Format NV12 or NV21
		a = ceil (width/256)
