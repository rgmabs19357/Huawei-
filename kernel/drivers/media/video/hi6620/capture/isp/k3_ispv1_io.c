...
int ispv1_csi_poweron(csi_index_t index)
{
    ...
    if (0 != get_cam_pwdn_status(DEBUGFS_PWDN_CSI)) {
    ...
    set_cam_pwdn_status(DEBUGFS_PWDN_CSI, 1);
...
void ispv1_csi_poweroff(csi_index_t index)
{
    ...
    set_cam_pwdn_status(DEBUGFS_PWDN_CSI, 0);
}
...
int ispv1_init_csi(csi_index_t index, csi_lane_t mipi_lane_count, u8 lane_clk)
{
    ...
    print_info("%s, csi debugfs pwdn config = %d, csi pwdn status = %d\n",
        __func__, get_cam_dbgfs_pwdncfg(), get_cam_pwdn_status(DEBUGFS_PWDN_CSI));
