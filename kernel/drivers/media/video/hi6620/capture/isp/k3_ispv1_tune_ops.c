...
void ispv1_switch_dns(camera_sensor *sensor, camera_state state, bool flash_on, u32 expo_line)
{
    ...
    unsigned uv_dns_base;
    unsigned raw_dns_sigma_base;
    unsigned raw_dns_gsl_base;
    unsigned raw_dns_rbsl_base;
    int idx;
    ...
    if(NULL == sensor->effect || CAMERA_USE_SENSORISP == sensor->isp_location){
        print_warn("sensor has no effect");
        return;
    }
    if(CAMERA_ZSL_OFF == this_ispdata->zsl_ctrl.zsl_state){
        dns = &(effect->dns.zsl_off);
        ...
        uv_dns_base = REG_ISP_PRE_UVDNS_BASE;

        raw_dns_sigma_base   = REG_ISP_PRE_RAWDNS_SIGMA_BASE;
        raw_dns_gsl_base     = REG_ISP_PRE_RAWDNS_GSL_BASE;
        raw_dns_rbsl_base    = REG_ISP_PRE_RAWDNS_RBSL_BASE;
    }else{
        dns = &(effect->dns.zsl_on);
        ...
        if(STATE_PREVIEW == state){
            ...
            uv_dns_base = REG_ISP_CAP_UVDNS_BASE;

            raw_dns_sigma_base = REG_ISP_CAP_RAWDNS_SIGMA_BASE;
            raw_dns_gsl_base     = REG_ISP_CAP_RAWDNS_GSL_BASE;
            raw_dns_rbsl_base    = REG_ISP_CAP_RAWDNS_RBSL_BASE;                   
            }
        }
    for(idx = 0; idx < DNS_MAX_STEP; idx++){
        SETREG8((uv_dns_base + idx), uv_dns[idx]);
        SETREG8((raw_dns_sigma_base + idx), sigma_dns_value[idx]);
        SETREG8((raw_dns_gsl_base + idx), raw_dns->gsl_coff[idx]);
        SETREG8((raw_dns_rbsl_base + idx), raw_dns->rbsl_coff[idx]);
    }
}
...
void ispv1_mannal_iso_dynamic_framerate(camera_sensor *sensor)
{
	...
	camera_frame_rate_state state = ispv1_get_frame_rate_state();
       ...
       u32 max_expo;
	u32 cur_expo = (ispv1_get_expo_line() >> 4);
