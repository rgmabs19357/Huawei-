...
static int ispv1_setreg_vcm_DLC_mode()
{
	...
	if ((vcm->moveLensAddr[0] == 0x0) && (vcm->moveLensAddr[1] == 0x0)) {
		...
		if (vcm->vcm_type == VCM_DW9714) {
			...
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xF260, I2C_16BIT, SCCB_BUS_MUTEX_WAIT);
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xA10D, I2C_16BIT, SCCB_BUS_MUTEX_WAIT);
		} else if (vcm->vcm_type == VCM_DW9714_Liteon) {
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xF208, I2C_16BIT, SCCB_BUS_MUTEX_WAIT);
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xA10D, I2C_16BIT, SCCB_BUS_MUTEX_WAIT);
		}
		ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xDC51, I2C_16BIT, SCCB_BUS_MUTEX_WAIT);
	} else {
...
static int ispv1_set_focus_area_done(focus_area_s *area, u32 zoom)
{
	...
	ret = k3_isp_yuvrect_to_rawrect(&yuv_in_full, &raw_rect);
		if (ret) {
			print_error("%s, line %d: error", __func__, __LINE__);
			return ret;
		}
...
static void ispv1_af_flash_check_open(void)
{
	...
	u8 frame_count = 0;
	if (get_focus_mode() == CAMERA_FOCUS_CAF_FORCE_AF)
		return;
...
bool ispv1_judge_skip_frame(void)
{
	...
	if ((sensor->fps > MIN_FRAME_RATE) && (stable_expo_ms <= (expo_time_ms / 2))) {
		ret = true;
	} else {
		ret = false;
	}
...
void ispv1_k3focus_run(void)
{
	...
	pos_info large_contrast_array[30];
	u32 large_curr = 0;
	pos_info large_top;
	...
	while ((schedule_case = ispv1_focus_need_schedule()) != -1) {
		...
		case FOCUS_SCHEDULE_CASE_VAF_MOVE:
			if (get_focus_stage() == AF_RUN_STAGE_FINE) {
				if (skip_frame == true) {
					if (trip->step_cnt == 0)
						frame_count += 2;
...
static void ispv1_af_range_cal(af_run_param *af_info, pos_info *curr, af_run_stage next_stage)
{
	...
	bool skip_frame = ispv1_judge_skip_frame();
	int large_stride = get_af_large_step(vcm->coarseStep, vcm->normalStrideRatio);
	int strideSeparator = vcm->offsetInit + vcm->strideOffset;
	strideSeparator += ((vcm->fullRange - strideSeparator) % large_stride);
	...
	case AF_RUN_STAGE_FINE:
		if (trip->direction == AF_RUN_DIRECTION_BACKWARD) {
			/*
			 * Modified for calculating the reasonable range in fine search stage by w00223743.
			 * Enlarge the range when skip_frame is true. As coarse search does not skip frame, algorithm should
			 * ensure the vcm can stop in the right position.
			 */
			if (skip_frame && top->code > strideSeparator) {
				trip->start_pos = top->code + (vcm->coarseStep << 1);
				trip->end_pos = top->code - (vcm->coarseStep << 1);
			} else if (skip_frame) {
				trip->start_pos = top->code + vcm->coarseStep;
				trip->end_pos = top->code - vcm->coarseStep;
			} else {
		...
		/*
		 * Modified for accompanying the enlarged range by w00223743.
		 */
		if (skip_frame && top->code > strideSeparator) {
			trip->step = vcm->coarseStep;
		} else {
