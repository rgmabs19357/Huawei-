...
static int ispv1_update_addr(u32 phyaddr,u32 statphyaddr,uv_offset *puvoffset);
...
#else
	reg5 |= ISP_SENSOR_INT_ENABLE | ISP_MACINT_SET_ENABLE | ISP_EXPO_RATIO_ENABLE;
#endif
	ispv1_config_idi(pic_attr->in_fmt, is_bypass_isp, &reg3c13, &reg3c15);

    /* init IDI */
    CMD_SET_IDI_CTRL(this_ispdata->idi);
            ...
            buffer_arr_t            *buf_arr)
{
	...
	SETREG16(ISP_IDI_CONTROL, this_ispdata->idi);
			     ...
			     camera_sensor *sensor, bool cold_boot, camera_scene scene)
{
	...
	*/
	...
	if (cold_boot == true)
		...
		reg5 = ISP_SENSOR_INT_ENABLE | ISP_MACINT_SET_DISABLE | ISP_EXPO_RATIO_ENABLE;
#endif
    ...
    print_info("idi=%d.",this_ispdata->idi);
    CMD_SET_IDI_CTRL(this_ispdata->idi);
...
int ispv1_poweron(void)
{
    ...
    if (0 == get_cam_pwdn_status(DEBUGFS_PWDN_ISP)) {
        ...
        if (clk_set_parent(isp_hw_data.isp_gated_clk, isp_hw_data.dsppll_clk)) {
    	...
    	if (chip_id == CS_CHIP_ID) {
    		...
    		if (clk_set_rate(isp_hw_data.isp_clk, ISP_CLOCK)) {
    		...
    		if (clk_set_rate(isp_hw_data.ispmipi_clk, ISP_MIPI_CLOCK)) {
    		...
    		if (isp_hw_data.isp_csi_clk == NULL || clk_enable(isp_hw_data.isp_csi_clk)) {
    			print_error("Error: %s, isp APB MUX clk enable failed", __func__);
    			return -EFAULT;
    		}

    		if (isp_hw_data.isp_hclk == NULL || clk_enable(isp_hw_data.isp_hclk)) {
        ...
        set_cam_pwdn_status(DEBUGFS_PWDN_ISP, 1);
    }

    print_info("%s, isp debugfs pwdn config = %d, isp pwdn status = %d\n",
        __func__, get_cam_dbgfs_pwdncfg(), get_cam_pwdn_status(DEBUGFS_PWDN_ISP));
    ...
    if (0 != get_cam_dbgfs_pwdncfg()) {
    	...
    	if (chip_id == CS_CHIP_ID) {
    		...
    		if (isp_hw_data.isp_csi_clk) {
    			clk_disable(isp_hw_data.isp_csi_clk);
    		}

    		if (isp_hw_data.isp_hclk) {
    			clk_disable(isp_hw_data.isp_hclk);
    		}
    	}
    	...
    	if (isp_hw_data.inter_ldo) {
    		rc = regulator_disable(isp_hw_data.inter_ldo);
    		if (rc)
        	{
        ...
        set_cam_pwdn_status(DEBUGFS_PWDN_ISP, 0);
    }
...
static int ispv1_frame_done_handler(camera_state state)
{
	...
	if ((isp_hw_data.frame_count == 5) && (state == STATE_PREVIEW)) {
		if ((FLASH_TESTING != this_ispdata->flash_flow) && (FLASH_FROZEN != this_ispdata->flash_flow)) {
			...
			{
				if (0 == this_ispdata->alc_awb_status)
				{
...
void zsl_stop_capture_with_preview_cmd_handler(void);

#define NR_FRAME_SKIP_TO_WAIT_FOR_WB	3
...
void zsl_stop_capture_with_preview_cmd_handler(void)
{
	...
	delta_jiffies = jiffies - delta_jiffies;
	print_info("@@@### %dms ellapsed since capture flash turned on", jiffies_to_msecs(delta_jiffies));
	this_ispdata->zsl_ctrl.history_buf_idx = GETREG8(ISP_CAP_STREAM_BUF_REG) & ISP_CAP_STREAM_BUF_MASK;
	print_info("want back idx=%d.ret buf idx=%d.",this_ispdata->zsl_ctrl.zsl_back_idx,this_ispdata->zsl_ctrl.history_buf_idx);
...
void zsl_wait_to_start_capture_with_flash(void)
{
	...
	if(true == this_ispdata->flash_on)
	{
		...
		if ((this_ispdata->sensor->sensor_index == CAMERA_SENSOR_PRIMARY) && (flashlight != NULL))
		{
			...
			{
				flashlight->turn_off();
				if (FLASH_PREFLASH_DONE != this_ispdata->flash_flow)
				{
					print_info("@@@### preflash not ready");
				}
			}
		}
		//set manual gc/ec/wb
...
static irqreturn_t ispv1_isr(int irq, void *dev_id)
{
	...
	cmd_id = ioread8_mutex((isp_hw_data.base + (COMMAND_FINISHED)));//GETREG8(COMMAND_FINISHED);

	if (irq_status_tmp & ISP_COMMAND_DONE_INT){
		if (cmd_id > 0xf0){
			cmd_id &= 0x0f;
			isp_hw_data.irq_val.mcu_cmd_finish|= mask_table[cmd_id - 1];
		}else if(cmd_id != 0){
			isp_hw_data.irq_val.mcu_cmd_finish |= (1 << cmd_id);
		}
		else{
		}
	}
...
static int ispv1_hw_init(struct platform_device *pdev, data_queue_t* data_queue)
{
	...
	if (chip_id == CS_CHIP_ID) {
		...
		isp_hw_data.isp_csi_clk = camera_csi_clk;
		...
		isp_hw_data.dsppll_clk = dsppll_clk;
		...
		isp_hw_data.isp_gated_clk = isp_gated_clk;
	}
...
fail:
	...
	if (chip_id == CS_CHIP_ID) {
		...
		if (isp_hw_data.dsppll_clk) {
			clk_put(isp_hw_data.dsppll_clk);
			isp_hw_data.dsppll_clk = NULL;
		}

		if (isp_hw_data.isp_gated_clk) {
			clk_put(isp_hw_data.isp_gated_clk);
			isp_hw_data.isp_gated_clk = NULL;
		}
	}
