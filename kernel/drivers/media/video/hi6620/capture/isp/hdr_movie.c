...
static int ispv1_load_sensor_setting(camera_sensor *sensor,char *filename);//for hdr_movie debug
...
static void ispv1_set_sensor_awb_gain(void)
{
	...
	if (this_ispdata->sensor->sensor_hdr_movie.set_awb_gain)
	{
		...
		this_ispdata->sensor->sensor_hdr_movie.set_awb_gain(b_gain, gb_gain, gr_gain, r_gain);
	}
}
