...
#define DDR_VALID_MAX_FREQ_PATH "/sys/devices/system/cpu/cpu0/cpufreq/scaling_available_ddrfrequencies"
...
void k3_isp_lock_ddr_freq(void)
{
    int  ddr_freq;

    ddr_freq = get_ddr_valid_maxfreq();
    if (ddr_freq <= 0) {
...
void k3_isp_cancel_cpuidle_vote(void)
{
