...
static int inter_init_zoomcoef(VPP_ZOOM_COEF_ADDR_S *pstZmeModule)
{
    ...
    if (true != get_hw_config_int("memory_config/vpp", &size, NULL))
    {
        logi("get_hw_config_int failed\n");
        return K3_FAILURE;
    }

    if (0 == size) {
        logi("vpp memory size is 0.\n");
        return K3_FAILURE;
    }
    ...
    pstZmeModule->pStartVirAddr
            = (unsigned int*)ioremap(VPP_RESERVED_MEM_ADDR, PAGE_ALIGN(size * SZ_1K));
    ...
    memset(pstZmeModule->pStartVirAddr, 0, PAGE_ALIGN(size * SZ_1K));
...
{
    BUG_ON(NULL == pstZmeModule);
    logi("enter inter_deinit_zoomcoef\n");
...
static int inter_init_dieinfo(VPP_DIE_ADDR_S *pstDieModule)
{
    ...
    logi("die addr, die_BusAddr = %#x, die_VirtAddr = %#x\n"
...
{
    BUG_ON(NULL == pstDieModule);
    logi("enter inter_deinit_dieinfo\n");
...
int inter_start(VPP_CONFIG_S* pVppConfig)
{
    ...
    if (k3_true == pVppConfig->bincrop_enable)
    {
        logi("crop enable\n");
