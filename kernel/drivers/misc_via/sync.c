...
static spinlock_t cbp_in_sdio_trans_spin = SPIN_LOCK_UNLOCKED;
...
extern int register_cbp_notifier(struct notifier_block*);
extern int unregister_cbp_notifier(struct notifier_block*);

static DECLARE_WAIT_QUEUE_HEAD(cbp_host_wake_wq);

/*0: need wait host wake, 1: just clock is ok*/
static int cbp_wait_event = 1;
static int cbp_host_event(struct notifier_block *this, unsigned long event, void *ptr)
{
/*event 1: resume, event 0: suspend*/
	pr_info("In Event 1: Event Number is %d\n", event);
	if (event == 0){
		cbp_wait_event = 0;
	}else if (event == 1){
		cbp_wait_event = 1;
	...
	msleep(500);
	wake_up(&cbp_host_wake_wq);
	return 0;
}

static struct notifier_block cbp_host_notifier =
{
    .notifier_call = cbp_host_event,
};

void VIA_trigger_signal(int i_on)
{
	pr_info("%s %d\n", __func__, __LINE__);
	/*do not disable clock when ap wake*/
	if (!i_on){
		pr_info("%s %d\n", __func__, __LINE__);
		return;
	}
	if (cbp_wait_event == 1){
		if (cbp_clock_enabled ==1){
			pr_info("%s %d\n", __func__, __LINE__);
			return;
		}
	}
	wait_event(cbp_host_wake_wq,  (cbp_host_waked == 1));
	pr_info("%s %d\n", __func__, __LINE__);
}
...
static irqreturn_t asc_irq_cp_wake_ap(int irq, void *data)
{
    ...
    if(level == cfg->polar){
        spin_lock_irqsave(&cbp_in_sdio_trans_spin, irq_flags);
        cbp_in_sdio_tras = 1;
        spin_unlock_irqrestore(&cbp_in_sdio_trans_spin, irq_flags);
...
{
    pr_info("%s  need set %d set ap_wake_cp wake\n", __func__, tx->cfg.gpio_wake);
...
{
    pr_info("%s  need set %d set ap_wake_cp sleep\n", __func__, tx->cfg.gpio_wake);
    if(tx->cfg.gpio_wake >= 0)
        oem_gpio_direction_output(tx->cfg.gpio_wake, !tx->cfg.polar);
}
...
static int __init asc_init(void)
{
    ...
    ret = register_cbp_notifier(&cbp_host_notifier);
    if (ret){
        pr_info("register test_notifier1 error\n");
        goto err_create_kobj;
    }
