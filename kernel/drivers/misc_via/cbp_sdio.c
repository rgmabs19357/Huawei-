...
static irqreturn_t gpio_irq_flow_ctrl(int irq, void *data)
{
	...
	if(level == cbp_flow_ctrl->wait_polar){
		...
		LOGPRT(LOG_DEBUG,  "%s: flow control is disable, can write now!\n", __func__);
...
/*if you need to wake sdio host and system up by using via's four wire sync mechanis,
*  please define WAKE_HOST_BY_SYNC  in Makefile;
*  if sdio host's data1 can wake sdio host and system up,please don't.
...
static int __devinit cbp_probe(struct platform_device *pdev)
{
	...
	ret = oem_get_gpio_number(CBP_VIATEL_ACK_STR,&plat->gpio_data_ack);
	if (ret)
		goto out;
	pr_err(">>>> %s get ack gpio %d\n", __func__, plat->gpio_data_ack);
	...
	ret = oem_get_gpio_number(CBP_VIATEL_FLOW_CTR_STR,&plat->gpio_flow_ctrl);
	if (ret)
		goto out;
	pr_info(">>>> %s get flow ctrl gpio %d\n", __func__, plat->gpio_flow_ctrl);
	...
	ret = oem_get_gpio_number(CBP_VIATEL_RST_IND_STR,&plat->gpio_rst_ind);
	if (ret)
		goto out;
	...
	ret = oem_get_gpio_number(CBP_VIATEL_AP_WK_MDM_STR,&plat->gpio_ap_wkup_cp);
	if (ret)
		goto out;
	pr_err(">>>> %s get gpio %d\n", __func__, plat->gpio_ap_wkup_cp);

	ret = oem_get_gpio_number(CBP_VIATEL_MDM_RDY_STR,&plat->gpio_cp_ready);
	if (ret)
		goto out;
	pr_err(">>>> %s get gpio %d\n", __func__, plat->gpio_cp_ready);

	ret = oem_get_gpio_number(CBP_VIATEL_MDM_WK_AP_STR,&plat->gpio_cp_wkup_ap);
	if (ret)
		goto out;
	pr_err(">>>> %s get gpio %d\n", __func__, plat->gpio_cp_wkup_ap);

	ret = oem_get_gpio_number(CBP_VIATEL_AP_RDY_STR,&plat->gpio_ap_ready);
	if (ret)
		goto out;
...
MODULE_DESCRIPTION("Via CBP SDIO driver");
