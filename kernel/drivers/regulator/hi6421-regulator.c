...
{
    return 1;
	rdev_info(dev, "hi6421_regulator_is_enabled func is called\n");
    #if 0
	int  regulator_id;
	u32 val;
...
{
    return 0;
	rdev_info(dev, "hi6421_regulator_enabled func is called\n");
    #if 0
...
{
    return 0;
    rdev_info(dev, "hi6421_regulator_disable func is called\n");
    #if 0
	int  regulator_id;
	struct hi6421_regulator_data *hi6421_regulator_data = rdev_get_drvdata(dev);
	regulator_id =  rdev_get_id(dev);
...
{
	#if 0
        rdev_info(dev, "hi6421_regulator_list_voltage func is called\n");
        #endif
	int  regulator_id;
	regulator_id =  rdev_get_id(dev);
...
{
	#if 0
        rdev_info(dev, "hi6421_regulator_get_voltage func is called\n");
        #endif
...
{
    return 0;
    rdev_info(dev, "hi6421_regulator_set_voltage func is called\n");
    #if 0
	struct hi6421_regulator_data *hi6421_regulator_data = rdev_get_drvdata(dev);
...
static unsigned int hi6421_regulator_get_mode(struct regulator_dev *dev)
{
	...
	rdev_info(dev, "hi6421_regulator_get_mode func is called\n");
    #if 0
	u32 val;
	struct hi6421_regulator_data *hi6421_regulator_data = rdev_get_drvdata(dev);
	int regulator_id = rdev_get_id(dev);
...
{
    return 0;
	rdev_info(dev, "hi6421_regulator_set_mode func is called\n");
    #if 0
	struct hi6421_regulator_data *hi6421_regulator_data = rdev_get_drvdata(dev);
	int regulator_id = rdev_get_id(dev);
	/*LDO0-LDO20(HI6421_LDO_ENTRY_ECO_MODE); BUCK0-BUCK5(HI6421_BUCK_ENTRY_ECO_MODE) ;
...
{
    return REGULATOR_MODE_NORMAL;
	rdev_info(dev, "hi6421_regulator_get_optimum_mode func is called\n");
    #if 0
	int regulator_id = rdev_get_id(dev);
...
#ifdef (CONFIG_LCD_TOSHIBA_MDW70 || CONFIG_LCD_JDI_OTM1282B)
static int hi_lcdio_regulator_enable(struct regulator_dev *dev)
{
    /*2¨´¡Á¡ÂGPIO */
    gpio_request(GPIO_5_2, "lcdio-vcc");
    gpio_direction_output(GPIO_5_2, 0);
    gpio_free(GPIO_5_2);

    return 0;
}

static int hi_lcdio_regulator_disable(struct regulator_dev *dev)
{
    /*2¨´¡Á¡ÂGPIO*/
    gpio_request(GPIO_5_2, "lcdio-vcc");
    gpio_direction_output(GPIO_5_2, 1);
    gpio_free(GPIO_5_2);

    return 0;
}

static struct regulator_ops hi_lcdio_vcc_ldo_ops = {
	.is_enabled = hi6421_regulator_is_enabled,
	.enable = hi_lcdio_regulator_enable,
	.disable = hi_lcdio_regulator_disable,
	.list_voltage = hi6421_regulator_list_voltage,
	.get_voltage = hi6421_regulator_get_voltage,
	.set_voltage = hi6421_regulator_set_voltage,
	.get_mode = hi6421_regulator_get_mode,
...
static int hi_lcdanalog_regulator_enable(struct regulator_dev *dev)
{
    /*2¨´¡Á¡ÂGPIO */
    gpio_request(GPIO_2_6, "lcdanalog-vcc");
    gpio_direction_output(GPIO_2_6, 0);
    gpio_free(GPIO_2_6);

    return 0;
}

static int hi_lcdanalog_regulator_disable(struct regulator_dev *dev)
{
    /*2¨´¡Á¡ÂGPIO*/
    gpio_request(GPIO_2_6, "lcdanalog-vcc");
    gpio_direction_output(GPIO_2_6, 1);
    gpio_free(GPIO_2_6);

    return 0;
}

static struct regulator_ops hi_lcdanalog_vcc_ldo_ops = {
	.is_enabled = hi6421_regulator_is_enabled,
	.enable = hi_lcdanalog_regulator_enable,
	.disable = hi_lcdanalog_regulator_disable,
	.list_voltage = hi6421_regulator_list_voltage,
	.get_voltage = hi6421_regulator_get_voltage,
	.set_voltage = hi6421_regulator_set_voltage,
	.get_mode = hi6421_regulator_get_mode,
...
{
        return 1;
	rdev_info(dev, "hi6421_chg_pump_is_enabled func is called\n");
    #if 0
	int  regulator_id;
	u32 val;
	struct hi6421_regulator_data *hi6421_regulator_data = rdev_get_drvdata(dev);
...
{
        return 0;
	rdev_info(dev, "hi6421_chg_pump_enable func is called\n");
    #if 0
	int  regulator_id;
	struct hi6421_regulator_data *hi6421_regulator_data = rdev_get_drvdata(dev);
	regulator_id =  rdev_get_id(dev);
...
{
    return 0;
	rdev_info(dev, "hi6421_chg_pump_disable func is called\n");
    #if 0
	int  regulator_id;
	struct hi6421_regulator_data *hi6421_regulator_data = rdev_get_drvdata(dev);
	regulator_id =  rdev_get_id(dev);
...
#ifdef CONFIG_LCD_SAMSUNG_LMS350DF04
	[HI6421_LDO16] = {
		.name = "LDO16",
		.id = HI6421_LDO16,
		.ops = &hi6421_ldo_ops,
...
#ifdef _DRV_LLT_
int hi6421_regulator_probe(struct platform_device *pdev)
#else
	...
	for (regulator_id = 0; regulator_id < ARRAY_SIZE(regulators); regulator_id++) {
		...
		if (IS_ERR(hi6421_regulator_data->rdev[regulator_id])) {
			...
			//ret = PTR_ERR(hi6421_regulator_data->rdev[regulator_id]);
                        printk("regualtor_id %d,, errno %d", regulator_id, hi6421_regulator_data->rdev[regulator_id]);
