    ...
    struct device_attribute *attr, char *buf, size_t count)
{
    ...
    mutex_lock(&rdev->mutex);
    _regulator_do_set_voltage(rdev,val,val);
    mutex_unlock(&rdev->mutex);
    return count;
}

static DEVICE_ATTR(microvolts, 0444, regulator_uV_show, regulator_uV_set);
...
int regulator_force_disable(struct regulator *regulator)
{
	...
	if (supply_rdev) {
		ret = regulator_disable(get_device_regulator(rdev_get_dev(supply_rdev)));
		if (ret) {
			printk(KERN_ERR "%s: failed to disabled regulator !\n", __func__);
		}
	}
...
err:
	...
	for (--i; i >= 0; --i) {
		...
		if (ret) {
			printk(KERN_ERR "%s: failed to disable regulator !\n", __func__);
			return ret;
		}
	}
	return ret;
...
static int add_regulator_attributes(struct regulator_dev *rdev)
{
	...
	if (ops->get_voltage || ops->get_voltage_sel) {
        if(NULL != strstr(saved_command_line,"androidboot.swtype=factory")) {
            dev_attr_microvolts.attr.mode = 0666;
        }
