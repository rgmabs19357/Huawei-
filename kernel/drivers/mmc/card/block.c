...
extern int mmc_blk_issue_packed_rq(struct mmc_queue *mq, struct request *req);
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if (rq_is_packed(req))  {
		int ret =  mmc_blk_issue_packed_rq(mq, req);
		return ret;
	}
	...
	do {
		...
		if (!mmc_host_is_spi(card->host) && rq_data_dir(req) != READ) {
			...
			do {
				...
				if(status & R1_ERROR){
					printk(KERN_ERR "CMD13 return error for TOSHIBA emmc, need retry.\n");
					gen_err = 1;
				}
				/*
...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
    ...
    ms_host = mmc_priv(card->host);

    if( (MMC_TYPE_MMC == card->type)&&(ms_host->quirks & MSHCI_QUIRK_EMMC_INIT_FINISH))
    {
...
static struct mmc_blk_data *mmc_blk_alloc(struct mmc_card *card)
{
	...
	if (card->host->caps2 & MMC_CAP2_PACKED_CMD) {
		info = &md->queue.packed_cmd_info;
		memset(info, 0, sizeof(struct packed_cmd_info));

		page = alloc_page(GFP_KERNEL);
		if (!page)
			goto packed_cmd_nomem;

		info->packed_read_requests = kmalloc(sizeof(struct request*) * card->ext_csd.max_packed_reads, GFP_KERNEL);
		info->packed_read_capacity = card->ext_csd.max_packed_reads;
		if (!info->packed_read_requests)
			goto packed_cmd_nomem;

		info->packed_write_requests = kmalloc(sizeof(struct request*) * card->ext_csd.max_packed_writes, GFP_KERNEL);
		info->packed_write_capacity = card->ext_csd.max_packed_writes;
		if (!info->packed_write_requests)
			goto packed_cmd_nomem;

		info->unpacked_requests = kmalloc(sizeof(struct request*) * MIN_PACKED_CMD_THRESHOLD, GFP_KERNEL);
		if (!info->unpacked_requests)
			goto packed_cmd_nomem;

		md->flags |= MMC_BLK_PACKED_CMD;
		md->queue.flags |= MMC_QUEUE_PACKED_CMD;
		info->hdr = page_address(page);

		return md;

packed_cmd_nomem:
		printk(KERN_ERR "error, packed command no memory, disable packed cmd\n");
		md->flags &= ~MMC_BLK_PACKED_CMD;
		md->queue.flags &= ~MMC_QUEUE_PACKED_CMD;
		...
		if (info->packed_read_requests)
			kfree(info->packed_read_requests);

		if (info->packed_write_requests)
			kfree(info->packed_write_requests);
	}
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if (MMC_TYPE_MMC == card->type){
        ms_host->quirks |= MSHCI_QUIRK_EMMC_INIT_FINISH;
	}
#endif
	mutex_unlock(&mmcpart_table_mutex);
    ...
    if(MMC_TYPE_MMC == card->type)
	{
      ...
      err = sys_mount("/dev/block/mmcblk0p11","/mnvm1:0","ext4", MS_NOATIME | MS_NOSUID | MS_NODEV, NULL);
	  if (err)
      {
         printk("module_init mount mmcblk0p11 fail\n");
	  }

	  /*NV2*/
      err = sys_mknod("/dev/block/mmcblk0p12",S_IFBLK|S_IRWXU|S_IRWXG|S_IRWXO,0xB30C);
      if(err < 0)
      {
         printk("module_init mknod mmcblk0p12 fail\n");
      }
      err = sys_mount("/dev/block/mmcblk0p12","/mnvm2:0","ext4", MS_NOATIME | MS_NOSUID | MS_NODEV, NULL);
	  if (err)
      {
         printk("module_init mount mmcblk0p12 fail\n");
	  }

      err = sys_mknod("/dev/block/mmcblk0p16",S_IFBLK|S_IRWXU|S_IRWXG|S_IRWXO,0xB310);
      if(err < 0)
      {
         printk("module_init mknod mmcblk0p16 fail\n");
      }
      err = sys_mount("/dev/block/mmcblk0p16","/mnvm3:0","ext4", MS_NOATIME | MS_NOSUID | MS_NODEV, NULL);
	  if (err)
      {
         printk("module_init mount mmcblk0p16 fail\n");
	  }

	  /*modemnvbkup*/
      err = sys_mknod("/dev/block/mmcblk0p7",S_IFBLK|S_IRWXU|S_IRWXG|S_IRWXO,0xB307);
      if(err < 0)
      {
         printk("module_init mknod mmcblk0p7 fail\n");
      }

      err = sys_mknod("/dev/block/mmcblk0p14",S_IFBLK|S_IRWXU|S_IRWXG|S_IRWXO,0xB30E);
      if(err < 0)
      {
         printk("module_init mknod mmcblk0p14 fail\n");
      }

      /* for ccore reset*/
      err = sys_mknod("/dev/block/mmcblk0p13",S_IFBLK|S_IRWXU|S_IRWXG|S_IRWXO,0xB30D);
      if(err < 0)
      {
         printk("module_init mknod mmcblk0p13 fail\n");
      }
      err = sys_mknod("/dev/block/mmcblk0p15",S_IFBLK|S_IRWXU|S_IRWXG|S_IRWXO,0xB30F);
      if(err < 0)
      {
         printk("module_init mknod mmcblk0p15 fail\n");
      }
