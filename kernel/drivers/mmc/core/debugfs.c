...
static int mmc_ios_show(struct seq_file *s, void *data)
{
	...
	case MMC_TIMING_UHS_SDR50:
	    str = "mmc high-speed uhs_sdr50";
		break;
	case MMC_TIMING_UHS_SDR104:
	    str = "mmc high-speed uhs_sdr104";
		break;
	case MMC_TIMING_UHS_DDR50:
	    str = "mmc high-speed ddr_50";
		break;
...
void mmc_add_card_debugfs(struct mmc_card *card)
{
	...
	sdxc_root = debugfs_create_dir("type", host->debugfs_root);
	if (IS_ERR(sdxc_root))
		return;
	if (!sdxc_root)
		goto sdxc_err;
...
sdxc_err:
	...
	dev_err(&card->dev, "failed to initialize sdxc debugfs\n");
}
