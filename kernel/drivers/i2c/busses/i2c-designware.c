...
int debug_on = 0;
void test_acpuidle_i2c_debug_on(int on)
{
    debug_on = on;
}
...
{
	struct i2c_dw_data *plat = dev->dev->platform_data;
	...
	if (!plat || !plat->dma_filter || !plat->dma_tx_param) {
...
static void reset_i2c_controller(struct dw_i2c_dev *dev)
{
	...
	if (plat && plat->exit)
		plat->exit();
	//plat->reset();
	if (plat && plat->reset_controller)
		plat->reset_controller();
	i2c_dw_init(dev);
	if (plat && plat->init)
		plat->init();
}
...
i2c_dw_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	pm_qos_debug_trace(pdev->id, "add request PM_QOS_CPU_INT_LATENCY");
	pm_qos_add_request(&plat->pm_qos_req, PM_QOS_CPU_INT_LATENCY, I2C_QOS+pdev->id);
...
/*#endif*/
	...
	pm_qos_debug_trace(pdev->id, "remove request PM_QOS_CPU_INT_LATENCY");
	pm_qos_remove_request(&plat->pm_qos_req);
...
static int __devinit dw_i2c_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(dev->clk)) {
		r = -ENODEV;
		dev_err(&pdev->dev, "dw_i2c_probe: dev->clk is error\n");
		goto err_free_mem;
	}
	clk_enable(dev->clk);

	plat = dev->dev->platform_data;

	plat->pm_qos_req.pm_qos_class = 0;
	
    if (plat->iomux_name){
    	/* Set iomux NORMAL, If set iomux failed, we still go on */
    	dev->iomux_block  = iomux_get_block(plat->iomux_name);
    	if (!dev->iomux_block){
    		dev_err(&pdev->dev, "Get i2c iomux_block %s failed\n", plat->iomux_name);
		    goto err_unuse_clocks;
        }

    	dev->block_config = iomux_get_blockconfig(plat->iomux_name);
    	if (!dev->block_config){
    		dev_err(&pdev->dev, "Get i2c block_config %s failed\n", plat->iomux_name);
		    goto err_no_iomux;
        }

		ret = blockmux_set(dev->iomux_block, dev->block_config, NORMAL);
		if (ret){
			dev_err(&pdev->dev, "Set %s iomux to NORMAL failed, ret=%d\n", plat->iomux_name, ret);
  		    goto err_no_iomux;
        }
    }
...
/*#if defined(CONFIG_ARCH_K3V2) || defined(CONFIG_ARCH_HI6620)*/
	if (plat && plat->init)
		plat->init();
	if (plat && plat->delay_sda)
		plat->delay_sda(ENABLE_DELAY_SDA);
/*#endif*/
	...
	k3v2_irqaffinity_register(dev->irq, 1);
#endif
...
err_iounmap:
	iounmap(dev->base);
err_no_iomux:
    dev->iomux_block = NULL;
    dev->block_config = NULL;
err_unuse_clocks:
	clk_disable(dev->clk);
	clk_put(dev->clk);
	dev->clk = NULL;
...
/*#if !defined(CONFIG_ARCH_K3V2) && !defined(CONFIG_ARCH_HI6620)
	if (plat && plat->delay_sda)
		plat->delay_sda(DISABLE_DELAY_SDA);
	clk_disable(dev->clk);
#endif*/
	clk_put(dev->clk);
	dev->clk = NULL;
	...
	writel(0, dev->base + DW_IC_ENABLE);
#ifdef CONFIG_ARCH_K3V2
	k3v2_irqaffinity_unregister(dev->irq);
#endif
	free_irq(dev->irq, dev);
#ifdef CONFIG_AUDIENCE
	if(1 == pdev->id) {
...
/*#endif*/
    ...
    if (plat && plat->iomux_name){
		ret = blockmux_set(dev->iomux_block, dev->block_config, LOWPOWER);
		if (ret){
			dev_err(&pdev->dev, "Set %s iomux to NORMAL failed, ret=%d\n", plat->iomux_name, ret);
     		return -EAGAIN;
       }
...
arch_initcall(dw_i2c_init_driver);
