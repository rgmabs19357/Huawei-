...
static unsigned int init_nr_desc_per_channel = PAGE_SIZE / ((sizeof(struct k3dma_desc) /K3_DMA_ALIGN + 1)  * K3_DMA_ALIGN);
...
/* ++add debug node++ */
static ssize_t k3dma_regs_show(struct device *dev, struct device_attribute *attr,
						char *buf);
static ssize_t k3dma_regs_fake_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count);
static DEVICE_ATTR(spi0_dma_debug, 0644, k3dma_regs_show, k3dma_regs_fake_store);
						...
						char *buf)
{
	...
	offset = buf;
	k3dma = (struct k3dma_device *)dev_get_drvdata(dev);
	if(!k3dma) {
		printk("%s: get driver data error", __func__);
		ret = snprintf(offset, PAGE_SIZE, "k3_dma == NULL\n");
		offset += ret;

		return (offset - buf);
	}
	for(index = HI6620_DMA_PHYCH_SPI0_RX; index < HI6620_DMA_PHYCH_SPI0_RX + 2; index++) {
		ch = &k3dma->phy_chans[index];
		chan_id = index;
		if(index == HI6620_DMA_PHYCH_SPI0_RX)
			ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": ============== SPI0 RX REGISTER DUMP ==============\n");
		else
			ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": ============== SPI0 TX REGISTER DUMP ==============\n");

		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": INT_STAT_OFFSET:            0x%x\n",
				dma_reg_read(k3dma->regs + INT_STAT_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": INT_TC1_OFFSET:             0x%x\n",
				dma_reg_read(k3dma->regs + INT_TC1_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": INT_ERR1_OFFSET:            0x%x\n",
				dma_reg_read(k3dma->regs + INT_ERR1_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": INT_ERR2_OFFSET:            0x%x\n",
				dma_reg_read(k3dma->regs + INT_ERR2_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": INT_TC1_RAW_OFFSET:         0x%x\n",
				dma_reg_read(k3dma->regs + INT_TC1_RAW_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": INT_ERR1_RAW_OFFSET:        0x%x\n",
				dma_reg_read(k3dma->regs + INT_ERR1_RAW_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": INT_ERR2_RAW_OFFSET:        0x%x\n",
				dma_reg_read(k3dma->regs + INT_ERR2_RAW_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CURR_CNT1_OFFSET:        0x%x\n",
				dma_reg_read(k3dma->regs + CX_CURR_CNT1_OFFSET(chan_id)));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CURR_CNT0_OFFSET:        0x%x\n",
				dma_reg_read(k3dma->regs + CX_CURR_CNT0_OFFSET(chan_id)));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CURR_SRC_ADDR_OFFSET:    0x%x\n",
				dma_reg_read(k3dma->regs + CX_CURR_SRC_ADDR_OFFSET(chan_id)));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CURR_DES_ADDR_OFFSET:    0x%x\n",
				dma_reg_read(k3dma->regs + CX_CURR_DES_ADDR_OFFSET(chan_id)));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_LLI_OFFSET:              0x%x\n",
				dma_reg_read(ch->ch_regs + CX_LLI_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_SRC_ADDR_OFFSET:         0x%x\n",
				dma_reg_read(ch->ch_regs + CX_SRC_ADDR_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_DES_ADDR_OFFSET:         0x%x\n",
				dma_reg_read(ch->ch_regs + CX_DES_ADDR_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_BINDX_OFFSET:            0x%x\n",
				dma_reg_read(ch->ch_regs + CX_BINDX_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CINDX_OFFSET:            0x%x\n",
				dma_reg_read(ch->ch_regs + CX_CINDX_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CNT1_OFFSET:             0x%x\n",
				dma_reg_read(ch->ch_regs + CX_CNT1_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CNT0_OFFSET:             0x%x\n",
				dma_reg_read(ch->ch_regs + CX_CNT0_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_CONFIG_OFFSET:           0x%x\n",
				dma_reg_read(ch->ch_regs + CX_CONFIG_OFFSET));
		offset += ret;
		ret = snprintf(offset, PAGE_SIZE, DEVICE_NAME ": CX_AXI_CONF_OFFSET:         0x%x\n",
...
{
	if (on) {
		dma_reg_write_mask_on(k3dma->regs + INT_TC1_MASK_OFFSET, HI6620_INT_MASK_ON);
		dma_reg_write_mask_on(k3dma->regs + INT_ERR1_MASK_OFFSET, HI6620_INT_MASK_ON);
		dma_reg_write_mask_on(k3dma->regs + INT_ERR2_MASK_OFFSET, HI6620_INT_MASK_ON);
	} else {
		dma_reg_write_mask_off(k3dma->regs + INT_TC1_MASK_OFFSET, HI6620_INT_MASK_OFF);
		dma_reg_write_mask_off(k3dma->regs + INT_TC2_MASK_OFFSET, HI6620_INT_MASK_OFF);
		dma_reg_write_mask_off(k3dma->regs + INT_ERR1_MASK_OFFSET, HI6620_INT_MASK_OFF);
		dma_reg_write_mask_off(k3dma->regs + INT_ERR2_MASK_OFFSET, HI6620_INT_MASK_OFF);
		dma_reg_write_mask_off(k3dma->regs + INT_ERR3_MASK_OFFSET, HI6620_INT_MASK_OFF);
	}
}

static void k3dma_clear_all_irq(struct k3dma_device *k3dma)
{
	dma_reg_write_mask_on(k3dma->regs + INT_TC1_RAW_OFFSET, HI6620_INT_MASK_ON);
	dma_reg_write_mask_on(k3dma->regs + INT_TC2_RAW_OFFSET, HI6620_INT_MASK_ON);
	dma_reg_write_mask_on(k3dma->regs + INT_ERR1_RAW_OFFSET, HI6620_INT_MASK_ON);
	dma_reg_write_mask_on(k3dma->regs + INT_ERR2_RAW_OFFSET, HI6620_INT_MASK_ON);
	dma_reg_write_mask_on(k3dma->regs + INT_ERR3_RAW_OFFSET, HI6620_INT_MASK_ON);
}
...
static int k3dma_get_phy_channel(struct k3dma_chan *k3chan)
{
	...
	spin_lock_irqsave(&k3chan->lock, flags);
	if (k3chan->phychan) {
		if(IS_HI6620_DMA_DEV(k3chan->phychan->chan_id))
		{
			if (!k3chan->phychan->serving)
				k3chan->phychan->serving = k3chan;
		}
		k3chan->phychan_hold++;
		spin_unlock_irqrestore(&k3chan->lock, flags);
		return 0;
...
static void k3dma_put_phy_channel(struct k3dma_chan *k3chan)
{
	...
	if (!ch) {
		printk(KERN_WARNING "%s: the point ch is NULL", __func__);
		return;
	}
	...
	if(!IS_HI6620_DMA_DEV(ch->chan_id))
	{
		k3chan->phychan = NULL;
	}
}
...
static irqreturn_t k3dma_interrupt(int irq, void *dev_id)
{
	...
	/* just handle interrupt of the special phy chan */
	for (i = DMACH_PHY_BEGIN; i < DMACH_PHY_MAX; i++) {
		mask = 0x1 << i;
...
static void k3dma_init_virtual_channels(struct k3dma_device *k3dma)
{
	...
	for (i = 0; i < k3dma->pd->nr_slave_channels; i++,
		...
		/* binding the phy chan to special number */
		k3chan->phychan = IS_HI6620_DMA_DEV(k3chan->cd.phy_ch) ?  \
			(&k3dma->phy_chans[k3chan->cd.phy_ch]) : NULL;
...
static int k3dma_probe(struct platform_device *pdev)
{
	...
	/*get clk*/
	k3dma->clk_axi0= clk_get(&pdev->dev, "clk_ap_dmac_axi0");
	if (IS_ERR(k3dma->clk_axi0)) {
		dev_err(&pdev->dev, "%s: k3dma clk_ap_dmac_axi0 clk_get failed\n",
			__func__);
		err = PTR_ERR(k3dma->clk_axi0);
		goto err_clk;
	}
	k3dma->clk_axi1 = clk_get(&pdev->dev, "clk_ap_dmac_axi1");
	if (IS_ERR(k3dma->clk_axi1)) {
		dev_err(&pdev->dev, "%s: k3dma clk_ap_dmac_axi1 clk_get failed\n",
			__func__);
		err = PTR_ERR(k3dma->clk_axi1);
		goto err_clk;
	}
	...
	err = clk_enable(k3dma->clk_axi0);
	if (err) {
		dev_err(k3dma->dma_common.dev, "%s: clk_ap_dmac_axi0 clock enable fail", __func__);
		goto err_clk_enable;
	}
	err = clk_enable(k3dma->clk_axi1);
	if (err) {
		dev_err(k3dma->dma_common.dev, "%s: clk_ap_dmac_axi1 clock enable fail", __func__);
		goto err_clk_enable;
	}
	...
	/*add debug node for spi0 dma*/
	err = device_create_file(&pdev->dev, &dev_attr_spi0_dma_debug);
	if(err) {
		dev_err(&pdev->dev, "%s: create dev attr file failed.", __func__);
		goto out_no_virtualchans;
	}
...
err_irq:
	clk_disable(k3dma->clk_axi0);
	clk_disable(k3dma->clk_axi1);
err_clk_enable:
	if (k3dma->hasacp)
...
err_clkgate_enable:
	clk_put(k3dma->clk_axi0);
	clk_put(k3dma->clk_axi1);
err_clk:
	iounmap(k3dma->regs);
	k3dma->regs = NULL;
err_release_r:
...
static int k3dma_resume_noirq(struct device *dev)
{
	...
	ret = clk_enable(k3dma->clk_axi0);
	if (ret) {
		dev_err(k3dma->dma_common.dev, "%s: clk_ap_dmac_axi0 clock enable fail", __func__);
	}
	ret = clk_enable(k3dma->clk_axi1);
	if (ret) {
		dev_err(k3dma->dma_common.dev, "%s: clk_ap_dmac_axi1 clock enable fail", __func__);
	}
