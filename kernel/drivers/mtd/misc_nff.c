	...
	switch (cmd) {
		case MISCRW:
			...
			if (MISC_READ == info.misc_operation) {
				/*read data from misc*/
                                if(ret = misc_read(mtd, offset, MISC_PWSTATE_DATA_SIZE, info.misc_data)){
                                       pr_err("[MISC][%s] misc_read error, ret = %d\n", __func__, ret);
                                       goto out;
                                }
                                pr_info("misc_ioctl:data from misc : %d\n", info.misc_data[0]);
				...
				/*write data to misc*/
                           pr_info("info.misc_operation = %d, info.misc_data[0] = %d\n");
				if(ret = misc_write(mtd, offset, MISC_PWSTATE_DATA_SIZE, info.misc_data)){
