    ...
    bool need_flush_cache, unsigned long magic_number, int valid_log_len)
{
    ...
    bool unlock_spinlock_in_the_end = false;
    ...
    if (!s_srecorder_log_info_saved)
    {
        ...
        volatile platform_special_reserved_mem_info_t *pmem_info = get_special_reserved_mem_info();
#else
        ...
        /* save log_buf's info in SRecorder's reserved mem */
        ...
        if (NULL != pmem_info)
        {
            /* save logcat buffers' info  in (16KB - 256) */
            ...
            pmem_info->crc32 = srecorder_get_crc32((unsigned char *)pmem_info, (unsigned long)
                (mem_info_size - sizeof(pmem_info->crc32) - sizeof(pmem_info->reserved)));
