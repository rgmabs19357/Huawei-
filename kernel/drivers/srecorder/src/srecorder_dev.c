...
/* flag indicates if flush cache when modules init */
#ifdef CONFIG_BOOT_NFF_LOG
static bool s_flush_cache = false;

#ifdef CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET
static bool s_reserve_mem_for_log_buf_boot_flag = false;
struct resource nff_log_buf_boot_flag_res = 
{
    .name  = "NFF log buf and boot flag",
    .start = 0,
    .end   = 0,
...
#ifdef CONFIG_BOOT_NFF_LOG
/**
    @function: void set_nff_log_info(unsigned long bootflag)
    @brief: set nff log info
    ...
    @note: 
*/
void set_nff_log_info(unsigned long bootflag)
{
    unsigned long log_buf = 0x0;
    unsigned long log_end = 0x0;
    unsigned long log_buf_len = 0x0;
    unsigned long mem_info_size = sizeof(platform_special_reserved_mem_info_t);
    
#ifdef CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET
    volatile platform_special_reserved_mem_info_t *bufp =  (platform_special_reserved_mem_info_t *)__va(PLAT_PHYS_OFFSET 
        + CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET);
#else
    volatile platform_special_reserved_mem_info_t *bufp =  (platform_special_reserved_mem_info_t *)
        __va(PLAT_PHYS_OFFSET + (KERNEL_TEXT_OFFSET - INITIAL_PAGE_TABLE_SIZE) - mem_info_size);
#endif

#ifdef CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET
    if (!s_reserve_mem_for_log_buf_boot_flag)
    {
        unsigned long boot_flag_base = (PLAT_PHYS_OFFSET + CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET);
        unsigned long boot_flag_size = mem_info_size;
        int ret = -1;

        ret = reserve_bootmem(boot_flag_base, boot_flag_size, BOOTMEM_EXCLUSIVE);
        if (ret < 0)
        {
            printk("NFF log buf and boot flag reservation failed - "
                "memory is in use (0x%lx)\n", (unsigned long)boot_flag_base);
            return;
        }

        printk("Reserving %luB of memory at %ldMB for NFF log buf and boot flag\n",
            (unsigned long)(boot_flag_size),(unsigned long)(boot_flag_base >> 20));
            
        nff_log_buf_boot_flag_res.start = boot_flag_base;
        nff_log_buf_boot_flag_res.end = boot_flag_base + boot_flag_size - 1;
        insert_resource(&iomem_resource, &nff_log_buf_boot_flag_res);
        s_reserve_mem_for_log_buf_boot_flag = true;
        memset((void *)bufp, 0, mem_info_size);
    } while (0);
#endif

    get_srecorder_log_buf_info(&log_buf, &log_buf_len);
    bufp->srecorder_log_buf = __pa(log_buf);
    bufp->srecorder_log_buf_len = log_buf_len;
    
    get_log_buf_info(&log_buf, &log_end, &log_buf_len);
    bufp->log_buf = __pa(*(unsigned long *)log_buf);
    bufp->log_buf_len = log_buf_len;
    bufp->boot_successfully_this_time = bootflag;
    
#ifdef CONFIG_DUMP_LOGCAT
    (void)get_logcat_buf_info(&(bufp->logcat_buf_info[0]));
    SRECORDER_PRINTK("log_main: %08lx, len: %08lx\n", bufp->logcat_buf_info[LOGCAT_MAIN].log_buf, 
        bufp->logcat_buf_info[LOGCAT_MAIN].log_buf_len);
    SRECORDER_PRINTK("log_system: %08lx, len: %08lx\n", bufp->logcat_buf_info[LOGCAT_SYSTEM].log_buf, 
        bufp->logcat_buf_info[LOGCAT_SYSTEM].log_buf_len);
    SRECORDER_PRINTK("log_events: %08lx, len: %08lx\n", bufp->logcat_buf_info[LOGCAT_EVENTS].log_buf, 
        bufp->logcat_buf_info[LOGCAT_EVENTS].log_buf_len);
    SRECORDER_PRINTK("log_radio: %08lx, len: %08lx\n", bufp->logcat_buf_info[LOGCAT_RADIO].log_buf, 
        bufp->logcat_buf_info[LOGCAT_RADIO].log_buf_len);
#endif

    bufp->crc32 = srecorder_get_crc32((unsigned char *)bufp, (unsigned long)(mem_info_size 
        - sizeof(bufp->crc32) - sizeof(bufp->reserved)));
    printk("##### srecorder_log_buf: 0x%lx srecorder_log_buf_len: 0x%lx log_buf: 0x%lx "
        "log_buf_len: 0x%lx boot flag: 0x% lx crc32: 0x%lx \n", 
        bufp->srecorder_log_buf, bufp->srecorder_log_buf_len, bufp->log_buf, bufp->log_buf_len, 
        bufp->boot_successfully_this_time, bufp->crc32);
}


#ifdef CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET
platform_special_reserved_mem_info_t *get_special_reserved_mem_info(void)
{
    return s_reserve_mem_for_log_buf_boot_flag ? ((platform_special_reserved_mem_info_t *)__va(PLAT_PHYS_OFFSET 
        + CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET)) : NULL;
}
#endif


/**
    @function: void set_flush_flag(bool flag)
...
/**
    @function: bool should_flush_cache_when_module_init(void)
    @brief: get flush flag

    @param: none

    @return: flush flag

    @note: Pls don't flush cache if we want machine to boot more faster
*/
bool should_flush_cache_when_module_init(void)
{
    return false; /* return s_flush_cache; */
}
#endif
...
static ssize_t srecorder_write(struct file *file, const char __user *data, size_t len, loff_t *ppos)
{
    ...
    case 'C': /* clean reserved buffer */
        {
            ...
            set_nff_log_info(BOOT_FAILED);
            mutex_unlock(&s_srecorder_mutex);
    ...
    case 'E': /* enable SRecorder */
        ...
        break;
#ifdef CONFIG_BOOT_NFF_LOG
    case 's': /* Set boot successfully flag */
    case 'S': /* Set boot successfully flag */
        {
            set_nff_log_info(BOOT_SUCCESSFULLY);
            SRECORDER_PRINTK("^_^_^ Set boot successfully flag!\n");
            mutex_unlock(&s_srecorder_mutex);
