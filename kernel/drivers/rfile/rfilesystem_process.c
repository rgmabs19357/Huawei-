 ...
 ******************************************************************************
  ...
  函数列表   :
              ...
              fxcopydirProxy
              fxdeletedirProxy
              rfile_calc_read_frame_num
              rfile_read_ccpu_data
              rfile_read_ccpu_pkt
              rfile_regProxyFunc
              rfile_sflags
              rfile_write_ccpu_pkt
              tds_binary_file_judge
  修改历史   :
  1.日    期   : 2013年1月26日
    修改内容   : 创建文件
...
/*****************************************************************************
 函 数 名  : rfile_regProxyFunc
 功能描述  : rfile_regProxyFunc
 输入参数  : void
 输出参数  : 无
 返 回 值  : void
 调用函数  :
...
/*****************************************************************************
 函 数 名  : rfile_write_ccpu_pkt
 功能描述  :  通过ICC通道把文件操作的返回值返回给CCPU
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
 调用函数  :
...
int  rfile_write_ccpu_pkt(int opspid,char *send_buf,unsigned int send_len)
{
    ...
    size = udi_write(rFileContext_a.rFileIccFd,send_buf,send_len);
	if (size != send_len)
	{
...
/*****************************************************************************
 函 数 名  : rfile_read_ccpu_pkt
 功能描述  : 读取CCPU发送过来的可维可测数据并进行参数检查
 输入参数  : RFILE_MNTN_HEADER *mntn_header
 输出参数  : 无
 返 回 值  : int
 调用函数  :
 被调函数  :
...
/*****************************************************************************
 函 数 名  : rfile_calc_read_frame_num
 功能描述  :
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
...
/*****************************************************************************
 函 数 名  : rfile_calc_massread_frame_num
 功能描述  :
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
...
/*****************************************************************************
 函 数 名  : rfile_xcopy_file
 功能描述  : 文件复制
 输入参数  : char *source_file
             char* dest_file
 输出参数  : 无
 返 回 值  : int
 调用函数  :
...
/*****************************************************************************
 函 数 名  : rfile_get_effectblk_addr
 功能描述  : 计算从分区开始的有效块数，返回实际地址
 输入参数  : fd               文件操作指针
             usr_logic_blk    用户希望起始块
             blkszie          存储介质blk大小
 输出参数  : 无
 返 回 值  : int
 调用函数  :
...
int rfile_mass_write(int fd,char* write_buf,unsigned int blksize,unsigned int erase_addr)
{
    ...
    if(sys_ioctl(fd, MEMERASE, (unsigned long)&erase_info)< 0)
    {
...
/*****************************************************************************
 函 数 名  : rfile_get_massread_op_para
 功能描述  : 获取远程文件操作参数
 输入参数  : char *buf
 输出参数  : 无
 返 回 值  : int
                                     ...
                                     RFILE_MASSREAD_HEAD *pmassread_head)
{
    ...
    {
        printk(KERN_INFO "rfile_get_massread_op_para: input para fial,buf= 0x%x,partition_name=0x%x \n",(unsigned int)buf,(unsigned int)partition_name);
...
/*****************************************************************************
 函 数 名  : rfile_get_masswrite_op_para
 功能描述  : 获取远程文件操作参数
 输入参数  : char *buf
 输出参数  : 无
 返 回 值  : int
...
/*****************************************************************************
 函 数 名  : rfile_mass_read
 功能描述  : mass 读
 输入参数  : int  fd
             char *buffer_temp
             char *buffer
             int   length
             RFILE_MASS_READ_PARA read_blk_para
                           ...
                           struct mtd_info_user mtd_info)
{
    ...
    {
       printk(KERN_INFO "rfile_mass_read_all: input para is fail,%x,%x \n",(unsigned int)buffer_temp,(unsigned int)buffer);
                           ...
                           struct mtd_info_user mtd_info)
{
    ...
    if((NULL == buffer_temp)||(NULL == buffer))
    {
       printk(KERN_INFO "rfile_mass_write_all: input para is fail,%x,%x \n",(unsigned int)buffer_temp,(unsigned int)buffer);
...
int rfile_xdelete(const char* pathname)
{
    ...
    while((ret = sys_old_readdir(fd,read_dirent,sizeof(struct linux_dirent)))!= 0)
    {
...
void fp_list_del_file(int fp_v)
{
...
/*****************************************************************************
 函 数 名  : bsp_ccpu_rest_rfile_handle
 功能描述  : CCPU单独复位时，rfile处理流程
 输入参数  : 无
 输出参数  : 无
 返 回 值  : int
 调用函数  :
...
int bsp_ccpu_rest_rfile_handle(void)
{
    ...
    {
        tmp = list_entry(p,fp_list_t,list);
        ret = bsp_linux_closedir((DRV_DIR_STRU*)(tmp->fp_v));
        if(ret != RFILE_OK)
        {
            printk(KERN_ERR "the dir close fail %s \n",((DRV_DIR_STRU*)(tmp->fp_v))->dd_dirent.d_name);
        }
    ...
    {
        tmp = list_entry(p,fp_list_t,list);
        ret = bsp_linux_fclose((FILE*)(tmp->fp_v));
        if(ret != RFILE_OK)
        {
            printk(KERN_ERR "the file close fail %x \n",((FILE*)(tmp->fp_v))->_file);
        }
        list_del(&tmp->list);
        kfree((const void*)tmp);
    }
...
int fopenProxy(RFILE_MNTN_HEADER *mnt_h, char * data)
{
    ...
    RFILE_DEBUGPL(0x1,"fopenProxy: fpath is %s, mode is %s \n",filename,mode);

    /*考虑到LINUX sys_open返回的值为一个整形数,当前在bsp_linux_fopen函数中进行封装，返回FILE**/
    if(NULL != rfile_linux_ops.bsp_fopen)
    {
        fp = (FILE *)(rfile_linux_ops.bsp_fopen)(filename,mode);
    }
    else
    {
...
int fcloseProxy(RFILE_MNTN_HEADER *mnt_h, char * data)
{
    ...
    fd = (FILE *)(pclose_head->fp);

    /*获取fopen是*/
    RFILE_DEBUGPL(0x1,"fcloseProxy:the fs_Set is %x \n",(unsigned int)fd);

    if (fp_check_file((int)fd)== 0)
    {
...
int freadProxy(RFILE_MNTN_HEADER *mnt_h, char * data)
{
    ...
    /*获取读数据的文件指针*/
    fp    = (FILE *)pread_head->fp;

    if (fp_check_file((int)fp)== 0)
    {
    ...
    while(totalframe--)
    {
        if (rd_sz > (BUF_LEN-head_len))
        {
            ...
            RFILE_DEBUGPL(0x1,"freadProxy: the read fp is %x ,the read len is %x, want size is %x \n",(unsigned int)fp,mntn_header->curlen,rd_sz);
        ...
        else
        {
            ...
            RFILE_DEBUGPL(0x1,"freadProxy: last frame the read fp is %x ,the read len is %x, want size is %x \n",(unsigned int)fp,mntn_header->curlen,rd_sz);
...
int fwriteProxy(RFILE_MNTN_HEADER *mnt_h, char * data)
{
    ...
    */
    ...
    if(mnt_h->curFrame < mnt_h->totalFrame)
    {
        ...
        RFILE_DEBUGPL(0x1,"fwriteProxy: the write fp is %x ,the write len is %x,the frame no is %x, totalfram is %x \n",(unsigned int)fp,wt_sz,g_curr_frame_num,mnt_h->totalFrame);
    ...
    else
    {
        ...
        RFILE_DEBUGPL(0x1,"fwriteProxy: the write fp is %x ,the write len is %x, the last frmae no is %x, total frame is %x \n",(unsigned int)fp,wt_sz,g_curr_frame_num,mnt_h->totalFrame);
...
int fseekProxy(RFILE_MNTN_HEADER *mnt_h, char * data)
{
    ...
    RFILE_DEBUGPL(0x1,"fseekProxy: fp is %x, offset is %lx, whence is %x\n",(unsigned int)fp,pseek_head->offset,pseek_head->whence);
...
int fremoveProxy(RFILE_MNTN_HEADER *mnt_h, char * data)
{
    ...
    RFILE_DEBUGPL(0x1,"fremoveProxy: pathname is %s\n",pathname);
...
int ftellProxy(RFILE_MNTN_HEADER *mnt_h, char * data)
{
    ...
    fp=(FILE *)ptell_head->fp;

    RFILE_DEBUGPL(0x1,"ftellProxy: fp is %x\n",(unsigned int)fp);
...
int frenameProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"frenameProxy: old pathname is %s, new pathname is %s\n",oldname_path,newname_path);
...
int faccessProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"faccessProxy: path is %s, mode is %x\n",path_dir,mode);
...
int fmkdirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"fmkdirProxy: pathname is %s\n",pathname);
...
int frmdirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"frmdirProxy: pathname is %s\n",pathname);
...
int fopendirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"fopendirProxy: dirname is %s\n",dirname);

    ret=(rfile_linux_ops.bsp_opendir)(dirname);

    if(ret != 0)
    {
        fp_list_add_dir((int)ret);
    }
...
int freaddirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"freaddirProxy: dir is %x\n",preaddir_head->dir);

    if (fp_check_dir((int)preaddir_head->dir)== 0)
    {
    ...
    ret = (rfile_linux_ops.bsp_readdir)(((DRV_DIR_STRU *)(preaddir_head->dir)),&dirbuf);
...
SEND_RESPONSE:
    ...
    memcpy(&res_readdir_msg.ret,&dirbuf,sizeof(DRV_DIRENT_STRU));
...
int fclosedirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"fclosedirProxy: dir is %x\n",pclosedir_head->dir);

    if (fp_check_dir((int)pclosedir_head->dir)== 0)
    {
    ...
    ret = (rfile_linux_ops.bsp_closedir)((DRV_DIR_STRU *)(pclosedir_head->dir));
...
int fstatdirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"fstatdirProxy: pathname is %s\n",pathname);
...
SEND_RESPONSE:
    ...
    memcpy(&res_stat_msg.ret,&stat_dir,sizeof(DRV_STAT_STRU));
...
int fxcopydirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"fxcopydirProxy: source_path_dir is %s,dest_path_dir is %s\n",source_path_dir,dest_path_dir);
...
int fxdeletedirProxy( RFILE_MNTN_HEADER *mnt_h, char * data )
{
    ...
    RFILE_DEBUGPL(0x1,"fxdeletedirProxy: pathname is %s\n",pathname);
...
/*****************************************************************************
 函 数 名  : fmassreadProxy
 功能描述  : 远程flash裸读接口
 输入参数  : RFILE_MNTN_HEADER *mnt_h
             char * data
 输出参数  : 无
 返 回 值  : int
 调用函数  : 如果成功 size 大于0 ； 失败 -1
 被调函数  :
...
FILE* bsp_linux_fopen( const char *filename, const char *mode )
{
    ...
    f = (FILE*)kmalloc(sizeof(FILE), GFP_KERNEL);
    if(NULL == f)
    {
...
int bsp_linux_remove( const char *pathname )
{
    ...
    if(ret < 0)
    {
        printk(KERN_INFO "bsp_linux_remove:the sys unlink is fail,the ret is %x\n",ret);
        set_fs(fs);
        up(&sem_remove);
...
int bsp_linux_rename( const char * oldname, const char * newname )
{
    ...
    if(ret < 0)
    {
        printk(KERN_INFO "bsp_linux_rename:the renamer is fail,the ret is %x \n",ret);
        set_fs(fs);
        up(&sem_rename);
        return RFILE_ERROR;
    }
...
int bsp_linux_access( const char *filename, int amode)
{
    ...
    if(0 != ret)
    {
        printk(KERN_INFO "bsp_linux_access:the access is fail,the ret is %x\n",ret);
        set_fs(fs);
        up(&sem_access);
...
int bsp_linux_rmdir( const char *pathname)
{
    ...
    if(ret < 0)
    {
        printk(KERN_INFO "bsp_linux_rmdir:the rmdir is fail,the ret is %x\n",ret);
        set_fs(fs);
        up(&sem_rmdir);
        return RFILE_ERROR;
    }
...
int bsp_linux_opendir( const char *dirname )
{
    ...
    DIR = (DRV_DIR_STRU*)kmalloc(sizeof(DRV_DIR_STRU), GFP_KERNEL);
    if(NULL == DIR)
    {
...
int bsp_linux_readdir( DRV_DIR_STRU *dirp,DRV_DIRENT_STRU* buf )
{
    ...
    {
        memset(read_dirent,0,sizeof(struct linux_dirent_rfile));
        ret = sys_old_readdir(dirp->dd_fd,read_dirent,sizeof(struct linux_dirent));
        if(ret < 0)
        {
...
int bsp_linux_xcopy( const char * source, const char * dest )
{
    ...
    {
        printk(KERN_INFO "bsp_linux_xcopy:the source %x or dest %x is null \n",(int)source,(int)dest);
...
int bsp_mass_read(const char *partionname,unsigned int offset,unsigned length,char *buffer)
{
    ...
    {
        printk(KERN_INFO "bsp_mass_read:the partionname %x or buffer %x is null \n",(int)partionname,(int)buffer);
...
/*****************************************************************************
 ...
 功能描述  : bsp_mass_write 存储介质裸写接口-nand
 输入参数  : const char *partionname
             unsigned int offset
             unsigned length
             char *buffer
 输出参数  : 无
...
int bsp_mass_write(const char *partionname,unsigned int offset,unsigned length,char *buffer)
{
    ...
    if((NULL == partionname)||(NULL == buffer))
    {
        printk(KERN_INFO "bsp_mass_write:the partionname %x or buffer %x is null \n",(int)partionname,(int)buffer);
    ...
    for(i = 0;i<arry_number;i++)
    {
        ...
        {
            RFILE_DEBUGPL(0x1,"bsp_mass_write: pathname %s isnot what we want!\n",partitions[i].name);
        }
