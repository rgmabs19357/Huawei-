...
*****************************************************************************/
/*****************************************************************************
 函 数 名  : rfile_write_cmd_to_ccpu
 功能描述  :  通过ICC通道把命令发送给CCPU
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
 调用函数  :
...
int  rfile_write_cmdtoccpu_pkt(int opspid,char *send_buf,unsigned int send_len)
{
    ...
    size = udi_write(rFileContext_cmd.rFileIccFd,send_buf,send_len);
	if (size != send_len)
	{
...
/*****************************************************************************
 函 数 名  : rfile_sync_ccpu
 功能描述  : 告诉CCPU，ACPU文件系统初始化完毕
 输入参数  : void
 输出参数  : 无
 返 回 值  : int
 调用函数  :
...
unsigned int rfile_icc_read_cmd_cb(unsigned int chenalId,signed int u32size)
{
	...
	/*ICC读取数据*/
	readSize = udi_read(rFileContext_cmd.rFileIccFd,rFileContext_cmd.sendBuf,u32size);
	if (readSize != u32size)
	{
...
int rfile_transfer_init(void)
{
	...
	rFileContext_cmd.rFileIccFd = udi_open(&rFileContext_cmd.rFileIccParam);
...
/*****************************************************************************
 函 数 名  : rfile_process_ccpu_pkt
 功能描述  : 远程文件操作处理
 输入参数  : RFILE_MNTN_HEADER *header
 输出参数  : 无
 返 回 值  : int
 调用函数  :
 被调函数  :
...
/*****************************************************************************
 函 数 名  : rfile_init_thread
 功能描述  : rfile_init_thread
 输入参数  : void
 输出参数  : 无
 返 回 值  : void
 调用函数  :
