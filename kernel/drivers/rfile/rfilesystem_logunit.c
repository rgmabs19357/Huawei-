 ...
 ******************************************************************************
  ...
  函数列表   :
              ...
              FlashLessLogAddInfoCurCodeFile
              FlashLessLogAddInfoOperation
              FlashLessLogAddInfoOthers
              FlashLessLogAddLogs
              FlashLessLogCanAdd
              FlashLessLogClearData
              FlashLessLogGetNameInfo
              FlashLessLogInit
              FlashLessLogPrint
...
#define     FLASHLESS_LOG_DIR                       "/data/modemlog/rfile_log.txt"
...
void FlashLessLogAddLogs(void)
{
    ...
    pMsg.ulFlag    = 0xAAAA;
    printk("RFILE: IFC Send!\n");
    if(0 != BSP_IFC_Send((void*)&pMsg, sizeof(bsp_flashless_log_ifcmsg)))
    {
        printk("FRILE: IFC Send Fail!\n");
    }
}
