...
static int minor_of_jank = 0;

#define LOGCTL_ENABLE_1 1
#define LOGCTL_DISABLE_0 0
#define NV_OPERATION_WRITE 0
#define LOGCTL_NV_NUM 301
#define LOGCTL_NV_VALID_SIZE 1
/* add log switch, control logmian ect logs can write in or not */
static atomic_t log_switch = ATOMIC_INIT(LOGCTL_DISABLE_0);
...
static int write_logctl_state_to_nv(void* szbuf)
{
    int ret;
    struct nve_info_user user_info;
	memset(&user_info, 0, sizeof(user_info));

	user_info.nv_operation = NV_OPERATION_WRITE;
	user_info.nv_number = LOGCTL_NV_NUM;
	user_info.valid_size = LOGCTL_NV_VALID_SIZE;
	strncpy(user_info.nv_name, "LOGCTL", sizeof(user_info.nv_name));
	user_info.nv_name[sizeof(user_info.nv_name) - 1] = '\0';
	memcpy(user_info.nv_data, szbuf, user_info.valid_size);
			 ...
			 unsigned long nr_segs, loff_t ppos)
{
	...
	ssize_t ret = 0;
        ...
        /* if log device is events or main which its priority is more than ANDROID_LOG_INFO, we also pass it */
            if (LOGCTL_ENABLE_1 == atomic_read(&log_switch)  || minor_of_events == log->misc.minor
            || ((minor_of_main == log->misc.minor) && (priority >= ANDROID_LOG_INFO))
			|| minor_of_power == log->misc.minor || minor_of_exception == log->misc.minor || minor_of_jank == log->misc.minor)
        {
            /* log it */
        }
        else
...
static ssize_t log_switch_write(struct file * file, const char __user * buf, size_t count, loff_t * pos)
{
	...
	if (val == logctl_nv)
		return count;
	len = min(count, STR_LEN_12);
	ret = copy_from_user(szbuf, buf, len);
	printk("In %s, count=%d, szbuf=%s\n", __FUNCTION__, count, szbuf);
    wrt_nv_ret = write_logctl_state_to_nv(szbuf);
    if (wrt_nv_ret < 0)
    {
        printk("%s, write nv error", __FUNCTION__);
        return 0;
    }
    return count;
}
...
static int __init logger_init(void)
{
...
#ifdef CONFIG_ANDROID_LOGGER_JANK
	ret = init_log(&log_jank);
	if (unlikely(ret))
		goto out;
#endif
    ...
    minor_of_jank = log_jank.misc.minor;
    printk("%s, minor_of_jank=%d\n", __FUNCTION__, minor_of_jank);
...
#ifdef CONFIG_K3_LOG
        log_power.log_buf_info = (volatile log_buffer_head *)log_buf_info + 5;
