...
#ifdef CONFIG_G2D
struct clk *g2d_core_clk;
struct clk *g2d_axi_clkhb;
struct clk *g2d_cfg_clk;
u32   g2dPoweroffstatus = 1;
#endif
...
static int __init gpu_init(void)
{
    ...
    ret = get_hw_config_int("memory_config/gpu", &size, NULL);
    if (false == ret) {
        printk("gpu_init: error memory_config gpu not found !\n");
        ret = -ENOMEM;
        goto out;
    } else {
        contiguousSize = PAGE_ALIGN(size *1024);
    }    
