...
struct pm_qos_request_list g2d_pm_qos_req;

#define _GC_OBJ_ZONE    gcvZONE_OS
#ifdef CONFIG_G2D
extern struct clk *g2d_core_clk;
extern struct clk *g2d_axi_clkhb;
extern struct clk *g2d_cfg_clk;
extern u32 g2dPoweroffstatus;
    ...
    do
    {
        ...
        if (Physical != ~0U)
        {
            for (i = 0; i < pageCount; i++)
            {
                ...
                if (0 != atomic_read(&pages[i]->_count))
                {
                    get_page(pages[i]);
                }
    ...
    )
{
    ...
    {
        if ((Os->device->kernels[Core] != NULL)) {
            //printk("!!!!!!!!! Disable int %d [Power=%d Clock=%d]\n", Core, Power, Clock);
            gckOS_SuspendInterruptEx(Os, Core);
            ret  = (gckOS_ReadRegisterEx(Os, Core, 0x0, &control));
            if (gcmIS_ERROR(ret))
            {
                printk(KERN_ERR "%s: gckOS_ReadRegisterEx fail!\n",__FUNCTION__);
                return ret;
            }
            control |= (1 << 19);//((((gctUINT32) (0x00000100)) & ~(((gctUINT32) (((gctUINT32) ((((1 ? 19:19) - (0 ? 19:19) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 19:19) - (0 ? 19:19) + 1))))))) << (0 ? 19:19))) | (((gctUINT32) ((gctUINT32) (1) & ((gctUINT32) ((((1 ? 19:19) - (0 ? 19:19) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 19:19) - (0 ? 19:19) + 1))))))) << (0 ? 19:19)));
            ret = (gckOS_WriteRegisterEx(Os, Core, 0x00000, control));
            if (gcmIS_ERROR(ret))
            {
                printk(KERN_ERR "%s: gckOS_WriteRegisterEx fail!\n",__FUNCTION__);
                return ret;
            }
        }
    }
        ...
        {
            ret = clk_enable(g2d_core_clk);
            if (ret) {
                printk(KERN_ERR "%s: Could not enable clk_g2d!\n",__FUNCTION__);
                //return ret; 
            }
        }
        else if (!Power && oldPowerState)
        {
            clk_disable(g2d_core_clk);
        }
#endif
    if (Power && Clock)
    {
        if (Os->device->kernels[Core] != NULL) {
            //printk("!!!!!!!!! Enable int %d [Power=%d Clock=%d]\n", Core, Power, Clock);
            gckOS_ResumeInterruptEx(Os, Core);
        }
    }
...
void _ResetG2D(void)
{
    /*reset g2d, write 1 to bit18,19,20, at least wait 0.5us*/
    phy_reg_writel(SOC_SC_OFF_BASE_ADDR,
                   SOC_SCTRL_SC_PERIPH_RSTEN7_ADDR(G2D_SCTRL_OFFSET),
                   18,
                   20,
                   0x7);
    udelay(1);
...
void _DisResetG2D(void)
{
    /*dis reset g2d, write 1 to bit18,19,20, at least wait 1us*/
    phy_reg_writel(SOC_SC_OFF_BASE_ADDR,
                   SOC_SCTRL_SC_PERIPH_RSTDIS7_ADDR(G2D_SCTRL_OFFSET),
                   18,
                   20,
                   0x7);
    udelay(1);
...
void _DisResetG2DDomain(void)
{
    /* dis reset G2D Domain ,write 1 to bit2, at least delay 1us */
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_RSTDIS0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
}

void _ResetG2DDomain(void)
{
    /* reset G2D Domain ,write 1 to bit2 */
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_RSTEN0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
}
...
void _IsoDisG2D(void)
{
    /* disolate G2D  ,write 1 to bit2*/
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_ISODIS0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
} 
...
void _IsoG2D(void)
{
    /* isolate G2D  ,write 1 to bit2*/
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_ISOEN0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
} 

void _ClkEnalbeG2DDomain(void)
{
    /* enable G2D Domain clk  ,write 1 to bit2*/
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_CLKEN0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
}

void _ClkDisalbeG2DDomain(void)
{
    /* Disable G2D Domain clk  ,write 1 to bit2*/
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_CLKDIS0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
}

int _G2DMtcmosIsOn(void)
{
    unsigned int val;

    val = phy_reg_readl(SOC_SC_ON_BASE_ADDR,
                        SOC_AO_SCTRL_SC_MTCMOS_STAT0_ADDR(G2D_AOSCTRL_OFFSET),
                        2,
                        2);
    if (1 == val) 
    {
        return G2D_MTCMOS_ON_STATUS;
    } 
    else 
    {
        return G2D_MTCMOS_OFF_STATUS;
    }
}

void _G2DMtcmosOn(void)
{
#ifdef CONFIG_MACH_HI6620OEM   
    int i;
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_EN0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
                   

    for(i = 0; i < G2D_POWERON_STATE_CHECK_COUNT; i++)
    {
    
        if (G2D_MTCMOS_ON_STATUS == _G2DMtcmosIsOn()) 
        {
            break;
        }
    }

    if (G2D_POWERON_STATE_CHECK_COUNT == i) 
    {
    	printk("%s mtcmos status no ready\n", __func__);
    }
    else
    {
    
        udelay(G2D_POWERON_DELAY_TIME);
    }
#endif    
    
}

void _G2DMtcmosOff(void)
{
#ifdef CONFIG_MACH_HI6620OEM   
    phy_reg_writel(SOC_SC_ON_BASE_ADDR,
                   SOC_AO_SCTRL_SC_PW_DIS0_ADDR(G2D_AOSCTRL_OFFSET),
                   2,
                   2,
                   0x1);
...
void _G2DPowerOn(void)
{
    /*Power on*/
    _G2DMtcmosOn();

    /*enalbe G2D Domain CLK */
    _ClkEnalbeG2DDomain();
    ...
    /*set G2D clk*/
    if (clk_set_rate(g2d_core_clk,  g2d_start_freq)) 
    {
        printk(KERN_ERR "G2D, %s: failed to set G2D clk rete, G2D clk = %p \n", __func__, g2d_core_clk);
    }
    g_g2d_current_freq = g2d_start_freq;
     /*Clock enable*/
    if (clk_enable(g2d_core_clk)) 
    {
        printk(KERN_ERR "%s: Could not enable g2d_core_clk!\n",__FUNCTION__);
    }
    
    if (clk_enable(g2d_axi_clkhb)) 
    {
        printk(KERN_ERR "%s: Could not enable g2d_axi_clkhb!\n",__FUNCTION__);
    }
    
    if (clk_enable(g2d_cfg_clk)) 
    {
        printk(KERN_ERR "%s: Could not enable g2d_cfg_clk!\n",__FUNCTION__);
    }
...
void _G2DPowerOff(void)
{
    ...
    /*Clock Off*/
    clk_disable(g2d_core_clk);
    clk_disable(g2d_axi_clkhb);   
    clk_disable(g2d_cfg_clk);

    g_g2d_current_freq = 0;

    /*Domain Clock Off*/
    _ClkDisalbeG2DDomain();
...
void _G2DClockGatingOn(
    IN gckOS Os,
    IN gceCORE Core)
{
#ifdef CONFIG_MACH_HI6620OEM   
    gctUINT32 control         = 0;
    if (gcmIS_ERROR(gckOS_ReadRegisterEx(Os, Core, 0x100, &control)))
    {
        printk(KERN_ERR "%s: gckOS_ReadRegisterEx fail!\n",__FUNCTION__);
        return;
    }
    control |= 1;
    if (gcmIS_ERROR(gckOS_WriteRegisterEx(Os, Core, 0x100, control)))
    {
        printk(KERN_ERR "%s: gckOS_WriteRegisterEx fail!\n",__FUNCTION__);
        return;
    }
    ...
    )
{
    ...
    if (!Power || !Clock)
    {
        ...
        if (Power && !oldPowerState)
        {
            pm_qos_add_request(&g2d_pm_qos_req, PM_QOS_CPU_INT_LATENCY, 0);
        
            _G2DPowerOn();
            _G2DClockGatingOn(Os, Core);
           
        }
        else if (!Power && oldPowerState)
        {
            if(g2dPoweroffstatus)
            {
                _G2DPowerOff();
                                
                pm_qos_remove_request(&g2d_pm_qos_req);
            }
        }
#endif
    if (Power && Clock)
