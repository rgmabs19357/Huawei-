    ...
    )
{
    ...
    if (gcPLS.reference)
    {
        gcmVERIFY_OK(gcoOS_AtomDestroy(gcvNULL, gcPLS.reference));
    ...
    )
{
    ...
    if (gcPLS.reference)
    {
        ...
        gcmVERIFY_OK(gcoOS_AtomDecrement(gcvNULL, gcPLS.reference, &reference));

        /* Check if all other threads exit. */
        if (reference == 1)
        {
            /* All threads exit.  Destroy PLS. */
            _PLSDestructor();
        }
    }
    ...
    )
{
    ...
    gcmONERROR(gcoOS_AtomConstruct(gcvNULL, &gcPLS.reference));

    /* Increment the reference. */
    gcmONERROR(gcoOS_AtomIncrement(gcvNULL, gcPLS.reference, gcvNULL));
    ...
    )
{
    ...
    if (gcPLS.processID != gcmGETPROCESSID())
    {
...
typedef struct _Property_Table {
	gctCHAR value[PROPERTY_VALUE_MAX];
	gctCHAR key[PROPERTY_KEY_MAX];
	struct _Property_Table * next;
...
{
#ifdef ANDROID
    static Property_Table * pt=gcvNULL;
    Property_Table *node=gcvNULL;

    gctCHAR property[PROPERTY_VALUE_MAX];
    gctSIZE_T klen=0,plen=0;
#endif
...
do
{
    ...
    if (property_get(VarName, property, gcvNULL) > 0)
    {
        ...
        gcoOS_StrLen(property,&plen);
        if(pt==gcvNULL) { /*first node*/
			gcoOS_Allocate(gcvNULL, sizeof(Property_Table), &node);
			node->next = gcvNULL;
			gcoOS_StrCopySafe(node->key,klen,VarName);
			gcoOS_StrCopySafe(node->value,plen,property);
			pt = node;
			*Value = node->value;
			break;
			}
		...
		if(node == gcvNULL)
			...
			*Value = node->value;
			...
			{
				if(gcoOS_StrCmp(property,node->value)!=gcvSTATUS_OK)
					{
						gcoOS_StrCopySafe(node->value,plen,property);
					}
				*Value = node->value;
    ...
    )
{
    ...
    if (gcoHAL_IsFeatureAvailable(gcvNULL,gcvFEATURE_NON_POWER_OF_TWO)
        == gcvSTATUS_TRUE)
    {
        return gcvFALSE;
    }

    static gctSTRING AppLists[] = {
        /* "com.gameloft.android.ANMP.GloftGANG" */ "\x9c\x90\x92\xd1\x98\x9e\x92\x9a\x93\x90\x99\x8b\xd1\x9e\x91\x9b\x8d\x90\x96\x9b\xd1\xbe\xb1\xb2\xaf\xd1\xb8\x93\x90\x99\x8b\xb8\xbe\xb1\xb8"
    };

    for(i = 0; i < gcmCOUNTOF(AppLists); i++)
    {
        if(gcoOS_DetectProcessByEncryptedName(AppLists[i]))
        {
            /* Find in application lists. */
            i--;
            break;
        }
    }

    return (i == gcmCOUNTOF(AppLists)) ? gcvFALSE : gcvTRUE;
}
