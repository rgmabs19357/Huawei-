...
{
    if (Orientation == gcvSURF_FLIP_X || Orientation == gcvSURF_FLIP_Y)
    {
        Orientation = gcvSURF_0_DEGREE;
    }

    if (*Relation == gcvSURF_FLIP_X || *Relation == gcvSURF_FLIP_Y)
    {
	...
	)
{
    ...
    gcmONERROR(gcsRECT_RelativeRotation(toRotation, &Rotation));
