    ...
    )
{
    ...
    if (ColorBits != gcvNULL)
    {
        ...
        */
        ...
        status = gcoHARDWARE_ConvertFormat(Format,
                                           &bitsPerPixel,
                                           gcvNULL);
    ...
    )
{
    ...
    gcmHEADER_ARG("Format=%d OriginX=%u OriginY=%u ColorConvert=%u "
                  "FgColor=0x%08x BgColor=0x%08x MonoBits=0x%016llx "
                  "ColorBits=0x%x Mask=0x%016llx",
                  Format, OriginX, OriginY, ColorConvert, FgColor, BgColor,
                  MonoBits, ColorBits, Mask);
    ...
    status = _BuildBrushBuffer(gcvNULL,
                               Format,
                               OriginX,
                               OriginY,
                               ColorConvert,
    ...
    if (brush == gcvNULL)
    {
        ...
        else
        {
            ...
            if (status != gcvSTATUS_OK)
            {
                ...
                gcmVERIFY_OK(gcmOS_SAFE_FREE(os, brush));
            ...
            status = gcoOS_Allocate(os, dataSize, &pointer);
    ...
    *Brush = brush;
    ...
    /* Success. */
    gcmFOOTER_ARG("*Brush=0x%x", *Brush);
    return gcvSTATUS_OK;
}
    ...
    )
{
    ...
    gcmHEADER_ARG("ColorConvert=%d Color=%x Mask=%llx Brush=0x%x",
                  ColorConvert, Color, Mask, Brush);
    ...
    )
{
    ...
    gcmHEADER_ARG("OriginX=%d OriginY=%d ColorConvert=%d FgColor=%x BgColor=%x"
                    "Bits=%lld Mask=%llx Brush=0x%x",
                    OriginX, OriginY, ColorConvert, FgColor, BgColor,
                    Bits, Mask, Brush);
    ...
    )
{
    ...
    gcmHEADER_ARG("OriginX=%d OriginY=%d Address=0x%x Format=%d Mask=%llx Brush=0x%x",
                    OriginX, OriginY, Address, Format, Mask, Brush);
    ...
    )
{
    ...
    {
        gcmVERIFY_OK(gcmOS_SAFE_FREE(gcvNULL, Brush->colorBits));
    }
    ...
    /* Free the gcoBRUSH object. */
    gcmVERIFY_OK(gcmOS_SAFE_FREE(gcvNULL, Brush));
    ...
    )
{
    ...
    /* Return status. */
    gcmFOOTER_ARG("*DataCount=%d status=%s", *DataCount, gcoOS_DebugStatus2Name(status));
    return status;
}
    ...
    do
    {
        if (Brush->colorBits != gcvNULL)
        {
            ...
            */
            ...
            /* Load the pattern. */
            status = gco2D_LoadColorBrush(Brush->engine,
                                          Brush->originX,
                                          Brush->originY,
                                          Node->physical,
                                          Brush->format,
        ...
        else if ((Brush->originX != ~0U) && (Brush->originY != ~0U))
        {
            /*
               Monochrome brush.
            */

            status = gco2D_LoadMonochromeBrush(Brush->engine,
                                               Brush->originX,
                                               Brush->originY,
                                               Brush->colorConvert,
                                               Brush->fgColor,
        ...
        else
        {
            ...
            */
            ...
            status = gco2D_LoadSolidBrush(Brush->engine,
                                          gcvSURF_UNKNOWN,
                                          Brush->colorConvert,
                                          Brush->fgColor,
                                          Brush->mask);
