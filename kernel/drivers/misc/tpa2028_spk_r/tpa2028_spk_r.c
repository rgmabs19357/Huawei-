...
static struct tpa2028_r_platform_data *pdata = NULL;
static struct i2c_client *this_client;

static struct iomux_block *tpa2028r_iomux_block = NULL;
static struct block_config *tpa2028r_block_config = NULL;
...
static long tpa2028_r_ioctl(struct file *file,
                            unsigned int cmd,
                            unsigned long arg)
{
    int ret = 0;
    ...
    case TPA2028_ENABLE:
        if (pdata->gpio_tpa2028_en) {
            ret = blockmux_set(tpa2028r_iomux_block, tpa2028r_block_config, NORMAL);
            if (0 > ret) {
    ...
    case TPA2028_DISABLE:
        if (pdata->gpio_tpa2028_en) {
            ...
            ret = blockmux_set(tpa2028r_iomux_block, tpa2028r_block_config, LOWPOWER);
            if (0 > ret) {
                           ...
                           const struct i2c_device_id *id)
{
    ...
    if (pdata->gpio_tpa2028_en) {
        ...
        tpa2028r_iomux_block = iomux_get_block(IOMUX_BLOCK_NAME);
        if (!tpa2028r_iomux_block) {
            loge("%s: get iomux block error", __FUNCTION__);
            return -ENODEV;
        }

        tpa2028r_block_config = iomux_get_blockconfig(IOMUX_BLOCK_NAME);
        if (!tpa2028r_block_config) {
            loge("%s: get block config error", __FUNCTION__);
            return -ENODEV;
        }

        /* request gpio */
        ret = gpio_request(pdata->gpio_tpa2028_en, TPA2028_R_NAME);
        if (ret < 0) {
...
{
    struct tpa2028_r_platform_data *platform_data = i2c_get_clientdata(client);
...
static int tpa2028_r_suspend(struct i2c_client *client, pm_message_t mesg)
{
    int ret = 0;
...
static int tpa2028_r_resume(struct i2c_client *client)
{
    int ret = 0;
...
#else
#define tpa2028_r_suspend NULL
#define tpa2028_r_resume NULL
#endif
...
static struct i2c_driver tpa2028_spk_r_driver = {
    ...
    .suspend  = tpa2028_r_suspend,
    .resume   = tpa2028_r_resume,
