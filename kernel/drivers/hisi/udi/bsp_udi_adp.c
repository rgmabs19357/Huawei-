...
#ifdef _DRV_LLT_
struct atomic_notifier_head android_usb_suspend_notifier_list;
struct atomic_notifier_head android_usb_resume_notifier_list;
#else
...
static void acm_event_notify(struct work_struct *data)
{
    ...
    if (acm->eventCB != NULL) {
        acm->eventCB(acm->cur_evt);

    }
    ACM_ADP_ERR("acm[%s] evt[%d]\n", acm->tty_name, acm->cur_evt);
}
/**************************************************************************
  ACM 适配实现
**************************************************************************/
...
*****************************************************************************/
int udi_acm_open(UDI_OPEN_PARAM *param, UDI_HANDLE handle)
{
    int ret = 0;
    unsigned int dev_id;
    ...
    BSP_UDI_SetPrivate(param->devid, (void *)ret);
...
int udi_acm_init(void)
{
    ...
    for (dev_num = UDI_USB_ACM_CTRL; dev_num < UDI_USB_ACM_MAX; dev_num++){
        (void)BSP_UDI_SetCapability((UDI_DEVICE_ID)UDI_BUILD_DEV_ID(UDI_DEV_USB_ACM, dev_num), UDI_USB_ACM_CAPA);
        (void)BSP_UDI_SetInterfaceTable((UDI_DEVICE_ID)UDI_BUILD_DEV_ID(UDI_DEV_USB_ACM, dev_num), pDrvInterface);
    }
...
*****************************************************************************/
int acm_open(unsigned int dev_id)
{
    int  i;
    int  dev_type;
    int  ldisc;
    ...
    port = (void *)acm_ncopy_port_ready((void *)tty);
    if (!port) {
        /* acm<-->tty<---X--->port<-->gser */
    ...
    p_acm_ctx->fd     = dev_fd;
    p_acm_ctx->tty    = (void *)tty;

    if (p_acm_ctx->ncopy) {
        acm_ncopy_init((int)p_acm_ctx);
    }
    ACM_ADP_ERR("leave\n");
...
*****************************************************************************/
int udi_acm_close(unsigned int acm_dev)
{
    mm_segment_t   fs         = 0;
    ...
    if (p_acm_ctx->ncopy) {
        acm_ncopy_uninit((int)p_acm_ctx);
    }
...
*****************************************************************************/
int udi_acm_read(int acm_dev, unsigned char *pbuff, unsigned int size)
{
    long fd;
    int len;
    mm_segment_t   fs         = 0;
...
*****************************************************************************/
int udi_acm_write(int acm_dev, unsigned char *pbuff, unsigned int size)
{
    long fd;
    int  len;
    long flag = 0;
    ...
    if (p_acm_ctx->writeDoneCB) {
        p_acm_ctx->writeDoneCB(pbuff,len);
    }
...
*****************************************************************************/
int udi_acm_ioctl(int acm_dev, int cmd, int arg)
{
    long fd;
    ...
    case ACM_IOCTL_WRITE_ASYNC:
        ...
        ret = acm_async_write(acm_dev,
                              (unsigned char *)async_buf->pBuffer,
                              async_buf->u32Size);
...
*****************************************************************************/
int acm_async_write(int acm_dev, unsigned char *pbuff, unsigned int size)
{
    long fd;
    ...
    ACM_ADP_DBG(" acm[%s] buf[%p] size[%d]\n", p_acm_ctx->tty_name, pbuff, size);
    ...
    if (p_acm_ctx->ncopy) {
        rc = acm_ncopy_start_tx((int)p_acm_ctx, pbuff, size);
        if (ADP_OK == rc) {
            len = size;
        }
    } else {
...
int acm_test_init(int acm_idx)
{
    ...
    sprintf(acm_ctx->thr_name, "acm_loop_%d", acm_idx);
    ACM_ADP_ERR("create thread [%s]\n", acm_ctx->thr_name);

    acm_ctx->thread = kthread_run(acm_loop_task, (void *)acm_idx, acm_ctx->thr_name);
...
int acm_send(int acm_idx, unsigned char *buf, unsigned int size, int loop)
{
    ...
    if (!acm_ctx->mem[OP_WRITE].buf)
    {
        acm_ctx->mem[OP_WRITE].buf = (unsigned char *)kmalloc(ACM_TEST_BUF_SIZE, GFP_KERNEL);
        if (!acm_ctx->mem[OP_WRITE].buf)
        {
    ...
    if (buf)
    {
        memcpy((void *)acm_ctx->mem[OP_WRITE].buf, (void *)buf, size);
    }
    else
    {
        memset((void *)acm_ctx->mem[OP_WRITE].buf, 'a', size);
    }
...
int acm_recv(int acm_idx, int loop, int dump)
{
    ...
    if (!acm_ctx->mem[OP_READ].buf)
    {
        acm_ctx->mem[OP_READ].buf = (unsigned char *)kzalloc(ACM_TEST_BUF_SIZE, GFP_KERNEL);
        if (!acm_ctx->mem[OP_READ].buf)
        {
...
int acm_loop(int acm_idx)
{
    ...
    if (!acm_ctx->mem[OP_LOOP].buf)
    {
        acm_ctx->mem[OP_LOOP].buf = (unsigned char *)kzalloc(ACM_TEST_BUF_SIZE, GFP_KERNEL);
        if (!acm_ctx->mem[OP_LOOP].buf)
        {
            ACM_ADP_ERR("buf malloc fail [%d]\n", acm_idx);
            return ADP_ERROR;
...
static int acm_loop_task(void *arg)
{
    ...
    while (1)
    {
        ...
        {
            buf  = acm_ctx->mem[acm_ctx->op_mode].buf;
        ...
        while (1)
        {
            if (OP_READ == acm_ctx->op_mode)
            {
                ...
                len = udi_acm_read((int)p_acm_ctx, buf, size);
                if (len <= 0)
                {
                ...
                else
                {
                    if (acm_ctx->op_dump)
                    {
                        ACM_ADP_ERR("acm[%d] recv [%d] bytes:%s\n", acm_idx, len, buf);
                    }
            ...
            else if (OP_WRITE == acm_ctx->op_mode)
            {
                ...
                len = acm_async_write((int)p_acm_ctx, buf, size);
                if (len != size)
                {
            ...
            else if (OP_LOOP == acm_ctx->op_mode)
            {
                ...
                else
                {
                    ...
                    length = acm_async_write((int)p_acm_ctx, buf, len);
...
exit:
    ...
    {
        if (acm_ctx->mem[i].buf)
        {
            kfree(acm_ctx->mem[i].buf);
            acm_ctx->mem[i].buf  = NULL;
            acm_ctx->mem[i].size = 0;
        }
    }
    ACM_ADP_ERR("acm[%d] test exit\n", acm_idx);
...
static int udiIccAdpOpen(UDI_OPEN_PARAM *param, UDI_HANDLE handle)
{
