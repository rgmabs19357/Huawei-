...
#define DRV_FILE_OPEN(name, flags) \
        fopen(name,flags)

#define DRV_FILE_CLOSE(fp) \
        fclose(fp)

#define DRV_FILE_READ(buff,size,number,fp) \
        fread(buff,size,number,fp)

#define DRV_FILE_WRITE(buff,size,number,fp) \
        fwrite(buff,size,number,fp)

#define DRV_FILE_RMFILE(fileName) \
        Stub_remove(fileName)

#define DRV_FILE_LSEEK(fp,offset,whence) \
        fseek(fp,offset,whence)

#define DRV_FILE_TELL(fp) \
        ftell(fp)

#define DRV_FILE_MKDIR(dirName) \
        mkdir(dirName)

#define DRV_FILE_RMDIR(path) \
        rmdir(path)

#define DRV_FILE_XCOPY(src, dest) \
        xcopy(src, dest)

#define DRV_FILE_ACCESS(name,flags) \
        access(name,flags)
...
/* 后续底软提供接口如下 */
#define DRV_FILE_OPEN(name, flags) \
        BSP_fopen(name,flags)

#define DRV_FILE_CLOSE(fp) \
        BSP_fclose(fp)

#define DRV_FILE_READ(buff,size,number,fp) \
        BSP_fread(buff,size,number,fp)

#define DRV_FILE_WRITE(buff,size,number,fp) \
        BSP_fwrite(buff,size,number,fp)

#define DRV_FILE_RMFILE(fileName) \
        BSP_remove(fileName)

#define DRV_FILE_LSEEK(fp,offset,whence) \
        BSP_fseek(fp,offset,whence)
...
#define DRV_FILE_ACCESS(name,flags) \
        BSP_access(name,flags)

#define DRV_FILE_OPENDIR(dirName) \
        BSP_opendir(dirName)

#define DRV_FILE_READDIR(pDir)\
        BSP_readdir(pDir)

#define DRV_FILE_CLOSEDIR(pDir)\
        BSP_closedir(pDir)
...
extern int   BSP_fwrite(const void *ptr, unsigned int size, unsigned int number, FILE *stream);
...
extern DRV_DIRENT_STRU* BSP_readdir(DRV_DIR_STRU *dirName);
extern int BSP_closedir(DRV_DIR_STRU *dirName);
#endif
