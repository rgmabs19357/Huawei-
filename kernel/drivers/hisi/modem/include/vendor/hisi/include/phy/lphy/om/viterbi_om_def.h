...
typedef struct
{
    UINT32 ulDlDciPower[3];                     /*下行DCI软件度量值*/
    UINT32 ulUlDciPower;                        /*上行DCI软件度量值*/
    UINT32 ulRsNoisePower;                      /*天线0与1的平均宽带噪声功率*/
    INT32  lDlDciSINR;                          /*下行DCI信干噪比*/
...
typedef struct __LPHY_VITERBI_RPT_PDCCH_GRANT_STAT_OM_IND__
{
    ...
    //UINT16 usCfi3Num;
    UINT16 ausCfiNum[3];
    /* END:   Modified by h00130263, 2013/6/8 */
    UINT16 usRsd1;
    UINT16 usUlGrantNum;
    UINT16 usDlGrantNum;
    /* BEGIN: Modified by h00130263, 2013/6/8   问题单号:v7r2_code_size_opt*/
...
{
    UINT32 ulPdcchErrorNum;         /*定义为无任何有效的DCI(包括上下行)就算一次 -> 算法需求, 但不清楚定义, 故DSP自己定义*/
    UINT32 ulPdcchTotalNum;         /*定义为只要VDL浮动中断上来就算一次, 注意对于DTX情况总数要累加, 但错误数亦累加*/
    UINT32 ulPdcchAccuErrorNum;     /*累积量*/
    UINT32 ulPdcchAccuTotalNum;     /*累积量*/
...
#define     OM_VITERBI_MAX_RPT_REG_IMG_LEN       ((0x230 - 0x100) / 4 + 1)
...
typedef struct __LPHY_VITERBI_PDCCH_RPT_REGS_IMAGE_OM_DEBUG__
{
    UINT16 usSysFn;
    UINT16 usSubFn;
    UINT32 aulRegImg[OM_VITERBI_MAX_RPT_REG_IMG_LEN];
...
/* BEGIN: Added by wujing, 2012/9/7   PN:CMCC_DT*/
/************************************************************
                           7.DT命令和对应的数据结构定义
************************************************************/
...
}LPHY_DT_REQ_VITERBI_ENUM;

typedef enum __LPHY_DT_IND_VITERBI__
{
    LPHY_DT_VITERBI_RPT_CFI_INFO_IND                   = OM_CMD_ID(LPHY_VITERBI_MID, OM_TYPE_DT, 0x1001),
    LPHY_DT_VITERBI_RPT_PDCCH_GRANT_STAT_IND,
    LPHY_DT_VITERBI_RPT_PDCCH_DL_GRANT_INFO_IND,
    LPHY_DT_VITERBI_RPT_PDCCH_BLER_IND,
}LPHY_DT_IND_VITERBI_ENUM;

/*DT上报CFI值*/
typedef struct __LPHY_DT_VITERBI_RPT_CFI_INFO_IND_STRU__
{
    UINT16 usSysFn;
    UINT16 usSubFn;
    UINT32 ulCfi;
}LPHY_DT_VITERBI_RPT_CFI_INFO_IND_STRU;

/*DT上报授权统计信息*/
typedef struct __LPHY_DT_VITERBI_RPT_PDCCH_GRANT_STAT_IND_STRU__
{
    UINT32 ulAccuUlGrantNum;
    UINT32 ulAccuDlGrantNum;
    UINT32 ulAccuCce1Num;
    ...
    UINT32 ulAccuDci3Num;
    UINT32 ulAccuDci3ANum;
} LPHY_DT_VITERBI_RPT_PDCCH_GRANT_STAT_IND_STRU;
...
/*DT上报PDCCH_BLER信息*/
typedef struct __LPHY_DT_VITERBI_RPT_PDCCH_BLER_IND_STRU__
{
    UINT32 ulPdcchAccuErrorNum;
    UINT32 ulPdcchAccuTotalNum;
}LPHY_DT_VITERBI_RPT_PDCCH_BLER_IND_STRU;

/* END:   Added by wujing, 2012/9/7 */
