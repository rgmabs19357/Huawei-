...
*
*   文 件 名 :  BSP_SOCP_DRV.h
*
*
*   描    述 :  SOCP模块用户头文件
*
*   修改记录 :  2011年02月20日  v1.00  y00171698  创建
*
*************************************************************************/
...
{
    BSP_U32                 u32ChanEn;      /*通道使能状态*/
    BSP_U32                 u32SetStat;     /*通道配置状态*/
    SOCP_ENCSRC_CHNMODE_E   eChnMode;      /* 数据结构类型 */
    socp_event_cb           event_cb;       /*通道事件回调函数*/
    socp_rd_cb              rd_cb;          /*通道RD事件回调函数*/
    BSP_U32                 u32LastRdSize;  /*最后一次获取的RD待释放的大小*/
    SOCP_RING_BUF_S         sEncSrcBuf;     /*源空间的信息*/
    SOCP_RING_BUF_S         sRdBuf;         /*RD空间信息*/
}SOCP_ENCSRC_CHAN_S;

typedef struct tagSOCP_ENCDST_CHAN_S
{
    BSP_U32                 u32ChanID;
    BSP_U32                 u32SetStat;    /* 通道已经或没有配置的标识*/
    BSP_U32                 u32Thrh;       /* 阈值*/
...
{
    BSP_BOOL                bInitFlag;                  /*标识当前SOCP初始化是否完成*/
    BSP_U32                 u32BaseAddr;                /*SOCP IP的基地址*/
#ifdef SOCP_ACORE
    BSP_U32                 u32EncSrcSemID;             /*编码源通道自处理任务的信号量*/
    BSP_U32                 u32EncDstSemID;             /*编码目的通道自处理任务的信号量*/
    BSP_U32                 u32DecSrcSemID;             /*解码源通道自处理任务的信号量*/
    BSP_U32                 u32DecDstSemID;             /*解码目的通道自处理任务的信号量*/
    BSP_U32                 u32EncDstFlushSemID;        /*编码目的通道Flush的信号量*/

    BSP_U32                 u32IntEncSrcHeader;         /*编码源通道消息头检测寄存器值*/
    BSP_U32                 u32IntEncSrcRD;             /*编码源通道RD检测寄存器值*/

    BSP_U32                 u32IntEncDstTfr;            /*编码目的通道传输寄存器值*/
    BSP_U32                 u32IntEncDstThresholdOvf;   /* 编码目的buffer阈值溢出 */
    BSP_U32                 u32IntDecSrcErr;            /*解码源通道错误寄存器*/
    BSP_U32                 u32IntDecDstTfr;            /*解码目的通道传输寄存器*/
    BSP_U32                 u32IntDecDstOvf;            /*解码目的通道溢出寄存器*/

    SOCP_ENCDST_CHAN_S      sEncDstChan[SOCP_MAX_ENCDST_CHN];   /*编码目的通道参数*/
    SOCP_DECSRC_CHAN_S      sDecSrcChan[SOCP_MAX_DECSRC_CHN];   /*解码源通道参数*/
    SOCP_DECDST_CHAN_S      sDecDstChan[SOCP_MAX_DECDST_CHN];   /*解码目的通道参数*/
#endif
    SOCP_ENCSRC_CHAN_S      sEncSrcChan[SOCP_MAX_ENCSRC_CHN];   /*编码源通道参数*/
}SOCP_GBL_STATE;

typedef struct tagSOCP_DEBUG_GBL_S
{
#ifdef SOCP_ACORE
    BSP_U32                 u32SocpAppEtrIntCnt;        /* 进入APP中断的次数*/
    BSP_U32                 u32SocpAppSucIntCnt;        /* 进入APP中断处理成功的次数*/
#endif
#ifdef SOCP_CCORE
    BSP_U32                 u32SocpModemEtrIntCnt;      /* 进入Modem中断的次数*/
    BSP_U32                 u32SocpModemSucIntCnt;      /* 进入Modem中断处理成功的次数*/
#endif
} SOCP_DEBUG_GBL_S;

typedef struct tagSOCP_DEBUG_ENCSRC_S
{
    BSP_U32 u32SocpStartEncSrcCnt[SOCP_MAX_ENCSRC_CHN];        /* SOCP启动编码源通道的次数*/
    BSP_U32 u32SocpStopEncSrcCnt[SOCP_MAX_ENCSRC_CHN];         /* SOCP关闭编码源通道的次数*/
    BSP_U32 u32SocpRegEventEncSrcCnt[SOCP_MAX_ENCSRC_CHN];     /* SOCP注册编码源通道事件的次数*/
    BSP_U32 u32SocpGetWBufEncSrcEtrCnt[SOCP_MAX_ENCSRC_CHN];      /* SOCP编码源通道尝试获得写buffer的次数*/
    BSP_U32 u32SocpGetWBufEncSrcSucCnt[SOCP_MAX_ENCSRC_CHN];      /* SOCP编码源通道成功获得写buffer的次数*/
    BSP_U32 u32SocpWriteDoneEncSrcEtrCnt[SOCP_MAX_ENCSRC_CHN];    /* SOCP编码源通道尝试写回buffer的次数*/
    BSP_U32 u32SocpWriteDoneEncSrcSucCnt[SOCP_MAX_ENCSRC_CHN];    /* SOCP编码源通道成功写回buffer的次数*/
    BSP_U32 u32SocpWriteDoneEncSrcFailCnt[SOCP_MAX_ENCSRC_CHN];    /* SOCP编码源通道写回buffer失败的次数*/
    BSP_U32 u32SocpGetRdBufEncSrcEtrCnt[SOCP_MAX_ENCSRC_CHN];     /* SOCP编码源通道尝试获得Rdbuffer的次数*/
    BSP_U32 u32SocpGetRdBufEncSrcSucCnt[SOCP_MAX_ENCSRC_CHN];     /* SOCP编码源通道成功获得Rdbuffer的次数*/
    BSP_U32 u32SocpReadRdDoneEncSrcEtrCnt[SOCP_MAX_ENCSRC_CHN];   /* SOCP编码源通道尝试写回Rdbuffer的次数*/
    BSP_U32 u32SocpReadRdDoneEncSrcSucCnt[SOCP_MAX_ENCSRC_CHN];   /* SOCP编码源通道成功写回Rdbuffer的次数*/
    BSP_U32 u32SocpReadRdDoneEncSrcFailCnt[SOCP_MAX_ENCSRC_CHN];   /* SOCP编码源通道写回Rdbuffer失败的次数*/
    BSP_U32 u32SocpEncSrcTskHeadCbCnt[SOCP_MAX_ENCSRC_CHN];      /* 任务中回调编码源通道包头错误中断处理函数完成的次数*/
    BSP_U32 u32SocpEncSrcTskHeadCbOriCnt[SOCP_MAX_ENCSRC_CHN];   /* 任务中回调编码源通道包头错误中断处理函数次数*/
    BSP_U32 u32SocpEncSrcTskRdCbCnt[SOCP_MAX_ENCSRC_CHN];        /* 任务中回调编码源通道Rd 完成中断处理函数完成的次数*/
    BSP_U32 u32SocpEncSrcIsrHeadIntCnt[SOCP_MAX_ENCSRC_CHN];      /* ISR中进入编码源通道包头错误中断次数*/
    BSP_U32 u32SocpEncSrcIsrRdIntCnt[SOCP_MAX_ENCSRC_CHN];        /* ISR进入编码源通道Rd 完成中断次数*/
} SOCP_DEBUG_ENCSRC_S;

#ifdef SOCP_ACORE
typedef struct tagSOCP_DEBUG_ENCDST_S
{
    BSP_U32 u32SocpRegEventEncDstCnt[SOCP_MAX_ENCDST_CHN];     /* SOCP注册编码目的通道事件的次数*/
    BSP_U32 u32SocpRegReadCBEncDstCnt[SOCP_MAX_ENCDST_CHN];    /* SOCP注册编码目的通道读数据回调函数的次数*/
    BSP_U32 u32SocpGetReadBufEncDstEtrCnt[SOCP_MAX_ENCDST_CHN];   /* SOCP编码目的通道尝试读数据的次数*/
    BSP_U32 u32SocpGetReadBufEncDstSucCnt[SOCP_MAX_ENCDST_CHN];   /* SOCP编码目的通道成功读数据的次数*/
    BSP_U32 u32SocpReadDoneEncDstEtrCnt[SOCP_MAX_ENCDST_CHN];     /* SOCP编码目的通道尝试写回目的buffer的次数*/
    BSP_U32 u32SocpReadDoneEncDstSucCnt[SOCP_MAX_ENCDST_CHN];     /* SOCP编码目的通道写回目的buffer成功的次数*/
    BSP_U32 u32SocpReadDoneEncDstFailCnt[SOCP_MAX_ENCDST_CHN];     /* SOCP编码目的通道写回目的buffer失败的次数*/
    BSP_U32 u32SocpReadDoneValidEncDstCnt[SOCP_MAX_ENCDST_CHN];    /* SOCP编码目的通道写回目的buffer size 不等于0 的次数*/
    BSP_U32 u32SocpEncDstTskTrfCbCnt[SOCP_MAX_ENCDST_CHN];        /* 任务中回调编码目的通道传输完成中断处理函数完成的次数*/
    BSP_U32 u32SocpEncDstTskTrfCbOriCnt[SOCP_MAX_ENCDST_CHN];        /* 任务中回调编码目的通道传输完成中断处理函数的次数*/
    BSP_U32 u32SocpEncDstTskOvfCbCnt[SOCP_MAX_ENCDST_CHN];        /* 任务中回调编码目的通道buf 溢出中断处理函数完成的次数*/
    BSP_U32 u32SocpEncDstTskOvfCbOriCnt[SOCP_MAX_ENCDST_CHN];     /* 任务中回调编码目的通道buf 溢出中断处理函数次数*/
    BSP_U32 u32SocpEncDstIsrTrfIntCnt[SOCP_MAX_ENCDST_CHN];        /* ISR中进入编码目的通道传输完成中断次数*/
    BSP_U32 u32SocpEncDstTskThresholdOvfCbCnt[SOCP_MAX_ENCDST_CHN];        /* 任务中回调编码目的通道buf阈值溢出中断处理函数完成的次数*/
    BSP_U32 u32SocpEncDstTskThresholdOvfCbOriCnt[SOCP_MAX_ENCDST_CHN];     /* 任务中回调编码目的通道buf阈值溢出中断处理函数次数*/
    BSP_U32 u32SocpEncDstIsrThresholdOvfIntCnt[SOCP_MAX_ENCDST_CHN];       /* ISR中进入编码目的通道buf阈值溢出中断次数*/
...
{
    BSP_U32 u32SocpStartDecSrcCnt[SOCP_MAX_DECSRC_CHN];        /* SOCP启动解码源通道的次数*/
    BSP_U32 u32SocpStopDecSrcCnt[SOCP_MAX_DECSRC_CHN];         /* SOCP关闭解码源通道的次数*/
    BSP_U32 u32SocpRegEventDecSrcCnt[SOCP_MAX_DECSRC_CHN];     /* SOCP注册解码源通道事件的次数*/
    BSP_U32 u32SocpGetWBufDecSrcEtrCnt[SOCP_MAX_DECSRC_CHN];      /* SOCP解码源通道尝试获得写buffer的次数*/
    BSP_U32 u32SocpGetWBufDecSrcSucCnt[SOCP_MAX_DECSRC_CHN];      /* SOCP解码源通道获得写buffer成功的次数*/
    BSP_U32 u32SocpWriteDoneDecSrcEtrCnt[SOCP_MAX_DECSRC_CHN];    /* SOCP解码源通道尝试写回buffer的次数*/
    BSP_U32 u32SocpWriteDoneDecSrcSucCnt[SOCP_MAX_DECSRC_CHN];    /* SOCP解码源通道写回buffer成功的次数*/
    BSP_U32 u32SocpWriteDoneDecSrcFailCnt[SOCP_MAX_DECSRC_CHN];    /* SOCP解码源通道写回buffer失败的次数*/
    BSP_U32 u32SocpDecSrcIsrErrIntCnt[SOCP_MAX_DECSRC_CHN];        /* ISR中进入解码源通道错误中断次数*/
    BSP_U32 u32SocpDecSrcTskErrCbCnt[SOCP_MAX_DECSRC_CHN];        /* 任务中调用解码源通道错误中断处理函数完成的次数*/
    BSP_U32 u32SocpDecSrcTskErrCbOriCnt[SOCP_MAX_DECSRC_CHN];     /* 任务中调用解码源通道错误中断处理函数次数*/
} SOCP_DEBUG_DECSRC_S;
#endif

typedef struct tagSOCP_DEBUG_DECDST_S
{
    BSP_U32 u32SocpRegEventDecDstCnt[SOCP_MAX_DECDST_CHN];     /* SOCP注册解码目的通道事件的次数*/
    BSP_U32 u32SocpRegReadCBDecDstCnt[SOCP_MAX_DECDST_CHN];    /* SOCP注册解码目的通道读数据回调函数的次数*/
    BSP_U32 u32SocpGetReadBufDecDstEtrCnt[SOCP_MAX_DECDST_CHN];   /* SOCP解码目的通道尝试读数据的次数*/
    BSP_U32 u32SocpGetReadBufDecDstSucCnt[SOCP_MAX_DECDST_CHN];   /* SOCP解码目的通道读数据成功的次数*/
    BSP_U32 u32SocpReadDoneDecDstEtrCnt[SOCP_MAX_DECDST_CHN];     /* SOCP解码目的通道尝试写回目的buffer的次数*/
    BSP_U32 u32SocpReadDoneDecDstSucCnt[SOCP_MAX_DECDST_CHN];     /* SOCP解码目的通道写回目的buffer成功的次数*/
    BSP_U32 u32SocpReadDoneDecDstFailCnt[SOCP_MAX_DECDST_CHN];     /* SOCP解码目的通道写回目的buffer失败的次数*/
    BSP_U32 u32SocpReadDoneZeroDecDstCnt[SOCP_MAX_DECDST_CHN];   /* SOCP解码目的通道写回目的buffer size 等于0 的次数*/
    BSP_U32 u32SocpReadDoneValidDecDstCnt[SOCP_MAX_DECDST_CHN];   /* SOCP解码目的通道写回目的buffer size 不等于0 的次数*/
    BSP_U32 u32SocpDecDstIsrTrfIntCnt[SOCP_MAX_DECDST_CHN];        /* ISR中进入解码目的通道传输完成中断次数*/
    BSP_U32 u32SocpDecDstTskTrfCbCnt[SOCP_MAX_DECDST_CHN];         /* 任务中回调解码目的通道传输完成中断处理函数完成的次数*/
    BSP_U32 u32SocpDecDstTskTrfCbOriCnt[SOCP_MAX_DECDST_CHN];      /* 任务中回调解码目的通道传输完成中断处理函数次数*/
    BSP_U32 u32SocpDecDstIsrOvfIntCnt[SOCP_MAX_DECDST_CHN];        /* ISR中进入解码目的通道buf 溢出中断次数*/
    BSP_U32 u32SocpDecDstTskOvfCbCnt[SOCP_MAX_DECDST_CHN];         /* 任务中进入解码目的通道buf 溢出中断处理函数完成的次数*/
    BSP_U32 u32SocpDecDstTskOvfCbOriCnt[SOCP_MAX_DECDST_CHN];      /* 任务中进入解码目的通道buf 溢出中断处理函数次数*/
} SOCP_DEBUG_DECDST_S;

typedef struct
{
    VOS_UINT32  ulStartTimeNum;
    VOS_UINT32  ulTimeOutNum;
    VOS_UINT32  ulEncDstRunNum;
    VOS_UINT32  ulStartTimeFailNum;
...
typedef struct tagSOCP_DEBUG_INFO_S
{
    ...
    SOCP_DEBUG_SDLOG_STRU   stSocpSDLogInfo;
    SOCP_ENCDST_SEM_STRU    stSocpEncDstSemInfo;
#endif
}SOCP_DEBUG_INFO_S;
...
*/
#define SOCP_CHN_BUSY                   ( (BSP_S32) 1)   /*通道忙*/
#define SOCP_CHN_IDLE                   ( (BSP_S32) 0)   /*通道闲*/
...
* 通道是否已经设置
*/
#define SOCP_CHN_SET                    ( (BSP_S32) 1)     /*通道已经配置*/
#define SOCP_CHN_UNSET                  ( (BSP_S32) 0)     /*通道没有配置*/
/*
* 编码旁路设置使能位
*/
#define SOCP_ENCSRC_BYPASS_ENABLE       ( (BSP_S32) 1)     /*通道旁路使能*/
#define SOCP_ENCSRC_BYPASS_DISABLE      ( (BSP_S32) 0)     /*通道旁路没有使能*/


/*
* 编码旁路设置使能位
*/
#define SOCP_DECSRC_DEBUG_ENBALE        ( (BSP_S32) 1)     /*通道DEBUG 使能*/
#define SOCP_DECSRC_DEBUG_DISBALE       ( (BSP_S32) 0)     /*通道DEBUG 没有使能*/

#define SOCP_CORE0_DEC_OUTOVFINT_MASK   ( (BSP_S32)(1))          /* 解码core0 目的BUFFER溢出中断 */
#define SOCP_DEC_INERRINT_MASK          ( (BSP_S32)(1<<1))       /* 编码输入检测错误中断 */
#define SOCP_CORE0_DEC_TFRINT_MASK      ( (BSP_S32)(1<<2))       /* 编码core0传输完成中断 */
#define SOCP_CORE1_DEC_TFRINT_MASK      ( (BSP_S32)(1<<3))       /* 解码core1 传输完成中断 */
#define SOCP_CORE1_DEC_OUTOVFINT_MASK   ( (BSP_S32)(1<<4))       /* 编码core1 目的BUFFER溢出中断*/
#define SOCP_MODEM_ENC_RDINT_MASK       ( (BSP_S32)(1<<10))      /* 编码RDBUFFER完成中断 */
#define SOCP_APP_ENC_RDINT_MASK         ( (BSP_S32)(1<<11))      /* 编码MODEMCPU包头检测错误中断 */
#define SOCP_APP_ENC_OUTOVFINT_MASK     ( (BSP_S32)(1<<12))      /* 编码RDBUFFER完成中断 */
#define SOCP_MODEM_ENC_FLAGINT_MASK     ( (BSP_S32)(1<<13))      /* 编码MODEMCPU包头检测错误中断 */
#define SOCP_APP_ENC_FLAGINT_MASK       ( (BSP_S32)(1<<14))      /* 编码RDBUFFER完成中断 */
#define SOCP_APP_ENC_TFRINT_MASK        ( (BSP_S32)(1<<15))      /* 编码MODEMCPU包头检测错误中断 */
...
#define SOCP_RD_TIMEOUT                 ((BSP_S32) 100)     /*待定*/
...
#define SOCP_REG_WRITE(reg, data)               COMM_DRV_REG_WRITE(g_strSocpStat.u32BaseAddr,   reg, data)
#define SOCP_REG_READ(reg, result)              COMM_DRV_REG_READ(g_strSocpStat.u32BaseAddr,    reg, result)
#define SOCP_REG_SETBITS(reg, pos, bits, val)   BSP_REG_SETBITS(g_strSocpStat.u32BaseAddr, reg, pos, bits, val)
#define SOCP_REG_GETBITS(reg, pos, bits)        BSP_REG_GETBITS(g_strSocpStat.u32BaseAddr, reg, pos, bits)
...
extern SOCP_GBL_STATE      g_strSocpStat;          /* 记录当前全局状态和通道配置信息 */

extern SOCP_DEBUG_INFO_S   g_stSocpDebugInfo;      /* 记录在SOCP运行状态中的过程信息 */
...
#ifdef SOCP_ACORE
extern BSP_VOID SocpHandlerEncDst(BSP_VOID);

extern BSP_U32 SocpAppInthandler(BSP_VOID);
#else
extern BSP_VOID SocpModemInthandler(BSP_VOID);
#endif

extern VOS_UINT32 OM_GetSlice(VOS_VOID);

extern BSP_VOID SOCP_LFixChan_Init(BSP_VOID);

extern BSP_VOID  SOCP_EnableLFixChan(VOID);
...
#elif((VOS_OS_VER == VOS_VXWORKS) || (VOS_OS_VER == VOS_RTOSCK))
#define SOCP_FLUSH_CACHE(ptr, size) \
    BSP_CACHE_DATA_FLUSH((void  *)(((BSP_U32)(ptr)) & (~0x1F)), (int)((((size) + (((BSP_U32)(ptr)) & 0x1F) + 0x1F)>>0x5)<<0x5))
#else
