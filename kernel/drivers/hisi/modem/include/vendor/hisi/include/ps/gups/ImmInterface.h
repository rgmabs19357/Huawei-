 ...
 ******************************************************************************
  文 件 名   : ImmInterface.h
  版 本 号   : 初稿
  生成日期   : 2011年12月1日
  最近修改   :
  功能描述   : PS内部ACPU零拷贝功能公共头文件
  函数列表   :
  说    明   : 头文件将在Android下使用, 使用C原始定义数据类型

  修改历史   :
  1.日    期   : 2011年12月1日
    修改内容   : 创建文件
  2.日    期   : 2012年8月11日
    修改内容   : 根据GIT代码策略调整头文件的位置, 并刷新接口
...
/*****************************************************************************
 函 数 名  : IMM_GetLocalFreeMemCnt
 功能描述  : A核查询A核最大挡空闲内存的数目。
 输入参数  : 无
 输出参数  : 无
 返 回 值  : 返回最大挡空闲内存的数目
 调用函数  :
