...
/*****************************************************************************
函 数 名  : MUX_PrintLog
功能描述  : 目前临时使用MUX打印函数，后续等A核打印可用后切换为PS_LOG，打印没有参数的打印
...
/*****************************************************************************
函 数 名  : MUX_PrintLog1
功能描述  : 目前临时使用MUX打印函数，后续等A核打印可用后切换为PS_LOG，打印有1个参数的打印
输入参数  : VOS_UINT32  ulModuleId      打印模块名
            VOS_UINT32  ulSubMod        打印子模块名
            VOS_UINT32  ulLevel         打印级别
            VOS_CHAR   *pcString        打印内容
...
/*****************************************************************************
函 数 名  : MUX_PrintLog2
功能描述  : 目前临时使用MUX打印函数，后续等A核打印可用后切换为PS_LOG，打印有2个参数的打印
输入参数  : VOS_UINT32  ulModuleId      打印模块名
            VOS_UINT32  ulSubMod        打印子模块名
            VOS_UINT32  ulLevel         打印级别
            VOS_CHAR   *pcString        打印内容
...
)
{
    ...
    if ( VOS_OK != lResult )
    {
        MUX_DBG_UL_CALLBACK_FAIL_NUM(1);
        MUX_PrintLog1(PS_PID_APP_MUX, 0, PS_PRINT_WARNING,
            "MUX, MUX_DlciUlHsicFreeDataBuf, call DRV_UDI_IOCTL return fail!\n",
                  lResult);
...
VOS_UINT32 MUX_OpenDevice(MUX_DEV_INFO_STRU *pstDevInfo)
{
    ...
    pstDevInfo->slUdiMuxAcmHdl = DRV_UDI_OPEN(&stParam);

    if (UDI_INVALID_HANDLE == pstDevInfo->slUdiMuxAcmHdl)
    {
        MUX_DBG_GET_INVALID_UDI_HANDLE_NUM(1);
        MUX_PrintLog(PS_PID_APP_MUX, 0, PS_PRINT_ERROR,
            "MUX, MUX_OpenDevice, Call DRV_UDI_OPEN return UDI_INVALID_HANDLE.\n");
        return VOS_ERR;
    }
