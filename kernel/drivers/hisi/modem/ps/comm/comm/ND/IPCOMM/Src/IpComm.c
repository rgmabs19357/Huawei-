...
)
{
    ...
    if ((usPayLoad + IP_IPV6_HEAD_LEN) > ulIpMsgLen)
    {
        if(1 == g_ulCnNd)
        {
            ...
            {
                vos_printf(" %02x %02x %02x %02x %02x %02x %02x %02x\n ",
...
/*****************************************************************************
 ...
 功能描述  : Computes the internet checksum (RFC 1071). NOTE
                it does not compute the 1 complement checksum.
 输入参数  : pbuf   -- 待计算的IP报文内容指针
             ulBytes -- 长度
 输出参数  : 无
 返 回 值  : A standard internet checksum..
 调用函数  :
 被调函数  :

 修改历史      :
  1.日    期   : 2011年6月19日
    修改内容   : [移植代码]参考Interpeak中checksum实现，全函数移植
*****************************************************************************/
...
/*****************************************************************************
 ...
 功能描述  : Calculates the internet checksum on IPv6 pseudo header.
 输入参数  : pstIp6SrcAddr - The source address.
              ...
              usLen         - The number of octets in the payload for which this is
                    pseudo header.
 输出参数  : 无
 返 回 值  : The 32-bit internet checksum.
 调用函数  :
 被调函数  :
...
/*****************************************************************************
 ...
 功能描述  : Computes the one complement internet checksum (RFC 1071).
 输入参数  : ulSum - The accumulated 32 bit internet sum.
 输出参数  : 无
 返 回 值  : A one complement standard internet checksum.
 调用函数  :
 被调函数  :
