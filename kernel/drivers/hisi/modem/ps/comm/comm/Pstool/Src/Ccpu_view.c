...
/******************************************************************************
   2 函数声明
******************************************************************************/
extern VOS_UINT32 LOG_GetTick(VOS_VOID);
...
/******************************************************************************
 Prototype      : CpuView_StartTimer
 Description    : 启动任务监测所使用的硬件时钟
 Input          :
 Output         :
 Return Value   :
 Calls          :
...
/******************************************************************************
 Prototype      : CpuView_StopTimer
 Description    : 停止任务监测所使用的硬件时钟
 Input          :
 Output         :
 Return Value   :
 Calls          :
...
VOS_UINT32 CpuView_GetTaskName(VOS_VOID)
{
    ...
    memset(g_astCpuTaskName, 0, sizeof(CPU_TASK_NAME_STRU) * CPU_VIEW_MAX_TASK_NUMBER);
...
VOS_VOID CpuView_OpenTaskWatch(VOS_VOID)
{
    ...
    DRV_VICINT_LVLCHG_HOOK_ADD((FUNCPTR_1)CpuView_IntLevChgInHook);
    DRV_VICINT_EXIT_HOOK_ADD((FUNCPTR_1)CpuView_IntLevChgOutHook);
...
VOS_VOID CpuView_Init(VOS_UINT32 ulRecordMode, VOS_UINT32 ulRecordCnt)
{
    ...
    */
    ...
    if (CPU_VIEW_MEMORY_MODE_DYNAMIC == ulRecordMode)
    {
        ...
        memset(g_pstCpuViewCtrl, 0, sizeof(CPU_VIEW_CTRL_STRU));
...
VOS_VOID CpuView_Close(VOS_VOID)
{
    ...
    VOS_CacheMemFree(g_pstCpuViewCtrl);
