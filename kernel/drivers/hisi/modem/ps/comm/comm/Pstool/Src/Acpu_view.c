...
#define CPU_VIEW_RECORD_DEFAULT_NUM             (10 * 1024)
#define CPU_VIEW_RECORD_MAX_NUM                 (20 * 1024)

#define CPU_VIEW_NO                             (0)
#define CPU_VIEW_YES                            (1)

#define CPU_VIEW_CLK_ID                         (TIMER_CCPU_CPUVIEW_ID)
...
void AcpuView_TaskSwitchHook(void *pOldTcb, void *pNewTcb)
{
    ...
    DRV_TIMER_GET_REST_TIME(CPU_VIEW_CLK_ID, TIMER_UNIT_MS, (unsigned int *)&ulCurrentTick);
    ...
    g_pstAcpuView[g_ulAcpuViewCnt].ulTick     = ulCurrentTick;
...
void AcpuView_OpenTaskWatch(void)
{
    ...
    {
        /*向操作系统注册任务钩子函数*/
        if ( OK != task_switch_hook_add((FUNC_VOID)AcpuView_TaskSwitchHook) )
        {
