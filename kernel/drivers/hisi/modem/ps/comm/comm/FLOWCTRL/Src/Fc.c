/******************************************************************************
  ...
  Version      : V300R002
  Date         : 2011-12-5
  Description  : 流控公共功能模块，包括流控策略、流控点管理等
  Function List:
        ---
        ---
        ---
...
)
{
    ...
    /*lint -e64*/
    PS_LOG(UEPS_PID_FLOWCTRL, PS_SUBMOD_NULL, ulLevel, pcString);
    /*lint +e64*/
    #else
...
)
{
    #ifdef __FC_RELEASE__
    /*lint -e64*/
    PS_LOG1(UEPS_PID_FLOWCTRL, PS_SUBMOD_NULL,
...
)
{
    #ifdef __FC_RELEASE__
    /*lint -e64*/
    PS_LOG2(UEPS_PID_FLOWCTRL, PS_SUBMOD_NULL,
...
)
{
    #ifdef __FC_RELEASE__
    /*lint -e64*/
    PS_LOG3(UEPS_PID_FLOWCTRL, PS_SUBMOD_NULL,
...
)
{
    #ifdef __FC_RELEASE__
    /*lint -e64*/
    PS_LOG4(UEPS_PID_FLOWCTRL, PS_SUBMOD_NULL,
...
)
{
    ...
    FC_POLICY_ChangePoint(enFcPrivatePolicyId, enFcId, enOldPri, enNewPri);
...
)
{
    ...
    {
        FC_POINT_SetFc(FC_POLICY_MASK(pFcPolicy->enPolicyId), pstPri->aenFcId[ulLoop]);
    }
...
)
{
    ...
    for (ulLoop = 0; ulLoop < pstPri->ucFcIdCnt; ulLoop++)
    {
        FC_POINT_ClrFc(FC_POLICY_MASK(pPolicy->enPolicyId), pstPri->aenFcId[ulLoop]);
    }
...
VOS_UINT32 FC_POLICY_Up(FC_POLICY_STRU *pPolicy)
{
    ...
    while (pPolicy->enDonePri < pPolicy->enHighestPri)
    {
        ...
        if (VOS_TRUE == pstPri->ucValid)
        {
            FC_POLICY_UpWithOnePri(pPolicy, pstPri);
            pPolicy->enDonePri++;
            break;
        }
    ...
    FC_MNTN_TracePolicy(ID_FC_MNTN_POLICY_UP, pPolicy);
...
VOS_UINT32  FC_POLICY_UpToTargetPri(FC_POLICY_STRU *pFcPolicy, FC_PRI_ENUM_UINT32 enTargetPri )
{
    ...
    while (pFcPolicy->enDonePri < enTargetPri)
    {
        ...
        if (VOS_TRUE == pstPri->ucValid)
        {
            FC_POLICY_UpWithOnePri(pFcPolicy, pstPri);
        }

        pFcPolicy->enDonePri++;
    }

    FC_MNTN_TracePolicy(ID_FC_MNTN_POLICY_UP, pFcPolicy);
...
VOS_UINT32 FC_POLICY_Down(FC_POLICY_STRU *pFcPolicy)
{
    ...
    while (pFcPolicy->enDonePri > FC_PRI_NULL)
    {
        ...
        if (VOS_TRUE == pstPri->ucValid)
        {
            FC_POLICY_DownWithOnePri(pFcPolicy, pstPri);
            pFcPolicy->enDonePri--;
            break;
        }
    ...
    FC_MNTN_TracePolicy(ID_FC_MNTN_POLICY_DOWN, pFcPolicy);
...
)
{
    ...
    pPolicy = FC_POLICY_Get(enPolicyId);
    FC_POLICY_DelPointForPri(pPolicy, enFcId, enPointPri);
...
)
{
    ...
    /* 刷新该流控点在流控策略的流控点维护 */
    FC_POLICY_DelPointForPri(pPolicy, enFcId, enPointOldPri);
    ...
    {
        FC_POINT_ClrFc(FC_POLICY_MASK(pPolicy->enPolicyId), enFcId);
    }
...
VOS_UINT32  FC_CFG_Init( VOS_VOID )
{
    ...
    if ( NV_OK != ulResult )
    {
        ...
        FC_CFG_SetDefaultValue(&g_stFcCfg);
...
VOS_UINT32  FC_Help( VOS_VOID )
{
    ...
    vos_printf("0 : FC_POLICY_ID_MEM\n");
    vos_printf("1 : FC_POLICY_ID_CPU_A\n");
    vos_printf("2 : FC_POLICY_ID_CDS\n");
    vos_printf("3 : FC_POLICY_ID_CST\n");
    vos_printf("4 : FC_POLICY_ID_GPRS\n");
    vos_printf("5 : FC_POLICY_ID_TMP\n");
    vos_printf("6 : FC_POLICY_ID_CPU_C\n");
    vos_printf("================POLICY ID define end================\n");

    vos_printf("\n================FC ID define start================\n");
    vos_printf("0  : FC_ID_BRIDGE_FORWARD_DISACRD\n");
    vos_printf("1  : FC_ID_USB_ETH\n");
    vos_printf("2  : FC_ID_WIFI_ETH\n");
    vos_printf("3  : FC_ID_NIC_1\n");
    vos_printf("4  : FC_ID_NIC_2\n");
    vos_printf("5  : FC_ID_NIC_3\n");
    vos_printf("6  : FC_ID_NIC_4\n");
    vos_printf("7  : FC_ID_NIC_5\n");
    vos_printf("8  : FC_ID_NIC_6\n");
    vos_printf("9  : FC_ID_NIC_7\n");
    vos_printf("10 : FC_ID_NIC_8\n");
    vos_printf("11 : FC_ID_NIC_9\n");
    vos_printf("12 : FC_ID_NIC_10\n");
    vos_printf("13 : FC_ID_NIC_11\n");
    vos_printf("14 : FC_ID_MODEM\n");
    vos_printf("15 : FC_ID_UL_RATE_1_FOR_CPU\n");
    vos_printf("16 : FC_ID_UL_RATE_2_FOR_CPU\n");
    vos_printf("17 : FC_ID_UL_RATE_3_FOR_CPU\n");
    vos_printf("18 : FC_ID_UL_RATE_4_FOR_CPU\n");
    vos_printf("19 : FC_ID_UL_RATE_5_FOR_CPU\n");
    vos_printf("20 : FC_ID_UL_RATE_6_FOR_CPU\n");
    vos_printf("21 : FC_ID_UL_RATE_7_FOR_CPU\n");
    vos_printf("22 : FC_ID_UL_RATE_8_FOR_CPU\n");
    vos_printf("23 : FC_ID_UL_RATE_9_FOR_CPU\n");
    vos_printf("24 : FC_ID_UL_RATE_10_FOR_CPU\n");
    vos_printf("25 : FC_ID_UL_RATE_11_FOR_CPU\n");
    vos_printf("26 : FC_ID_UL_RATE_1_FOR_TMP\n");
    vos_printf("27 : FC_ID_UL_RATE_2_FOR_TMP\n");
    vos_printf("28 : FC_ID_UL_RATE_3_FOR_TMP\n");
    vos_printf("29 : FC_ID_UL_RATE_4_FOR_TMP\n");
    vos_printf("30 : FC_ID_UL_RATE_5_FOR_TMP\n");
    vos_printf("31 : FC_ID_UL_RATE_6_FOR_TMP\n");
    vos_printf("32 : FC_ID_UL_RATE_7_FOR_TMP\n");
    vos_printf("33 : FC_ID_UL_RATE_8_FOR_TMP\n");
    vos_printf("34 : FC_ID_UL_RATE_9_FOR_TMP\n");
    vos_printf("35 : FC_ID_UL_RATE_10_FOR_TMP\n");
    vos_printf("36 : FC_ID_UL_RATE_11_FOR_TMP\n");
    vos_printf("================FC ID define end================\n");


    vos_printf("\n================FC Debug func start================\n");
    vos_printf("FC SetThreshold Func        : FC_SetThreshold (ulPolicyId, ulParam1, ulSetThreshold, ulStopThreshold)\n");
    vos_printf("FC SetDevLev Func           : FC_SetDebugLev (ulLev)\n");
    vos_printf("FC SetEnableMask Func       : FC_SetFcEnableMask (ulEnableMask)\n");
    vos_printf("================FC Debug func end================\n");


    vos_printf("\n================FC Point information start================\n");
    vos_printf("FC Point information Start\n");
    vos_printf("FC Point Num            : %d\n", g_stFcPointMgr.ulPointNum);
    ...
    for (ulFcIdLoop = 0; ulFcIdLoop < g_stFcPointMgr.ulPointNum; ulFcIdLoop++)
    {
        ...
        vos_printf("FC ID                   : %d\n", pFcPoint->enFcId);
        vos_printf("Modem ID                : %d\n", pFcPoint->enModemId);
        vos_printf("ulPolicyMask        : 0x%x\n", pFcPoint->ulPolicyMask);
    ...
    vos_printf("FC EnableMask           : 0x%x\n", g_stFcCfg.ulFcEnbaleMask);

    for (enPolicyId = FC_PRIVATE_POLICY_ID_MEM_MODEM_0; enPolicyId < FC_PRIVATE_POLICY_ID_BUTT; enPolicyId++)
    {
        pPolicy = FC_POLICY_Get(enPolicyId);

        vos_printf("POLICY ID               : %d\n", enPolicyId);
        vos_printf("PriCnt              : %d\n", pPolicy->ucPriCnt);
        ...
        for (enFcPri = FC_PRI_1; enFcPri < FC_PRI_BUTT; enFcPri++)
        {
            ...
            {
                vos_printf("FcID of Pri %d       : ", enFcPri);

                for (ulFcIdIndex = 0; ulFcIdIndex < pPri->ucFcIdCnt; ulFcIdIndex++)
                {
                    vos_printf("%d, ", pPri->aenFcId[ulFcIdIndex]);
                }
    ...
    vos_printf("================FC POLICY STATUS End================\n");
