...
#define DICC_LOG(Level, String)\
    {\
        (PS_LOG(LOCAL_UEPS_PID_DICC, 0, (Level), (String)));\
        /*lint --e{506, 774}*/\
...
#define DICC_LOG1(Level, String, Para1)\
    {\
        (PS_LOG1(LOCAL_UEPS_PID_DICC, 0, (Level), (String), (Para1)));\
        /*lint --e{506, 774}*/\
        if (DICC_BBIT == g_ucDiccStability)\
        {\
            if (((Level) == PS_PRINT_WARNING) || ((Level) == PS_PRINT_ERROR))\
...
#define DICC_LOG2(Level, String, Para1, Para2)\
    {\
        (PS_LOG2(LOCAL_UEPS_PID_DICC, 0, (Level), (String), (Para1), (Para2)));\
        /*lint --e{506, 774}*/\
        if (DICC_BBIT == g_ucDiccStability)\
        {\
            if (((Level) == PS_PRINT_WARNING) || ((Level) == PS_PRINT_ERROR))\
...
#define DICC_LOG3(Level, String, Para1, Para2, Para3)\
    {\
        (PS_LOG3(LOCAL_UEPS_PID_DICC, 0, (Level), (String), (Para1), (Para2), (Para3)));\
        /*lint --e{506, 774}*/\
        if (DICC_BBIT == g_ucDiccStability)\
        {\
            if (((Level) == PS_PRINT_WARNING) || ((Level) == PS_PRINT_ERROR))\
...
#define DICC_LOG4(Level, String, Para1, Para2, Para3, Para4)\
    {\
        (PS_LOG4(LOCAL_UEPS_PID_DICC, 0, (Level), (String), (Para1), (Para2), (Para3), (Para4)));\
        /*lint --e{506, 774}*/\
        if (DICC_BBIT == g_ucDiccStability)\
        {\
            if (((Level) == PS_PRINT_WARNING) || ((Level) == PS_PRINT_ERROR))\
...
VOS_VOID  DICC_SndEvent(DICC_MNTN_EVENT_ID_ENUM_UINT16 enEventId,
                        ...
                        DICC_CPU_ID_ENUM_UINT8  enCpuId)
{
    PS_OM_EVENT_IND_STRU               *pstDiccEvent;
    VOS_UINT32                          ulLength;
    ...
    {
        DICC_LOG(PS_PRINT_ERROR, "[DICC_SndEvent] alloc event fail");
        return;
    }

    /*填写消息内容*/
    pstDiccEvent->ulLength      = ulLength - 4; /* 不包括ulLength 所占用的字节 */
    ...
    {
        DICC_LOG(PS_PRINT_ERROR, "[LOCAL CPU_ID: DICC_SndEvent] OM_Event fail");
    }
 #elif (OSA_CPU_ACPU == VOS_OSA_CPU)
    if (VOS_OK != OM_AcpuEvent((PS_OM_EVENT_IND_STRU*)pstDiccEvent))
    {
        DICC_LOG(PS_PRINT_ERROR, "[LOCAL CPU_ID: DICC_SndEvent] OM_Event fail");
    }
 #endif
#else
    /* 计划在PC机上测试A核, 但当前OAM组没有把OM_AcpuEvent在WIN32上实现好,
       报不出事件, 暂时用OM_Event上报 */
    ...
    if (DICC_CPU_ID_ACPU == enCpuId)
    {
        if (VOS_OK != OM_AcpuEvent((PS_OM_EVENT_IND_STRU*)pstDiccEvent))
        {
            DICC_LOG(PS_PRINT_ERROR, "[LOCAL CPU_ID: DICC_SndEvent] OM_AcpuEvent fail");
        }
    }
#endif
                          ...
                          DICC_CPU_ID_ENUM_UINT8       enCpuId)
{
    ...
    if (PS_TRUE == pstIsrRecord->enOpResult)    /* 正确操作中断, 记录下轨迹即可 */
    {
        ...
        if (DICC_SERVICE_ISR_ENABLE == pstIsrRecord->enIsrOp)
        {
            ...
            pstDFx->astChanStatus[ucChanId].stIsrOp.ulEnableOpTime = OM_GetSlice();
            pstDFx->astChanStatus[ucChanId].stIsrOp.ulEnableCnt++;
        }
        else
        {
            pstDFx->astChanStatus[ucChanId].stIsrOp.ulDisableOpPid  = pstIsrRecord->ulPid;
            pstDFx->astChanStatus[ucChanId].stIsrOp.ulDisableOpTime = OM_GetSlice();
    ...
    else    /* 当DiableIsr, EnableIsr参数错误时, 记录到Debug中, 只记录下最近一次的情形 */
    {
        if (DICC_SERVICE_ISR_ENABLE == pstIsrRecord->enIsrOp)
        {
            ...
            pstDFx->stIsrDbg.ulInvalidEnableOpTime  = OM_GetSlice();
        }
        else
        {
            pstDFx->stIsrDbg.ulInvalidDisableOpPid  = pstIsrRecord->ulPid;
            pstDFx->stIsrDbg.ulInvalidDisableOpTime = OM_GetSlice();
        }
    }
}    /* DICC_RecordIsrOp */
               ...
               DICC_CHAN_ID_ENUM_UINT8 ucChnId, DICC_CPU_ID_ENUM_UINT8 enCurrCpuId)
{
    ...
    pstDFx->astChanStatus[ucChnId].stStatistics.ulLastTrigIsrTime = OM_GetSlice();
#endif
...
/*Add by y45445 for PS FUSION PC ST 20120109 begin*/
    ...
    if (DICC_CPU_ID_CCPU == enCurrCpuId)
    {
        ...
        lIsrRet = DRV_IPC_INTSEND(enDstCore, (IPC_INT_LEV_E)ulDiccIsrIntLev);
    }
    else
    {
                                  ...
                                  DICC_CPU_ID_ENUM_UINT8  enCpuId)
{
    ...
    if (DICC_CHAN_RECVER_OPEN_MASK == (DICC_CHAN_RECVER_OPEN_MASK & pstDiccChnCtrl->ucStateMask))
    {
        if (0 != TTF_RING_Q_GetCnt(ulPid, (TTF_RING_Q_ST *)(&(pstDiccChnCtrl->stRingBuffer))))
        {
            ...
            pstDFx->astChanStatus[ucChnId].stStatistics.ulLastRemoveTime = OM_GetSlice();
#endif
