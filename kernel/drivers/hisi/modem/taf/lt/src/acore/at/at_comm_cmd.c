...
{
    FTM_SET_LTCOMMCMD_REQ_STRU stSetReq = {EN_LTCOMMCMD_DEST_LDSP_CT, 0, {0}};
 ...
 */
    ...
    if( gastAtParaList[4].usParaLen > FTM_MAX_COMM_CMD_LEN*2)
    {
        return AT_CME_INCORRECT_PARAMETERS;
    }
    ...
    ulCRC = usCRC_H<<16;
    ulCRC = ulCRC + usCRC_L;
    ulCRC_Cal = Calc_CRC32(gastAtParaList[4].aucPara, gastAtParaList[4].usParaLen);
    
    printk("LTCOMMCMD DEST:%d, LEN:%d, CRC_L:%d,CRC_H:%d, CRC_CAL:%d, DATA:%s\n", 
        (VOS_INT)gastAtParaList[0].ulParaValue,
        (VOS_INT)gastAtParaList[1].ulParaValue,
        (VOS_INT)gastAtParaList[2].ulParaValue,
        (VOS_INT)gastAtParaList[3].ulParaValue,
        (VOS_INT)ulCRC_Cal,
    ...
    stSetReq.ulCmdDest = (FTM_LTCOMMCMD_DEST)(gastAtParaList[0].ulParaValue);
    stSetReq.ulDataLen = (VOS_UINT32)(gastAtParaList[1].ulParaValue)/2;
    for(i=0; i<stSetReq.ulDataLen; i++)
    {
        stSetReq.cData[i] = Hec2ToVal(gastAtParaList[4].aucPara+2*i);
		/* printk("%02X", (VOS_UINT8)stSetReq.cData[i]); */
    }

    ulRst = atSendFtmDataMsg(MSP_SYS_FTM_PID, ID_MSG_FTM_SET_LTCOMMCMD_REQ, ucClientId, 
        (VOS_VOID*)(&stSetReq), sizeof(stSetReq));
