...
/*    ulNVLen = (ulNVLen > 128)?128:ulNVLen;*/
    ...
    gstAtSendData.usBufLen = 0;
    gstAtSendData.usBufLen = (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN, (VOS_CHAR*)pgucAtSndCodeAddr,(VOS_CHAR*)pgucAtSndCodeAddr,
        "^NVRD: %d,%d,%d,", usNVID, usOffset, usRdLen);
...
VOS_UINT32 atIsmcoexCnfProc(VOS_UINT8 ucClientId, VOS_VOID *pMsgBlock)
{
    OS_MSG_STRU* pEvent = NULL;
    SET_ISMCOEX_CNF_STRU *pstIsmcoex = NULL;

    pEvent = (OS_MSG_STRU*)(((MsgBlock*)pMsgBlock)->aucValue);
    pstIsmcoex = (SET_ISMCOEX_CNF_STRU *)pEvent->ulParam1;

    if(AT_SUCCESS == pstIsmcoex->ulErrCode)
    {
        VOS_MemCpy(g_astCnfIsmcoex, g_astIsmcoex, L4A_ISMCOEX_BANDWIDTH_NUM * sizeof(ISMCOEX_INFO_STRU));
    }

    CmdErrProc(ucClientId, pstIsmcoex->ulErrCode, 0, NULL);
