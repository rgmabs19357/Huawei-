 ...
 ******************************************************************************
  文 件 名   : Taf_MmiStrParse.c
  版 本 号   : 初稿
  生成日期   : 2008年3月14日
  最近修改   :
  功能描述   : 对应用输入的字串进行解析，输出对应的操作和相应的参数
  函数列表   :
  修改历史   :
  1.日    期   : 2008年3月14日
    修改内容   : 创建文件
        ...
        while ((j < MN_MMI_MAX_PARA_NUM) && (VOS_FALSE == bEndReached))
        {
            if (VOS_NULL_PTR != pcNextStop)
            {
                PS_MEM_CPY(&aucTmpVal[j][0],
                           pcNextStart,
                           (VOS_UINT32)(pcNextStop - pcNextStart));
...
)
{
    ...
    *pulErrCode = MN_ERR_NO_ERROR;
    ...
    if (VOS_TRUE == MMI_DecodeScAndSi(pMmiStr, pMmiOpParam, &stScSiPara, ppOutRestMmiStr))
    {
        ...
        {
            VOS_StrNCpy((VOS_CHAR*)pMmiOpParam->PinReq.aucOldPin,
                        (VOS_CHAR*)stScSiPara.acSia,
                        VOS_StrLen(stScSiPara.acSia) + 1);
            VOS_StrNCpy((VOS_CHAR*)pMmiOpParam->PinReq.aucNewPin,
    ...
    *pulErrCode = MN_ERR_NO_ERROR;
                                      ...
                                      ppOutRestMmiStr))
    {
                            ...
                            MMI_Max(VOS_StrLen(stScSiPara.acSib), VOS_StrLen(stScSiPara.acSic))))
        {
            ...
            VOS_StrNCpy((VOS_CHAR*)pMmiOpParam->RegPwdReq.aucOldPwdStr,
                        (VOS_CHAR*)stScSiPara.acSia,
                        VOS_StrLen(stScSiPara.acSia) + 1);
            VOS_StrNCpy((VOS_CHAR*)pMmiOpParam->RegPwdReq.aucNewPwdStr,
                        (VOS_CHAR*)stScSiPara.acSib,
                        VOS_StrLen(stScSiPara.acSib) + 1);
            VOS_StrNCpy((VOS_CHAR*)pMmiOpParam->RegPwdReq.aucNewPwdStrCnf,
