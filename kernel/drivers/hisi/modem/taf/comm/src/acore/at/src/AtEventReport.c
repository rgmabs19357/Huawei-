...
/*****************************************************************************
 Prototype      : At_UnicodeFormatPrint
 Description    : 电话本姓名编码转换函数
 Input          : pucDst   --- 目的字串
                  pucSrc   --- 源字串
                  Dcs      --- 编码类型，81或82
 Output         :
...
)
{
    ...
    /* 申请AT_ADS_PDP_STATUS_IND_STRU消息 */
...
VOS_VOID AT_ReportSysCfgQryCmdResult(
    TAF_PH_SYS_CFG_EX_STRU             *pstSysCfg,
    VOS_UINT8                           ucIndex,
    VOS_UINT16                         *pusLength
)
{
...
/*****************************************************************************
 Prototype      : At_HexString2AsciiNumPrint
 Description    : Unicode到Unicode打印转换
 Input          : ucNumType--- 号码类型
                  pucDst   --- 目的字串
                  usDstLen --- 目的字串长度
...
/*****************************************************************************
 Prototype      : At_QryParaRspCpamProc
 Description    : 参数查询结果Cpam的上报处理
 Input          : usClientId --- 用户ID
                  OpId       --- 操作ID
                  QueryType  --- 查询类型
                  pPara      --- 结果
 ...
 History        : ---
  1.Date        : 2005-04-19
    Modification: Created function
  2.日    期   : 2013年01月11日
    修改内容   : DSDA GUNAS C CORE项目，增加异常查询处理
*****************************************************************************/
...
VOS_VOID AT_QryParaRspPrefPlmnProc(VOS_VOID *pMsg)
{
    VOS_UINT32                          ulResult = AT_FAILURE;
    VOS_UINT16                          usLength = 0;
...
/*****************************************************************************
 函 数 名  : AT_QryParaRspCsqlvlExtProc
 功能描述  : E5全球版本信号格数查询命令 ^CSQLVLEXT响应事件处理
 输入参数  : VOS_UINT8 ucIndex
 输出参数  : 无
 返 回 值    : VOS_UINT32
 调用函数  :
...
/*****************************************************************************
 函 数 名  : AT_QryParaRspCsqlvlProc
 功能描述  : +CSQLVL响应事件处理，输出查询信号格数(autochecker工具查询)，包括RSCP 等级和RSCP 的绝对值
 输入参数  : VOS_UINT8 ucIndex
 输出参数  : 无
 返 回 值  : VOS_UINT32
 调用函数  :
...
#ifdef FEATURE_UPGRADE_TL
/*****************************************************************************
 Prototype      : AT_USIM_BcdToAscii
 Description    : At_USIMMsgProc SIM消息处理函数调用函数
 Input          :
 Output         :
 Return Value   : ---
 Calls          : ---
 ...
 History        : ---
*****************************************************************************/
VOS_VOID AT_USIM_BcdToAscii(VOS_UINT8 ucBcdNumLen,VOS_UINT8 *pucBcdNum, VOS_UINT8 *pucAsciiNum, VOS_UINT8 *pucLen)
{
...
/*****************************************************************************
 Prototype      : AT_USIM_CheckContentValidity
 Description    : At_USIMMsgProc SIM消息处理函数调用函数
 Input          :
 Output         :
 Return Value   : ---
 ...
 History        : ---
*****************************************************************************/
VOS_UINT32 AT_USIM_CheckContentValidity(SI_PB_CONTENT_STRU *pstPBContent, VOS_UINT8 *pContent)
{
    if ((VOS_NULL_PTR == pContent)||(VOS_NULL_PTR == pstPBContent))
    {
        printk("AT_USIM_CheckContentValidity: Input NULL PTR\n");
    ...
    {
        printk("SI_PB_CheckContentValidity: The PhoneBook Content is Empty\n");
        
        return VOS_ERR;
    }
    else
    {
        printk("SI_PB_CheckContentValidity: The PhoneBook Content is Not Empty\n");
...
/*****************************************************************************
 Prototype      : AT_USIM_DecodePBName
 Description    : At_USIMMsgProc SIM消息处理函数调用函数
 Input          :
 Output         :
 Return Value   : ---
 ...
 History        : ---
*****************************************************************************/
VOS_VOID AT_USIM_DecodePBName(VOS_UINT8 ucNameMax, VOS_UINT8 *pucName,
...
/*****************************************************************************
 Prototype      : AT_USIM_TransPBFromate
 Description    : At_USIMMsgProc SIM消息处理函数调用函数
 Input          :
 Output         :
 Return Value   : ---
 ...
 History        : ---
*****************************************************************************/
VOS_VOID AT_USIM_TransPBFromate(SI_PB_CONTENT_STRU *pstPBContent, VOS_UINT16 usIndex, VOS_UINT8 *pContent, SI_PB_RECORD_STRU *pstRecord)
{
    VOS_UINT32 ulResult;
    VOS_UINT8  ucXdnNumLen = 0;

    ulResult = AT_USIM_CheckContentValidity(pstPBContent, pContent);/*检查当前的内容是否有效*/
    ...
    else
    {
        ...
        AT_USIM_DecodePBName(pstPBContent->ucNameLen, pContent,
        ...
        AT_USIM_BcdToAscii(ucXdnNumLen, &pContent[pstPBContent->ucNameLen+2],
...
/*****************************************************************************
 Prototype      : AT_USIM_GetBitFromBuf
 Description    : At_USIMMsgProc SIM消息处理函数调用函数
 Input          :
 Output         :
 Return Value   : ---
 ...
 History        : ---
*****************************************************************************/
VOS_UINT32 AT_USIM_GetBitFromBuf(VOS_UINT8 *pucDataBuf, VOS_UINT32 ulBitNo)
{
    VOS_UINT32  ulOffset;
    VOS_UINT8   ucBit;
...
/*****************************************************************************
 Prototype      : At_USIMMsgProc
 Description    : SIM消息处理函数调用函数
 Input          :
 Output         :
 Return Value   : ---
 Calls          : ---
 ...
 History        : ---
*****************************************************************************/
TAF_VOID At_USIMMsgProc(MsgBlock* pMsg)
{
    PS_USIM_GET_FILE_CNF_STRU   *pstMsg;
    TAF_UINT8                   ucIndex = 0;
    VOS_UINT32                  ulResult = AT_FAILURE;
    SI_PB_EVENT_INFO_STRU       pEvent;
    VOS_UINT32                  i = 0;
    SI_PB_CONTENT_STRU          pPBContect;
    VOS_UINT8                   *pucContent;
    VOS_UINT32                  usLength=0;

    pstMsg = (PS_USIM_GET_FILE_CNF_STRU*)pMsg;

    AT_LOG1("At_USIMMsgProc pEvent->ClientId",    pstMsg->ulSendPara);
    AT_LOG1("At_USIMMsgProc Event Error",         pstMsg->ulErrorCode);

    if (AT_FAILURE == At_ClientIdToUserId(pstMsg->ulSendPara, &ucIndex))
    {
        AT_ERR_LOG1("At_PbMsgProc At_ClientIdToUserId FAILURE", pstMsg->ulSendPara);
        return;
    }

    AT_STOP_TIMER_CMD_READY(ucIndex);

    AT_LOG1("At_PbMsgProc ucIndex is 0x%x\n",ucIndex);
    AT_LOG1("gastAtClientTab[ucIndex].CmdCurrentOpt",gastAtClientTab[ucIndex].CmdCurrentOpt);

    pPBContect.enPBType = PB_MISDN;
    pPBContect.ucRecordLen = pstMsg->usEfLen;
    pPBContect.usTotalNum  = pstMsg->ucTotalNum;
    pPBContect.ucNameLen   = pstMsg->usEfLen-SI_PB_EXCEPT_NAME;  // - SI_PB_EXCEPT_NAME
    pPBContect.ucNumberLen = pstMsg->usEfLen;
    pPBContect.ulExtInfoNum   = PB_MSISDN_CONTENT;
    pPBContect.pContent = pstMsg->aucEf;

    for(i=0; i<pstMsg->ucTotalNum; i++)     /*根据读取的电话本索引范围循环*/
    {
        VOS_MemSet((VOS_UINT8 *)&pEvent.PBEvent.PBReadCnf, 0, sizeof(SI_PB_EVENT_READ_CNF_STRU));

        AT_USIM_TransPBFromate(&pPBContect, (VOS_UINT16)(i), pPBContect.pContent, &pEvent.PBEvent.PBReadCnf.PBRecord);

        pucContent += pPBContect.ucRecordLen;/*内容指针按照记录长度偏移*/
        ...
        pEvent.PBLastTag = 0;
        pEvent.PBError = TAF_ERR_NO_ERROR;

        ulResult = At_PbCNUMCmdPrint(ucIndex,&usLength,pgucAtSndCodeAddr,&pEvent);
...
#if (FEATURE_VSIM == FEATURE_ON)
/*****************************************************************************
 函 数 名  : At_StkHvsmrIndPrint
 功能描述  : HVSMR上报主动命令的打印输出函数
 输入参数  : ucIndex --用户索引
             pEvent -- 指向待打印的数据结构
 输出参数  : 无
 返 回 值  : 无
...
/*****************************************************************************
 函 数 名  : AT_RcvDrvAgentHvpdhSetCnf
 功能描述  : 处理DH秘钥设置结果
 输入参数  : VOS_VOID *pMsg - 消息指针
 输出参数  : 无
 返 回 值  : VOS_VOID
...
/*****************************************************************************
 函 数 名  : AT_PhEOPlmnSetCnfProc
 功能描述  : 处理TAF_MMA_EVT_USER_CFG_OPLMN_SET_CNF
