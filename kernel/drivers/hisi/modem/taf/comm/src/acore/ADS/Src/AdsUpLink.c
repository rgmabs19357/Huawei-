...
{
    AT_PDP_STATUS_ENUM_UINT8            enPdpStatus;
    VOS_UINT8                           ucRabId;
    AT_ADS_QCI_TYPE_ENUM_UINT8          enPrio;
    ...
    ucRabId        = pstPdpStatusInd->ucRabId;
    enPrio         = pstPdpStatusInd->enQciType;
    enPdpStatus    = pstPdpStatusInd->enPdpStatus;
    ...
    {
        ADS_WARNING_LOG1(ACPU_PID_ADS_UL, "ADS_UL_RcvAtPdpStatusInd: ucRabId is ", ucRabId);
        return VOS_ERR;
    }
    ...
    {
        enPrio = AT_ADS_QCI_TYPE_QCI9_NONGBR;
    }
        ...
        for (ucRabId = ADS_RAB_ID_MIN; ucRabId <= ADS_RAB_ID_MAX; ucRabId++)
        {
            if (VOS_OK == ADS_UL_IsQueueExistent(ucInstanceIndex, ucRabId))
            {
                ...
                ADS_UL_ClearQueue(ucRabId,
                                  ...
                                  &(pstAdsUlCtx->astAdsUlQueue[ucRabId].stSpinLock));
            }
        }
    }
    else
...
VOS_UINT32 ADS_UL_RcvAdsUlMsg(MsgBlock *pMsg)
{
    ...
    switch(pstMsg->ulMsgName)
    {
        case ID_CCPU_ADS_UL_RESET_START_IND:
            ADS_UL_RcvCcpuResetStartInd(pMsg);
            break;

        case ID_CCPU_ADS_UL_RESET_END_IND:
