...
VOS_UINT32 At_QrySecuBootPara(VOS_UINT8 ucIndex)
{
    ...
    {
        gastAtClientTab[ucIndex].CmdCurrentOpt = AT_CMD_SECUBOOT_QRY;             /*设置当前操作模式 */
...
VOS_UINT32 At_QryCgcattPara(VOS_UINT8 ucIndex)
{
    ...
    ulRst  = AT_FillAndSndAppReqMsg(gastAtClientTab[ucIndex].usClientId,
                                    ...
                                    (VOS_VOID*)&ucType,  /*要求查询CS和PS的注册状态*/
                                    sizeof(VOS_UINT8),
                                    I0_WUEPS_PID_MMA);
...
VOS_UINT32 At_QryAutoAttach(TAF_UINT8 ucIndex)
{
    if (AT_SUCCESS == Taf_ParaQuery(gastAtClientTab[ucIndex].usClientId,
                                    ...
                                    TAF_NULL_PTR))
    {
        gastAtClientTab[ucIndex].CmdCurrentOpt = AT_CMD_CAATT_READ;             /*设置当前操作模式 */
        return AT_WAIT_ASYNC_RETURN;                                            /* 等待异步事件返回 */
    }
    else
    {
...
VOS_UINT32 AT_QrySysCfgExPara(VOS_UINT8 ucIndex)
{
    ...
    stSysCfgExSetPara.ucCmdType = TAF_PH_CMD_QUERY;
    ...
    if (AT_SUCCESS == ulRst)
    {
        /* 设置当前操作模式为AT_CMD_SYSCFGEX_READ,等待异步事件返回 */
...
TAF_UINT32 At_QrySysCfgPara(TAF_UINT8 ucIndex)
{
    ...
    stSysCfgSetPara.ucCmdType           = TAF_PH_CMD_QUERY;
...
{
    /* 给MMA发送新参数查询消息TAF_PH_HOMEPLMN_VALUE_PARA */
    if(AT_SUCCESS == Taf_ParaQuery(gastAtClientTab[ucIndex].usClientId,
                                   0,
...
{
    /* 给MMA发送消息，查询新参数类型TAF_PH_ANQUERY_VALUE_PARA */
    if(AT_SUCCESS == Taf_ParaQuery(gastAtClientTab[ucIndex].usClientId,
                                   0,
...
VOS_UINT32 AT_QryAcpuMemInfoPara(TAF_UINT8 ucIndex)
{
    ...
    /*该命令只需要调用TTF的接口即可，由TTF向SDT上报内存信息，AT命令本身不需要显示*/
...
VOS_UINT32 AT_QryLogCfgPara(VOS_UINT8 ucIndex)
{
    ...
    if(NV_OK != NV_ReadEx(MODEM_ID_0, en_NV_ID_SOCP_SDLOG_CFG, (VOS_VOID *)(&stSocpCfg), sizeof(NV_SOCP_SDLOG_CFG_STRU)))
    {
        return AT_ERROR;
    }

    ulResult = BSP_SOCP_GetSDLogCfg(&stSocpCfg.ulTimeOutValue);
