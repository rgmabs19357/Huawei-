...
VOS_VOID AT_ReadIpv6BackProcExtCauseNV(VOS_VOID)
{
    ...
    AT_NV_IPV6_BACKPROC_EXT_CAUSE_STRU  stNvBackProcExtCause;
    ...
    PS_MEM_SET(&stNvBackProcExtCause, 0x00, sizeof(AT_NV_IPV6_BACKPROC_EXT_CAUSE_STRU));

    ulRslt = NV_ReadEx(MODEM_ID_0,
                       en_NV_Item_IPV6_BACKPROC_EXT_CAUSE,
                       &stNvBackProcExtCause,
                       sizeof(AT_NV_IPV6_BACKPROC_EXT_CAUSE_STRU));

    if ( (NV_OK == ulRslt)
      && (VOS_TRUE == stNvBackProcExtCause.ulActiveFlag) )
    {
        for (ulCnt = 0 ; ulCnt < AT_PS_IPV6_BACKPROC_EXT_CAUSE_MAX_NUM; ulCnt++)
        {
            /* 检查(E)SM Cause是否有效 */
            if (!AT_PS_IS_SM_CAUSE_VALID(stNvBackProcExtCause.aucSmCause[ulCnt]))
            {
                continue;
            }
            else
            {
                pstPsBackProcExtCause->aenPsCause[ulCauseNum] = AT_PS_CONVERT_SM_CAUSE_TO_PS_CAUSE(stNvBackProcExtCause.aucSmCause[ulCnt]);
                ulCauseNum++;
            }
        }
    }
