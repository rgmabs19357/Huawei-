...
{
    PS_MEM_SET(ADS_FILTER_DBG_GET_STATS_ARRAY_ADDR(), 0, sizeof(VOS_UINT32) * ADS_FILTER_ORIG_PKT_BUTT);
    return;
}
...
VOS_VOID ADS_FILTER_ShowIPv6Addr(VOS_VOID)
{
    ...
    for (ucLoop = 0; ucLoop < ADS_IPV6_ADDR_LEN; ucLoop++)
    {
        ...
        vos_printf("%02x", pucIPv6Addr[ucLoop]);
    }
