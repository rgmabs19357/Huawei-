...
)
{
    switch (pstMsg->ulMsgName)
    {
        ...
        case AT_MMA_OM_MAINTAIN_INFO_IND :
            AT_RcvMmaOmMaintainInfoInd((struct MsgCB *)pstMsg);
            break;
#endif


        case AT_MMA_USIM_STATUS_IND :
            /* 增加USIM状态消息处理函数，保存USIM相关信息 */
            AT_RcvAtMmaUsimStatusInd(pstMsg);
            break;

        case AT_MMA_CRPN_QUERY_RSP:
            AT_RcvMmaCrpnQueryRsp(pstMsg);
            break;

        case AT_MMA_CMM_SET_CMD_RSP:
            AT_RcvMmaCmmSetCmdRsp(pstMsg);
            break;


        case AT_MMA_CIPHER_INFO_QUERY_CNF:
            AT_RcvMmaCipherInfoQueryCnf(pstMsg);
            break;

        case AT_MMA_LOCATION_INFO_QUERY_CNF:
            AT_RcvMmaLocInfoQueryCnf(pstMsg);
            break;
        case AT_MMA_SIMLOCK_STAUS_QUERY_CNF:
            AT_RcvSimLockQryRsp(pstMsg);
            break;
        case AT_MMA_AC_INFO_QUERY_CNF:
            AT_RcvMmaAcInfoQueryCnf(pstMsg);
            break;

        case AT_MMA_CERSSI_INFO_QUERY_CNF:
            AT_RcvMmaCerssiInfoQueryCnf(pstMsg);
            break;
        case AT_MMA_COPN_INFO_QUERY_CNF:
            AT_RcvMmaCopnInfoQueryCnf(pstMsg);
            break;

        case AT_MMA_SIM_INSERT_RSP:
            AT_RcvMmaSimInsertRsp(pstMsg);
            break;
        case AT_MMA_CPOL_INFO_QUERY_CNF:
            AT_QryParaRspPrefPlmnProc(pstMsg);
            break;
        default:
            break;
    }
...
/*****************************************************************************
 函 数 名  : AT_RcvDrvAgentSetSecuBootRsp
 功能描述  : ^SECUBOOT的设置回复处理函数
 输入参数  : VOS_VOID *pMsg
 输出参数  : 无
 返 回 值  : VOS_UINT32
 调用函数  :
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2012年2月03日
    修改内容   : 新生成函数
*****************************************************************************/
VOS_VOID AT_RcvSimLockQryRsp(VOS_VOID *pMsg)
{
    VOS_UINT8                           ucIndex;
    VOS_UINT32                          ulRet;
...
#ifndef __PS_WIN32_RECUR__
VOS_VOID AT_RcvMmaOmMaintainInfoInd(
    struct MsgCB                       *pstMsg
)
{
