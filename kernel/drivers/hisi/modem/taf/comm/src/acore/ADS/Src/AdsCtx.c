...
)
{
    ...
    /* 队列加锁 */
    VOS_SpinLockIntLock(pstSpinLock, ulLockLevel);

    ulQueueCnt = IMM_ZcQueueLen(pstQueue);

    /* 队列操作完成解锁 */
    VOS_SpinUnlockIntUnlock(pstSpinLock, ulLockLevel);
...
)
{
    ...
    ADS_UL_ClearQueue(ucRabId,
                      ...
                      ADS_UL_GET_QUEUE_LINK_SPINLOCK(ucInstanceIndex, ucRabId));
...
VOS_VOID ADS_InitUlCtx(VOS_UINT8 ucInstanceIndex)
{
    ...
    for (i = 0; i < ADS_RAB_ID_MAX + 1; i++)
    {
        ...
        pstAdsSpecCtx->stAdsUlCtx.astAdsUlQueue[i].enPrio          = AT_ADS_QCI_TYPE_BUTT;
