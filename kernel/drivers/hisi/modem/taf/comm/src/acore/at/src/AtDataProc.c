...
*****************************************************************************/
AT_ADS_QCI_TYPE_ENUM_UINT8 AT_QosParaTransferToQci(
    TAF_UMTS_QOS_STRU                   *pQos
)
{
    AT_ADS_QCI_TYPE_ENUM_UINT8                 enQci;
    ...
    */
    ...
    if (AT_APP_QOS_TRAFFIC_CLASS_CONVERSATIONAL == pQos->ucTrafficClass)
    {
        ...
        {
            enQci       = AT_ADS_QCI_TYPE_QCI1_GBR;
        }
        else if (pQos->usTransDelay >= AT_APP_QOS_TRANSFER_DELAY_150MS)
        {
            enQci       = AT_ADS_QCI_TYPE_QCI2_GBR;
        }
        else
        {
            enQci       = AT_ADS_QCI_TYPE_QCI3_GBR;
        }
    }
    else if (AT_APP_QOS_TRAFFIC_CLASS_STREAMING == pQos->ucTrafficClass)
    {
        enQci       = AT_ADS_QCI_TYPE_QCI4_GBR;
    }
    else if (AT_APP_QOS_TRAFFIC_CLASS_INTERACTIVE == pQos->ucTrafficClass)
    {
        if (AT_APP_QOS_TRAFFIC_HANDLING_PRIORITY_1 == pQos->ucTraffHandlePrior)
        {
            ...
            {
                enQci       = AT_ADS_QCI_TYPE_QCI5_NONGBR;
            }
            else
            {
                enQci       = AT_ADS_QCI_TYPE_QCI6_NONGBR;
            }
        }
        else if (AT_APP_QOS_TRAFFIC_HANDLING_PRIORITY_2 == pQos->ucTraffHandlePrior)
        {
            enQci       = AT_ADS_QCI_TYPE_QCI7_NONGBR;
        }
        else
        {
            enQci       = AT_ADS_QCI_TYPE_QCI8_NONGBR;
        }
    }
    else
    {
        enQci       = AT_ADS_QCI_TYPE_QCI9_NONGBR;
    }
...
VOS_UINT32 AT_SendAdsPdpStatusInd(
    VOS_UINT32                          ulPid,
    AT_ADS_PDP_STATUS_IND_STRU         *pstAdsInd
)
{
    AT_ADS_PDP_STATUS_IND_STRU          *pstMsg;

    /* 申请AT_ADS_PDP_STATUS_IND_STRU消息 */
    pstMsg = (AT_ADS_PDP_STATUS_IND_STRU *)PS_ALLOC_MSG_WITH_HEADER_LEN(
                            WUEPS_PID_AT,
                            sizeof(AT_ADS_PDP_STATUS_IND_STRU));
    ...
    PS_MEM_SET((VOS_CHAR*)pstMsg + VOS_MSG_HEAD_LENGTH,
               0x00,
               sizeof(AT_ADS_PDP_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    /* 填写消息头 */
    pstMsg->ulReceiverCpuId = VOS_LOCAL_CPUID;
    pstMsg->ulReceiverPid   = ulPid;
    pstMsg->enMsgId         = ID_AT_ADS_PDP_STATUS_IND;
...
{
    VOS_UINT32                          ulRet;
    AT_ADS_PDP_STATUS_IND_STRU          stAdsInd;

    PS_MEM_SET(&stAdsInd, 0x00, sizeof(AT_ADS_PDP_STATUS_IND_STRU));
    ...
    /* 填写参数 */
    stAdsInd.enPdpStatus    = AT_ADS_PDP_STATUS_MODIFY;
...
)
{
    ...
    /* 填写参数 */
    stAdsInd.enPdpStatus    = AT_ADS_PDP_STATUS_ACT;
...
)
{
    ...
    /* 填写参数 */
    stAdsInd.enPdpStatus    = AT_ADS_PDP_STATUS_DEACT;
    stAdsInd.enPdpType      = pstEvent->enPdpType;
    stAdsInd.ucRabId        = pstEvent->ucRabId;
    stAdsInd.enQciType      = AT_ADS_QCI_TYPE_BUTT;
...
)
{
    ...
    stPdpPrimContextExt.enPdpType   = ucPdpType;
    stPdpPrimContextExt.ucCid       = ucCid;

    /* 设置PDP上下文 */
    if (VOS_ERR == TAF_AGENT_SetPdpCidPara(usClientId,
                                          &stPdpPrimContextExt))
    {
        AT_ERR_LOG("AT_NdisPdpActOrig: ERROR: Set Pdp Context Fail.");
        return;
    }
...
VOS_VOID AT_AppPsRecoverState(VOS_VOID)
{
    ...
    {
         /* 通知上下行ADS PDP 去激活 */
         AT_SendAdsPdpStatusInd(MODEM_ID_0, pstAppPdpEntity->ucIpv4Cid,AT_ADS_PDP_STATUS_DEACT,ACPU_PID_ADS_DL,VOS_NULL_PTR);
         AT_SendAdsPdpStatusInd(MODEM_ID_0, pstAppPdpEntity->ucIpv4Cid,AT_ADS_PDP_STATUS_DEACT,ACPU_PID_ADS_UL,VOS_NULL_PTR);
    ...
    {
         /* 通知上下行ADS PDP 去激活 */
         AT_SendAdsPdpStatusInd(MODEM_ID_0, pstAppPdpEntity->ucIpv6Cid,AT_ADS_PDP_STATUS_DEACT,ACPU_PID_ADS_DL, VOS_NULL_PTR);
         AT_SendAdsPdpStatusInd(MODEM_ID_0, pstAppPdpEntity->ucIpv6Cid,AT_ADS_PDP_STATUS_DEACT,ACPU_PID_ADS_UL, VOS_NULL_PTR);
    ...
    {
        /* 通知上下行ADS PDP 去激活 */
         AT_SendAdsPdpStatusInd(MODEM_ID_0, pstAppPdpEntity->ucIpv4v6Cid,AT_ADS_PDP_STATUS_DEACT,ACPU_PID_ADS_DL, VOS_NULL_PTR);
         AT_SendAdsPdpStatusInd(MODEM_ID_0, pstAppPdpEntity->ucIpv4v6Cid,AT_ADS_PDP_STATUS_DEACT,ACPU_PID_ADS_UL, VOS_NULL_PTR);
...
)
{
    ...
    if (VOS_ERR == TAF_AGENT_SetPdpCidPara(usClientId,
                                          &stPdpPrimContextExt))
    {
        AT_ERR_LOG("AT_AppPdpActOrig: ERROR: Set Pdp Context Fail.");
        return;
    }
...
)
{
    ...
    {
        if (AT_SUCCESS == AT_LenStr2IpAddr(pstEvent->stPdpAddr.aucIpv4Addr, (VOS_UINT8 *)&ulIpAddr))
        {
            pstPsModemCtx->aulIpAddrRabIdMap[pstEvent->ucRabId - AT_PS_RABID_OFFSET] = VOS_NTOHL(ulIpAddr);
        }
    }
...
/*****************************************************************************
 ...
 输入参数  : RNIC_RM_NET_ID_ENUM_UINT8           enRnicRmNetId
...
)
{
    ...
    TAF_PDP_PRIM_CONTEXT_EXT_STRU       stCidParamInfo;
    TAF_PS_DIAL_PARA_STRU               stPsDialParamInfo;
    VOS_UINT32                          ulRslt;

    PS_MEM_SET(&stPsDialParamInfo, 0x00, sizeof(TAF_PS_DIAL_PARA_STRU));
    PS_MEM_SET(&stCidParamInfo, 0x00, sizeof(TAF_PDP_PRIM_CONTEXT_EXT_STRU));

    /* 填写CID上下文参数 */
    AT_GetPdpContextFromAtDialParam(&stCidParamInfo, pstCallDialParam);

#ifdef DMT
    ulRslt = MN_APS_SetPdpCidPara(usClientId, &stCidParamInfo);
#else
    /* 设置CID上下文参数 */
    ulRslt = TAF_AGENT_SetPdpCidPara(usClientId, &stCidParamInfo);

    if (VOS_OK != ulRslt)
    {
        AT_ERR_LOG("AT_PS_SetupCall: Set CID parameter failed.");
        return VOS_ERR;
    }
#endif
