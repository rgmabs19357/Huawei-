...
#if (OSA_CPU_ACPU == VOS_OSA_CPU)
extern VOS_VOID OM_AcpuICCInfoRecord(MsgBlock *pMsgCtrlBlk);
#endif
...
VOS_VOID VOS_ExecuteAwakeFun(MsgBlock *pstMsgCtrlBlk)
{
    ...
    if ( VOS_NULL_PTR == g_pfnVosAwakeFunHook )
    {
        return;
    }
    
    (g_pfnVosAwakeFunHook)(pstMsgCtrlBlk);
...
#if (OSA_CPU_ACPU == VOS_OSA_CPU)
    OM_AcpuICCInfoRecord(pMsgCtrlBlk);
#endif
