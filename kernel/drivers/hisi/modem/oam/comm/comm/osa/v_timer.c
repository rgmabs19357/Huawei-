...
VOS_VOID VOS_Stop26MHardTimer(VOS_VOID)
{
    ...
    if ( VOS_OK != DRV_TIMER_STOP(VOS_26M_TIMER_ID) )
    {
        g_st26MSocTimerInfo.ulStopErrCount++;
    }
...
VOS_VOID VOS_Start26MHardTimer( VOS_UINT32 value )
{
    ...
    if ( VOS_OK != DRV_TIMER_START(VOS_26M_TIMER_ID, (FUNCPTR_1)VOS_26MDualTimerIsr, VOS_26M_TIMER_ID, value, TIMER_ONCE_COUNT,TIMER_UNIT_NONE) )
    {
...
VOS_UINT32 VOS_Get26MHardTimerElapsedTime(VOS_VOID)
{
    ...
    if ( VOS_OK != DRV_TIMER_GET_REST_TIME(VOS_26M_TIMER_ID, TIMER_UNIT_NONE, (VOS_UINT*)&ulTempValue) )
    {
            ...
            while ( VOS_NULL_PTR != vos_Timer_expire_head_Ptr )
            {
                ...
                if ( VOS_NULL_PTR == vos_Timer_expire_head_Ptr->CallBackFunc )
                {
                    ...
                    if ( VOS_NULL_PTR != pstExpireMsg )
                    {
                        ...
                        pstExpireMsg->hTm    = (HTIMER)(vos_Timer_expire_head_Ptr->TimeOutValueInMilliSeconds);
                           ...
                           VOS_UINT32 ulFileID, VOS_INT32 usLineNo )
{
    ...
    {
        TimerId = *((VOS_UINT32 *)(*phTm));
        if(TimerId <= vos_TimerCtrlBlkNumber)
        {
    ...
    {
        DRV_SYSTEM_ERROR(VOS_CHECK_TIMER_RANG, (VOS_INT)ulFileID, (VOS_INT)usLineNo, (VOS_CHAR *)phTm, 128);
    }
    
    VOS_SIMPLE_FATAL_ERROR(VOS_CHECK_TIMER_NOT_EQUAL);
    ...
    VOS_UINT32 ulFileID, VOS_INT32 usLineNo)
{
    ...
    if( VOS_NULL_PTR == Timer )
    {
        ...
        VOS_TimerDump(START_26M_RELTIMER_FAIL_TO_ALLOCATE, ulFileID, usLineNo);
    ...
    VOS_UINT32 ulFileID, VOS_INT32 usLineNo)
{
    ...
    if(Timer == VOS_NULL_PTR)
    {
        ...
        VOS_TimerDump(START_CALLBACK_RELTIMER_FAIL_TO_ALLOCATE, ulFileID, usLineNo);
...
VOS_VOID VOS_TimerDump(VOS_INT lModId, VOS_UINT32 ulFileID, VOS_UINT32 ulLineNo)
{
    ...
    {
        DRV_SYSTEM_ERROR(lModId, (VOS_INT)ulFileID, (VOS_INT)ulLineNo,
