...
/*****************************************************************************
 函 数 名  : OM_SDWriteBuffer
 功能描述  : 将trace写到SD卡中
 输入参数  : pucData - 指向待写入的数据
             uslen - 待写入数据长度
 输出参数  : 无
 返 回 值  : VOS_OK - 成功
             VOS_ERR - 失败
...
/*****************************************************************************
 函 数 名  : OM_SDReadCluster
 功能描述  : 读SD卡中的数据到缓冲区中并发给PC侧
 输入参数  : ulPacketSn - 需要读的包序号
 输出参数  : 无
 返 回 值  : 无
...
/*****************************************************************************
 函 数 名  : OM_SDGetReadPosition
 功能描述  : 获取SD卡中起始读的ClusterID
 输入参数  : ulNeedClusterNum - 需要读的Cluster数
 输出参数  : 无
 返 回 值  : 无
...
/*****************************************************************************
 函 数 名  : OM_SDParaCheck
 功能描述  : 处理PC侧发过来的读SD卡的消息
 输入参数  : ulDataSize - 用户需要的trace大小
             pulErrNo - 指向错误号
 输出参数  : 无
 返 回 值  : 无
...
/*****************************************************************************
 函 数 名  : OM_SDReadReqProc
 功能描述  : 处理PC侧发过来的读SD卡的消息
 输入参数  : pstReadSDReq - 指向PC->OM请求数据包结构体的指针
 输出参数  : 无
 返 回 值  : 无
...
/*****************************************************************************
 函 数 名  : OM_ReadSdMsgProc
 功能描述  : 处理PC侧发过来的读SD卡的消息
 输入参数  : pstRspPacket - 指向PC->OM请求数据包结构体的指针
             pstRspFuncPtr - 指向响应函数的指针
 输出参数  : 无
 返 回 值  : 无
...
VOS_UINT32 OM_FSInitCfgFile(VOS_VOID)
{
    ...
    {
        /* 判断LOG目录是否已经存在 */
        if (SD_FS_OPEN(g_acFSLogDir, O_DIRECTORY, 0) < 0)
        {
            /* 如果目录不存在则创建LOG目录 */
...
#if (FEATURE_ON == FEATURE_COMPRESS_WRITE_LOG_FILE)
/*****************************************************************************
 函 数 名  : OM_AddHeadInfoToFile
 功能描述  : 在Log文件增加文件头信息
 输入参数  : pstOmUcmxHeadInfo :待填充文件头信息
...
/*****************************************************************************
 函 数 名  : OM_FSCheckFileIsFull
 功能描述  : 检查文件是否满
 输入参数  : ulLength    - 被写入文件的长度
 输出参数  : 无
 返 回 值  : VOS_OK  写入成功
             VOS_ERR 写入失败
...
/*****************************************************************************
 函 数 名  : OM_FSWriteLogFile
 功能描述  : 将LOG日志写入到文件系统的LOG文件中
 输入参数  : pucLogData - 指向被写入LOG内容的指针
             ulLength   - 被写入的长度
 输出参数  : 无
 返 回 值  : VOS_OK  写入成功
             VOS_ERR 写入失败
...
/*****************************************************************************
 函 数 名  : OM_FSWriteLogProc
 功能描述  : 根据Nv项配置，将收到Log数据处理后写到文件系统
 输入参数  : pucLogData - 指向被写入LOG数据的指针
             ulLength   - 被写入的长度
 输出参数  : 无
 返 回 值  : VOS_OK  写入成功
             VOS_ERR 写入失败
