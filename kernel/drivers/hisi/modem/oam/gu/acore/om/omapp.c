...
/*Added by j00174725 for BBP数采项目,2012-12-03,end */
...
/*****************************************************************************
 Prototype      : OM_AcpuSendResult
 Description    : OM sends the execution result to tool side.
 Input          : ucFuncType      - The function type of msg.
                  ulResult        - The execution result.
...
/*****************************************************************************
 Prototype      : OM_AcpuSendContent
 Description    : OM sends the execution content to tool side.
 Input          : ucFuncType     - The pointer of the sequence number.
                  pstOmToAppMsg  - Points to the OM_APP_MSG_EX_STRU struct.
                  usReturnPrimId - The returned PrimId.
 Output         : void
 Return Value   : void
...
/*****************************************************************************
 函 数 名  : OM_AcpuCheckMsgPID
 功能描述  : 配置trace和event
 输入参数  : pulModuleIds  - 需要配置的module id
...
/*****************************************************************************
 函 数 名  : OM_AcpuShowPSConfigInfo
 功能描述  : 获取Trace event配置信息
 输入参数  : ulModuleId  - 需要配置的module id
             ulModuleNum   - 模块数目
...
OM_FUNCID_PROC_STRU g_astAcpuOmFuncIdProcTbl[OM_FUNCID_MAX_NUM] =
{
    ...
    {OM_BBP_FUNC,      Om_AcpuBbpConfigMsgProc,     LEVEL_NORMAL}   /* BBP可维可测项目增加,j00174725 2012-11-26 */
};


/*****************************************************************************
 Prototype       :   OM_AcpuRegisterReqProc
 Description     :   对外接口，用来注册FuncId/ModuleId对应的处理函数
 Input           :   ucModuleId  - Indicates the type of current module.
                     pReqPro     - Points to the function pointer of handle the current module.
 Output          :   void
...
/*****************************************************************************
 Prototype       :   OM_AcpuRegisterDrvCB
 Description     :   调用底软接口注册函数
 Input           :   VOS_VOID
 Output          :   VOS_VOID
 Return Value    :   VOS_VOID
...
/*****************************************************************************
 Prototype       : OM_AcpuStartEventTimer
...
VOS_VOID OM_AcpuConnectInfoReqSnd(VOS_VOID)
{
    ...
    *(VOS_UINT16*)(pMsg->aucValue) = OM_CONNECT_INFO_REQ;
    ...
    VOS_SendMsg(ACPU_PID_OMAGENT, pMsg);
...
/*****************************************************************************
 Prototype       : OM_AcpuTimerMsgProc
 Description     : Handle all timer messages sent to OM.
...
/*****************************************************************************
 函 数 名  : OM_AcpuICCInfoFileName
 功能描述  : 获取当前保存文件名
 输入参数  : 无
 输出参数  : 无
 返 回 值  : VOS_CHAR * cFileName
 调用函数  : 无
 被调函数  :
...
/*****************************************************************************
 函 数 名  : OM_AcpuIccInfoWriteLog
 功能描述  : 将ICC可维可测内容记录至文件中
...
/*****************************************************************************
 函 数 名  : OM_AcpuICCInfoInit
 功能描述  : OM记录可维可测信息初始化接口
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
 调用函数  :
...
VOS_UINT32  OM_AcpuICCInfoInit( VOS_VOID )
{
    ...
    VOS_StartRelTimer(&g_AcpuIccLogTimerId, ACPU_PID_OM, stInfoCfg.ulTimeOutValue,
...
/*****************************************************************************
 函 数 名  : OM_AcpuICCInfoRecord
 功能描述  : A核记录ICC消息信息接口
 输入参数  : MsgBlock *pMsgCtrlBlk
 输出参数  : 无
 返 回 值  : 无
 调用函数  :
 被调函数  :
...
/*****************************************************************************
 Prototype       : OM_AutoConfigProc
 Description     : ACPU初始化完毕，通知CCPU进行自动配置消息处理
 Input           : VOS_VOID
 Output          : None
 Return Value    : VOS_VOID
 ...
 History         : ---
    ...
    Date         : 2013-12-29
    Modification : Modify for Flash log function
 *****************************************************************************/
...
/*****************************************************************************
 Prototype       : OM_AcpuCcoreResetCB
 Description     : Ccore ReSet callback function
 Input           : None
 Output          : None
 Return Value    : VOS_UINT32
...
/*****************************************************************************
 Prototype       : Om_ShowCcpuResetInfo
 Description     : show Ccpu Reset info
 Input           : None
 Output          : None
 Return Value    : VOS_UINT32
