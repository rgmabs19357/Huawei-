...
VOS_UINT32 Diag_GuAirMessageTest( VOS_UINT32 ulSize);
VOS_UINT32 Diag_GuAirMessageTest( VOS_UINT32 ulSize)
{
	VOS_UINT32 ret =0;
	VOS_UINT16 ulMsgLen =30;
	VOS_UINT8 * pRptMessage =NULL;/*lint !e958 */
	VOS_UINT8 ucMsg[100] ={1,1,1,1,};
    ...
    /*lint -e539*/
	pRptMessage = (VOS_UINT8*) diag_HeapAllocD(sizeof(DIAG_GU_AIR_MSG_LOG_STRU)+100);
    if(NULL == pRptMessage)
    {
		return ERR_MSP_FAILURE;
    }
	((DIAG_GU_AIR_MSG_LOG_STRU*)pRptMessage)->usPrimId = ID_WAS_OM_OTA_IND;
	((DIAG_GU_AIR_MSG_LOG_STRU*)pRptMessage)->usOtaMsgID = 0x0FC0;
	((DIAG_GU_AIR_MSG_LOG_STRU*)pRptMessage)->ucUpDown = DIAG_SIDE_NET;
	((DIAG_GU_AIR_MSG_LOG_STRU*)pRptMessage)->pData = ucMsg;
    /*lint +e539*/
	...
	if(g_HsoConn)
	{
		...
		diag_CfgSetGlobalBitValue(&g_ulDiagCfgInfo,DIAG_CFG_GU_AIR_BIT,DIAG_CFG_SWT_OPEN);
	}
	diag_printf("DIAG_ReportGUAirMessageLog !g_ulDiagCfgInfo =%d\n",g_ulDiagCfgInfo);

	ret = DIAG_ReportGUAirMessageLog(pRptMessage,ulMsgLen);
	return ret;
}
...
VOS_UINT32 Diag_LayerMessageTest( VOS_UINT32 ulSize)
{
	...
	VOS_UINT32 ret;
	DIAG_CMD_LOG_CAT_LAYER_REQ_STRU stLayerReq = {0};
	...
	if(g_HsoConn)
	{
		...
		stLayerReq.ulModuleId = 150;
		stLayerReq.ulIsDestModule = 0;
		stLayerReq.ulSwitch = DIAG_CMD_REQ_LOG_CFG_FILTER_SWT_OPEN;
		diag_CfgSetLayerSwt(&stLayerReq, sizeof(DIAG_CMD_LOG_CAT_LAYER_REQ_STRU));

		stLayerReq.ulModuleId = 152;
		stLayerReq.ulIsDestModule = 1;
		stLayerReq.ulSwitch = 1;
		diag_CfgSetLayerSwt(&stLayerReq, sizeof(DIAG_CMD_LOG_CAT_LAYER_REQ_STRU));
	}
...
VOS_UINT32 Diag_PrintMessageTest( VOS_UINT32 ulLevelFilter)
{
	...
	if(g_HsoConn)
	{
		...
		pstPrintSwtReq = (DIAG_CMD_LOG_CAT_PRINT_REQ_STRU*)diag_HeapAllocD(sizeof(DIAG_CMD_LOG_CAT_PRINT_REQ_STRU));
        if(NULL == pstPrintSwtReq)
        {
            return ERR_MSP_MALLOC_FAILUE;
        }
		pstPrintSwtReq->ulModuleId = 152;/*lint !e539 */
...
VOS_UINT32 Diag_UserPlaneMessageTest( VOS_UINT32 ulMsgId,VOS_UINT32 ulSwitch)
{
	...
	DIAG_CMD_LOG_CAT_USERPLANE_REQ_STRU stUserPlaneSwtReq = {0};
	VOS_UINT8 ucMsg[100] ={2,2,2,2,};
	...
	if(g_HsoConn)
	{
		...
		stUserPlaneSwtReq.ulMsgId = ulMsgId;
		stUserPlaneSwtReq.ulSwitch = ulSwitch;
		ret = diag_CfgSetUserPlaneSwt(&stUserPlaneSwtReq,sizeof(DIAG_CMD_LOG_CAT_USERPLANE_REQ_STRU));
	}
...
VOS_UINT32 Diag_CmdMessageTest( VOS_UINT32 id,VOS_UINT32 ulSize)
{
	...
	ret = DIAG_ReportCommand(id,ulSize,ucMsg);/*lint !e734 */
	return ret;
}
...
#if (FEATURE_SAMPLE_V7_ONLY == FEATURE_ON)
typedef enum
{
...
VOS_VOID Test_Diag_Mntn_Drx(DIAG_ENUM_TYPE_E eDiagEnumType)
{
   ...
   /* para->time_stamp         = 0;\*/
	...
	pDiagReqHeadStru = (MSP_DIAG_DATA_REQ_STRU *)(pDiagHeadStru+sizeof(MSP_DIAG_HEAD_STRU));
...
VOS_UINT32 ulSerId, VOS_UINT32 ulStrId, VOS_UINT32 ulMailboxId, VOS_UINT32 ulOmSamPleId)
{
   ...
   /* para->time_stamp         = 0;\*/
	...
	psDrxSample = (DIAG_CMD_DRX_SAMPLE_REG_WR_REQ_STRU *)(pDiagReqHeadStru +sizeof(MSP_DIAG_DATA_REQ_STRU));
