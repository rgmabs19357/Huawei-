...
*****************************************************************************/
INLINE VOS_UINT32 diag_CheckMsgPara(DIAG_AIR_MSG_LOG_STRU* pstRptMessage)
{
...
*****************************************************************************/
INLINE VOS_UINT32 diag_GetLayerMsgCfg(VOS_UINT32 ulCatId, VOS_UINT32 ulMsgId)
{
...
VOS_UINT32 diag_GetLayerSrcCfg(VOS_UINT32 ulModuleId)
{
    ...
    /*lint +e539*/
	if(DIAG_CFG_MODULE_IS_VALID(ulModuleId))/*lint !e539 */
	{
		if(DIAG_CFG_SWT_OPEN == g_LayerSrcModuleCfg[ulModuleId - VOS_PID_DOPRAEND])
		{
			return	ERR_MSP_SUCCESS;
		}
	}
...
{
        /*lint -save -e40*/
	if(DIAG_CFG_MODULE_IS_VALID((VOS_INT32)ulModuleId))
	{
		if(DIAG_CFG_SWT_OPEN == g_LayerDecModuleCfg[ulModuleId - VOS_PID_DOPRAEND])
		{
			return	ERR_MSP_SUCCESS;
		}
	}
...
*****************************************************************************/
INLINE VOS_UINT32 diag_GetUserPlaneCfg(VOS_UINT32 ulMsgId)
{
    VOS_UINT32 i = 0;
...
*****************************************************************************/
INLINE VOS_UINT32 diag_GetPrintCfg(VOS_UINT32 ulModuleId, VOS_UINT32 ulLevel)
{
    ...
    ulLevelFilter = (1<<(7-ulLevel));/*lint !e701 */
    ...
    else
    {
        /*lint -save -e40*/
        /*获取模块打印开关状态*/
        if(DIAG_CFG_MODULE_IS_INVALID(ulModuleId))
        {
            return  ERR_MSP_CFG_LOG_NOT_ALLOW;
        }
...
VOS_UINT32 DIAG_PrintfV_TDS( VOS_CHAR* pszFmt, ...)
{
    ...
    va_list arg;
    VOS_UINT32 ulParamLen =0;
    VOS_UINT8* pstSocpPackt = NULL;
    DIAG_CMD_LOG_PRINT_RAW_TXT_IND_STRU* pRptInfo = NULL;
    VOS_UINT32 ulDataLength =0;
    VOS_CHAR ucDiagPrintData[DIAG_PRINTF_MAX_LEN]={0};

    /* 只记录调用次数*/
    DIAG_DEBUG_SDM_FUN(EN_SDM_DIAG_PRINTFV, 0, 0, 0);

    /*检查DIAG是否初始化且HSO是否连接上*/
    if(!DIAG_IS_CONN_ON)
    {
        DIAG_DEBUG_SDM_FUN(EN_SDM_DIAG_PRINTFV_ERR, ERR_MSP_NO_INITILIZATION, 0, 1);
        return ERR_MSP_NO_INITILIZATION;
    }

    /*给HSO的打印字符串形式如下:pszFileName[ulLineNum]data。HSO根据中括号[]去截取相应的信息*/
    ulParamLen = VOS_nsprintf(ucDiagPrintData,DIAG_PRINTF_MAX_LEN,"%s[%d]",NULL,NULL);/*lint !e732 */

    /*ulParamLen 未包含\0*/
    va_start(arg, pszFmt);
    /*lint -e732*/
    ulParamLen = VOS_nvsprintf(ucDiagPrintData + ulParamLen,\
                 DIAG_PRINTF_MAX_LEN - ulParamLen,(const VOS_CHAR *) pszFmt, arg);/*lint !e732 */
    /*lint +e732*/
    va_end(arg);
...
VOS_UINT32 DIAG_PrintfV(VOS_UINT32 id, VOS_CHAR* pszFileName, VOS_UINT32 ulLineNum, VOS_CHAR* pszFmt, ...)
{
    ...
    /*给HSO的打印字符串形式如下:pszFileName[ulLineNum]data。HSO根据中括号[]去截取相应的信息*/
	ulParamLen = VOS_nsprintf(ucDiagPrintData,DIAG_PRINTF_MAX_LEN,"%s[%d]",pszFileName,ulLineNum);/*lint !e732 !e539*/
...
{
	return DIAG_ReportCommand((VOS_UINT16)ulID, ulDataSize, pData);
