...
VOS_VOID NV_MemDump(VOS_UINT32 ulErrorNO, VOS_UINT32 ulFileID,VOS_INT32 ulLineNo,VOS_VOID *pData, VOS_UINT32 ulDataLen)
{
    ...
    {
        DRV_SYSTEM_ERROR((VOS_INT)ulErrorNO, (VOS_INT)ulFileID,
    ...
    *pulDumpBuffer++ = g_ulNVInitEndSlice;
    ...
    DRV_SYSTEM_ERROR(NV_MEMCPY_PARA_ERROR, (VOS_INT)ulFileID,
                (VOS_INT)ulLineNo, (VOS_CHAR *)pData, (VOS_INT)ulDataLen);
...
{
    VOS_UINT32 aulExcInfo[2];

    if ( ((VOS_UINT32)Src < NV_MAGICNUM_ADDR)                                  /*小于NV共享缓冲区的起始地址*/
        ||((VOS_UINT32)Src > (NV_MAGICNUM_ADDR+NV_BUFFER_SIZE))                   /*大于于NV共享缓冲区的终结地址*/
        ||(((VOS_UINT32)Src+ulnbytes) > (VOS_UINT32)(NV_MAGICNUM_ADDR+NV_BUFFER_SIZE)) )      /*数据内容大于于NV共享缓冲区的终结地址*/
    {
        LogPrint4("ERROR:NV_MemCpy big:file %d, l %d, Src is 0x%x, Len is %d.\r\n",
                    (VOS_INT)ulFileID, (VOS_INT)usLineNo, (VOS_INT)Src, (VOS_INT)ulnbytes);

        aulExcInfo[0] = (VOS_UINT32)Src;
        aulExcInfo[1] = ulnbytes;
                                ...
                                VOS_UINT32              ulLength)
{
    ...
    /* 判断是否属于安全NV */
    if((VOS_YES == DRV_SEC_CHECK()) && (g_ulCFDNvFileId == stNvIdReturnInfo.usFileId))
    {
    ...
    /* 获取IPC硬件锁 */
    if(BSP_OK != DRV_IPC_SEMTAKE(IPC_SEM_NVIM, NV_IPC_TIME_FOREVER))
    {
    ...
    /* 记录Slice */
    NV_RecordNvWriteSlice(usID, DRV_GET_SLICE());
