...
MAILBOX_LOCAL unsigned long test_mailbox_get_bound_width(
...
*****************************************************************************/
MAILBOX_EXTERN unsigned long test_mailbox_get_time_stamp(void)
{
...
*****************************************************************************/
char* test_mailbox_msg_pack(unsigned long recv_code, unsigned long size)
{
    struct mb_st_msg *usr_msg = MAILBOX_NULL;
    int pack_size = (size + sizeof(struct mb_st_msg));
    
    if((pack_size > (unsigned long)sizeof(g_test_mailbox_data_str)) ||
                ...
                unsigned long           mail_len)
{
    ...
    switch (st_msg->test_id)
    {
        ...
        case MAILBOX_BOARDST_ID_LOOP_BACK:
            ...
            {
                printk("eror test_mailbox_msg_cb: pCbHandle is NULL\n");
            ...
            {
                if (MAILBOX_ERRO != pCbHandle->check_ret)
                {
...
void test_mailbox_msg_create_task(FUNCPTR entryPtr,
                ...
                unsigned long SendMailCode,
                unsigned long RecvMailCode,
                ...
                struct tm_mb_cb * mb_cb)
{
    ...
    unsigned long FullWaitTimes = try_times;
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2012年11月20日
    修改内容   : 新生成函数
*****************************************************************************/
unsigned long test_mailbox_msg_get_code(
                unsigned long dst_id,
                unsigned long carrier_id,
                unsigned long *send_code,
                unsigned long *recv_code)
{
    unsigned long src_id = MAILBOX_LOCAL_CPUID;
...
*****************************************************************************/
unsigned long test_mailbox_msg_multi_send(
                unsigned long dst_id,
                unsigned long carrier_id,
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2012年11月20日
    修改内容   : 新生成函数
*****************************************************************************/
unsigned long test_mailbox_msg_single_test(
                unsigned long DstId,
                unsigned long MsgLen,
                unsigned long MsgNum,
                unsigned long DelayNum )
{
    unsigned long DelayTick = 2;
    unsigned long Priority  = 100;
    unsigned long TaskNum   = 1;
    unsigned long CarrierID = 1;
    unsigned long TryTimes = 3;
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2012年11月20日
    修改内容   : 新生成函数
*****************************************************************************/
unsigned long test_mailbox_msg_multi_test(
                unsigned long dst_id,
                unsigned long task_num,
                unsigned long msg_num,
                unsigned long delay_num)
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2012年11月20日
    修改内容   : 新生成函数
*****************************************************************************/
void test_mailbox_msg_reg(unsigned long channel_id)
{
    /*通道中的第一个mail_code为保留测试号，这个mail_code和此通道的channel_id一致*/
    unsigned long recv_code = channel_id;
...
long acore_print_p0(const char *string, int strlen)
{
    printk("%s", string);
    return 0;
}

long acore_fopen(const char *filename, int name_len, char *mode, int mode_len)
{
    int     oflags;
    int     ret = 0;
    int     appendflag = 0;
...
long acore_fread( void *ptr, unsigned int count, int fp)
{
    int ret = 0;
    unsigned long fs;
...
long acore_fwrite( void *buf, unsigned int count, int fp)
{
    int ret = 0;
    unsigned long fs;
...
long acore_fseek(int fp, long offset, int whence )
{
    int ret = 0;
    unsigned long fs;
...
//s00227525 add
long acore_memcmp(const void * src, int src_len, const void * dest, int dest_len)
{
...
typedef long (*TASK_IFC_FUNC)(struct acpu_ifc_test_data *para);

long acpu_ifc_test_ccpu_entry(unsigned int len, unsigned int loop_num)
{
...
long acpu_ifc_test_ccpu(struct acpu_ifc_test_data *para)
{
    unsigned int len = para->l;
    unsigned int loop_num = para->n;
    long result = acpu_ifc_test_ccpu_entry(len, loop_num);
    return result;
}

long acpu_ifc_test_mcu_entry(unsigned int len, unsigned int loop_num)
{
    unsigned char *src = NULL;
    unsigned char *dst = NULL;
    unsigned int i = 0;
...
long acpu_ifc_test_mcu(struct acpu_ifc_test_data *para)
{
    unsigned int len = para->l;
    unsigned int loop_num = para->n;
    long result = acpu_ifc_test_mcu_entry(len, loop_num);
    return result;
}

 long acpu_ifc_test_main(unsigned int len, unsigned int loop_num)
 {
...
long acpu_ifc_base_test1_ccpu(int aa, int bb, int cc)
{
    int a = aa, b = bb, c = cc;
...
long acpu_ifc_base_test2_ccpu(int count)
{
    int *a, *b, *c;
    int i = 0;
