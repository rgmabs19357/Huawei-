...
{
    volatile unsigned int tmp_val;
    unsigned int timer1_addr;
    unsigned int sctrl_addr = (unsigned int)IO_ADDRESS(SOC_SC_ON_BASE_ADDR);

    timer1_base = ioremap(OSA_TIMER_REG_BASE, OSA_TIMER_REG_SIZE);
    if (!timer1_base)
    {
