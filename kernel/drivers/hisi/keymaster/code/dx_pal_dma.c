                        ...
                        uint32_t  physBuffAddr)
{
	...
	/*Size 4M*/
	gMemPoolLen = 4 * 1024 * 1024;  
	gMemVirtBaseAddr  = dma_alloc_coherent(NULL, gMemPoolLen, &gMemPhysBaseAddr, GFP_KERNEL);
	if(gMemVirtBaseAddr == 0)
	{
...
void DX_PAL_DmaTerminate()
{
	...
	dma_free_coherent(NULL, gMemPoolLen, gMemVirtBaseAddr, gMemPhysBaseAddr);
...
{
	*ppVirtBuffAddr = gen_pool_alloc(mem_pool, buffSize);
