...
int km_sign_data(void *arg)
{
        ...
        status = CRYS_RSA_PRIM_Sign(&(pri_table[key_blob->hash_private]),&tmp_prim_data,sign_data->data_in,
...
int km_verify_data(void *arg)
{
        ...
        status = CRYS_RSA_PRIM_Verify(&(key_blob->public_key),&tmp_prim_data,verify_data->data_in,
...
int km_get_key(void *arg)
{
        ...
        status = CRYS_RSA_Get_PubKey(&(key_blob->public_key),Exponent_ptr, &ExponentSize_ptr,
 ...
 */
int km_ioctl(struct file *filp,unsigned int cmd, unsigned long arg)
{
        int status;
        int ret;
        if(NULL == (void *)arg) {
