...
#define HW_DESC_DUMP(pDesc) do {            			\
	...
	PRINTF("0x%08X, ", (unsigned int)(pDesc)->word[0]);  	\
	PRINTF("0x%08X, ", (unsigned int)(pDesc)->word[1]);  	\
	PRINTF("0x%08X, ", (unsigned int)(pDesc)->word[2]);  	\
	PRINTF("0x%08X, ", (unsigned int)(pDesc)->word[3]);  	\
	PRINTF("0x%08X\n", (unsigned int)(pDesc)->word[4]);  	\
	PRINTF("---------------------------------------------------\n\n");    \
} while (0)
#else
 ...
 * NOTE: Set the LAST INDICATION bit for all DOUT descriptors of any type. AXI counters increase only when using DLLI/MLLI
 * 
 * \param pDesc pointer HW descriptor struct 
 * \param dmaMode The DMA mode: NO_DMA, SRAM, DLLI, MLLI, CONSTANT
...
#define HW_DESC_SET_DOUT_TYPE(pDesc, dmaMode, doutAdr, doutSize, axiId, axiNs)										\
	do {		                                                                                        					\
		...
		DX_CC_REG_FLD_SET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD3, DOUT_LAST_IND, (pDesc)->word[3], 1);						\
