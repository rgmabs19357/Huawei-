...
#define MSENSOR_PIN "compass_int"
...
static int akm8963_gpio_init(enum pull_updown new_mode, char *pin_name)
{
	int ret = 0;
...
static int akm_regulator_configure(struct akm8963_platform_data *pdata, struct i2c_client *client)
{
	int rc;
...
static int  akm_regulator_enable(struct akm8963_platform_data *pdata)
{
int rc = 1;
...
static int  akm_regulator_disable(struct akm8963_platform_data *pdata)
{
	int rc = 1;
...
static ssize_t akm8963_set_cal_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
       int err = 0;
...
printk("------akm8963_mode_store----1-\n");
	...
	if (false == get_value_as_int(buf, count, &mode))
		return -EINVAL;
printk("------akm8963_mode_store--mode=%d--1-\n", mode);
...
{
    ssize_t count;
    count = snprintf(buf, PAGE_SIZE, "AKM AKM8963");
    return count;
}
static struct device_attribute akm8963_attributes[] = {
	...
	__ATTR(enable_ori, 0664, akm8963_enable_ori_show, akm8963_enable_ori_store),
	__ATTR(delay_acc,  0664, akm8963_delay_acc_show,  akm8963_delay_acc_store),
	__ATTR(delay_mag,  0664, akm8963_delay_mag_show,  akm8963_delay_mag_store),
	__ATTR(delay_ori,  0664, akm8963_delay_ori_show,  akm8963_delay_ori_store),
	__ATTR(set_cal,  0664, akm8963_set_cal_show,  akm8963_set_cal_store),
	__ATTR(compass_info, 0664, attr_get_accl_info, NULL),
#ifdef AKM8963_DEBUG_IF
	__ATTR(mode,  0664, NULL, akm8963_mode_store),
	__ATTR(bdata, 0664, akm8963_bdata_show,NULL ),
	__ATTR(asa,   0664, akm8963_asa_show, NULL),
#endif
	__ATTR_NULL,
};
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int i;
	int error = 1;
	if (!hsad_get_akm8963_type()) {
		dev_info(&client->dev, "%s: compass akm8963 device not exits\n", __func__);
		return  -ENXIO;
	}
	...
	if (pdata) {
		...
		s_akm->akm_gpio = pdata->gpio_DRDY;
	} else {
	...
	if (error) {
		dev_err(&client->dev, "Failed to intialize akm_regulator hardware\n");
		}
	...
	if (err < 0)
		goto exit2;
	err = set_sensor_chip_info(COMPASS, "AKM AKM8963");
	if (err) {
		dev_err(&client->dev, "set_sensor_chip_info error\n");
	}
	/*err = akm8963_gpio_init(PULLDOWN, MSENSOR_PIN);
	if (err < 0) {
		dev_err(&client->dev, "failed to init gpio\n");
		goto exit2;
	}*/
	     // phy_reg_writel(REG_BASE_IOC_ON,
            //           0x868/* IOCG026: 0x800 + 26*4  gpio_3_3-->dev_iomux.c*/,
            //           0,
             //          1,
              //         0x10/*pull down, disable pull up*/);
	...
	if (s_akm->irq == 0) {
		...
		err = gpio_request(pdata->gpio_DRDY, "akm_DRDY_gpio");
		if (err) {
...
exit5:
	if (s_akm->irq >= 0)
		gpio_free(irq_to_gpio(s_akm->irq));
exit4:
