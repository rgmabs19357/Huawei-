...
#elif SENSOR_TYPE == 5
#define YAMAHA_SENSOR_NAME "light"
#elif SENSOR_TYPE == 6
#define YAMAHA_SENSOR_NAME "pressure"
#elif SENSOR_TYPE == 7
#define YAMAHA_SENSOR_NAME "temperature"
#elif SENSOR_TYPE == 8
#define YAMAHA_SENSOR_NAME "proximity"
#elif SENSOR_TYPE == 9
#define YAMAHA_SENSOR_NAME "gravity"
#elif SENSOR_TYPE == 10
#define YAMAHA_SENSOR_NAME "linear_acceleration"
#elif SENSOR_TYPE == 11
#define YAMAHA_SENSOR_NAME "rotation_vector"
#endif
...
#ifdef YAS_SENSOR_KERNEL_DEVFILE_INTERFACE
	make_event(ev, EV_ABS, ABS_CONTROL_REPORT, (data->enabled<<16) | value);
...
{
	struct input_dev *input_data = this_data;//to_input_dev(dev);
...
sensor_probe(struct platform_device *pdev)
{
	...
	input_set_abs_params(input_data, ABS_CONTROL_REPORT, INT_MIN, INT_MAX,
			0, 0);
	input_data->name = YAMAHA_SENSOR_NAME;
    input_data->dev.parent = &pdev->dev;
	...
	rt = sysfs_create_group(&pdev->dev.kobj,
...
{
	sensor_pdev = platform_device_register_simple(YAMAHA_SENSOR_NAME, 0, NULL, 0);
