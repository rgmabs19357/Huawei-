...
#define SENSITIVITY_6G		183		/* ug/LSB	*/
#define SENSITIVITY_8G		244		/* ug/LSB	*/
#define SENSITIVITY_16G		732		/* ug/LSB	*/
...
/* acc kernel error code start from 0x03
 * 0x01: cannot open sensor (report from userspace directly)
 * 0x02: cannot open sysfs fail (report from userspace directly)
 * 0x03: calibrate offset fail
 * 0x04: write offset to nv fail
 * 0x05: read offset from nv fail
 * */
#define ERR_MSG_NONE_ERR	0x00
#define ERR_MSG_OPEN_SENSOR	0x01
#define ERR_MSG_OPEN_SYSFS	0x02
#define ERR_MSG_CAL_OFFSET	0x03
#define ERR_MSG_WRITE_NV	0x04
#define ERR_MSG_READ_NV		0x05

static char *cal_err_msg[] = {
	[ERR_MSG_NONE_ERR] = "0x00 calibrate pass",
	[ERR_MSG_OPEN_SENSOR] = "0x01 cannot open sensor",
	[ERR_MSG_OPEN_SYSFS] = "0x02 cannot open sysfs",
	[ERR_MSG_CAL_OFFSET] = "0x03 calibrate failed, over threshold",
	[ERR_MSG_WRITE_NV] = "0x04 write nv error",
	[ERR_MSG_READ_NV] = "0x05 read nv error",
};

/* mark calibrate err*/
static atomic_t cal_err_code = ATOMIC_INIT(0);
...
struct lsm330_acc_data {
	...
	u8 reg_addr;
#endif
   struct  regulator *acc_vdd;
   struct  regulator *acc_io_vdd;
};
...
static void lis3dsh_acc_set_init_register_values(struct lsm330_acc_data *acc)
{
...
{
	int err;
	accl_data[0] = xyz[0]*1024/1000000;
	accl_data[1] = xyz[1]*1024/1000000;
	accl_data[2] = xyz[2]*1024/1000000;

	if (1 == calibration_value)
	{
        printk("this is calibrate funstion  calibrate_count =%d\n",calibrate_count);
	    //compute accel
		calibrate_count ++;
		...
		temp[2] = 1024*Z_SCALE/ (accel[2] /10);
						...
						||temp[2]>Z_SCALE_BIG||temp[2]<Z_SCALE_SMALL)
			{
				...
				atomic_set(&cal_err_code, 0x03);
				printk("WRONG_CALIBRATION_VALUE   |temp_offset| bigger than 200mg/2; temp_offset= %d %d %d \n",temp[0],temp[1],temp[2]);
			}
			else
			{
				printk("RIGHT_CALIBRATION_VALUE   |temp_offset| smaller than 200mg/2;temp_offset = %d %d %d \n",temp[0],temp[1],temp[2]);
				err=write_gsensor_offset_to_nv();
				if(err<0)
				{
					return_calibration = WRONG_CALIBRATION;
					atomic_set(&cal_err_code, 0x04);
					printk("WRONG_CALIBRATION because of write NV error \n");
				}
				else
				{
					msleep(10);
					err=read_gsensor_offset_from_nv();
					if(err<0)
					{
						return_calibration = WRONG_CALIBRATION;
						atomic_set(&cal_err_code, 0x05);
						printk("WRONG_CALIBRATION because of read NV error \n");
					}
					else
					{
						if(gsensor_offset[0] == temp[0]&& gsensor_offset[1] == temp[1]&&gsensor_offset[2] == temp[2])
							return_calibration = RIGHT_CALIBRATION;
						else
							return_calibration = WRONG_CALIBRATION;
						printk("lsm330_acc:the CALIBRATION status is %d; gsensor_offset=[%d,%d,%d]\n",return_calibration,gsensor_offset[0],gsensor_offset[1],gsensor_offset[2]);
					}
				}
			}
	...
	input_report_abs(acc->input_dev, ABS_X, accl_data[0] - gsensor_offset[0] * SCALE);
	input_report_abs(acc->input_dev, ABS_Y, accl_data[1] - gsensor_offset[1] * SCALE);
    if(gsensor_offset[2]>0)
        input_report_abs(acc->input_dev, ABS_Z, accl_data[2]*gsensor_offset[2]/Z_SCALE);
    else   
	    input_report_abs(acc->input_dev, ABS_Z, accl_data[2]);
	input_sync(acc->input_dev);
}

static int acc_regulator_configure(struct lsm330_acc_data *pdata, struct i2c_client *client)
{
	int rc;
	/*if (on == false)
		goto hw_shutdown;*/

	pdata->acc_vdd = regulator_get(&client->dev, SENSORS_ANALOG_VDD);
	if (IS_ERR(pdata->acc_vdd)) {
		rc = PTR_ERR(pdata->acc_vdd);
		dev_err(&client->dev,
			"Regulator get failed acc-vdd rc=%d\n", rc);
		return rc;
	}

	rc = regulator_set_voltage(pdata->acc_vdd, 2850000,
							2850000);
		if (rc) {
			dev_err(&client->dev,
       ...
       pdata->acc_io_vdd = regulator_get(&client->dev, SENSOR_IO_VDD);
	if (IS_ERR(pdata->acc_io_vdd)) {
		rc = PTR_ERR(pdata->acc_io_vdd);
		dev_err(&client->dev,
...
static int  acc_regulator_enable(struct lsm330_acc_data *pdata)
{
int rc = 1;
rc = regulator_enable(pdata->acc_vdd);
	if (rc) {
		printk(KERN_ERR "Regulator acc-vdd enable failed rc=%d\n", rc);
		return rc;
		}

	rc = regulator_enable(pdata->acc_io_vdd);
	if (rc) {
		printk(KERN_ERR "Regulator acc_io_vdd enable failed rc=%d\n", rc);
		return rc;
	}
	return rc;
}
static int  acc_regulator_disable(struct lsm330_acc_data *pdata)
{
	int rc = 1;
      rc = regulator_disable(pdata->acc_vdd);
	if (rc) {
		printk(KERN_ERR "Regulator acc-vdd disable failed rc=%d\n", rc);
		return rc;
		}

	rc = regulator_disable(pdata->acc_io_vdd);
	if (rc) {
		printk(KERN_ERR "Regulator acc_io_vdd disable failed rc=%d\n", rc);
		return rc;
	}
      return rc;
}
...
{
	int err;
      //acc_regulator_enable(acc);
       if (down_interruptible(&acc->gsensor_sem)){
	     pr_err("lsm330_acc_enable lock sem failed!\n");
           return -EAGAIN;
          }
...
{
       if (down_interruptible(&acc->gsensor_sem)){
             pr_err("lsm330_acc_disable lock sem failed!\n");
	      return -EAGAIN;
        }
...
//read offset value
static int read_gsensor_offset_from_nv()
{
	int ret;
	struct nve_info_user user_info;

	user_info.nv_operation = 1;
	user_info.nv_number = 307;
	...
	gsensor_offset[0] = user_info.nv_data[0];
	gsensor_offset[1] = user_info.nv_data[1];
	gsensor_offset[2] = user_info.nv_data[2];
	if (gsensor_offset[0] > OFFSET_BIG)
		gsensor_offset[0] = gsensor_offset[0] - 256;
	if (gsensor_offset[1] > OFFSET_BIG)
		gsensor_offset[1] = gsensor_offset[1] - 256;
	if (gsensor_offset[2] > Z_SCALE_BIG)
		gsensor_offset[2] = Z_SCALE_BIG;
	printk( "nve_direct_access read gsensor_offset (%d %d %d )\n",gsensor_offset[0],gsensor_offset[1],gsensor_offset[2]);
       first_start_flag = 0;
	return 0;
}
...
static int write_gsensor_offset_to_nv()
{
	int ret;
	struct nve_info_user user_info;
...
static ssize_t attr_set_calibrate_status(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	unsigned long val = simple_strtol(buf, NULL, 10);
	atomic_set(&cal_err_code, val);
	return size;
}

static ssize_t attr_get_calibrate_status(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", cal_err_msg[atomic_read(&cal_err_code)]);
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(calibrate, 0664, attr_get_calibrate, attr_set_calibrate),
	__ATTR(calibrate_status, 0664, attr_get_calibrate_status, NULL),
#ifdef DEBUG
...
static int lsm330_acc_later_resume(struct early_suspend *h)
{
	struct lsm330_acc_data *acc = container_of(h, struct lsm330_acc_data, early_suspend);
	//pr_err("----------- lsm330acc later resume begin-----\n");
...
static int lsm330_acc_early_suspend(struct early_suspend *h)
{
	struct lsm330_acc_data *acc = container_of(h, struct lsm330_acc_data, early_suspend);
         //pr_err("----------- lsm330 early suspend begin-----\n");
      if (down_interruptible(&acc->gsensor_sem)){
             pr_err("lsm330_acc_early_suspend lock sem failed!\n");
             return -EAGAIN;
      }
...
/* set regulator*/
	error = acc_regulator_configure(acc, client);
	if (error) {
		dev_err(&client->dev, "Failed to intialize acc_regulator hardware\n");
		goto err_freedata;
	}
      acc_regulator_enable(acc);
	...
	acc->client = client;
	sema_init(&acc->gsensor_sem, 0);
	i2c_set_clientdata(client, acc);
	chipid = LSM330_WHO_AM_I;
	err = lsm330_acc_i2c_read(acc, &chipid, 1);
	if (err < 0) {
		dev_err(&client->dev, "Error reading WHO_AM_I: is device available/working?\n");
		goto exit_check_functionality_failed;
	}

	if (chipid != WHOAMI_LSM330_ACC) {
		dev_err(&client->dev,
			"device unknown. Expected: 0x%x, Replies: 0x%x\n", WHOAMI_LSM330_ACC, chipid);
		err = -ENODEV;
		goto exit_check_functionality_failed;
	}

	dev_info(&client->dev, "Read lsm330 acc chip ok, ID is 0x%x\n", chipid);
	err = set_sensor_chip_info(ACC, "ST LSM330");
	if (err) {
	...
	acc->early_suspend.suspend = lsm330_acc_early_suspend;
	acc->early_suspend.resume = lsm330_acc_later_resume;
