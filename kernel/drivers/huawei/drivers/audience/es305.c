...
static struct audience_img img;
...
/* CLOCK 26M LDO_1 */
//static struct regulator *g_regu_buck6 = NULL;

extern void hi6401_lock_sysclk(bool);
...
static void es305_poweron(void)
{
    ...
    if (ES305_POWER_OFF == es305_power) {
        ...
        hi6401_lock_sysclk(true);

#if 0
        ret = regulator_enable(g_regu_buck6);
        if (0 > ret)
...
#if 0
        ret = regulator_disable(g_regu_buck6);
        if (0 > ret)
            pr_err("%s: regulator buck6 disable failed\n", __FUNCTION__);
#endif
        hi6401_lock_sysclk(false);
    }
}
...
static void es305_hw_reset(void)
{
    ...
    ret = blockmux_set(es305_iomux_block, es305_block_config, LOWPOWER);
	if (0 > ret) {
...
#if 0
    /* get ldo_1 */
    g_regu_buck6 = regulator_get(&client->dev, ES305_IO_VDD);
    if (IS_ERR(g_regu_buck6)) {
        pr_err("%s: Could not get regulator : %d\n", __FUNCTION__, IS_ERR(g_regu_buck6) );
        return -ENODEV;
    }
#endif
