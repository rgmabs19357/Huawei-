...
static void bq2419x_monitor_battery_ntc_japan_charging(struct bq2419x_device_info *di)
{
    ...
    dev_info(di->dev,
        "%s: japan Status:batt_temp is %d , ntc_temp is %d , batt_volt is %d",
    ...
    if(charge_current_temp == 1){
        ...
        di->currentmA = charge_current_temp;
    }
    ...
    dev_info(di->dev, "japan iin = %d, ichg = %d\n", di->cin_limit, di->currentmA);
...
static void bq2419x_monitor_battery_ntc_charging(struct bq2419x_device_info *di)
{
    ...
    dev_info(di->dev,
        "%s: Status:batt_temp is %d , ntc_temp is %d , batt_volt is %d",
        __FUNCTION__, batt_temp, get_hot_temperature(), di->battery_voltage);
...
static int is_aux_batt_exist(struct bq2419x_device_info *di)
{
    unsigned short volt = -1;
    int channel = HKADC9;
    int aux_batt_exist = 0;
    if (!(get_hw_config_int(AUX_BATT, &aux_batt_exist, NULL ))){
        return 0;
    }
    ...
    if (hkadc_get_shared_value(channel, &volt) != 0){
        ...
        dev_err(di->dev, "HKADC9 read error\n");
        return -1;
    }
    hkadc_debug_uninit(channel);
    if((volt >= AUX_EXIST_MIN_MV)&&(volt <= AUX_EXIST_MAX_MV))
        return 1;
    else
        return 0;
}
static void bq2419x_start_usb_charger(struct bq2419x_device_info *di)
{
    ...
    dev_info(di->dev, "%s, -->START USB CHARGING, batt current=%dmA batt volt=%dmV\n"
...
static void bq2419x_start_ac_charger(struct bq2419x_device_info *di)
{
    ...
    dev_info(di->dev, "%s, -->START AC CHARGING, batt current=%dmA batt volt=%dmV\n"
                         , __func__, di->currentmA, di->voltagemV);
...
static void bq2419x_stop_charger(struct bq2419x_device_info *di)
{
    ...
    cancel_delayed_work(&di->otg_int_work);
...
static void bq2419x_otg_int_work(struct work_struct *work) {
    ...
    dev_info(di->dev, "reg09=0x%x first\n",read_reg);
    msleep(100);
    bq2419x_read_byte(di, &read_reg, CHARGER_FAULT_REG09);
    dev_info(di->dev, "reg09=0x%x then\n", read_reg);
                  ...
                  const char *buf, size_t count)
{
    ...
    if(val == 1){
        ...
        if((val < IINLIM_100) || (val > IINLIM_3000))
            return -EINVAL;
    }
    cin_limit_temp = val;
    di->cin_limit = cin_limit_temp;
                  ...
                  const char *buf, size_t count)
{
    ...
    if(val == 1){
        ...
        if((val < ICHG_512) || (val > ICHG_3000))
            return -EINVAL;
    }
     charge_current_temp = val;
    di->cin_limit = charge_current_temp;
    bq2419x_config_current_reg(di);
...
int bq2419x_mhl_set_iin(int flag)
{
    ...
    dev_info(di->dev,"mhl_limit_flag = %d\n", flag);
                  ...
                  const char *buf, size_t count)
{
    ...
    dev_info(di->dev,"set temp_current_iin = %d\n", input_current_iin);
                  ...
                  const char *buf, size_t count)
{
    ...
    else if(val == 0 || val <= ICHG_LOW_TEMP)
        input_current_ichg = ICHG_LOW_TEMP;
    else
    ...
    dev_info(di->dev,"set temp_current_ichg = %d\n", input_current_ichg);
                           ...
                           const struct i2c_device_id *id)
{
    ...
    di->gpio_cd = pdata->gpio_cd;//ENABLE_BQ2419x_CHARGER;
    di->gpio_int = pdata->gpio_int;
    //di->gpio_otg = pdata->gpio_otg;
    ...
    if (ret) {
          dev_err(&client->dev, "could not request charger_cd\n");
          ret = -ENOMEM;
          goto err_io;
    }
    ...
    if(ret) {
        dev_err(di->dev,"could not request gpio_int\n");
        goto err_gpio_int;
    }
