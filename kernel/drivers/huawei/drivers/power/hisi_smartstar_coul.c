...
#define SMARTSTAR_COUL_EVT(fmt,args...) do { printk("[smartstar]" fmt, ## args); } while (0)
#define SMARTSTAR_COUL_INF(fmt,args...) do { printk("[smartstar]" fmt, ## args); } while (0)
#endif
...
void hisi_smartstar_print_cc_reg(int c)
{
    ...
    if (cnt)
        SMARTSTAR_COUL_ERR("reg: cc_in = 0x%lx, cc_out = 0x%lx\n", g_cc_in, g_cc_out);
}
#endif
/**
    ...
    if (di) {
        if (SR_DEVICE_WAKEUP == sr_cur_state) {
            ...
            if (di->sr_resume_time > 0) {
                SMARTSTAR_COUL_ERR("[SR]%s(%d): di->sr_resume_time = %ld\n", __func__, __LINE__, di->sr_resume_time);
                di->sr_resume_time = 0;
            }
...
static int __init early_parse_pmu_nv_addr_cmdline(char *p)
{
    ...
    nv_info_addr = (struct ss_coul_nv_info *)simple_strtoul(buf, &endptr, 16);
...
static int __init move_pmu_nv_info(void)
{
    ...
    pmu_nv_addr = mem_remap_nocache(nv_info_addr, sizeof (struct ss_coul_nv_info));

    SMARTSTAR_COUL_EVT("pmu_nv_addr=0x%x\n", (unsigned int)pmu_nv_addr);

    memcpy(&my_nv_info, pmu_nv_addr, sizeof(my_nv_info));
    return 0;
}
core_initcall_sync(move_pmu_nv_info);


int fastboot_save_nv_info_error(void)
{
    unsigned short ocvreg = 1;
...
static int read_nv_info(struct smartstar_coul_device *di)
{
    ...
    if (ret) {
        ...
        memset(pinfo->temp,  0, sizeof(pinfo->real_fcc));
        memset(pinfo->real_fcc,  0, sizeof(pinfo->real_fcc));
        SMARTSTAR_COUL_INF("read nv partion failed, ret=%d\n", ret);
        return -1;
    }
    ...
    SMARTSTAR_COUL_INF("v_a=%d,v_b=%d,c_a=%d,c_b=%d,cycles=%d, reg_c=%d, reg_v=%d, start_cc=%ld\n"
        ,pinfo->v_offset_a, pinfo->v_offset_b, pinfo->c_offset_a, pinfo->c_offset_b, pinfo->charge_cycles 
        , pinfo->calc_ocv_reg_c, pinfo->calc_ocv_reg_v, pinfo->start_cc);
...
int smartstar_battery_temperature_tenth_degree(void)
{
	...
	SMARTSTAR_COUL_ERR("use 27510 for temperature = %d\n",use_bq27510);
        inited = 1;
    }

    if (use_bq27510){
        extern int hisi_bq27510_battery_temperature(void);
        return hisi_bq27510_battery_temperature()*10;
    }else{
        int ret = 0;
        short temperature;
        unsigned short volt;
        ...
        if(ret != 0){ // 0 == HKADC_OK
            if (once_read_ok){
                SMARTSTAR_COUL_ERR("read battery temperature from nv. temp = %d\n", di->nv_info.hkadc_batt_temp);
                return di->batt_temp;
            }else{
        ...
        if(once_read_ok == 0){
            once_read_ok = 1;
            SMARTSTAR_COUL_EVT("read battery temperature success, battery temperature = %d\n", temperature);
        }
        ...
        if (pre_temperature == -999){
            if(temperature >= -40 && temperature <= 80){
                ...
                SMARTSTAR_COUL_ERR("read battery temperature error, temperature over 80 degree, return pre temperatrue: %d.\n",pre_temperature);
...
int is_smartstar_fcc_debounce(void) {
    ...
    if (!di) {
        SMARTSTAR_COUL_ERR("%s, di is NULL\n", __func__);
        return ret;
    }
    ...
    if (batt_fcc < (fcc * 85 / 100)  || batt_fcc > (fcc * 115 / 100)) {
        ret = 1;
        SMARTSTAR_COUL_ERR("%s, fcc_from_temp=%d, batt_fcc=%d, ret=%d\n", __func__, fcc, batt_fcc, ret);
    }
...
void clear_pmu_error_mode(void)
{
    unsigned char val = 0;
...
int boost_enable_threshold = 3500;
...
extern unsigned int reg_read_u32(unsigned int pAddr);
	...
	for (i=0; i<ARRAY_SIZE(reg_array); i++){
		printk(KERN_ERR "[ACORE DUMP]" "reg[0x%x] = 0x%x\n", reg_array[i], reg_read_u32(reg_array[i]));
	}
...
static void boost_check_work(struct work_struct *work)
{
    ...
    static int pmu_error_count = 0;
    static long last_cc=0;
    static int last_vol = 0;
    static int last_cur = 0;
    unsigned char regval = 0;
    int vreg_val = 0;
    int creg_val = 0;

    SMARTSTAR_REGS_READ(SMARTSTAR_VOL_FIFO_BASE, &vreg_val, 2);
    SMARTSTAR_REGS_READ(SMARTSTAR_CUR_FIFO_BASE, &creg_val, 2);

    if(last_vol ==smartstar_battery_voltage_uv() && last_cc ==calculate_cc_uah() &&  last_cur == smartstar_battery_current_ua() && vreg_val == creg_val)
    {
        pmu_error_count++;
        SMARTSTAR_COUL_INF("pmu work error, voltage reg val is 0x%x, current reg val is 0x%x\n", vreg_val, creg_val);
    }
    else{
        pmu_error_count = 0;
    }

	last_vol = smartstar_battery_voltage_uv();
	last_cc = calculate_cc_uah();
	last_cur = smartstar_battery_current_ua();

    regval = SMARTSTAR_REG_READ(SMARTSTAR_COUL_CTRL_REG);
    if((regval & 0x80 == 0) || pmu_error_count >= 5)
    {
        dump_acore_reg();
        clear_pmu_error_mode();
    }
   ...
   else if (cali_cnt % (CALIBRATE_INTERVAL / di->soc_work_interval) == 1)
   {
    ...
    if (di->charging_state == CHARGING_STATE_CHARGE_DONE){
        ...
        if (charged_cnt%(CHARGED_OCV_UPDATE_INTERVAL/CALCULATE_SOC_MS) == 0){
        	...
        	SMARTSTAR_COUL_INF("sleep_cc=%ld, sleep_time=%ld\n", sleep_cc, sleep_time);
            ...
            if (sleep_time <= 0){
            	...
            	SMARTSTAR_COUL_INF("sleep_current=%ld\n", sleep_current);
...
static irqreturn_t coul_irq_cb(int irq,  void *_di)
{
    ...
    if (i == IRQ_MAX){
        SMARTSTAR_COUL_ERR("IRQ[%d], not know!\n", irq);
        return IRQ_NONE;
    }
...
int ss_offset_show(void)
{
    ...
    SMARTSTAR_COUL_ERR("C_offset: a=%d, b=%d uA V_offset: a=%d, b=%d uV, r_pcb=%d uohm\n",
        pinfo->c_offset_a, pinfo->c_offset_b, pinfo->v_offset_a, pinfo->v_offset_b, di->r_pcb);
    return 0;
}
...
#endif
{
    ...
    if (0 != retval) {
        printk("failed to create sysfs entry(abs_cc): %d\n", retval);
        return -1;
    }
    ...
    if (!is_smartstar_battery_exist()) {
        ...
        //retval = -EPERM;
        di->batt_data = get_battery_data(di->batt_id_vol, product_name);
        di->batt_exist = 0;
...
coul_no_battery:
    ...
    coul_ops->is_battery_exist = is_battery_exist;
	...
	pm_message_t state)
{
    ...
    if (wakeup_time > SR_MAX_RESUME_TIME) {
        clear_sr_time_array();
        SMARTSTAR_COUL_INF("[SR]%s(%d): wakeup_time(%ld) > SR_MAX_RESUME_TIME(%d)\n", __func__, __LINE__, wakeup_time, SR_MAX_RESUME_TIME);
    }
        ...
        SMARTSTAR_COUL_ERR("[SR]%s(%d): wakeup_time=%ld, di->sr_suspend_time=%ld, di->sr_resume_time=%ld\n",
...
static long sr_get_duty_ratio(void) {
    ...
    if (total_sleep_time + total_wakeup_time >= SR_TOTAL_TIME) {
        ...
        SMARTSTAR_COUL_INF("[SR]%s(%d): total_wakeup=%lds, total_sleep=%lds, duty_ratio=%ld\n",
...
static int sr_need_update_ocv(struct smartstar_coul_device *di) {
    ...
    if (last_sleep_time > SR_DELTA_SLEEP_TIME &&
        ...
        SMARTSTAR_COUL_INF("[SR]%s(%d): need_update, last_sleep=%lds, last_wakeup=%lds, duty_ratio=%ld\n",
            __func__, __LINE__, last_sleep_time, last_wakeup_time, duty_ratio);
        return 1;
    }
    else {
        SMARTSTAR_COUL_INF("[SR]%s(%d): no_need_update, last_sleep=%lds, last_wakeup=%lds, duty_ratio=%ld\n",
            __func__, __LINE__, last_sleep_time, last_wakeup_time, duty_ratio);
        return 0;
    }
}
...
static int ocv_could_update(struct smartstar_coul_device *di)
{
    ...
    {
        SMARTSTAR_COUL_INF("[SR]Can't update ocv, sleep_current=%ld ma, sleep_time=%ld s\n", sleep_current, sleep_time);
        return 0;
    }
    ...
    if (sleep_time > 0) {
        ...
        SMARTSTAR_COUL_ERR("[SR]%s(%d): sleep_time = %ld\n",  __func__, __LINE__, sleep_time);
        return 0;
    }

    SMARTSTAR_COUL_INF("[SR]going to update ocv, sleep_time=%lds, sleep_current=%ld ma\n", sleep_time, sleep_current);
    return 1;
}
...
static int hisi_smartstar_coul_resume(struct platform_device *pdev)
{
    ...
    if (sr_sleep_time >= 0) {
        ...
        SMARTSTAR_COUL_ERR("[SR]%s(%d): sr_sleep_time = %ld\n", __func__, __LINE__, sr_sleep_time);
    }
