...
#define RX_NUMBER 88  //f01 control_base_addr + 56
#define TX_NUMBER 89  //f01 control_base_addr + 57
...
struct rmi_function_container *g_f54_fc;
...
int rmi_start_mmi_test(char * buf, size_t count);
int rmi_get_mmi_test_result(char * buf);
...
static ssize_t rmi_f54_mmi_test_store(struct device *dev,struct device_attribute *attr, 
	const char *buf, size_t count)
{
	return rmi_start_mmi_test(buf, count);
};
...
static ssize_t rmi_f54_mmi_test_show(struct device * dev,struct device_attribute * attr,char * buf)
{
	...
	return rmi_get_mmi_test_result(buf);
};

static void mmi_deltacapacitance_test(struct rmi_function_container *fc, size_t count)
{
...
static void mmi_rawcapacitance_test(struct rmi_function_container *fc, size_t count)
{
...
int rmi_start_mmi_test(char * buf, size_t count)
{
	...
	struct rmi_function_container *fc = g_f54_fc;
	...
	} else {/*I2C communication successfully, go on test*/
		memcpy(buf_f54test_result, "0P-", (strlen("0P-")+1));
		mmi_rawcapacitance_test(fc, count);

		INIT_COMPLETION(mmi_sync);
		wait_for_completion(&mmi_sync);
		mmi_deltacapacitance_test(fc, count);
                mmi_add_static_data(data);
	}
	command = 1;
	result = rmi_write_block(fc->rmi_dev, f01_fc->fd.command_base_addr, &command, 1);
	if (result < 0) {
		dev_err(&fc->dev, "failed to write command to f01 reset!\n");
		return result;
	}

	dev_info(&fc->dev, "TP MMI test result: %s\n", buf_f54test_result);
	return count;
}
