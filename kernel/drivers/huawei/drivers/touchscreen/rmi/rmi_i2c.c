...
extern int hsad_get_rmi_enable(void);
...
static int synaptics_touchpad_gpio_setup(struct syna_gpio_data *gpio_data)
{
        int retval=0;
...
static void synaptics_touchpad_gpio_free(struct syna_gpio_data *gpio_data)
{
       struct syna_gpio_data *data =gpio_data;
        pr_warn("%s: Free touchpad gpio %d and %d.",
...
static int __devexit rmi_i2c_remove(struct i2c_client *client)
{
	...
	synaptics_touchpad_gpio_free(pd->gpio_data);
