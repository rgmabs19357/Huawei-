...
#include <linux/fhd_rmi.h>
#include "fhd_rmi_driver.h"
...
/*-----------false triggering processing start ------------*/
#include <linux/time.h>
#include <linux/rtc.h>
#include <linux/android_alarm.h>
#include <linux/ktime.h>

#define YMIN                146						 //upper virtualkey zone
#define YMAX                2030					//bottom virtualkey zone
#define XMIN                50						//left area
#define XMAX                1188					//right area
#define BACKL               216
#define BACKR               336
#define HOMEL               564
#define HOMER               684
...
#define PEN_TIMES           8
#define PEN_MOV_THD         30					//(pixels)
#define PEN_MOV_LENGTH      100				//move length (pixels)
#define PEN_RELESE_TIME     400				//latency of finger report after pen released (ms)
#define FINGER_REL_TIME     400				//the time pen checked after finger released shouldn't less than this value(ms)
#define STATE1_0_LATENCY    500				//latency of state 1 to 0 (ms)
#define STATE0_2_LATENCY    500				//latency of state 0 to 2 (ms)
#define PEN_TOUCH_INTER     150				//the interval between two pen touch must longer than PEN_TOUCH_INTER

#define ZERO0_STATE         0
#define FINGER1_STATE       1					//finger state
#define PEN2_STATE          2					//pen state

#define PEN_SENS_ADDR       0x008f			//pen sensitivity setting addr
#define PEN_SENS_HIGH       160				//high sensitivity
#define PEN_SENS_LOW        144				//low sensitivity

static struct timespec curr_time = {0,0}; 
static struct timespec pen_start_time = {0,0};
static struct timespec pre_pen_time = {0,0};
static struct timespec pre_finger_time = {0,0};

static u8  touch_state = 0;							//0-0 state, 1-finger state, 2-pen state
u8  pen_release_flag = 0;
...
int pen_rels_x;
int pen_rels_y;
/*------------fasle triggering processing end -------------*/
...
static struct device_attribute attrs[] = {
	...
	__ATTR(clip, RMI_RO_ATTR_1, f11_clip_show, f11_clip_store),
	__ATTR(offset, RMI_RO_ATTR_1, f11_offset_show, f11_offset_store),
	__ATTR(swap, RMI_RO_ATTR_1, f11_swap_show, f11_swap_store),
	__ATTR(relreport, RMI_RO_ATTR_1, f11_relreport_show, f11_relreport_store),
	__ATTR(maxPos, RMI_RO_ATTR, f11_maxPos_show, rmi_store_error),
#if RESUME_REZERO
	__ATTR(rezeroOnResume, RMI_RO_ATTR_1, f11_rezeroOnResume_show,
		f11_rezeroOnResume_store),
	__ATTR(rezeroWait, RMI_RO_ATTR_1, f11_rezeroWait_show,
...
/*-----------------false triggering processing start ----------------*/
/*if point is in display zone or virturalkey area, return 1; if forbidden zone, return 0*/
static u8 available_area_check(int x, int y)
{
	if (((x > XMIN) && (x < XMAX) && (y > YMIN) && (y < YMAX))
		|| (((x > BACKL) && (x < BACKR)) || ((x > HOMEL) && (x < HOMER)) || ((x > MENUL) && (x<MENUR))))
		return 1;
	return 0;
}
/*----------------fasle triggering processing end ------------------*/
...
static void rmi_f11_abs_pos_report(struct f11_2d_sensor *sensor,
					u8 finger_state, u8 n_finger,  struct rmi_device *rmi_dev)
{
	struct f11_2d_data *data = &sensor->data;
	struct rmi_f11_2d_axis_alignment *axis_align = &sensor->axis_align;
	...
	/*-----------false triggering processing start ------------*/
	if (touch_state != FINGER1_STATE) {
		/*a pen release when not in finger state*/

		if (pen_release_flag == 0)
			pen_release_flag = 1;
		else {
			pen_rels_x = -PEN_MOV_THD;
			pen_rels_y = -PEN_MOV_THD;
		}
		ktime_get_ts(&pre_pen_time);
	}
	else {
		/*this is a finger release*/

		ktime_get_ts(&pre_finger_time);
		pre_pen_time.tv_sec = 0;
		pre_pen_time.tv_nsec = 0;
	}
	return;
	}
	/*------------fasle triggering processing end -----------*/
...
//		__func__, n_finger, finger_state, x, y, z, w_max, w_min);
	...
	if (finger_state == F11_PRESENT) {
		/*this is a finger signal*/

		if (touch_state != FINGER1_STATE) {
			touch_state = FINGER1_STATE;
			retval = rmi_write(rmi_dev, PEN_SENS_ADDR, PEN_SENS_LOW);
			if (retval) {
				dev_err(rmi_dev, "%s : failed to write %d to 0x%x\n",
					__func__, PEN_SENS_LOW, PEN_SENS_ADDR);
				return retval;
			}
			/*decrease pen sensitivity at finger state*/
			pen_release_flag = 0;
		}

		/*
		**below, report finger after pen has released 400ms.
		**this solves false triggering, when a finger down, may be recognized as pen, then finger.
		*/
		if ((pre_pen_time.tv_sec != 0) || (pre_pen_time.tv_nsec != 0)) {
			ktime_get_ts(&curr_time);
			interval_time = (curr_time.tv_sec -pre_pen_time.tv_sec)*1000
						+ (curr_time.tv_nsec-pre_pen_time.tv_nsec)/1000000;
			/*obtain the interval from pen to finger*/

			if (interval_time < 0 || interval_time > PEN_RELESE_TIME) {
				pre_pen_time.tv_sec = 0;
				pre_pen_time.tv_nsec = 0;
				report_flag = 1;
			}
				...
				ktime_get_ts(&pre_pen_time);
				/*keep don't report all this time until next touch down*/
	        }
		...
		/*this is a pen signal*/

		if (touch_state == ZERO0_STATE) {
			tch_pen_times ++;
			if (tch_pen_times >= PEN_TIMES) {
				touch_state = PEN2_STATE;
				tch_pen_times = 0;
			}
			/*touch_state is set as pen when 8 pen signals has come*/

			else if (tch_addr_x - x > PEN_MOV_LENGTH || x - tch_addr_x > PEN_MOV_LENGTH
					|| tch_addr_y - y > PEN_MOV_LENGTH || y - tch_addr_y > PEN_MOV_LENGTH)
				touch_state = PEN2_STATE;
			/*touch_state is set as pen when pen touching addr is 100*/

			else {
				if ((pen_start_time.tv_sec != 0) || (pen_start_time.tv_nsec !=0)) {
					ktime_get_ts(&curr_time);
					interval_time = (curr_time.tv_sec - pen_start_time.tv_sec)*1000
								+ (curr_time.tv_nsec-pen_start_time.tv_nsec)/1000000;
					if (interval_time < 0 || interval_time > STATE0_2_LATENCY)
						touch_state = PEN2_STATE;
				}
				else
					ktime_get_ts(&pen_start_time);
			}
			/*touch_state is set as pen state after pen signal lasts 500ms*/

			if (touch_state == PEN2_STATE) {
				retval = rmi_write(rmi_dev, PEN_SENS_ADDR, PEN_SENS_HIGH);
				if (retval) {
					dev_err(rmi_dev, "%s : failed to write %d to 0x%x\n",
						__func__, PEN_SENS_HIGH, PEN_SENS_ADDR);
					return retval;
				}
			}
			/*increase pen sensitivity at pen state*/
		}
		else if (touch_state == PEN2_STATE) {
			/*pen signal comes*/
			...
			if (pen_release_flag == 0)
				...
				interval_time = (curr_time.tv_sec - pre_pen_time.tv_sec)*1000
							+ (curr_time.tv_nsec - pre_pen_time.tv_nsec)/1000000;
				if (interval_time < 0 || interval_time > PEN_TOUCH_INTER)
				{
					report_flag = 1;
					pen_release_flag = 0;
				}
				/*if the time between previous pen released and current pen down is more than 150ms, report it*/

				else if (pen_rels_x - x >= PEN_MOV_THD || x - pen_rels_x >= PEN_MOV_THD
					|| pen_rels_y - y >= PEN_MOV_THD || y - pen_rels_y >= PEN_MOV_THD) 
				{
					report_flag = 1;
					pen_release_flag = 0;
				}
				/*if the distance between previous pen to current pen over 30 pixels, report it*/
			}
			pen_rels_x = x;
			pen_rels_y = y;
		}
		else {
			/*pen signal comes when in finger state*/

			ktime_get_ts(&curr_time);
			interval_time = (curr_time.tv_sec - pre_finger_time.tv_sec)*1000
						+ (curr_time.tv_nsec - pre_finger_time.tv_nsec)/1000000;
			if (interval_time > 0 && interval_time < FINGER_REL_TIME)
				ktime_get_ts(&pre_finger_time);
				/*if the time between finger release and pen touch is too short, take it as a error check*/

			else {
				retval = available_area_check(x, y);
				if (retval == 1) {
					/*only normal area process pen touch*/

					if (interval_time < 0 || interval_time > STATE1_0_LATENCY) {
						touch_state = ZERO0_STATE;
						tch_pen_times = 0;
						tch_addr_x = x;
						tch_addr_y = y;
						ktime_get_ts(&pen_start_time);
...
static void rmi_f11_finger_handler(struct f11_2d_sensor *sensor, struct rmi_device *rmi_dev)
{
	...
	for (i = 0, finger_pressed_count = 0; i < sensor->nbr_fingers; i++) {
		...
		if (sensor->data.abs_pos)
			rmi_f11_abs_pos_report(sensor, finger_state, i, rmi_dev);
...
static int rmi_f11_register_devices(struct rmi_function_container *fc)
{
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		* phys name for input device */
		...
		input_dev->name="synaptics";//add by chenpeng
...
int fhd_rmi_f11_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	struct rmi_device *rmi_dev = fc->rmi_dev;
	struct f11_data *f11 = fc->data;
	...
	for (i = 0; i < f11->dev_query.nbr_of_sensors + 1; i++) {
		...
		rmi_f11_finger_handler(&f11->sensors[i], rmi_dev);
...
static struct rmi_function_handler function_handler = {
	...
	.reset = rmi_f11_reset,
	.attention = fhd_rmi_f11_attention,
...
static int __init rmi_f11_module_init(void)
{
	...
	error = fhd_rmi_register_function_driver(&function_handler);
	if (error < 0) {
...
{
	fhd_rmi_unregister_function_driver(&function_handler);
}
