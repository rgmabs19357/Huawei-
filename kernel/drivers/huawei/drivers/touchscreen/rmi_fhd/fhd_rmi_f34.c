...
#include "fhd_rmi_f34.h"

#define READ_CONFIGID		//add by chenpeng for read configid
#define BLK_SZ_OFF	3
...
static struct device_attribute attrs[] = {
	...
	__ATTR(cmd, RMI_RO_ATTR_1,
	       rmi_fn_34_cmd_show, rmi_fn_34_cmd_store),
	__ATTR(bootloaderid, RMI_RO_ATTR_1,
	...
	__ATTR(blocknum, RMI_RO_ATTR_1,
	       rmi_fn_34_blocknum_show, rmi_fn_34_blocknum_store),
	__ATTR(rescanPDT, RMI_RO_ATTR_1,
	       rmi_show_error, rmi_fn_34_rescanPDT_store),
#ifdef READ_CONFIGID
	__ATTR(configid, RMI_RO_ATTR,
		rmi_fn_34_configid_show, rmi_store_error),/*for read configid*/
#endif
};

struct bin_attribute fhd_dev_attr_data = {
	.attr = {
		 .name = "data",
		 .mode = 0664},
	.size = 0,
...
err_remove_sysfs:
	sysfs_remove_bin_file(&fc->dev.kobj, &fhd_dev_attr_data);
...
int fhd_rmi_f34_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	int retval;
...
static struct rmi_function_handler function_handler = {
	...
	.reset = rmi_f34_reset,
	.attention = fhd_rmi_f34_attention,
					...
					char *buf)
{
	...
	int error;
	rmi_dev = fc->rmi_dev;
	config_addr = fc ->fd.control_base_addr;
	error = rmi_read_block(rmi_dev,config_addr,configid,4);
	if (error < 0) {
		dev_err(dev, "%s : Could not read configid\n",
		       __func__);
		return error;
	}
