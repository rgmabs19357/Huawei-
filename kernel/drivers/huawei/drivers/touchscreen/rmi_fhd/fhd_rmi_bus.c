...
DEFINE_MUTEX(fhd_rmi_bus_mutex);
...
struct bus_type fhd_rmi_bus_type = {
	.name		= "fhd_rmi",
	.match		= rmi_bus_match,
	.probe		= rmi_bus_probe,
...
int fhd_rmi_register_phys_device(struct rmi_phys_device *phys)
{
	...
	rmi_dev->phys = phys;
	rmi_dev->dev.bus = &fhd_rmi_bus_type;
...
EXPORT_SYMBOL(fhd_rmi_register_phys_device);

void fhd_rmi_unregister_phys_device(struct rmi_phys_device *phys)
{
...
EXPORT_SYMBOL(fhd_rmi_unregister_phys_device);

int fhd_rmi_register_driver(struct rmi_driver *driver)
{
	driver->driver.bus = &fhd_rmi_bus_type;
	return driver_register(&driver->driver);
}
EXPORT_SYMBOL(fhd_rmi_register_driver);
...
void fhd_rmi_unregister_driver(struct rmi_driver *driver)
{
	bus_for_each_dev(&fhd_rmi_bus_type, NULL, driver, __rmi_driver_remove);
	driver_unregister(&driver->driver);
}
EXPORT_SYMBOL(fhd_rmi_unregister_driver);
...
int fhd_rmi_register_function_driver(struct rmi_function_handler *fh)
{
	struct rmi_function_list *entry;
	struct rmi_function_handler *fh_dup;

	fh_dup = fhd_rmi_get_function_handler(fh->func);
	if (fh_dup) {
	...
	bus_for_each_dev(&fhd_rmi_bus_type, NULL, fh, __rmi_bus_fh_add);

	return 0;
}
EXPORT_SYMBOL(fhd_rmi_register_function_driver);
...
void fhd_rmi_unregister_function_driver(struct rmi_function_handler *fh)
{
	struct rmi_function_list *entry, *n;

	/* notify devices of the removal of the function handler */
	bus_for_each_dev(&fhd_rmi_bus_type, NULL, fh, __rmi_bus_fh_remove);
...
EXPORT_SYMBOL(fhd_rmi_unregister_function_driver);

struct rmi_function_handler *fhd_rmi_get_function_handler(int id)
{
	struct rmi_function_list *entry;
...
EXPORT_SYMBOL(fhd_rmi_get_function_handler);
...
static int rmi_register_character_device(struct device *dev, void *data)
{
	...
	mutex_lock(&fhd_rmi_bus_mutex);
	list_add_tail(&char_dev->list, &char_driver->devices);
	mutex_unlock(&fhd_rmi_bus_mutex);
...
int fhd_rmi_register_character_driver(struct rmi_char_driver *char_driver)
{
	...
	char_driver->driver.bus = &fhd_rmi_bus_type;
	...
	/* notify devices of the removal of the function handler */
	bus_for_each_dev(&fhd_rmi_bus_type, NULL, char_driver,
			 rmi_register_character_device);

	return 0;
}
EXPORT_SYMBOL(fhd_rmi_register_character_driver);


int fhd_rmi_unregister_character_driver(struct rmi_char_driver *char_driver)
{
...
EXPORT_SYMBOL(fhd_rmi_unregister_character_driver);

static int __init rmi_bus_init(void)
{
	int error;

	mutex_init(&fhd_rmi_bus_mutex);
	INIT_LIST_HEAD(&rmi_supported_functions.list);
	INIT_LIST_HEAD(&rmi_character_drivers.list);

#ifdef CONFIG_RMI4_DEBUG
	rmi_debugfs_root = debugfs_create_dir(fhd_rmi_bus_type.name, NULL);
	if (!rmi_debugfs_root)
	...
	error = bus_register(&fhd_rmi_bus_type);
	if (error < 0) {
...
#endif
	bus_unregister(&fhd_rmi_bus_type);
}
