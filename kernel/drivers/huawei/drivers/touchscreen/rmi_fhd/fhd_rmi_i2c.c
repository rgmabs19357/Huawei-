				  ...
				  const struct i2c_device_id *id)
{
	...
	error = regulator_set_voltage(pdata->vdd,2800000,2800000);
	if(error < 0){
	...
	error = set_touch_chip_info("synaptics_7020");
	if (error) {
	...
	error = fhd_rmi_register_phys_device(rmi_phys);
	if (error) {
		dev_err(&client->dev,
...
err_unregister:
	fhd_rmi_unregister_phys_device(rmi_phys);
err_gpio:
	if (pdata->gpio_config)
...
static int __devexit rmi_i2c_remove(struct i2c_client *client)
{
	...
	disable_device(phys);
	fhd_rmi_unregister_phys_device(phys);
	kfree(phys->data);
	kfree(phys);
