...
int fhd_rmi_set_bits(struct rmi_device *rmi_dev, u16 address,
		 unsigned char bits)
{
...
EXPORT_SYMBOL(fhd_rmi_set_bits);

/* Utility routine to clear bits in a register. */
int fhd_rmi_clear_bits(struct rmi_device *rmi_dev, u16 address,
		   unsigned char bits)
{
	unsigned char reg_contents;
	int retval;
...
EXPORT_SYMBOL(fhd_rmi_clear_bits);
			     ...
			     struct rmi_function_container *fc)
{
	...
	if (!fc->fh) {
		struct rmi_function_handler *fh =
			fhd_rmi_get_function_handler(fc->fd.function_number);
		if (!fh) {
...
static int process_interrupt_requests(struct rmi_device *rmi_dev)
{
	...
	if (data->f01_container->irq_mask && data->f01_container->fh->attention) {
		fhd_u8_and(irq_bits, irq_status, data->f01_container->irq_mask,
				data->num_of_irq_regs);
		if (fhd_u8_is_any_set(irq_bits, data->num_of_irq_regs))
			data->f01_container->fh->attention(
					data->f01_container, irq_bits);
	}

	fhd_u8_and(irq_status, irq_status, data->current_irq_mask,
	       data->num_of_irq_regs);
	...
	list_for_each_entry(entry, &data->rmi_functions.list, list)
		if (entry->irq_mask && entry->fh && entry->fh->attention) {
			fhd_u8_and(irq_bits, irq_status, entry->irq_mask,
			       data->num_of_irq_regs);
			if (fhd_u8_is_any_set(irq_bits, data->num_of_irq_regs)) {
				     ...
				     u16 page_start)
{
	...
	struct device *dev = &rmi_dev->dev;
	struct rmi_function_handler *fh =
		fhd_rmi_get_function_handler(0x01);
	struct rmi_device_platform_data *pdata;
...
#ifdef CONFIG_RMI4_FWLIB
	if (has_f34)
		fhd_rmi4_fw_update(rmi_dev, &f01_pdt, &f34_pdt);
	else
...
static int rmi_driver_probe(struct rmi_device *rmi_dev)
{
	...
	retval = fhd_rmi_driver_f01_init(rmi_dev);
	if (retval < 0) {
...
{
	return fhd_rmi_register_driver(&sensor_driver);
}

static void __exit rmi_driver_exit(void)
{
	fhd_rmi_unregister_driver(&sensor_driver);
}
