...
static struct device_attribute fn_01_attrs[] = {
	...
	__ATTR(wakeup_threshold, RMI_RO_ATTR_1,
	       rmi_fn_01_wakeup_threshold_show,
		rmi_fn_01_wakeup_threshold_store),
	__ATTR(doze_holdoff, RMI_RO_ATTR_1,
	...
	__ATTR(reset, RMI_RO_ATTR_1,
	       rmi_show_error, rmi_fn_01_reset_store),
	__ATTR(allow_doze, RMI_RO_ATTR_1,
	       rmi_show_error, rmi_fn_01_allow_doze_store),
...
int fhd_rmi_set_bit_field(struct rmi_device *rmi_dev,
				     ...
				     const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &allow_doze) != 1)
		return -EINVAL;
	if (allow_doze < 0 || allow_doze > 1)
		return -EINVAL;

	/* Per spec, 0 has no effect, so we skip it entirely. */
	if (allow_doze) {
		commands.allow_doze = 1;
		retval = rmi_write_block(fc->rmi_dev, fc->fd.command_base_addr,
				&commands.reg, sizeof(commands.reg));
		if (retval < 0) {
			dev_err(dev, "%s: failed to issue allow_doze command, "
				"error = %d.", __func__, retval);
			return retval;
		}
	}
...
int fhd_rmi_driver_f01_init(struct rmi_device *rmi_dev)
{
