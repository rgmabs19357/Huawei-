...
struct bin_attribute fhd_dev_rep_data = {
	.attr = {
		 .name = "rep_data",
		 .mode = RMI_RO_ATTR},
	.size = 0,
...
bool fhd_attrs_ctrl_regs_exist[ARRAY_SIZE(attrs_ctrl_regs)];
...
static void rmi_f54_free_memory(struct rmi_function_container *fc)
{
	...
	sysfs_remove_bin_file(&fc->dev.kobj, &fhd_dev_rep_data);
	if (f54) {
...
static int rmi_f54_create_sysfs(struct rmi_function_container *fc)
{
	...
	for (reg_num = 0; reg_num < ARRAY_SIZE(attrs_ctrl_regs); reg_num++) {
		if (fhd_attrs_ctrl_regs_exist[reg_num]) {
	...
	retval = sysfs_create_bin_file(&fc->dev.kobj, &fhd_dev_rep_data);
	if (retval < 0) {
...
static int rmi_f54_initialize(struct rmi_function_container *fc)
{
	...
	fhd_attrs_ctrl_regs_exist[reg_num] = true;
	reg_num++;
...
int fhd_rmi_f54_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	struct rmi_driver *driver = fc->rmi_dev->driver;
	char fifo[2];
	struct rmi_fn_54_data *data = fc->data;
...
static struct rmi_function_handler function_handler = {
	...
	.config = NULL,
	.reset = rmi_f54_reset,
	.attention = fhd_rmi_f54_attention,
