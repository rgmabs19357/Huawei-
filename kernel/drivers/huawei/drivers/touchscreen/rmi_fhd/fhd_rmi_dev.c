...
#define CHAR_DEVICE_NAME "fhd_rmi"
#define DEVICE_CLASS_NAME "fhd_rmidev"
...
static int __init rmidev_init(void)
{
	...
	error = fhd_rmi_register_character_driver(&rmidev_driver);
	if (error)
...
{
	pr_debug("%s: exiting.\n", __func__);
	fhd_rmi_unregister_character_driver(&rmidev_driver);
	class_destroy(rmidev_device_class);
}
