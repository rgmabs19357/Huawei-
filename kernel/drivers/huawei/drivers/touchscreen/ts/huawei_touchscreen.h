...
#define TS_DEV_NAME "touchscreen"
...
#define TS_RAWDATA_BUFF_MAX 1024
...
#define TS_LOG_INFO(format, arg...)  printk(KERN_ERR "[TS INF][%s]" format , __FUNCTION__ , ## arg)
#define TS_LOG_ERR(format, arg...)  printk(KERN_ERR "[TS ERR][%s]" format , __FUNCTION__ , ## arg)
#define TS_LOG_DEBUG(format, arg...)  do{			\
		if(g_ts_log_cfg)	\
			printk(KERN_INFO "[TS DBG][%s]" format , __FUNCTION__ , ## arg);}while(0)
