...
static int fwu_enter_flash_prog(void)
{
	...
	if (retval < 0)
      ...
      TS_LOG_INFO("fwu->command = 0x%x, fwu->flash_status = 0x%x\n", fwu->command, fwu->flash_status);
...
int synaptics_fw_update(void)
{
	...
	if (retval < 0) {
		TS_LOG_ERR("Failed reset device\n");
		return retval;
	}
	...
	if (f01_device_status.flash_prog)
		TS_LOG_INFO("Flash status %d\n",
				f01_device_status.status_code);

	if (f01_device_status.flash_prog) {
		TS_LOG_INFO("Device is in flash prog mode 0x%02X\n", f01_device_status.status_code);
		retval = 0;
		goto exit;
	}
...
int synaptics_fw_data_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		TS_LOG_INFO("Failed to read PDT properties, assuming 0x00\n");
...
int synaptics_get_fw_data_boot(char *file_name)
{
	...
	snprintf(firmware_name, sizeof(firmware_name), "rmi4/%s.img", file_name);
...
bool synaptics_check_fw_version(void)
{
	...
	unsigned char *fw_image = fwu->fw_image;
