...
static void cypress_fw_watchdog_work(struct work_struct *work);
static void cypress_fw_watchdog_timer(unsigned long data);
static void cypress_fw_start_wd_timer(void);
static void cypress_fw_stop_wd_timer(void);
...
static int cypress_power_on_reset(void)
{
	...
	rc = regulator_set_voltage(vdd, 2800000, 2800000);
	if(rc < 0){
	...
	if (rc < 0) {
		TS_LOG_ERR("%s vbus enable error:%d \n", rc);
		return -EINVAL;
	}
	msleep(5);
	return rc;
...
static int cypress_glove_switch(struct ts_glove_info *info)
{
	...
	if (core_data.process_state != CY_STATE_NORMAL) {
		TS_LOG_INFO("not in noarmal state\n");
		return -EIO;
	}
...
static int cypress_sysinfo_offsets(void)
{
	...
	rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), &core_data.sysinfo.sysinfo_data, sizeof(core_data.sysinfo.sysinfo_data));
	if (rc < 0) {
...
static int cypress_sysinfo_test(void)
{
	...
	rc = cypress_i2c_read(&core_data.sysinfo.sysinfo_merged_data.test_ofs, 1, test, test_size);
	if (rc < 0) {
...
static int cypress_sysinfo_cydata(void)
{
	...
	rc = cypress_i2c_read(&core_data.sysinfo.sysinfo_merged_data.cydata_ofs, 1, buf, cydata_size);
	if (rc < 0) {
...
static int cypress_sysinfo_pcfg(void)
{
	...
	rc = cypress_i2c_read(&core_data.sysinfo.sysinfo_merged_data.pcfg_ofs, 1, pcfg, pcfg_size);
	if (rc < 0) {
...
static int cypress_sysinfo_opcfg(void)
{
	...
	rc = cypress_i2c_read(&core_data.sysinfo.sysinfo_merged_data.opcfg_ofs, 1, opcfg, opcfg_size);
	if (rc < 0) {
		TS_LOG_ERR("Failed to read cydata rc = %d\n", rc);
		goto free_opcfg;
	}
...
static int cypress_exec_cmd(u8 mode, u8 *cmd_buf, size_t cmd_size, u8 *return_buf, size_t return_buf_size)
{
	...
	for (i = 0; i < I2C_RW_TRIES + 2; i++) {
		...
		rc = cypress_i2c_read(&cmd_ofs, 1, &command, sizeof(command));
		if (rc < 0) {
	...
	reg_addr = cmd_ofs + 1;
	rc = cypress_i2c_read(&reg_addr, 1, return_buf, return_buf_size);
	if (rc < 0) {
...
static int cypress_power_up(void)
{
	int rc = 0;

	vdd = regulator_get(NULL, TP_ANALOG_VDD);
	if (IS_ERR(vdd)) {
	...
	vbus = regulator_get(NULL, TP_IO_VDD);
	if (IS_ERR(vbus)) {
	...
	if(rc < 0){
		dev_err(cypress_dev, "%s: failed to set cypress vbus\n", __func__);
		return -EINVAL;
	}
#endif
	rc = regulator_enable(vbus);
...
static void cypress_power_off(void)
{
...
static int cypress_exit_bl_into_sysinfo(void)
{
	...
	rc = cypress_i2c_write(reg_addr, ldr_exit, sizeof(ldr_exit));
	if(rc < 0){
...
static int cypress_ldr_verify_row_command()
{
	u16 crc = 0;
	int i = 0;
	int rc = 0;
...
static int cypress_ldr_verify_chksum_command()
{
	u16 crc = 0;
	int i = 0;
	int rc = 0;
...
static cypress_ldr_verify_row_result(u8 *stat_ret, size_t num_stat_byte,
			     size_t status_size, unsigned long timeout_ms)
{
	return cypress_command_result(stat_ret, num_stat_byte, status_size, timeout_ms);
}

static cypress_ldr_verify_chksum_result(u8 *stat_ret, size_t num_stat_byte,
			     size_t status_size, unsigned long timeout_ms)
{
	return cypress_command_result(stat_ret, num_stat_byte, status_size, timeout_ms);
}

static cypress_ldr_exit_result(u8 *stat_ret, size_t num_stat_byte,
			     size_t status_size, unsigned long timeout_ms)
{
	return cypress_command_result(stat_ret, num_stat_byte, status_size, timeout_ms);
}
...
static int cypress_load_app(const u8 *fw, int fw_size)
{
	...
	TS_LOG_DEBUG("state = %d, p=%p, fw=%p, fw_size=%d\n", state, p, fw, fw_size);
LOOP:
	switch (state) {
		...
		case FW_UP_LDR_INT_RESULT:
			...
			break;
		case FW_UP_ROW_PRO:
			if (false == fw_wd_first) {
				cypress_fw_start_wd_timer();
				fw_wd_first = true;
			}
			...
			rc2 = cypress_prog_row_command(core_data.ld.row_image);
			if (rc1 < 0 || rc2 < 0){
				state = FW_UP_FAIL;
			} else {
...
static void cypress_fw_watchdog_work(struct work_struct *work)
{
	...
	static unsigned char equal_count = 0;
	bool force_update = false;

	if (core_data.process_state == CY_STATE_NORMAL) {
		TS_LOG_INFO("in noarmal state, stop the fw update watchdog\n");
		return;
	}

	mutex_lock(&core_data.fw_update_wd_mutex);
	if (old_count != core_data.load_app_count) {
		force_update = false;
		equal_count = 0;
	} else {
		equal_count ++;
	...
	old_count = core_data.load_app_count;
	mutex_unlock(&core_data.fw_update_wd_mutex);

	if (true == force_update) {
		TS_LOG_INFO("continue fw update, core_data.load_app_count =%d\n", core_data.load_app_count);
		cypress_fw_update_boot(NULL);
	}
...
static void cypress_fw_watchdog_timer(unsigned long data)
{
	if (!work_pending(&core_data.fw_watchdog_work))
		schedule_work(&core_data.fw_watchdog_work);
...
static void cypress_fw_start_wd_timer(void)
{
	TS_LOG_INFO("start wd\n");
	mod_timer(&core_data.fw_watchdog_timer, jiffies +
			msecs_to_jiffies(50));
	return;
}
...
{
	TS_LOG_INFO("stop wd\n");
	del_timer(&core_data.fw_watchdog_timer);
	cancel_work_sync(&core_data.fw_watchdog_work);
	core_data.load_app_count = 0;
	return;
}
		...
		struct ts_device_data *chip_data, struct platform_device *ts_dev)
{
	...
	/*set device name for get regulator*/
	rc = cypress_power_up();
	if (rc < 0) {
		TS_LOG_ERR("Failed to power up, rc = %d\n", rc);
		goto regulator_err;
	}

	rc = cypress_gpio_request();
	if(rc < 0){
		TS_LOG_ERR("Failed to config reset gpio, rc = %d\n", rc);
		goto regulator_err;
	}
...
{
	int rc = NO_ERR;
	/*start an fw update wd timer*/
	mutex_init(&core_data.fw_update_wd_mutex);
	core_data.load_app_count  = 0;
	INIT_WORK(&(core_data.fw_watchdog_work), cypress_fw_watchdog_work);
	setup_timer(&(core_data.fw_watchdog_timer),
		cypress_fw_watchdog_timer, (unsigned long)(&core_data));
	rc = cypress_initialize_device();
	if (rc < 0) {
...
static int cypress_xy_worker(struct ts_fingers *info)
{
	...
	enum cypress_tch_abs abs;
	enum cypress_event_id event_id;
	u8 num_cur_tch;
	...
	readElementOffset += returnedElement;			//calc number of element that has been read
	 while (leftOverElement > 0){
		...
		if (readByte >= (I2C_BUF_MAX_SIZE - elementStartOffset)) {
			rc = cypress_i2c_read(&elementStartOffset, 1, core_data.rawdate.ic_buf + dataIdx, I2C_BUF_MAX_SIZE - elementStartOffset);
			dataIdx += (I2C_BUF_MAX_SIZE - elementStartOffset);
		} else {
			rc = cypress_i2c_read(&elementStartOffset, 1, core_data.rawdate.ic_buf + dataIdx, readByte);
			dataIdx += readByte;
		}
		if (rc < 0) {
			TS_LOG_ERR("Error on read rc = %d\n", rc);
...
static int cypress_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	if (TS_ACTION_READ == info->op_action) {
...
cypress_get_rawdata:
	if (TS_ACTION_UNDEF == info->op_action) {
		if (rc1 < 0) {
			memcpy(info->buff, "0F-1F-2P-3P", sizeof("0F-1F-2P-3P"));
		} else if (rc2  < 0) {
			memcpy(info->buff, "0P-1F-2P-3P", sizeof("0P-1F-2P-3P"));
		} else {
			memcpy(info->buff, "0P-1P-2P-3P", sizeof("0P-1P-2P-3P"));
		}
	}
