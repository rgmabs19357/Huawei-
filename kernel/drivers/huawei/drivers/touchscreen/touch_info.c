...
#include <linux/err.h>
#include <linux/rmi.h>
#include <cypress/cyttsp4_core.h>
...
extern int rmi_start_mmi_test(char * buf, size_t count);
extern int rmi_get_mmi_test_result(char * buf);
...
static ssize_t touch_mmi_test_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int rc = 0;
	if (dev == NULL) {
	...
	} else if (0 == strcmp(touch_chip_info, TOUCH_INFO_RMI3250)) {  //rmi 3250
		return rmi_get_mmi_test_result(buf);
	}
}

/*start tp's mmi test*/
static ssize_t touch_mmi_test_store(struct device *dev,
...
static DEVICE_ATTR(touch_mmi_test, 0664,
				   touch_mmi_test_show, touch_mmi_test_store);
