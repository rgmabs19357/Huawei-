...
static int cyttsp4_check_ttconfig_crc(struct cyttsp4_device *ttsp)
{
	...
	rc = cyttsp4_get_crc_from_params(ttsp, &cfg_crc_new);
	if (rc)
		return 0;
...
static void cyttsp4_fw_upgrade(struct work_struct *fw_upgrade)
{
	...
	struct cyttsp4_touch_firmware *fw=NULL;//hgz
	struct cyttsp4_device *ttsp = data->ttsp;
	struct device *dev = &ttsp->dev;
	int rc;
...
#if CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_MANUAL_TTCONFIG_UPGRADE
...
exit:
	if (!rc) {
#ifdef CALIBRATE_AFTER_TTCONFIG_UPGRADE
		/* set up startup call back */
		dev_vdbg(dev, "%s: Adding callback for calibration\n",
				__func__);
		rc = cyttsp4_subscribe_attention(ttsp, CY_ATTEN_STARTUP,
		...
		u8 *bin_config_data, int bin_config_size, u8 **start, int *len)
{
	...
	if (cyttsp4_check_version_(ttsp, fw_ver_config,
