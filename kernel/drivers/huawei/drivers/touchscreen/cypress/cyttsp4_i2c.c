...
extern int hsad_get_cyttsp4_enable(void);
...
/*add for esd re-power begin*/
static struct cyttsp4_client{
	//struct i2c_client client;
	struct regulator *vdd;
	struct regulator *vbus;
}cy_client;

int cyttsp4_power_on_reset(struct device *dev)
{
	int err = 0;
	/*disable regulator first*/
	regulator_disable(cy_client.vbus);
	regulator_disable(cy_client.vdd);
	msleep(5);
	err = regulator_enable(cy_client.vdd);
	if (err < 0) {
		dev_err(dev, "%s vdd enable error:%d \n", __func__, err);
	}
	err = regulator_enable(cy_client.vbus);
	if (err < 0) {
		dev_err(dev, "%s vbus enable error:%d \n", __func__, err);
	}
	msleep(5);
	return err;
}
EXPORT_SYMBOL(cyttsp4_power_on_reset);
