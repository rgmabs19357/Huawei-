...
int touch_type_state;
EXPORT_SYMBOL(touch_type_state);
#define HI_BYTE(x) (u8)(((x) >> 8) & 0xFF)
#define LO_BYTE(x) (u8)((x) & 0xFF)

extern int set_touch_chip_info(const char *chip_info);
extern int cyttsp4_power_on_reset(struct device *dev);
...
static int cyttsp4_startup_(struct cyttsp4_core_data *cd)
{
	...
	rc = cyttsp4_reset_and_wait(cd);
	if (rc < 0) {
		dev_err(cd->dev, "%s: Error on h/w reset rc = %d\n", __func__, rc);
		cyttsp4_power_on_reset(&cd->dev);
		rc = cyttsp4_reset_and_wait(cd);
		if (rc < 0)
			dev_err(cd->dev, "%s: Error on h/w reset twice  r=%d\n", __func__, rc);
	}
...
static struct device* core_dev = NULL;

int cyttsp4_get_glove_switch(u8 *status)
{
	struct cyttsp4_core_data *cd;
	u8 cmd_buf[CY_CMD_OP_GET_PARA_CMD_SZ];
	u8 return_buf[CY_CMD_OP_GET_PARA_RET_SZ];
	...
	if(NULL == core_dev) {
	    pr_err("%s core_dev==NULL\n", __func__);
	    return -1;
	}
 	cd = dev_get_drvdata(core_dev);

	cmd_buf[0] = CY_CMD_OP_GET_PARA;
	cmd_buf[1] = CY_OP_PARA_SCAN_TYPE;

	pm_runtime_get_sync(core_dev);

	rc = request_exclusive(cd, cd->core, CY_CORE_REQUEST_EXCLUSIVE_TIMEOUT);
	if (rc < 0) {
		dev_err(core_dev, "%s: fail get exclusive ex=%p own=%p\n",
				__func__, cd->exclusive_dev, cd->core);
		goto exit_put;
	}
	...
	rc = cyttsp4_exec_cmd(cd, CY_MODE_OPERATIONAL,
			...
			CY_COMMAND_COMPLETE_TIMEOUT);
	if (rc < 0) {
		dev_err(core_dev, "%s: exec cmd error.\n", __func__);
		goto exit_release;
	}

	dev_dbg(core_dev, "%s: return_buf=0x%x,0x%x,0x%x\n", __func__,
			return_buf[0], return_buf[1], return_buf[2]);

	if (return_buf[0] != CY_OP_PARA_SCAN_TYPE) {
		dev_err(core_dev, "%s: return data error.\n", __func__);
		rc = -EINVAL;
		goto exit_release;
	}
	...
	if (rc == 0) {
		...
		*status = 1;
		...
		dev_err(core_dev, "error detected\n");
		return -1;
	}

exit_release:
	if (release_exclusive(cd, cd->core) < 0) {
		dev_err(core_dev, "%s: fail to release exclusive\n", __func__);
	}
exit_put:
	pm_runtime_put(core_dev);
	return 0;
}
EXPORT_SYMBOL(cyttsp4_get_glove_switch);
...
int cyttsp4_set_glove_switch(u8 status)
{
	struct cyttsp4_core_data *cd;
	u8 cmd_buf[CY_CMD_OP_SET_PARA_CMD_SZ];
	u8 return_buf[CY_CMD_OP_SET_PARA_RET_SZ];
	...
	default:
		mutex_unlock(&cd->system_lock);
		dev_err(core_dev, "%s: Invalid signal status=%d\n", __func__,
				(int)status);
		rc = -EINVAL;
		goto exit;
	}
	...
	dev_dbg(core_dev, "%s: return_buf=0x%x,0x%x\n", __func__,
			return_buf[0],	return_buf[1]);
exit_release:
	if (release_exclusive(cd, cd->core) < 0)
		/* Don't return fail code, mode is already changed. */
		...
		dev_vdbg(core_dev, "%s: pass release exclusive\n", __func__);
exit_put:
	pm_runtime_put(core_dev);
exit:
	return rc;
}
EXPORT_SYMBOL(cyttsp4_set_glove_switch);
/*
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	rc= set_touch_chip_info(TOUCH_INFO_CYPRESS);
	if (rc) {
		dev_err(cd->dev, "set_touch_chip_info error\n");
	}
...
error_attr_create:
	free_irq(cd->irq, cd);
	del_timer(&cd->watchdog_timer);//hgz
error_request_irq:
error_gpio_irq:
