...
static int lm3642_read_reg8(u8 reg, u8 *val)
{
	...
	if (NULL == lm3642_client || NULL == lm3642_client->adapter) {
 ...
 **************************************************************************
 */
int lm3642_set_gpio(unsigned gpio, int value)
{
    int ret = -EINVAL;
    char *label;
    ...
    if (ret < 0) {
        print_error("%s: request flash gpio - %d(%s) failed, result = %d.", __func__, gpio, label, ret);
        return -EINVAL;
    }

    ret = gpio_direction_output(gpio, value);
    if (ret < 0) {
        print_error("%s: fail to set flash gpio - %d(%s), result = %d.", __func__, gpio, label, ret);
    }
...
static int lm3642_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(flash_config_type == 2) { //process long press key 4s
        ...
        flash_mask_vdd = regulator_get(&client->dev, "FLASH_MASK_IO_VDD");
...
static int __init lm3642_module_init(void)
{
	...
	get_hw_config_int("camera/miniisp", &val, NULL);
	if (val == 1)
		mini_register_camera_flash(&lm3642_intf);
	else
