...
static int RG_Ratio_Typical = 0x26E;
static int BG_Ratio_Typical = 0x2E9;
static u8 otp_group_idx = 3;
static struct otp_struct ov5648_foxconn_otp;
#endif

static effect_params effect_ov5648_foxconn = {
#include "effect_ov5648_foxconn.h"
};
...
static framesize_s ov5648_foxconn_framesizes[] = {
	{0, 0, 1280, 960, 1408, 1320, 30, 30, 0x18D, 0x14B, 0x100, VIEW_FULL, RESOLUTION_4_3, false, false, {ov5648_foxconn_framesize_quarter, ARRAY_SIZE(ov5648_foxconn_framesize_quarter)} },
	{0, 0, 2592, 1952, 2816, 2088, 15, 15, 0x138, 0x104, 0xC9, VIEW_FULL, RESOLUTION_4_3, false, false, {ov5648_foxconn_framesize_full, ARRAY_SIZE(ov5648_foxconn_framesize_full)} },
};
...
/* get value */
	...
	ov5648_foxconn_otp.version_id = (ov5648_foxconn_otp.vendor_version) & 0x0f;
	ov5648_foxconn_otp.vendor_version = (ov5648_foxconn_otp.vendor_version >> 4) & 0x0f;
	print_info("S: enter %s   year = %d    mouth = %d   data = %d", __func__,ov5648_foxconn_otp.year,ov5648_foxconn_otp.month,ov5648_foxconn_otp.date);
...
{
	print_info("enter %s  r_gain = %d   g_gain = %d  b_gain = %d", __func__,R_gain,G_gain,B_gain);
	if (R_gain > 0x400) {
...
static int ov5648_foxconn_enum_framesizes(struct v4l2_frmsizeenum *framesizes)
{
	...
	u32 this_max_index = ARRAY_SIZE(ov5648_foxconn_framesizes) - 1;
...
{
	int max_index = ARRAY_SIZE(ov5648_foxconn_framesizes) - 1;
...
#endif    
	if (i >= ARRAY_SIZE(ov5648_foxconn_framesizes)) {
...
static int ov5648_foxconn_init_reg(void)
{
	int size = 0;
	u8 i;
	print_info("Enter Function:%s  , initsize=%d", __func__, sizeof(ov5648_foxconn_init_regs));
        #if 0
	...
	mdelay(5);
	size = ARRAY_SIZE(ov5648_foxconn_init_regs);
...
static int ov5648_foxconn_framesize_switch(camera_state state)
{
	...
	if (next_frmsize_index >= ARRAY_SIZE( ov5648_foxconn_framesizes )){
...
static int ov5648_foxconn_stream_on(camera_state state)
{
	...
	return ov5648_foxconn_framesize_switch(state);
}
...
static int ov5648_foxconn_init(void)
{
	...
	if (false == ov5648_foxconn_check) {
        if (check_suspensory_camera("OV5648FX") != 1) {
...
static void ov5648_foxconn_set_default(void)
{
	...
	ov5648_foxconn_sensor.frmsize_list = ov5648_foxconn_framesizes;
	...
	ov5648_foxconn_sensor.get_sensor_aperture = NULL;
	...
	ov5648_foxconn_sensor.info.orientation = 90;
	ov5648_foxconn_sensor.lane_clk = CLK_450M;
	ov5648_foxconn_sensor.effect = &effect_ov5648_foxconn;
