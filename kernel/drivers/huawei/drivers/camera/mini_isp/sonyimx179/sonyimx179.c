...
#define SONYIMX179_APERTURE_FACTOR              240 //F2.4
#define SONYIMX179_EQUIVALENT_FOCUS	            26  //26mm
...
static mini_framesize_s sonyimx179_framesizes[] = {
    /*1280x720, hdr movie*/
	{0, 0, 1280, 720, 4800, 4800, 30, 30, 0x2F3, 0x275,0x100, VIEW_FULL, RESOLUTION_16_9,false,false,{sonyimx179_rgb_framesize_1280x720_HDR, ARRAY_SIZE(sonyimx179_rgb_framesize_1280x720_HDR)},3},
	...
	{0, 0, 1920, 1088, 4800, 4800, 27, 27, 100, 120, 0x100, VIEW_FULL, RESOLUTION_16_9,false,false,{sonyimx179_rgb_framesize_1280x720_HDR, ARRAY_SIZE(sonyimx179_rgb_framesize_1280x720_HDR)},4},
	{0, 0, 3264, 2448, 4800, 4800, 20, 20, 100, 120, 0x100, VIEW_FULL, RESOLUTION_4_3,false,false,{sonyimx179_rgb_framesize_1280x720_HDR, ARRAY_SIZE(sonyimx179_rgb_framesize_1280x720_HDR)},1},
	{0, 0, 3264, 2448, 4800, 4800, 20, 20, 100, 120, 0x100, VIEW_ZOOM_MODE, RESOLUTION_4_3,false,false,{sonyimx179_rgb_framesize_1280x720_HDR, ARRAY_SIZE(sonyimx179_rgb_framesize_1280x720_HDR)},1},
				 ...
				 struct v4l2_frmsize_discrete *fs, int flag, camera_setting_view_type view_type,bool zsl_preview)
{
    ...
    if(HDR_MOVIE_ON == sonyimx179_sensor.sensor_hdr_movie.hdrInfo.hdr_on) {
    	...
    	if ((VIEW_FULL == view_type) || (VIEW_ZOOM_MODE == view_type)){
    		for (i = 0; i < size; i++) {
    			...
    			if ((sonyimx179_framesizes[i].width >= fs->width)
    			    && (sonyimx179_framesizes[i].height >= fs->height)
    			    && (view_type == sonyimx179_framesizes[i].view_type)
    			    && (mini_camera_get_resolution_type(fs->width, fs->height)
    			    <= sonyimx179_framesizes[i].resolution_type)) {
        				fs->width = sonyimx179_framesizes[i].width;
        				fs->height = sonyimx179_framesizes[i].height;
...
static int sonyimx179_stream_on(camera_state state)
{
	...
	if (state == STATE_PREVIEW){
		...
		frmsize_index = sensor->frmsize_list[frmsize_index].mini_isp_index;
		camera_agent_take_pictures(sensor->sensor_index, frmsize_index,hv_flip,0,0,0,0);		
	}
	return 0;
}
...
static int sonyimx179_get_equivalent_focus()
{
	return SONYIMX179_EQUIVALENT_FOCUS;
}
	...
	if(0 == get_sensor_iopower()) {
      ...
      /*main camera*/
		...
		mini_k3_ispio_power_init(M_CAMERA_CORE_VDD, LDO_VOLTAGE_10V, LDO_VOLTAGE_10V);	/*core 1.05V - pri camera*/
         ...
         sonyimx179_sensor.needSetmode = true;
	return 0;
}
...
static void sonyimx179_set_default(void)
{
	...
	sonyimx179_sensor.ap_writeAE_delay = 0; /* 5 expo and gain registers, 6000 us is enough */
#else
	...
	sonyimx179_sensor.sensor_rgb_type = SENSOR_RGGB;/* changed by y00231328. add bayer order*/
	...
	sonyimx179_sensor.get_equivalent_focus = sonyimx179_get_equivalent_focus;
