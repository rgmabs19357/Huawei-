 ...
 * mini_isp_cmd_misp
 *
 *  Author: 	Dingwei (dingwei@163.com)
 *  Date:  	2013/11/9
 *  Version:	1.0
 *  History:	2013/11/9      Frist add driver for misp chardev
 *
 * ----------------------------------------------------------------------------
 *
...
static ssize_t misp_cdev_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	print_info("buff[0] = %c",buff[0]);
	if(-1 == buff[0]) {
		print_error("without cmd");
		return -1;
	}else if('0' == buff[0]) {
		print_info("open denoise");	
		camera_agent_set_denoise_mode(1);
	}else if('1' == buff[0]) {
		print_info("close denoise");
...
{
	print_info("enter %s cmd = %u",__func__,cmd);
	int ret =0;
	int ioarg = 0;
	int index_cmd = 0;
	if(_IOC_TYPE(cmd)!=MINIISP_CMD_MAGIC)
	{
		print_info("MAGIC error");
		return -EINVAL;
	}
	switch(cmd)
	{
		case MINIISP_CMD_CLOSE_DENOISE:
			camera_agent_set_denoise_mode(0);
			print_info("MINIISP_CMD_CLOSE_DENOISE OK");
			break;
		case MINIISP_CMD_OPEN_DENOISE:
			camera_agent_set_denoise_mode(1);
			print_info("MINIISP_CMD_OPEN_DENOISE OK");
			break;
		default:
			print_info("NO CMD");
			return -EINVAL;
...
static void misp_cdev_setup(struct mispcdev *dev,int index,dev_t  *devp)
{
	...
	print_info("misp_cdev_setup mispcdev_Major = %d mispcdev_Min = %d",mispcdev_Major,mispcdev_Min);
	int err;
...
static int __init misp_cdev_init(void)
{
	...
	print_info("misp_cdev_init mispcdev_Major = %d mispcdev_Min = %d",mispcdev_Major,mispcdev_Min);
	if(ret<0)
	{
		print_info("register_chrdev_region error %d",ret);
		return -1;
	}
