/*
 * sonyimx214 sensor driver
 *
 *  Author: 	Zhoujie (zhou.jie1981@163.com)
 *  Date:  	2013/01/05
 *  Version:	1.0
 *  History:	2013/01/05      Frist add driver for sonyimx214
 *
 * ----------------------------------------------------------------------------
 *
...
#define SONYIMX214_EQUIVALENT_FOCUS	            28  //28mm
...
static mini_camera_capability sonyimx214_cap[] = {
	...
	{V4L2_CID_FOCUS_MODE, FOCUS_MODE},
...
//static _mini_effect_params  *effect=NULL; //z00174260

static mini_framesize_s sonyimx214_framesizes[] = {
	//{0, 0, 1920, 1088, 4800, 4800, 30, 30, 0x2F3, 0x275, 0x100, VIEW_FULL, RESOLUTION_16_9,false,false,{sonyimx214_rgb_framesize_1280x720_HDR, ARRAY_SIZE(sonyimx214_rgb_framesize_1280x720_HDR)},4},
	{0, 0, 2096, 1552, 4800, 4800, 30, 30, 0x2F3, 0x275,0x100, VIEW_FULL, RESOLUTION_4_3,false,false,{sonyimx214_rgb_framesize_1280x720_HDR, ARRAY_SIZE(sonyimx214_rgb_framesize_1280x720_HDR)},0 },
	{0, 0, 4208, 3120, 4800, 4800, 10, 10, 0x2F3, 0x275,0x100, VIEW_FULL, RESOLUTION_4_3,false,false,{sonyimx214_rgb_framesize_1280x720_HDR, ARRAY_SIZE(sonyimx214_rgb_framesize_1280x720_HDR)},1 },
};
 ...
 **************************************************************************
 */
 /* del to remove compile warning by c00220250, 20131230 */
 #if 0
				 ...
				 struct v4l2_frmsize_discrete *fs, int flag, camera_setting_view_type view_type,bool zsl_preview)
{
	...
	if (state == STATE_PREVIEW) {
                 if (sonyimx214_sensor.preview_frmsize_index != i) {
                    sonyimx214_sensor.needSetmode = true;
                 }
 ...
 **************************************************************************
*/
/* del by c00220250 to remove compile warning , 20131230 */
#if 0
...
extern int misp_g_motion;
extern int misp_g_multidenoise;
static int sonyimx214_stream_on(camera_state state)
{
	...
	if (state == STATE_PREVIEW){
		...
		camera_agent_take_pictures(sensor->sensor_index, frmsize_index, hv_flip, 0, 0, (u8)misp_g_multidenoise, (u8)misp_g_motion);
	}
	return 0;
}
...
static int sonyimx214_get_equivalent_focus()
{
	return SONYIMX214_EQUIVALENT_FOCUS;
}
...
static void sonyimx214_set_default(void)
{
	...
	sonyimx214_sensor.ap_writeAE_delay = 0; /* 5 expo and gain registers, 6000 us is enough */
#else
	...
	sonyimx214_sensor.sensor_rgb_type = SENSOR_RGGB;/* changed by y00231328. add bayer order*/
	...
	sonyimx214_sensor.get_equivalent_focus = sonyimx214_get_equivalent_focus;
