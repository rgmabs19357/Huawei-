...
#define S5K4E1GA_FOXCONN_I2C_RETRY_TIMES    5
...
static effect_params effect_s5k4e1ga_foxconn = {
#include "effect_s5k4e1ga_foxconn_edge.h"
};

static framesize_s s5k4e1ga_foxconn_framesizes[] = {
	{0, 0, 1280, 960, 2982, 1072, 30, 30, 0x141, 0x10C, 0x100, VIEW_FULL, RESOLUTION_4_3, false, false, {s5k4e1ga_foxconn_framesize_1280x960, ARRAY_SIZE(s5k4e1ga_foxconn_framesize_1280x960)} },
	//{0, 0, 1920, 1088, 2982, 1200, 27, 27, 0x141, 0x10C, 0x200, VIEW_FULL, RESOLUTION_4_3, false, false, {s5k4e1ga_foxconn_framesize_1080p, ARRAY_SIZE(s5k4e1ga_foxconn_framesize_1080p)} },
	{0, 0, 2592, 1952, 2982, 2148, 15, 15, 0x141, 0x10C, 0x100, VIEW_FULL, RESOLUTION_4_3, false, false, {s5k4e1ga_foxconn_framesize_full, ARRAY_SIZE(s5k4e1ga_foxconn_framesize_full)} },
};
...
static int s5k4e1ga_foxconn_read_reg(u16 reg, u8 *val)
{
	return k3_ispio_read_reg(s5k4e1ga_foxconn_sensor.i2c_config.index,
				 s5k4e1ga_foxconn_sensor.i2c_config.addr, reg, (u16 *)val, s5k4e1ga_foxconn_sensor.i2c_config.val_bits,s5k4e1ga_foxconn_sensor.i2c_config.addr_bits);
}
...
static int s5k4e1ga_foxconn_write_reg(u16 reg, u8 val, u8 mask)
{
	return k3_ispio_write_reg(s5k4e1ga_foxconn_sensor.i2c_config.index,
			s5k4e1ga_foxconn_sensor.i2c_config.addr, reg, val, s5k4e1ga_foxconn_sensor.i2c_config.val_bits, mask,s5k4e1ga_foxconn_sensor.i2c_config.addr_bits);
}
...
static int s5k4e1ga_foxconn_write_seq(const struct _sensor_reg_t *seq, u32 size, u8 mask)
{
	...
	return k3_ispio_write_seq(s5k4e1ga_foxconn_sensor.i2c_config.index,
			s5k4e1ga_foxconn_sensor.i2c_config.addr, seq, size, s5k4e1ga_foxconn_sensor.i2c_config.val_bits, mask,s5k4e1ga_foxconn_sensor.i2c_config.addr_bits);
}
...
/*
 **************************************************************************
 * FunctionName: s5k4e1ga_foxconn_init_isp_reg;
 ...
 **************************************************************************
*/
static int s5k4e1ga_foxconn_init_isp_reg(void)
{
	int size = 0;

	camera_sensor *sensor = &s5k4e1ga_foxconn_sensor;

        size = CAMERA_MAX_SETTING_SIZE;

        s5k4e1ga_foxconn_write_isp_seq(sensor->effect->isp_settings, size);
...
static int s5k4e1ga_foxconn_init_reg(void)
{
	...
	if(E_CAMERA_SENSOR_FLIP_TYPE_H_V == get_secondary_sensor_flip_type()) {
		s5k4e1ga_foxconn_write_reg(S5K4E1GA_FLIP, 0x03, 0x00); //turn camera layout
...
static int s5k4e1ga_update_flip(u16 width, u16 height)
{
	...
	if (old_flip != new_flip) { 
		k3_ispio_update_flip((s5k4e1ga_foxconn_sensor.old_flip ^ new_flip) & 0x03, width, height, (old_flip ^ new_flip) & 0x03 );
    		print_info("secondary_sensor_flip_type = %d", get_secondary_sensor_flip_type());

		if(E_CAMERA_SENSOR_FLIP_TYPE_H_V == get_secondary_sensor_flip_type()){
			s5k4e1ga_foxconn_write_reg(S5K4E1GA_FLIP, s5k4e1ga_foxconn_sensor.vflip ? 0x00 : 0x02, ~0x02);
			s5k4e1ga_foxconn_write_reg(S5K4E1GA_FLIP, s5k4e1ga_foxconn_sensor.hflip ? 0x00 : 0x01, ~0x01);
		}else{
			s5k4e1ga_foxconn_write_reg(S5K4E1GA_FLIP, s5k4e1ga_foxconn_sensor.vflip ? 0x02 : 0x00, ~0x02);
			s5k4e1ga_foxconn_write_reg(S5K4E1GA_FLIP, s5k4e1ga_foxconn_sensor.hflip ? 0x01 : 0x00, ~0x01);
		}
...
static int s5k4e1ga_foxconn_check_sensor(void)
{
    ...
    for(retry = 0; retry < S5K4E1GA_FOXCONN_I2C_RETRY_TIMES; retry++) {
    	s5k4e1ga_foxconn_read_reg(0x0000, &idh);
    	s5k4e1ga_foxconn_read_reg(0x0001, &idl);

    	id = ((idh << 8) | idl);
    	print_info("s5k4e1ga_foxconn product id:0x%x, retrytimes:%d", id, retry);
...
void s5k4e1ga_foxconn_set_gain(u32 gain)
{
	gain = gain << 1;
...
void s5k4e1ga_foxconn_set_exposure(u32 exposure)
{
	exposure = exposure >> 4;
	s5k4e1ga_foxconn_write_reg(S5K4E1GA_FOXCONN_EXPOSURE_REG_H, (exposure >> 8) & 0xff, 0x00);
	s5k4e1ga_foxconn_write_reg(S5K4E1GA_FOXCONN_EXPOSURE_REG_L, exposure & 0xff, 0x00);
...
static int s5k4e1ga_foxconn_init(void)
{
	...
	if (false == s5k4e1ga_check) {
        if (check_suspensory_camera("S5K4E1GA") != 1) {
...
static void s5k4e1ga_foxconn_set_default(void)
{
	...
	s5k4e1ga_foxconn_sensor.init_isp_reg = s5k4e1ga_foxconn_init_isp_reg;
	...
	if(E_CAMERA_SENSOR_FLIP_TYPE_H_V == get_secondary_sensor_flip_type()) {
    	s5k4e1ga_foxconn_sensor.sensor_rgb_type = SENSOR_GBRG;
    } else {
    	s5k4e1ga_foxconn_sensor.sensor_rgb_type = SENSOR_GRBG;
    }
	s5k4e1ga_foxconn_sensor.update_flip = s5k4e1ga_update_flip;
	...
	s5k4e1ga_foxconn_sensor.get_capability = NULL;
	...
	s5k4e1ga_foxconn_sensor.skip_frames = 2; 
	...
	s5k4e1ga_foxconn_sensor.i2c_config.index = I2C_SECONDARY;
	...
	s5k4e1ga_foxconn_sensor.i2c_config.addr_bits = I2C_16BIT;
	...
	s5k4e1ga_foxconn_sensor.ap_writeAE_delay = 0;
#else
	...
	s5k4e1ga_foxconn_sensor.set_gain = s5k4e1ga_foxconn_set_gain;
	s5k4e1ga_foxconn_sensor.set_exposure = s5k4e1ga_foxconn_set_exposure;
	...
	s5k4e1ga_foxconn_sensor.get_vts_reg_addr = s5k4e1ga_foxconn_get_vts_reg_addr;
	...
	s5k4e1ga_foxconn_sensor.info.focal_length = 345; /* 3.45mm*/
	s5k4e1ga_foxconn_sensor.info.h_view_angle = 66; /*  66.1 degree */
	s5k4e1ga_foxconn_sensor.info.v_view_angle = 50;
	s5k4e1ga_foxconn_sensor.lane_clk = CLK_500M;
	s5k4e1ga_foxconn_sensor.hflip = 0;
	s5k4e1ga_foxconn_sensor.vflip = 0;
	s5k4e1ga_foxconn_sensor.old_flip = 0;
    s5k4e1ga_foxconn_sensor.effect = &effect_s5k4e1ga_foxconn;
	s5k4e1ga_foxconn_sensor.support_summary = false;
//	s5k4e1ga_foxconn_sensor.vts_change = 0;
	...
	s5k4e1ga_foxconn_sensor.get_capability = s5k4e1ga_get_capability;

	s5k4e1ga_foxconn_sensor.isp_idi_skip = false;
}
