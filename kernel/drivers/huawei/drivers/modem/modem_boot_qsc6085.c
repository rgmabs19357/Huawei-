...
void uart1_set_gpio_for_runtime_suspend(void)
{
...
void uart1_set_gpio_for_runtime_resume(void)
{
	int k,m;
	if(modem_driver_plat_data)
	{
...
static void modem_boot_change_state(int state)
{
...
static int modem_boot_probe(struct platform_device *pdev)
{
    ...
    if (modem_driver_plat_data) {
		...
		dev_info(&pdev->dev, "%s. gpio request success.\n", __func__);
    }
    else{
        dev_err(&pdev->dev, "%s. pdev->dev.platform_data is NULL.\n",__func__,status);
        goto error;
    }
