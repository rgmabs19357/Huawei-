...
#define PRODUCT_NVRAM_PATH  "/vendor/firmware/%s.txt"
...
void 
dhd_common_init_nvpath(void)
{
	char product_buf[PRODUCT_TYPE_LEN] = {0};
	char nvram_path[NVRAM_PATH_LEN]={0};
    
	memset(nv_path, 0, sizeof(nv_path));
	if (get_wifi_nvram_name(product_buf)){
		
		sprintf(nvram_path, PRODUCT_NVRAM_PATH, product_buf);
		bcm_strcpy_s(nv_path, sizeof(nv_path), nvram_path);
		DHD_ERROR(("%s: boardid nvram path:%s\n", __FUNCTION__,nv_path));
		return;
	}else{
		...
		DHD_ERROR(("%s: default nvram path:%s\n", __FUNCTION__,nv_path));
		return ;
	}
}
              ...
              wl_event_msg_t *event, void **data_ptr)
{
	...
	*data_ptr = &pvt_data[1];
	...
	case WLC_E_FIFO_CREDIT_MAP:
		DHD_ERROR(("%s: PROP_TXSTATUS get wlfc event WLC_E_FIFO_CREDIT_MAP\n", __FUNCTION__));
