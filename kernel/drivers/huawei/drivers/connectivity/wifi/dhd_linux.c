...
#define PRODUCT_TYPE_LEN   48
bool g_emo = FALSE;
bool g_wifilte_coex = FALSE;
...
/* define huawei version string to easily check what is the modification
    every change in bcmdhd should increase the number of huawei version
*/
#define	HUAWEI_VERSION_STR ", HUAWEI-2014.003"
...
dhd_open(struct net_device *net)
{
	...
	if (strlen(firmware_path) != 0) {
		...
		HW_PRINT((WIFI_TAG"set path: %s\n", firmware_path));
		strncpy(fw_path, firmware_path, sizeof(fw_path)-1);
		if (g_wifilte_coex){
			strncat(fw_path, "_lte", 4);
			HW_PRINT((WIFI_TAG"dhd_open fw_path: %s\n", fw_path));
		}
		firmware_path[0] = '\0';
	}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (strlen(firmware_path) != 0) {
		...
		if (g_wifilte_coex){
			strncat(fw_path, "_lte", 4);
			HW_PRINT((WIFI_TAG"dhd_attach fw_path: %s\n", fw_path));
		}
	}
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if(g_emo){
		/* HT40/20 setting */
		uint32 mimo_bw_cap = 1;
		HW_PRINT((WIFI_TAG"set station HT40 enable\n"));
		bcm_mkiovar("mimo_bw_cap", (char *)&mimo_bw_cap, 4, iovbuf, sizeof(iovbuf));
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
				sizeof(iovbuf), TRUE, 0)) < 0) {
				DHD_ERROR(("%s mimo_bw_cap for ap&station failed  %d\n", __FUNCTION__, ret));
		}
	}
#endif
...
static void judge_product()
{
	char product_buf[PRODUCT_TYPE_LEN] = {0};
	
	if (get_wifi_nvram_name(product_buf)){
             if(strstr(product_buf, "EMO")){
                  DHD_ERROR(("%s: is emo wifi, product_buf: %s\n", __FUNCTION__, product_buf));
                  g_emo = TRUE;
             }
       }
	if(product_type("SOPHIA_TL") || product_type("OSCAR_TL")){ //lte & wifi coexist
		g_wifilte_coex = TRUE;
		HW_PRINT((WIFI_TAG"use wifi&lte coexistent firmware.\n"));
	}
}
