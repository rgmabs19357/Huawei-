...
#define WIFI_DRIVER_AMERICAN_COUNTRY_NAME_NUM        10
/*refer to wireless-regdb-2013.11.27*/
const char *american_country_list[WIFI_DRIVER_AMERICAN_COUNTRY_NAME_NUM] =  
{                                                                           
      "CA", "DO", "GD", "GT", "GU", "PA", "PR", "TW", "US", "UZ"
};  
...
extern int dhd_wifi_shutdown(void);
...
int is_special_country(char *country)
{
    int i = 0;

    /* Japan */
    if(!strncasecmp(country, "JP", WLC_CNTRY_BUF_SZ))
        return 1;

    /* American */
    for(i=0; i<WIFI_DRIVER_AMERICAN_COUNTRY_NAME_NUM; i++)
    {
        if(!strncasecmp(american_country_list[i], country, WLC_CNTRY_BUF_SZ))
        {
            return 1;
        }
    }
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	/* CUSTOMER_SET_COUNTRY feature is define for only GGSM model */
		...
		if(!(is_special_country(country_code))){
                    char country[WLC_CNTRY_BUF_SZ] = "AU";
                    printk("change country code from %s to: %s \n", country_code, country);
                    bytes_written = wldev_set_country(net, country, true, true);
                }
                else{
