...
struct bluesleep_info {
	...
	spinlock_t rw_lock; /*Lock for state transitions */
 	...
 	char *blockName; /*name of iomux block*/	
...
static inline int bluesleep_can_sleep(void)
{
	...
	return (LPM_BT_WAKE_DEASSERT == gpio_get_value(bsi->ext_wake)) &&
		(LPM_HOST_WAKE_DEASSERT == gpio_get_value(bsi->host_wake));
#else
    /* check if gpio ext_wake and gpio host_wake are both deasserted */
	return (LPM_BT_WAKE_DEASSERT == gpio_get_value(bsi->ext_wake)) &&
		(LPM_HOST_WAKE_DEASSERT == gpio_get_value(bsi->host_wake)) &&
...
#endif
{
	if (test_bit(BT_ASLEEP,	&bsi->flags)) {
		...
		gpio_set_value(bsi->ext_wake, LPM_BT_WAKE_ASSERT);
	...
	*/
	/* Here we depend on the status of gpio ext_wake, for stability	*/
		if (LPM_BT_WAKE_DEASSERT == gpio_get_value(bsi->ext_wake)) {
...
static void	bluesleep_hostwake_task(unsigned long data)
{
	...
	if (LPM_HOST_WAKE_ASSERT == gpio_get_value(bs_info->host_wake))
		bluesleep_rx_busy();
	else
		bluesleep_rx_idle();
...
static void	bluesleep_outgoing_data(struct bluesleep_info *bs_info)
{
	...
	if (LPM_BT_WAKE_DEASSERT == gpio_get_value(bs_info->ext_wake)) {
...
static void	bluesleep_tx_timer_expire(unsigned long	data)
{
	...
	if (!test_bit(BT_TXDATA, &bs_info->flags)) {
		...
		gpio_set_value(bs_info->ext_wake, LPM_BT_WAKE_DEASSERT);
		bluesleep_tx_idle();
	} else {
 ...
 */
#ifdef _DRV_LLT_
int bluesleep_start(struct bluesleep_info *bs_info)
#else
...
#endif
{
	...
	/* assert BT_WAKE */
	gpio_set_value(bs_info->ext_wake, LPM_BT_WAKE_ASSERT);
 ...
 */
#ifdef _DRV_LLT_
void	bluesleep_stop(struct bluesleep_info *bs_info)
#else
					...
					int	count, int *eof, void *data)
{
	...
	*eof = 1;
	return sprintf(page, "btwake:%u\n",
				LPM_BT_WAKE_ASSERT == gpio_get_value(bs_info->ext_wake));
}
			...
			off_t	offset,	int	count, int *eof, void *data)
{
	...
	*eof = 1;
	return sprintf(page, "hostwake:	%u\n",
				LPM_BT_WAKE_ASSERT == gpio_get_value(bs_info->host_wake));
}
...
static int	bluesleep_io_init(struct bluesleep_info *bs_info)
{
	...
	if (NULL == bs_info->blockName) {
		BT_ERR("%s:	block name is null\n", __func__);
		ret = -ENXIO;
		goto err_iomux_ops;
    }

	bs_info->blockTemp	= iomux_get_block(bs_info->blockName);
	...
	bs_info->configTemp = iomux_get_blockconfig(bs_info->blockName);
...
#ifdef _DRV_LLT_
int create_bluesleep_proc_sys(struct bluesleep_info *bs_info)
#else
...
#ifdef _DRV_LLT_
int  bluesleep_probe(struct platform_device *pdev)
#else
static int __devinit bluesleep_probe(struct platform_device *pdev)
#endif
{
	int	ret = 0;
    struct bt_dev_info *bt_info = (struct bt_dev_info *)pdev->dev.platform_data;
    ...
    if (!bt_info) {
		BT_ERR("%s: bt device information is null\n", __func__);
		ret = -ENXIO;
		goto free_bsi;
    }

    bsi->ext_wake = bt_info->host_wake_bt;
    bsi->host_wake = bt_info->bt_wake_host;
#ifndef CONFIG_MACH_HI6620OEM	
    bsi->blockName = bt_info->iomux_pm;
	...
	/* deassert bt wake */
	ret	= gpio_direction_output(bsi->ext_wake, LPM_BT_WAKE_DEASSERT);
	if (ret) {
