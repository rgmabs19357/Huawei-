...
#define MAX_WIFI_NVRAM_NAME_LEN 25
...
bool get_wifi_nvram_name(char *nvram_name)
{
    if(nvram_name == NULL)
        return false;
        
    if(get_hw_config_string("wifi/nvram", nvram_name, MAX_WIFI_NVRAM_NAME_LEN, NULL)){
        return true;
    }
    else{
        return false;
...
EXPORT_SYMBOL(get_wifi_nvram_name);
