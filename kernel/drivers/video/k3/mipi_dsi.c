...
void debug_lcd_parse_cmd_str(char ** str)
{
    ...
    while (NULL != (cmd_seq = strsep(str, " \\"))) {
        ...
        if (0 == cmd_seq_num) {         
            ...
            if (strnicmp(cmd_seq, "-1", 2) == 0) {
                ...
                strict_strtol(cmd_seq, 0, &g_mipi_lcd_debug_info.lcd_reg_msb);
            }
        } else if (cmd_seq_num == 2) {
             strict_strtol(cmd_seq, 0, &g_mipi_lcd_debug_info.lcd_reg_lsb);
        } else {
            if (g_mipi_lcd_debug_info.ops_type == LCD_DEBUG_OPS_TYPE_READ) {
                strict_strtol(cmd_seq, 0, &g_mipi_lcd_debug_info.para_num);  /* the reg para num needed to read  */
                ...
                if (0 == strict_strtol(cmd_seq, 0, &g_mipi_lcd_debug_info.para_array[g_mipi_lcd_debug_info.para_num])) {
                    ...
                    k3fb_logi("g_mipi_lcd_debug_info.para_array[%d] = %d \n ",g_mipi_lcd_debug_info.para_num - 1,
    ...
    if (DTYPE_GEN_WRITE == g_mipi_lcd_debug_info.cmd_type) {
        if (1 == g_mipi_lcd_debug_info.para_num) {
            ...
            k3fb_loge("generic write must have para! para_num = %d \n", g_mipi_lcd_debug_info.para_num);
        }
    }
    ...
    k3fb_logi("g_mipi_lcd_debug_info.cmd_type = 0x%x \n"
              " g_mipi_lcd_debug_info.lcd_reg_msb  = 0x%x \n "
              " g_mipi_lcd_debug_info.lcd_reg_lsb  = 0x%x \n "
              " g_mipi_lcd_debug_info.ops_type = %d (0:write,1:read) \n"
              " g_mipi_lcd_debug_info.para_num = %d \n", g_mipi_lcd_debug_info.cmd_type,
...
void debug_lcd_write_lcd_reg()
{
    ...
    mipi_dsi_cmds_tx(&cmd_set, 1, k3fd_reg_base_edc0);
...
void debug_lcd_read_lcd_reg()
{
    ...
    k3fb_logi("g_mipi_lcd_debug_info.para_num = %d \n", g_mipi_lcd_debug_info.para_num);

    packet_size_cmd_set.dtype = DTYPE_MAX_PKTSIZE;
    packet_size_cmd_set.vc    = 0;

    set_dsi_mode(k3fd_reg_base_edc0, CMD_MODE);

    mdelay(100);

    /* TODO: to define the dlen */
    packet_size_cmd_set.dlen = g_mipi_lcd_debug_info.para_num;

    mipi_dsi_max_return_packet_size(&packet_size_cmd_set, k3fd_reg_base_edc0);
    ...
    /* send cmd */
    set_MIPIDSI_GEN_HDR(k3fd_reg_base_edc0, cmd_reg);

    for (i = 0; i < max_times; i++) {
        mdelay(20);

        g_mipi_lcd_debug_info.para_array[i] = inp32(k3fd_reg_base_edc0 + MIPIDSI_GEN_PLD_DATA_OFFSET);        

        k3fb_logi("cmd_reg = 0x%x, lcd_reg_val = 0x%x \n", cmd_reg, g_mipi_lcd_debug_info.para_array[i]);
    }    

    set_dsi_mode(k3fd_reg_base_edc0, HSPD_MODE);

    mdelay(20);

    return;
}
void debug_lcd_send_cmd_set()
{     
...
void debug_lcd_build_cmd_to_str(char * buf)
{
    ...
    if (-1 == g_mipi_lcd_debug_info.lcd_reg_msb) {
        ...
        sprintf(buf + 3, "0x%02x ", g_mipi_lcd_debug_info.lcd_reg_msb);
        buf_para_start = 8;
    }

    /* LSB of LCD_REG */
    sprintf(buf + buf_para_start, "0x%02x ", g_mipi_lcd_debug_info.lcd_reg_lsb);    
    ...
    if (LCD_DEBUG_OPS_TYPE_WRITE == g_mipi_lcd_debug_info.ops_type) {        
        ...
        if (g_mipi_lcd_debug_info.para_num > 0) {
            /* Para_num */
            sprintf(buf + buf_para_start, "0x%02x ", g_mipi_lcd_debug_info.para_num);
        }
        ...
        for (i = 0; i < g_mipi_lcd_debug_info.para_num; i++) {
            ...
            sprintf(buf + buf_para_start, "0x%02x ", g_mipi_lcd_debug_info.para_array[i]);
        }
    } else {      
        ...
        /* Para_num */
        sprintf(buf + buf_para_start, "0x%02x\n", g_mipi_lcd_debug_info.para_num);

        buf_para_start += 5;
        
        for (i = 0; i < read_times; i++) {               
            if ((buf_para_start + 11) > DEBUG_LCD_CMD_STR_MAX_BYTE) {
                k3fb_loge(" buf_para_start + 11 is overflow, buf_para_start + 11:", buf_para_start + 11);
                return;
            }

            sprintf(buf + buf_para_start, "0x%08x\n", g_mipi_lcd_debug_info.para_array[i]);
...
int __init mipi_lcd_debug_init(void)
{
    ...
    debugfs_create_file(LCD_DEBUG_FILE_NAME, 0666, lcd_debugfs_dir, NULL, &debug_lcd_cmd_fops);
