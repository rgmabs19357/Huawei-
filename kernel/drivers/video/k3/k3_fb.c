...
static void k3fb_te_inc(struct k3_fb_data_type *k3fd, bool te_should_enable);
static void k3fb_te_dec(struct k3_fb_data_type *k3fd, bool te_should_disable);
...
static int k3_fb_register(struct k3_fb_data_type *k3fd)
{
	...
	if (k3fd->index == 0) {
		...
		mutex_init(&k3fd->vsync_info.irq_lock);
		k3fd->use_fake_vsync = false;
