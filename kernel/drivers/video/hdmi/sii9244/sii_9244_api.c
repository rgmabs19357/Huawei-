...
void usb_switch_contorl(int switch_1_state, int switch_2_state)
{
	...
	gpio_direction_output(mhl_pdata->gpio_switch_1, switch_1_state);
	gpio_direction_output(mhl_pdata->gpio_switch_2, switch_2_state);

	gpio_free(mhl_pdata->gpio_switch_1);
	gpio_free(mhl_pdata->gpio_switch_2);
...
void set_dcdc_mode(int value)
{
	...
	mhl_pdata = mhl_Sii9244_page0->dev.platform_data;
	if (!mhl_pdata) {
		TX_API_PRINT((KERN_ERR "Mhl:API:%s:%d:mhl_pdata is NULL!\n", __func__, __LINE__));
		return;
	}
	if (gpio_request(mhl_pdata->gpio_dcdc, "DCDC_MODE") < 0) {
		TX_API_PRINT((KERN_ERR "Mhl:API:%s:%d: Mhl DCDC GPIO request failed\n", __func__, __LINE__));
		return;
	}
	if (gpio_direction_output(mhl_pdata->gpio_dcdc, value) < 0) {
		TX_API_PRINT((KERN_ERR "Mhl:API:%s:%d: Mhl DCDC GPIO direction failed\n", __func__, __LINE__));
	}
