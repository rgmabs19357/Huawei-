...
#define CHARGE_TEMP_PROTECT_ENABLE   1
#define CHARGE_TEMP_PROTECT_DISABLE  0
static int      g_ChargeTempProtectFlag = CHARGE_TEMP_PROTECT_ENABLE;
...
struct k3_bq24161_device_info *k3_bq24161_di;
enum{
...
k3_bq24161_charge_status(struct k3_bq24161_device_info *di)
{
	...
	{
        dev_err(di->dev, " POWER_SUPPLY_OVERVOLTAGE = 0x%x\n",read_reg[1]);
	...
	if((read_reg[1] & 0x06) == BQ24161_FAULT_BAT_OVP)
	{
        dev_err(di->dev, " Battery OVERVOLOTAGE = 0x%x\n",read_reg[1]);
    ...
    if ((read_reg[0] & BQ24161_VUSB_FAULT) == BQ24161_CHARGING_FROM_USB) {
        ...
        if ((COUL_BQ27510 == hisi_coulometer_type())
            &&(!is_hisi_battery_full())) {
            /*enable bq2416x charger high impedance mode*/
            di->hz_mode = 1;
            k3_bq24161_write_byte(di, di->control_reg | di->hz_mode, REG_CONTROL_REGISTER);
            /*disable bq2416x charger high impedance mode*/
...
void setChargeTempProtectFlag(int flag)
{
    g_ChargeTempProtectFlag = flag;
    return;
}
...
static int k3_bq24161_check_battery_temperature_threshold(void)
{
    ...
    if(CHARGE_TEMP_PROTECT_DISABLE == g_ChargeTempProtectFlag)
    {       
...
static void k3_bq24161_stop_charger(struct k3_bq24161_device_info *di)
{
	...
	/*ocking_notifier_call_chain(&notifier_list_bat, events, NULL);*/
    hisi_coul_charger_event_rcv(events);
...
k3_bq24161_charger_update_status(struct k3_bq24161_device_info *di)
{
	...
	if (read_reg[0] & 0x7) {
		di->cfg_params = 1;
		dev_err(di->dev, "CHARGER STATUS 0x%x\n", read_reg[0]);
	}
...
void k3_bq24161_sche_usbcrg_work(enum usb_charger_type event)
{
	struct k3_bq24161_device_info *di = k3_bq24161_di;
