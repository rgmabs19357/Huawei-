...
static int calc_capacity_from_voltage(void)
{
    ...
    /*battery isn't exist, so use extern power supply*/
	battery_voltage = hisi_power_supply_voltage();
...
static int k3_battery_monitor_capacity_changed(struct k3_battery_monitor_device_info *di)
{
	...
	if (!di->bat_exist) {
		...
		curr_temperature = hisi_battery_temperature();
#if 0
				...
				void *_data)
{
    ...
    case BATTERY_LOW_WARNING:
        ...
        dev_warn(di->dev, "%s BATTERY_LOW_WARNING\n", __func__);
		break;
	...
	if (!di->bat_exist)
	    ...
	    di->capacity = hisi_battery_capacity();
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
        val->intval = hisi_power_supply_voltage();
		break;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TIME_TO_EMPTY_AVG:
		val->intval = hisi_battery_tte();
		break;
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
	case POWER_SUPPLY_PROP_TIME_TO_FULL_AVG:
		val->intval = hisi_battery_ttf();
		break;
...
static int __devinit k3_battery_monitor_probe(struct platform_device *pdev)
{
    ...
    /*check whether coul module is ready*/
    if (!(is_hisi_coul_ready())) {
		dev_err(&pdev->dev, "%s failed, check coul module installed...\n", __func__);
		ret = -EINVAL;
		goto err_pdata;
    }
    ...
    if (is_hisi_battery_reach_threshold()) {
        dev_warn(&pdev->dev, "%s soc of battery reach threshold\n", __func__);
        wake_lock(&low_power_lock);
    }
