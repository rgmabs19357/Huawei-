...
struct i2c_client *client_atmel_test;
...
int enable_ldo_power_for_device(struct device *dev)
{
	...
	uint32_t gpio, gpio1;
	gpio = GPIO_5_5;
	ret = gpio_request(gpio, "tp_power_1.8V");
	if (ret) {
        printk(KERN_ERR "error request gpio_5_5 fail\n");
		return ret;
	}
	ret = gpio_direction_output(gpio, 0);

    if(ret){
        printk(KERN_ERR "error request gpio_5_5 set fail\n");
        gpio_free(gpio);
        return ret;
    }

	gpio1 = GPIO_5_4;
	ret = gpio_request(gpio1, "tp_power_2.85V");
	if (ret) {
        printk(KERN_ERR "error request gpio_5_4 fail\n");
		gpio_free(gpio);
		return ret;
	}
    ...
    if(ret){
        printk(KERN_ERR "error request gpio_5_4 set fail\n");
        gpio_free(gpio);
        gpio_free(gpio1);
        return ret;
    }
...
#if 0
int check_delta(struct atmel_ts_data *ts)
{
...
#endif
	/*
	Atmel touchscreen hardware reqires that the reset pin must be 0 during power-up. The digital and analog power
	supplies can be applied in any order.
	*/
	ret = gpio_request(ts->pdata->gpio_reset, "gpio_tp_reset");
	if (ret) {
		dev_err(&client->dev, "k3ts, %s: gpio_request failed for gpio_rest %d, ret = %d\n",
			__func__, ts->pdata->gpio_reset, ret);
		goto err_request_gpio_reset_failed;
	}
	gpio_direction_output(ts->pdata->gpio_reset, 0);

	ret = enable_ldo_power_for_device(&client->dev);
	if (ret) {
		dev_err(&client->dev, "k3ts, %s: enable_ldo_power_for_device failed for  ret = %d\n",
			__func__, ret);
		goto err_request_gpio_reset_failed;
	}
	if (ts->power)
...
int atmel_ts_suspend_test(void)
{
    int ret;
    if (NULL == client_atmel_test){
        return -1;
    }
    ret = atmel_ts_suspend(client_atmel_test, PMSG_SUSPEND);
    return ret;
}

int atmel_ts_resume_test(void)
{
    int ret;
    if (NULL == client_atmel_test){
        return -1;
    }
    ret = atmel_ts_resume(client_atmel_test);
    return ret;
}
