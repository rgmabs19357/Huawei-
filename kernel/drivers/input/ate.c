...
#include "../huawei/drivers/touchscreen/ts/huawei_touchscreen.h"
...
static int inputevent_data_read(void)
{
    ...
    set_fs(KERNEL_DS);
    pos = 0;
    ret = vfs_read(fp, buf, sizeof(buf), &pos);
    if (ret < 0)
        goto exit;
    ...
    ret = vfs_read(fp, buf_name, sizeof(buf_name), &pos);
    if (ret < 0)
        goto exit;
    if('\0' != buf_name[0]) {
...
#endif
#ifdef HISI_K3_PLATFORM
    strcpy(ate_dt->idf.dev_name[0], TS_DEV_NAME);
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
