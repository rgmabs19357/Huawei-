...
static unsigned int g_keyback = 1;
...
struct k3v2_gpio_key {
	...
	int    			volume_down_irq;    /*volumn down key irq.*/
 	int    			key_back_irq;    /*volumn down key irq.*/
};
...
static void k3v2_gpio_keyup_work(struct work_struct *work)
{
	...
	keyup_value = gpio_get_value(GPIO_1_4);
...
static void k3v2_gpio_keydown_work(struct work_struct *work)
{
	...
	keydown_value = gpio_get_value(GPIO_1_5);
...
static void k3v2_gpio_keyback_work(struct work_struct *work)
{
	struct k3v2_gpio_key *gpio_key = container_of(work,
		struct k3v2_gpio_key, gpio_keyback_work.work);

	unsigned int keyback_value = 0;
	unsigned int report_action = GPIO_KEY_RELEASE;

	keyback_value = gpio_get_value(GPIO_1_6);
	if (g_keydown == keyback_value)
		return;
	else
		g_keydown = keyback_value;
	...
	printk(KERN_DEBUG "[gpiokey]back key %u action %u\n", KEY_BACK, report_action);
...
static int __devinit k3v2_gpio_key_probe(struct platform_device* pdev)
{
	...
	INIT_DELAYED_WORK(&(gpio_key->gpio_keyback_work), k3v2_gpio_keyback_work);
	...
	/*get volume-down-key irq.*/
	gpio_key->key_back_irq = platform_get_irq(pdev, 2);
	...
	/*
	 * support failing irq that means back-key is pressed,
	 * and rising irq which means back-key is released.
	 */
	err = request_irq(gpio_key->key_back_irq, k3v2_gpio_key_irq_handler, IRQF_NO_SUSPEND | IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, pdev->name, gpio_key);
	if (err) {
		dev_err(&pdev->dev, "Failed to request release interupt handler!\n");
		goto err_request_irq;
	}
