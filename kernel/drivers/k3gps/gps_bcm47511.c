...
static int __devinit k3_gps_bcm_probe(struct platform_device *pdev)
{
	...
	/* Get reset gpio */
	res = platform_get_resource_byname(pdev, IORESOURCE_IO, GPIO_GPS_BCM_RESET_NAME);
	if (!res) {
	...
	gpio_direction_output(gps_bcm->gpioid_ret, 1);
	dev_dbg(&pdev->dev,  "High reset\n");

	/* Get ref_clk gpio, if failed, we still go on  */
	res = platform_get_resource_byname(pdev, IORESOURCE_IO, GPIO_GPS_BCM_REFCLK_NAME);
	if (!res) {
	...
	/* Gps power gpio (VDDIO 1.8V), Only for FPGA, if failed, we still go on */
	res = platform_get_resource_byname(pdev, IORESOURCE_IO, GPIO_GPS_BCM_VDDIO_ENABLE_NAME);
	if (!res) {
		...
		gpio_direction_output(gps_bcm->gpioid_power, 1);
		dev_dbg(&pdev->dev,  "High power\n");
	}

	/* Set 32KC clock */
	gps_bcm->clk = clk_get(NULL, CLK_REF32K);
