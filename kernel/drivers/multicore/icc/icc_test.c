...
BSP_S32 icc_write_task_tmp(void *data)
{
    ...
    while (g_asendlen[id])
    {
        ...
        {
            if (0 != down_timeout(&g_semIccSendTask[id], msecs_to_jiffies(2000)))
            {
                /*ICC_TEST_DEBUG("%s  %d  FAILED\n",__FUNCTION__, __LINE__);*/
            }
...
BSP_S32 BSP_ICC_ST_RW_STREAM_005(void)
{
    ...
    g_u32IccSendTaskId[1] = kthread_run(icc_write_task, (void *)1, taskName);
...
上来不及*/
BSP_S32 BSP_ICC_ST_RW_STREAM_006(void)
{
    char taskName[30] = {0};
    DEV_INIT();
    g_arecvtotal[1] = 0;
    g_asendtotal[1] = 0;
    g_arecvtotalb[1] = ICC_TEST_LARGE_DATA_LEN_TMP;
    sema_init(&g_semIccSendTask[1], SEM_EMPTY);
    if(BSP_OK != open_channel_large(1))
    {
        /*ICC_TEST_DEBUG("%s  %d  FAILED\n", __FUNCTION__, __LINE__);*/
    ...
    /*ICC_TEST_DEBUG(taskName,"iccTest%02d",1);*/
    g_u32IccSendTaskId[1] = kthread_run(icc_write_task_tmp, (void *)1, taskName);
    if (IS_ERR(g_u32IccSendTaskId[1]))
    {
        kthread_stop(g_u32IccSendTaskId[1]);
        return ERROR;
...
BSP_S32 BSP_ICC_ST_MNTN_002(void)
{
    ...
    DEV_INIT();
    /* 利用打开无效通道，产生通道打开失败 */
    CHECK_RET(BSP_ICC_Open(32, &attr), BSP_ERR_ICC_INVALID_PARAM);
