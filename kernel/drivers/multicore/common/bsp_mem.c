...
spinlock_t g_ipc_sem_mem_lock = SPIN_LOCK_UNLOCKED;
/**************************************************************************
  宏实现
**************************************************************************/
...
#define MEM_SPIN_INIT() memset((VOID*)MEM_CORESHARE_MEM_ADDR, 0, MEMORY_RAM_CORESHARE_SYNC_RESV+MEMORY_RAM_CORESHARE_MEM_RESV)
        ...
        case MEM_ICC_DDR_POOL:
            {
                ...
                pAllocInfo->memPoolInfo.u32BaseAddr = (BSP_U32)DDR_VIRT_TO_PHY(MEM_ICC_DDR_POOL_BASE_ADDR);
...
LOCAL INLINE VOID* memPoolAlloc(MEM_ALLOC_INFO* pAllocInfo, BSP_U32 u32Size)
{
    ...
    u32RetAddr = DDR_PHY_TO_VIRT(u32RetAddr);
...
LOCAL INLINE VOID* memGetItem(MEM_ALLOC_INFO* pAllocInfo, BSP_U32 cnt, BSP_U32 u32PoolType, BSP_U32 u32Size)
{
    ...
    {
	    pItem = (VOID*)DDR_PHY_TO_VIRT(((unsigned int)*ppHead));
...
INLINE VOID memFree(BSP_U32 u32PoolType, BSP_VOID* pMem, BSP_U32 u32Size)
{
    ...
    pAllocInfo->allocList[cnt] = (BSP_VOID *)DDR_VIRT_TO_PHY((BSP_U32)pMem);
...
int memUsrInitFunction(void)
{
 ...
 #if 0
    memset((VOID*)MEM_ICC_CTRL_CHAN_ADDR, 0, MEMORY_AXI_ICC_SIZE);
    memset((VOID*)MEM_AXI_DICC_ADDR, 0, MEMORY_AXI_DICC_SIZE);
    memset((VOID*)ECS_NV_BASE_ADDR_VIRT, 0, 16);
    memset((VOID*)MEM_AXI_USB_CSHELL_ADDR, 0, MEMORY_AXI_USB_CSHELL_SIZE);
    /*monan*/
    /*V3R2 CS f00164371，这段内存是因为屏蔽了低功耗后新添加的，在后面低功耗合入后，
      需要删除该段代码，与zhangshichu、liuyongfu确认*/
    memset((void*)MEM_AXI_TEMP_PROTECT_ADDR, (int)0x00, (unsigned int)MEMORY_AXI_TEMP_PROTECT_SIZE);
#else
    memset((VOID*)ECS_NV_BASE_ADDR_VIRT, 0, 16);
#endif
    return 0;
...
#ifdef CHIP_BB_HI6620
    (BSP_VOID)BSP_SYNC_UnLock(SYNC_MODULE_MEM, 1);
#endif
...
BSP_S32 BSP_MEM_DumpAllocList(BSP_U32 u32Size, BSP_U32 u32PoolType)
{
    ...
    pCurAddr = (VOID*)DDR_PHY_TO_VIRT(pAllocInfo->allocList[u32Item]);/*lint124,modify by z00212992,2012.4.17*/
