...
extern int pmu_set_alarm_time(unsigned long time);
...
int alarm_set_rtc_alarm(long time_sec,bool enable_irq)
{
	...
	if (alarm_value < (ALARM_AHEAD_TIME + rtc_current_time)) {
                printk(KERN_DEBUG "%s: power off alarm %lu is close, skip it\n", __FUNCTION__, alarm_value);
                return -1;
	}
	...
	printk(KERN_DEBUG "%s: [%d-%d-%d] [%d:%d:%d] %lu\n", __FUNCTION__,
		...
		alarm_value);
	pmu_set_alarm_time(alarm_value);
