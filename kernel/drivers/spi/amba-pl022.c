...
#define SSP_TXFIFOCR_MASK_INT		(0x07UL << 3)
/*
 * SSP RX FIFO Register - SSP_TXFIFOCR
 */
#define SSP_RXFIFOCR_MASK_DMA		(0x07UL << 0)
#define SSP_RXFIFOCR_MASK_INT		(0x07UL << 3)
/*#endif*/
...
struct pl022 {
	...
	/* DMA settings */
#if defined (CONFIG_DMA_ENGINE) || defined(CONFIG_MACH_HI6620OEM)
...
static void giveback(struct pl022 *pl022)
{
	...
	struct pl022_ssp_controller *plat;
	...
	if (plat && plat->exit)
		plat->exit();
	*/
	if (plat && plat->iomux_name) {
		if (blockmux_set(pl022->iomux_block, pl022->block_config, LOWPOWER))
			dev_err(&(pl022->adev->dev), "set iomux to LOWPOWER failed\n");
	}
...
/*#if defined(CONFIG_ARCH_K3V2) || defined(CONFIG_ARCH_HI6620)*/
	dev_dbg(&pl022->adev->dev,
		"%s entry: cs_change: %d, len: %d, msg->actual_length: %d\n",
		__func__, pl022->cur_transfer->cs_change,
		pl022->cur_transfer->len, msg->actual_length);
/*#endif*/
...
static int configure_dma(struct pl022 *pl022)
{
	...
	dev_dbg(&pl022->adev->dev, "%s: pl022->read: %d, pl022->write: %d\n",
		__func__, pl022->read, pl022->write);
	/*
	...
	case SSP_TX_32_OR_MORE_EMPTY_LOC:
		tx_conf.dst_maxburst = 16;
		break;
	case SSP_TX_64_OR_MORE_EMPTY_LOC:
		tx_conf.dst_maxburst = 16;
		break;
	default:
	...
	ret = sg_alloc_table(&pl022->sgt_rx, rx_pages, GFP_NOWAIT);
	if (ret)
		goto err_alloc_rx_sg;

	ret = sg_alloc_table(&pl022->sgt_tx, tx_pages, GFP_NOWAIT);
	if (ret)
	...
	dev_dbg(&pl022->adev->dev, "rx_sglen=%u, tx_sglen=%u\n",
								rx_sglen, tx_sglen);
...
err_rxdesc:
	...
	dma_unmap_sg(txchan->device->dev, pl022->sgt_tx.sgl,
		     pl022->sgt_tx.nents, DMA_TO_DEVICE);
err_tx_sgmap:
	dev_err(&pl022->adev->dev, "err_tx_sgmap\n");
	dma_unmap_sg(rxchan->device->dev, pl022->sgt_rx.sgl,
		     pl022->sgt_tx.nents, DMA_FROM_DEVICE);
err_rx_sgmap:
	dev_err(&pl022->adev->dev, "err_rx_sgmap\n");
	sg_free_table(&pl022->sgt_tx);
err_alloc_tx_sg:
	dev_err(&pl022->adev->dev, "tx sg_alloc_table error\n");
	sg_free_table(&pl022->sgt_rx);
err_alloc_rx_sg:
	dev_err(&pl022->adev->dev, "rx sg_alloc_table error\n");
	return -ENOMEM;
}
...
/*#if defined(CONFIG_ARCH_K3V2) || defined(CONFIG_ARCH_HI6620)*/
	dev_dbg(&pl022->adev->dev,
		"pl022_interrupt_handler: irq_status= 0x%x.\n", irq_status);
/*#endif*/
...
/*#if defined(CONFIG_ARCH_K3V2) || defined(CONFIG_ARCH_HI6620)*/
	...
	*/
    if (plat && plat->iomux_name) {
		if (blockmux_set(pl022->iomux_block, pl022->block_config, NORMAL))
			dev_err(&(pl022->adev->dev), "set iomux to NORMAL failed");
    }
/*#endif*/
	restore_state(pl022);
	flush(pl022);
	...
	if ((freq <= max_tclk) && (freq >= min_tclk)) {
		...
		if (cpsdvsr != 0) {
			dev_info(&pl022->adev->dev,
				"SSP Effective Frequency is %u, SSPCLK=%u\n",
				(rate / (cpsdvsr * (1 + scr))), rate);
...
static int pl022_setup(struct spi_device *spi)
{
	...
	chip->cpsr = 0;
/*#if defined(CONFIG_ARCH_K3V2) || defined(CONFIG_ARCH_HI6620)*/
	dev_dbg(&spi->dev, "com_mode: %d, enable_dma: %d\n",
		chip_info->com_mode, (pl022->master_info)->enable_dma);
/*#endif*/
...
static ssize_t pl022_debug_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	int ret = 0, clk = 0, pclk = 0, rst = 0;
	char *offset = NULL;
	...
	if (pl022 == NULL) {
		dev_err(dev, "%s no driver data\n", __func__);
		return 0;
	}

	offset = buf;
	ret = snprintf(offset, PAGE_SIZE, "[amba pl022 debug reg]\n");

	offset += ret;
	ret = snprintf(offset, PAGE_SIZE, "CR0=0x%x, CR1=0x%x, SR=0x%x, CPSR=0x%x\n",
										readw(SSP_CR0(pl022->virtbase)),
										readb(SSP_CR1(pl022->virtbase)),
										readb(SSP_SR(pl022->virtbase)),
										readb(SSP_CPSR(pl022->virtbase)));

	offset += ret;
	ret = snprintf(offset, PAGE_SIZE, "IMSC=0x%x, RIS=0x%x, MIS=0x%x\n",
										readb(SSP_IMSC(pl022->virtbase)),
										readb(SSP_RIS(pl022->virtbase)),
										readb(SSP_MIS(pl022->virtbase)));

	offset += ret;
	ret = snprintf(offset, PAGE_SIZE, "DMACR=0x%x, TXFIFOCR=0x%x, RXFIFOCR=0x%x\n",
										readb(SSP_DMACR(pl022->virtbase)),
										readb(SSP_TXFIFOCR(pl022->virtbase)),
										readb(SSP_RXFIFOCR(pl022->virtbase)));

	clk = phy_reg_readl(REG_BASE_SC_OFF, SOC_SCTRL_SC_PERIPH_CLKSTAT1_ADDR(0),
																	15, 15);
	pclk= phy_reg_readl(REG_BASE_SC_OFF, SOC_SCTRL_SC_PERIPH_CLKSTAT1_ADDR(0),
																	14, 14);
	rst = phy_reg_readl(REG_BASE_SC_OFF, SOC_SCTRL_SC_PERIPH_RSTSTAT1_ADDR(0),
																	7, 7);
	offset += ret;
	ret = snprintf(offset, PAGE_SIZE, "CLK=%d, PCLK=%d, RST=%d\n",
												clk, pclk, rst);
	offset += ret;
	ret = snprintf(offset, PAGE_SIZE, "rx: %p, rxend: %p, tx: %p, txend: %p\n",
							pl022->rx, pl022->rx_end, pl022->tx, pl022->tx_end);
	offset += ret;

	/* drop reset signal control */
	plat = pl022->master_info;
	if (plat && plat->init)
		plat->init();
...
static DEVICE_ATTR(pl022_debug, S_IRUGO, pl022_debug_show, NULL);
...
pl022_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	get_hw_config_int("camera/miniisp", &use_ssp_driver, NULL);
	if (!use_ssp_driver) {
		status = -ENODEV;
		dev_warn(&adev->dev, "not configed to use ssp drvier\n");
		goto err_no_dev;
	}
	...
	/* config plat iomux setting */
	if (platform_info->iomux_name) {
		pl022->iomux_block = iomux_get_block(platform_info->iomux_name);
		if (!pl022->iomux_block) {
			dev_err(&adev->dev, "get iomux block failed\n");
			status = -EIO;
			goto err_no_iomux;
		}

		pl022->block_config = iomux_get_blockconfig(platform_info->iomux_name);
		if (!pl022->block_config) {
			dev_err(&adev->dev, "get iomux block failed\n");
			status = -EIO;
			goto err_no_iomux;
		}

		status = blockmux_set(pl022->iomux_block, pl022->block_config, NORMAL);
		if (status){
			dev_err(&adev->dev, "set iomux to NORMAL failed\n");
			status = -EIO;
			goto err_no_iomux;
		}

		dev_info(&adev->dev, "init ssp iomux success\n");
	}

	/* config plat clk&rst control */
	if (platform_info->init) {
		platform_info->init();
		dev_info(&adev->dev, "init ssp clock source\n");
	}
	...
	dev_info(&adev->dev, "CR0=0x%08x CR1=0x%08x SR=0x%08x CPSR=0x%08x\n",\
							readw(SSP_CR0(pl022->virtbase)),
							readb(SSP_CR1(pl022->virtbase)),
							readb(SSP_SR(pl022->virtbase)),
							readb(SSP_CPSR(pl022->virtbase)));
	...
	device_create_file(dev, &dev_attr_pl022_debug);
	...
	*/
	if (platform_info->iomux_name) {
		if (blockmux_set(pl022->iomux_block, pl022->block_config, LOWPOWER))
			dev_err(&adev->dev, "set iomux to LOWPOWER failed\n");
	}
 ...
 err_no_irq:
 err_no_iomux:
	pl022->iomux_block = NULL;
	pl022->block_config = NULL;
	clk_put(pl022->clk);
 err_no_clk:
...
static int pl022_suspend(struct amba_device *adev, pm_message_t state)
{
	...
	dev_info(&adev->dev, "pl022_suspend+\n");
...
static int pl022_resume(struct amba_device *adev)
{
	...
	dev_info(&adev->dev, "pl022_resume+\n");
