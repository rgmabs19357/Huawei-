...
#endif
#ifdef UMP_MW3_PCLINT
#define UMP_INLINE 
#else 
#define UMP_INLINE inline
#endif
...
struct ump_mw3
{
    unsigned char     reset_rtt_min:1;
    unsigned char     first_ack:1;
    unsigned char     phase:1;
    unsigned char     zero:1;
    unsigned char     reset_rtt_max:1; 
    unsigned char     rtt_phase:1;
    unsigned char     filter_phase:1;
    unsigned char     est_phase:1;
    unsigned char     ssthresh_phase:1;
    unsigned char     reset_rcved:1;
    unsigned char     mss_phase:1;
    unsigned char     reset_mw3:1;
    unsigned char     rcved_una:1;
    unsigned char     is_reno:1;
    unsigned char     reset_bw_est:1;
    unsigned char     ssthresh_choose:1;
...
static UMP_INLINE void update_rtt_max(struct mw3 *m)
{
    if (!m->rtt_phase) 
    {
...
static UMP_INLINE void update_rtt_min(struct mw3 *m)
{
    if (m->reset_rtt_min) 
    {
        m->rtt_min = m->rtt + g_rtt_para;
...
static void tcp_mw3_pkts_acked(struct sock *sk, u32 cnt, s32 rtt)
{
    ...
    if(m->reset_rtt_max)
    {
        m->rtt = (u32)usecs_to_jiffies(tp->srtt);
        update_rtt_max(m);
    }
    else if(rtt > 0) 
    {
        m->rtt = (u32)usecs_to_jiffies(rtt);
        update_rtt_min(m);
    }
    return ;
}

static UMP_INLINE u32 mw3_do_filter(struct sock *sk,u32 a, u32 b)
{
    struct mw3 *m = inet_csk_ca(sk);
...
static UMP_INLINE void mw3_fast_bw(struct sock *sk)
{
    struct tcp_sock *tp = tcp_sk(sk);
    struct mw3 *m = inet_csk_ca(sk);
    u32 delta;
...
UMP_INLINE static  void mw3_other_ack(struct sock *sk)
{
    struct tcp_sock *tp = tcp_sk(sk);
    struct mw3 *m = inet_csk_ca(sk);
    u32 delta;
...
static UMP_INLINE u32 update_m_cwnd(struct sock *sk)
{
    struct tcp_sock *tp = tcp_sk(sk);
    struct mw3 *m = inet_csk_ca(sk);
...
static void tcp_cong_avoid_mw3(struct sock *sk, u32 w, u32 in_flight)
{
    struct tcp_sock *tp = tcp_sk(sk);
    struct mw3 *m = inet_csk_ca(sk);
    if(m->loss_rate)
...
static u32 tcp_mw3_ssthresh(struct sock *sk)
{
    struct tcp_sock *tp = tcp_sk(sk);
    struct mw3 *m = inet_csk_ca(sk);
    u32 a = 0;
...
static void tcp_mw3_cwnd_event(struct sock *sk, enum tcp_ca_event event)
{
    switch(event)
    {
...
static void tcp_mw3_info(struct sock *sk, u32 ext,struct sk_buff *skb)
{
    ...
    if (ext & (1 << (INET_DIAG_VEGASINFO - 1))) 
    {
        ...
        (void)nla_put(skb, INET_DIAG_VEGASINFO, (int)sizeof(info), &info);
    }
    return ;
}
...
{
    .flags              = (unsigned long )TCP_CONG_RTT_STAMP,
    .init               = tcp_mw3_init,
...
static int g_revd = 0;
static void ump_kernel_receive(struct sk_buff *skb);
...
static int  ump_kernel_netlink_init(void)
{
    if(!g_nl_sk)
    {
        g_nl_sk = netlink_kernel_create(&init_net,NETLINK_MW3_PROL,0,
                (void (*)(struct sk_buff *skb) )ump_kernel_receive,NULL,THIS_MODULE);
        if (NULL == g_nl_sk)
        {
            (void)printk(KERN_EMERG "netlink_kernel_create failure !\n");
            return -1;
        }
    }
    return 0;
...
static void ump_kernel_receive(struct sk_buff *skb)
{
    ...
    if(((__u32)(nlh->nlmsg_len) < ((__u32)NLMSG_HDRLEN)) || ((__u32)skb->len <(__u32) nlh->nlmsg_len))
    {
        goto out;
    }
    ...
    {     
        u32 len = actual_len - 2 * sizeof(int);
        if( len != LENGTH)
        {
            (void)printk(KERN_EMERG "line:%d ump length is wrong.\n",__LINE__);
            goto out;
        }
        else if(!g_revd)
        {
            ump_handle_data(data->data);
            (void)printk(KERN_EMERG"line:%d ump data set success!\n",__LINE__);
        }
    }
out:
    return;
