...
#define CRASH_REASON_BOOT_NFF "\nCrash reason: boot-nff "
#define CRASH_DEFAULT_TIME_BOOT_NFF "Crash time: Unknown\n"
...
#ifdef CONFIG_DUMP_LOGCAT
static inline unsigned long dump_logcat(char *pdst, logcat_buf_info *plogcat_buf_info, int count, int write_header);
#endif
...
static inline unsigned long dump_logcat(char *pdst, logcat_buf_info *plogcat_buf_info, int count, int write_header)
{
    ...
    {
        pstart = (char *)((unsigned long)pdst + keyword_len);
    }
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (0x0 != plogcat_buf_info[i].log_buf)
        {
    ...
    {
        copy_data((char *)pdst, LOGCAT_KEYWORD, keyword_len);
    }

    return (0 != write_header) ? (log_header_size + keyword_len + data_len) : (keyword_len + data_len);
}
#endif
...
#endif
{
    ...
    {
        data_len = dump_logcat((char *)(dmesg_header_addr + data_len), plogcat_buf_info, LOGCAT_TYPE_MAX, 1);
        pheader->data_length += data_len;
    }
#else
    /* 3. dump dmesg */
		...
		int arch_id)
{
    ...
    do
    {
        ...
        unsigned long temp_buf = (unsigned long)(output_start - KERNEL_TEXT_OFFSET
                + CONFIG_SRECORDER_TEMPBUF_ADDR_FROM_PHYS_OFFSET);

#ifdef CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET
        volatile platform_special_reserved_mem_info_t *pmem_info = (platform_special_reserved_mem_info_t *)
            (output_start - KERNEL_TEXT_OFFSET + CONFIG_SRECORDER_SPECIAL_MEM_ADDR_FROM_PHYS_OFFSET);
#else
        volatile platform_special_reserved_mem_info_t *pmem_info = (platform_special_reserved_mem_info_t *)
        ...
        boot_crc32 = get_crc32((unsigned char*)pmem_info, (unsigned long)(mem_info_len
            - sizeof(pmem_info->crc32) - sizeof(pmem_info->reserved)));
        ...
        /* check nff */
        if (BOOT_FAILED == pmem_info->boot_successfully_this_time)
        {
            int log_header_size = sizeof(srecorder_info_header_t);
            int keyword_len = str_len(CRASH_REASON_KEYWORD);
            unsigned long data_len = 0x0;
            unsigned long dst = 0x0;

            pmem_header = (psrecorder_reserved_mem_header_t)pmem_info->srecorder_log_buf;
            ...
            dst = temp_buf + mem_header_len + pmem_header->data_length + log_header_size + keyword_len;

            /* add boot nff reason and time */
            data_len += strcpy_valid_char((char *)(dst + data_len), (char *)CRASH_REASON_BOOT_NFF, str_len(CRASH_REASON_BOOT_NFF));
            data_len += strcpy_valid_char((char *)(dst + data_len), (char *)CRASH_DEFAULT_TIME_BOOT_NFF, str_len(CRASH_DEFAULT_TIME_BOOT_NFF));

            /* dump dmesg */
            data_len += strcpy_valid_char((char *)(dst + data_len), BOOT_LOG_START_TAG, str_len(BOOT_LOG_START_TAG));
            data_len += strcpy_valid_char((char *)(dst + data_len), (char *)pmem_info->log_buf, pmem_info->log_buf_len);
            data_len += strcpy_valid_char((char *)(dst + data_len), BOOT_LOG_END_TAG, str_len(BOOT_LOG_END_TAG));

            /* dump logcat */
#ifdef CONFIG_DUMP_LOGCAT
            if (0x0 != (pmem_header->dump_ctrl_bits[LOGCAT_BIT9 / 32] & (((unsigned long)1 << (LOGCAT_BIT9 % 32)))))
            {
                data_len += dump_logcat((char *)(dst + data_len), pmem_info->logcat_buf_info, LOGCAT_TYPE_MAX, 0);
            }
#endif
            write_info_header((unsigned long)(temp_buf + mem_header_len + pmem_header->data_length),
                NFF_LOG_BIT10, keyword_len + data_len, CRASH_REASON_KEYWORD);

            pmem_header = (psrecorder_reserved_mem_header_t)temp_buf;
            pmem_header->magic_num = SRECORDER_MAGIC_NUM;
            pmem_header->data_length += (log_header_size + keyword_len + data_len);
