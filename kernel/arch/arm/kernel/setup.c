...
#ifndef CONFIG_DEBUG_IRQ_WARNING
struct stack {
	u32 irq[3];
	u32 abt[3];
	u32 und[3];
...
{
	int i;
	unsigned long ucpufreq;

	struct cpufreq_policy *policy = cpufreq_cpu_get(0);

	if (policy != NULL)
		ucpufreq = policy->max /1000;
	else
		ucpufreq = 1200;

	ucpufreq = (ucpufreq*829)/416;

	seq_printf(m, "Processor\t: %s rev %d (%s)\n",
		   cpu_name, read_cpuid_id() & 15, elf_platform);

#if defined(CONFIG_SMP)
	/* show each present CPU
         * comment off for_each_online_cpu(i) {
         */
