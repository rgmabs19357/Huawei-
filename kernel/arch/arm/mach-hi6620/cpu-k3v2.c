...
unsigned int *g_acpu_chip_max_freq = (unsigned int*)ACPU_CHIP_MAX_FREQ;

#define CPU_FREQ_MIN (208000)
...
static inline void cpumax_handle(void)
{
	...
	while (gcpu_num_limit.max < cpu_online_num) {
		...
		if (ret) {
			printk(KERN_ERR "%s: CPU:%d down failed ret=%d\n",__FUNCTION__, cpu_online_num-1, ret);
		}
...
static inline void cpumin_handle(void)
{
	...
	while (gcpu_num_limit.min > cpu_online_num) {
		...
		if (ret) {
			printk(KERN_ERR "%s: CPU:%d up failed ret=%d\n",__FUNCTION__, cpu_online_num, ret);                      
		}
		cpu_online_num = num_online_cpus();
	}
}
	...
	void *v)
{
	...
	ipps_set_current_policy(&ipps_client, IPPS_OBJ_GPU, &upolicy);   
	return 0;
}
...
static void k3v2_qos_remove_notifier(void)
{
	...
	QOS_REMOVE_NOTIFY(PM_QOS_CPU_INT_LATENCY, cpuilde_int_notifier);
...
#define store_chip_one(file_name, object) \
static ssize_t store_##file_name	\
	(struct cpufreq_policy *policy, const char *buf, size_t count)	\
{	\
						\
...
store_chip_one(chip_max_freq, 0);
...
cpufreq_freq_attr_rw(chip_max_freq);
...
static irqreturn_t Ipc2IntHandler(int irq, void *dev_id)
{
    ...
    printk(KERN_INFO "Ipc2IntHandler, irq=%d.\n", irq);
