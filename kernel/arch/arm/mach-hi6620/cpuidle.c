...
struct pm_qos_request_list pm_cpu_idle_suspend;

static int cpuidle_pm_notify(struct notifier_block *nfb, unsigned long action, void *ignored)
{
	switch (action) {
	case PM_SUSPEND_PREPARE:
		pm_qos_add_request(&pm_cpu_idle_suspend, PM_QOS_CPU_INT_LATENCY, IDLE_QOS);
		disable_hlt();
		pr_info("PM_SUSPEND_PREPARE for CPUIDLE\n");
		return NOTIFY_OK;
	case PM_POST_RESTORE:
	case PM_POST_SUSPEND:
		pm_qos_remove_request(&pm_cpu_idle_suspend);
		enable_hlt();
			...
			struct cpuidle_state *state)
{
    ...
    {
        /*PRINT_PWC_DBG(PWC_SWITCH_CPUIDLE,"before.tv_sec:0x%lx\n",before.tv_sec);*/
    ...
    if(enter_state >= CPU_IDLE_C1)
    {
        ...
        PRINT_PWC_DBG(PWC_SWITCH_CPUIDLE,"enter_lowpm state(%d),system time:%d private time:%d\n",enter_state,expected_us, schedule_time);
        ...
        if(expected_us > state->target_residency)
        {
            expected_us -= state->exit_latency;
            if (BSP_OK != DRV_TIMER_STOP(CPU_IDLE_TIMER)) {
                printk(KERN_ERR "stop the timer failed !\n");
            }
            PRINT_PWC_DBG(PWC_SWITCH_CPUIDLE,"##DRV_TIMER_START CPU_IDLE.\r\n");
            DRV_TIMER_START((unsigned int)CPU_IDLE_TIMER, 0, (int)0, (expected_us / MSEC_PER_SEC), TIMER_ONCE_COUNT, TIMER_UNIT_MS);
        }
        else/*predict_us not eq expect_us ,return to C0*/
        {
            PRINT_PWC_DBG(PWC_SWITCH_CPUIDLE,"###select fail,expected_us:%d,state->target_residency:%d\n",expected_us, state->target_residency);
            enter_state = CPU_IDLE_C0;
        }
    }
    ...
    {
        pwrctrl_deep_sleep(PM_SUSPEND_MEM);
    }
#if 0
...
static struct cpuidle_state cpuidle_set[] = {
	...
	[1] = {
		.enter			= enter_lowpm,
		.exit_latency		= 5000,/*1000000*/
		.target_residency	= 10000,/*1600000*/
		.flags			= CPUIDLE_FLAG_TIME_VALID,
		.name			= "LOW_POWER_C1",
		.desc			= "ARM power down",
		.power_usage    = 2,
	...
	[2] = {
		.enter			= enter_lowpm,
		.exit_latency		= 15000,/*15000*/
		.target_residency	= 30000,/*30000*/
		.flags			= CPUIDLE_FLAG_TIME_VALID,
		.name			= "LOW_POWER_C2",
	...
	[3] = {
		.enter			= enter_lowpm,
		.exit_latency		= 25000,/*25000*/
		.target_residency	= 50000,/*50000*/
		.flags			= CPUIDLE_FLAG_TIME_VALID,
		.name			= "LOW_POWER_C3",
		.desc			= "ARM pd&clk switch to 32Khz",
		.power_usage    = 0,
	},
};
...
int __init cpuidle_init(void)
{
	...
	wdt_base = (unsigned int)ioremap(REG_BASE_WD0, REG_WD0_IOSIZE);
