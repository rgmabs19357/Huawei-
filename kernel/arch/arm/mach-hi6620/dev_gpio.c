...
extern unsigned int bsp_config_read(unsigned int config_id_addr,unsigned int size,unsigned char* buffer);
...
static int __init amba_gpio_dev_init(void)
{
	int i;
	int ret = -1;
    unsigned int config_ret ;
    char ccpu_gpio_int_group_num = -1;

	edb_trace(1);
    config_ret = bsp_config_read(CONFIG_CCPU_GPIO_INT_GROUP,CONFIG_CCPU_GPIO_INT_SIZE,&ccpu_gpio_int_group_num);
    if(0 != config_ret)
    {
        pr_err("%s:bsp_config_read err!\n",__func__);
    }
    printk("ccpu_gpio_int_group_num %d\n",ccpu_gpio_int_group_num);

    if((ccpu_gpio_int_group_num >=0)&&(ccpu_gpio_int_group_num < ARRAY_SIZE(amba_gpio_devs)))
    {
        struct amba_device *d = amba_gpio_devs[ccpu_gpio_int_group_num];
        ((struct pl061_platform_data*)(d->dev.platform_data))->irq_base = (unsigned)-1;
    }
