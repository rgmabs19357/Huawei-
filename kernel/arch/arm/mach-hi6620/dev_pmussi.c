...
static void __inline__ pmussi_rstdis(void)
{
    ...
    *(volatile int *)SOC_AO_SCTRL_SC_MCU_SUBSYS_CTRL3_ADDR(sctrl_addr) = \
    	(0x00ff0019);
#endif
}
...
static void  pmussi_ctrl_init(void)
{
    ...
    if (di->clk) {
        ret = clk_enable(di->clk);
        if (ret) {
            printk("%s:failed to enable clock\n", __func__);
        }
    }
    if (di->regulator) {
        ret = regulator_enable(di->regulator);
        if (ret) {
            printk("%s:failed to enable regulator\n", __func__);
        }
    }
...
static void pmussi_ctrl_exit(void)
{
    ...
    if (di->regulator) {
        ret = regulator_disable(di->regulator);
        if (ret) {
