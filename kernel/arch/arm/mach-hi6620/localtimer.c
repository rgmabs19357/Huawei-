...
static void local_timer_clkenable(int cpu)
{
	...
	/*cpu1*/
	if (cpu == 1) {
		writel(TIMER6_ENABLE_BIT, IO_ADDRESS(SOC_SC_ON_BASE_ADDR) + SC_PERIPH_CLKEN4);
	}

	if (cpu == 2) {
		writel(TIMER7_ENABLE_BIT, IO_ADDRESS(SOC_SC_ON_BASE_ADDR) + SC_PERIPH_CLKEN4);
	}

	if (cpu == 3) {
		writel(TIMER8_ENABLE_BIT, IO_ADDRESS(SOC_SC_ON_BASE_ADDR) + SC_PERIPH_CLKEN4);
	}
