...
{\
    unsigned int ret;\
    int timeout = I2C_RESET_CONTROLLER_TIMEOUT;\
\
    phy_reg_writel(REG_BASE_SC_OFF,\
        SOC_SCTRL_SC_PERIPH_RSTDIS1_ADDR(CALC_REG_OFFSET),\
        SOC_SCTRL_SC_PERIPH_RSTDIS1_periph_rstdis1_ ## reg_name ## _START,\
        SOC_SCTRL_SC_PERIPH_RSTDIS1_periph_rstdis1_ ## reg_name ## _END, 1);\
\
    while (timeout--) { \
        ret = phy_reg_readl(REG_BASE_SC_OFF, \
            SOC_SCTRL_SC_PERIPH_RSTSTAT1_ADDR(CALC_REG_OFFSET),\
            SOC_SCTRL_SC_PERIPH_RSTSTAT1_periph_rststat1_ ## reg_name ## _START,\
            SOC_SCTRL_SC_PERIPH_RSTSTAT1_periph_rststat1_ ## reg_name ## _END);\
        \
        if ((!timeout) && ret){\
...
\
    phy_reg_writel(REG_BASE_SC_OFF,\
        SOC_SCTRL_SC_PERIPH_RSTEN1_ADDR(CALC_REG_OFFSET),\
        SOC_SCTRL_SC_PERIPH_RSTEN1_periph_rsten1_ ## reg_name ## _START,\
        SOC_SCTRL_SC_PERIPH_RSTEN1_periph_rsten1_ ## reg_name ## _END, 1);\
\
    while (timeout--) { \
        ret = phy_reg_readl(REG_BASE_SC_OFF, \
            SOC_SCTRL_SC_PERIPH_RSTSTAT1_ADDR(CALC_REG_OFFSET),\
...
static void I2C ## chip_id ## _reset(void)\
{\
    /*to do*/\
    return;\
}
...
static void I2C ## chip_id ## _reset_controller(void)\
{\
    /*to do*/\
    return;\
}

I2C_INIT_FUNC(0, 9i2c0)
I2C_INIT_FUNC(1, 10i2c1)
I2C_INIT_FUNC(2, 11i2c2)
I2C_INIT_FUNC(3, 12i2c3)

I2C_EXIT_FUNC(0,9i2c0)
I2C_EXIT_FUNC(1,10i2c1)
I2C_EXIT_FUNC(2,11i2c2)
I2C_EXIT_FUNC(3,12i2c3)
