...
{
    int i = 0;
    void __iomem *timerMap;

    for(i=0;i<PWRCTRL_ACPU_TIMER_TOTAL_NUM-1;i++)
    {
        ...
        g_ulPWRCTRLTimerAddr[i] = SOC_TIMER_TIMERN_LOAD_ADDR((unsigned int)timerMap,0);
        //printk(KERN_INFO "pwrctrl_timer_addr_remap g_ulPWRCTRLTimerAddr %d :0x%x\n",i,g_ulPWRCTRLTimerAddr[i]);
        g_ulPWRCTRLTimerAddr[i+1] = SOC_TIMER_TIMERN_LOAD_ADDR((unsigned int)timerMap,1);
                                           ...
                                           u32_t *pTimerID, u32_t *pNextScheduleTime)
{
    ...
    for (i = 0; i < PWRCTRL_ACPU_TIMER_TOTAL_NUM; i++)
    {
        ...
        /*对于非always timer 在系统从睡眠中醒来后，可以继续执行计数，当前计数值会保存到内存中*/
        /*if (((0 == ulAllTimer) && (0 == ((1 << i) & PWRCTRL_ALWAYSON_TIMER)))
            || (0 == ulState)||(TIMER_ACPU_IDLE_ID == i))*//* not always on timer */
        if ((PWRCTRL_FALSE == ulState)||(TIMER_ACPU_IDLE_ID == i))
        {
