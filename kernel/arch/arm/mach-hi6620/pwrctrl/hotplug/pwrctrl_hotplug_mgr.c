...
extern void secondary_jump(void);
...
local_t int pwrctrl_cpu_up_verify (void)
{
    ...
    if(cpu_online_num<pwrctrl_hotplug_cpuqos.cpu_number_max)
    {
        PRINT_PWC_DBG(PWC_SWITCH_AUTOHOTPLUG,"pwrctrl_cpu_up_verify ok:  cpu_number_max:%d online_num:%d\n",
                pwrctrl_hotplug_cpuqos.cpu_number_max,cpu_online_num);
        return RET_OK;
    }
    else
    {
    ...
    if((RET_OK == pwrctrl_is_func_on(PWC_SWITCH_AUTOHOTPLUG))&&(PWRCTRL_INIT_FLAG == g_pwc_init_flag))
    {
		...
		if(ret == 0)
		{
	  	    update_idletime(cpu_online_num);		 
                ...
                if(ret)
                {
                    printk(KERN_ERR "CPU:%d up failed ret=%d,online_num=%d\n",cpu_online_num, ret,num_online_cpus());
                }
            }
            else
            {
        ...
        else if((cnt_thread_sub > pwrctrl_hotplug_para.hotplug_down_level[cpu_online_num-1].cnt_task)&&(cnt_load_sub > pwrctrl_hotplug_para.hotplug_down_level[cpu_online_num-1].cnt_load))
        {
            if(RET_OK == pwrctrl_cpu_down_verify())
            {
                ...
                ret = cpu_down(cpu_online_num-1);
                if(ret)
                {
                    printk(KERN_ERR "CPU:%d down failed ret=%d,online_num=%d\n",cpu_online_num-1, ret,num_online_cpus());
                }
            }
            else
            {
 ...
 s32_t pwrctrl_hotplug_timer_init (void)
{
	...
	INIT_DELAYED_WORK_DEFERRABLE(&pwrctrl_hotplug_wk, (work_func_t)pwrctrl_hotplug_timer_workqueue);
...
{
    return cancel_delayed_work_sync(&pwrctrl_hotplug_wk);
...
{
    u32_t *ppara=(u32*)&pwrctrl_hotplug_para;
 ...
 int __init pwrctrl_hotplug_init(void)
{
    ...
    secondary_phy_addr = (volatile unsigned int *)MEMORY_AXI_SEC_CORE_BOOT_ADDR;
    *secondary_phy_addr = (volatile unsigned int)secondary_jump;
    /* *secondary_phy_addr = virt_to_phys(k3v2_secondary_startup);*/

    PWC_TRACE(PWC_LOG_ERROR, "MEMORY_AXI_SEC_CORE_BOOT_ADDR=0x%x\n",(unsigned int)secondary_phy_addr);
...
void pwrctrl_hotplug_mntn (void)
{
    ...
    {
        PWC_TRACE(PWC_LOG_INFO, "index: %d cnt_load:%d, cpu_load: %d, avr_task: %d\n",i,photplug_level[i].cnt_load,photplug_level[i].cpu_load,photplug_level[i].avr_task);
    }
    ...
    for(i=0;i<NR_CPUS_NUMBER;i++)
    {
        ...
        PWC_TRACE(PWC_LOG_INFO, " cpu index:%d, down_count: %d,down_time:%d,down_power:%d,note_mcu:%d\n",
            ...
            precord[i].down_power_time,precord[i].down_notemcu_time);
