...
#define RMNET_IOC_ENABLE_MODEM_STREAMING	        (N_RMNET<<2  | 1)
#define RMNET_IOC_DISABLE_MODEM_STREAMING	        (N_RMNET<<2  | 2)
...
static int flg_modem_streaming = 0;
/************************************************************/
...
static int rmnet_ioctl(struct tty_struct *tty, struct file *file,
		   unsigned int cmd, unsigned long arg)
{
	pr_info("%s++[%d]\n", __FUNCTION__,cmd);

	switch (cmd) {
	case RMNET_IOC_ENABLE_MODEM_STREAMING:
		flg_modem_streaming = 1;
		break;

	case RMNET_IOC_DISABLE_MODEM_STREAMING:
		flg_modem_streaming = 0;
		break;
...
static inline int init_input_ip_pkt(struct rmnet_private *p, int ver, const unsigned char* hdr)
{
	...
	if (p->pkt_len + RMNET_ETH_HDR_SIZE > RMNET_MTU_SIZE) {
