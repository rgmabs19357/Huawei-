...
#endif
#else
	arm_add_memory(PLAT_PHYS_OFFSET, HISI_BASE_MEMORY_SIZE);
#endif
	return;
}
...
void __init hisik3_init_cache(void)
{
	...
	l2x0_init(p, 0x3c470000, 0xc2000fff);
...
static struct hisik3_mmc_platform_data sdio_plat_data = {
	.ocr_mask = MMC_VDD_165_195	| MMC_VDD_27_28 | MMC_VDD_28_29
		...
		| MMC_VDD_35_36,
	.caps = MMC_CAP_4_BIT_DATA | MMC_CAP_SDIO_IRQ | MMC_CAP_SD_HIGHSPEED | MMC_CAP_NONREMOVABLE | MMC_CAP_CLOCK_GATING,
	.quirks = MSHCI_QUIRK_ALWAYS_WRITABLE |
...
static struct xmm_power_plat_data  xmm_plat_data = {
	...
	.post_suspend = &hisik3_usbh_data.post_suspend,
	.pre_resume = &hisik3_usbh_data.pre_resume,
};
...
static struct balong_power_plat_data  balong_plat_data = {
	.flashless = false,
	.gpios = {
		[BALONG_RESET] 	   = {95, GPIOF_IN,  "balong_rst"},
		[BALONG_POWER_ON] 	   = {96, GPIOF_OUT_INIT_LOW, "balong_pow"},
		[BALONG_PMU_RESET]    = {97, GPIOF_IN, "balong_pmu_rst"},
		[BALONG_HOST_ACTIVE]  = {115, GPIOF_OUT_INIT_LOW, "host_act"},
		[BALONG_HOST_WAKEUP]  = {113, GPIOF_IN, "host_wake"},
		[BALONG_SLAVE_WAKEUP] = {114, GPIOF_OUT_INIT_LOW, "slave_wake"},
		[BALONG_POWER_IND]    = {94, GPIOF_IN, "power_ind"},
		[BALONG_RESET_IND]    = {98, GPIOF_IN, "reset_ind"},
	},
	.block_name = "block_moden_ap_ctrl",
	.echi_device = &hisik3_usb_ehci,
	.post_suspend = &hisik3_usbh_data.post_suspend,
	.pre_resume = &hisik3_usbh_data.pre_resume,
};
...
#if 0
const char * get_modem_type(void)
{
        pr_info("get_modem_type = [%s]\n",modem_name);
        return modem_name;
}
EXPORT_SYMBOL(get_modem_type);
#endif
void set_modem_type(const char* name)
{
        pr_info("set_modem_type = [%s]\n",name);
        modem_name = name;
}
EXPORT_SYMBOL(set_modem_type);
