			     ...
			     unsigned migratetype, bool skip_hwpoisoned_pages)
{
	...
	for (pfn = start_pfn;
		...
		if (page &&
			...
			undo_pfn = pfn;
			cma_log(CMA_LOG_EMERG, "set_migratetype_isolate_cma fail! %s\n", __FUNCTION__);
			goto undo;
		}
	}
	return 0;
				  ...
				  bool skip_hwpoisoned_pages)
{
	...
	while (pfn < end_pfn) {
		...
		if (PageBuddy(page)) {
			...
			if (get_freepage_migratetype(page) != MIGRATE_ISOLATE) {
				...
				cma_log(CMA_LOG_DEBUG, "%s1: page:0x%08lx, end_page:0x%08lx\n", __FUNCTION__, page_to_pfn(page),  page_to_pfn(end_page));
		...
		else
		{
			 ...
			 * system, and page_count() is not 0.
			 */
				cma_log(CMA_LOG_EMERG, "%s: PageHWPoison page:0x%08lx\n", __FUNCTION__, page_to_pfn(page));
			}
			else{
				cma_log(CMA_LOG_EMERG, "%s: page:0x%08lx private:0x%08lx\n", __FUNCTION__, page_to_pfn(page), page_private(page));
				dump_page(page);
			}
	...
	if (pfn < end_pfn)
	{
		cma_log(CMA_LOG_EMERG, "%s: pfn:0x%08lx, end_pfn:0x%08lx end_pfn-pfn:0x%08lx\n", __FUNCTION__, pfn,  end_pfn, end_pfn - pfn);
		return 0;
	}
	return 1;
}
			...
			bool skip_hwpoisoned_pages)
{
	...
	if ((pfn < end_pfn) || !page)
	{
		cma_log(CMA_LOG_EMERG, "%s: pfn:0x%lx end_pfn:0x%lx page:%p\n", __FUNCTION__, pfn, end_pfn, page);
		return -EBUSY;
	}
	/* Check all pages are free or Marked as ISOLATED */
	zone = page_zone(page);
