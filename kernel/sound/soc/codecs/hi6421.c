...
#define HI6421_PLL_RESET_TIMES    ( 10 )
...
enum headset_voltage { /* mV */
    ...
    HI6421_PB_VOLTAGE_MAX    = 1055,/*900*/
};
...
{
    unsigned int i = 0;
    pr_info("%#04x : %#04x", HI6421_REG_AUDID_LDO/*0x36*/, hi6421_reg_read(g_codec, HI6421_REG_AUDID_LDO/*0x36*/));
...
/* _reg_set REG VAL */
unsigned int _reg_set(unsigned int reg, unsigned int value)
{
    hi6421_reg_write(g_codec, reg, value);
    return hi6421_reg_read(g_codec, reg);
}

unsigned int _reg_get(unsigned int reg)
{
    unsigned int val = 0;
    val = hi6421_reg_read(g_codec, reg);
    pr_info("_reg_get: %#04x = %#04x\r\n", reg, val);
...
static void hi6421_pll_work_func(struct work_struct *work)
{
    ...
    while(1) {
        ...
        if (loop++ > HI6421_PLL_RESET_TIMES) {
            break;
        }
...
void hi6421_btn_down_work_func(struct work_struct *work)
{
    ...
    if (HI6421_JACK_BIT_HEADSET ==  hs_status) {
        ret = hisi_adc_get_value(ADC_PB_DETECT, NULL);
...
void hi6421_audio_plug_in_work_func(struct work_struct *work)
{
    ...
    codec = priv->codec;
    wake_lock_timeout(&priv->wake_lock,
            HI6421_HKADC_SLEEP_TIME +  HI6421_HS_WAKE_LOCK_TIMEOUT);

    if (0 != priv->counter--) {
        pr_info("%s(%u): There is still delay work in workqueue", __FUNCTION__, __LINE__);
        return;
    }
    msleep(HI6421_HS_PLUG_DETECT_TIME);
#ifdef CONFIG_K3_ADC
    ...
    if (HI6421_JACK_BIT_HEADSET != hs_status) {
        /* power off micbias if headset pluged out or headphone pluged in */
        printk(" %s(%u) hkadc off\n", __FUNCTION__, __LINE__);
...
static int hi6421_codec_probe(struct snd_soc_codec *codec)
{
    ...
    priv->check_voltage = hisi_adc_get_value(ADC_500K, NULL);
...
#ifdef CONFIG_MFD_HI6421_IRQ
#ifdef CONFIG_K3_ADC
    free_irq(pdata->irq[3],codec);  /*IRQ_BSD_BTN_PRESS_DOWN*/
    if(priv->headset_plug_in_delay_wq) {
...
/* de-reset */
static void hi6421_dereset(struct platform_device *pdev)
{
    struct hi6421_codec_platform_data *pdata = dev_get_platdata(&pdev->dev);

    pr_info("%s Begin", __FUNCTION__);

    if (0 != pdata->gpio_reset) {
        gpio_set_value(pdata->gpio_reset, 1);
    }

    /* LDOAUDIO */
    hi6421_reg_write(0, HI6421_REG_AUDID_LDO/*0x36*/, 0x71/*AUDIO_NORMAL_MODE, EN*/);
...
static int __devinit hi6421_probe(struct platform_device *pdev)
{
    ...
    /* get pmuspi clock */
    g_clk = clk_get(NULL, pdata->clk_name);
    if (IS_ERR(g_clk)) {
        pr_err("%s : Could not get clk_name\n", __FUNCTION__);
        return -ENODEV;
    }

    /* get audio_ldo */
    g_regu = regulator_get(&pdev->dev, pdata->regulator_name);
    if (IS_ERR(g_regu)) {
        pr_err("%s: Could not get regulator name\n", __FUNCTION__);
        goto probe_err;
    }
    ...
    if (0/*NULL*/ != pdata->gpio_reset) {
        ret = gpio_request(pdata->gpio_reset, pdata->gpio_label);
        if (0 > ret) {
            pr_err("%s: gpio request reset pin failed", __FUNCTION__);
            goto gpio_err;
        }
        ret = gpio_direction_output(pdata->gpio_reset, 0);
        if (0 > ret) {
            pr_err("%s: request reset gpio direction failed", __FUNCTION__);
            goto gpio_err;
        }
        hi6421_dereset(pdev);
    }
