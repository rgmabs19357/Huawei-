...
void move_srecorder_log(void)
{
    ...
    {
        /* invalid log, return */
        printk("~_~_~_~ [SRecorder]: Magic number=%08lx reset flag=%lx data length=%lx "
    ...
    printk("^_^_^_^ [SRecorder]: Magic number=%08lx reset flag=%lx data length=%lx\n",
        pheader->magic_num, pheader->reset_flag, pheader->data_length);
 ...
 */
    ...
    do
    {
        char *recovery = strstr(command_line, "enter_recovery");
        
      if ((NULL != recovery)
          && (0 != simple_strtol(recovery + strlen("enter_recovery="), NULL, 10)))
        {
...
static noinline int init_post(void)
{
	...
	printk(KERN_INFO "Time tracking: run init_process .\n");
