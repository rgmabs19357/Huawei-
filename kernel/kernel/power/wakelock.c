...
#ifdef CONFIG_K3_LOG
extern struct semaphore k3log_sys_sema;
#endif
...
struct pm_qos_request_list g_specialpolicy;
...
static int print_lock_stat(struct seq_file *m, struct wake_lock *lock)
{
	...
	ktime_t screen_off_time = lock->stat.screen_off_time;
	char *lock_type = NULL;
	...
	return seq_printf(m,
		     "\"%s%s\"\t%d\t%d\t%d\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t\"%s\"\t%d\t%d\n",
		     lock->name, lock_type, lock_count, expire_count,
		     lock->stat.wakeup_count, ktime_to_ns(active_time),
		     ktime_to_ns(total_time),
		     ktime_to_ns(prevent_suspend_time), ktime_to_ns(max_time),
		     ktime_to_ns(lock->stat.last_time),
		     ktime_to_ns(screen_off_time),
		     lock->comm, lock->pid, lock->tgid); /* Current command name, process ID and thread group ID */
}
...
static int wakelock_stats_show(struct seq_file *m, void *unused)
{
	...
	ret = seq_puts(m, "name\tcount\texpire_count\twake_count\tactive_since"
			"\ttotal_time\tsleep_time\tmax_time\tlast_change\tsleep_since\tcommand_name\tpid\ttgid\n");
...
static void wake_unlock_stat_locked(struct wake_lock *lock, int expired)
{
	...
	if (lock->flags & WAKE_LOCK_PREVENTING_SUSPEND) {
		...
		lock->stat.screen_off_time = ktime_add(
			lock->stat.screen_off_time, duration);
...
static void update_sleep_wait_stats_locked(int done)
{
	...
	if (done) {
		list_for_each_entry(lock, &inactive_locks, link) {
			lock->stat.screen_off_time = ktime_set(0, 0);
		}
	}
	list_for_each_entry(lock, &active_wake_locks[WAKE_LOCK_SUSPEND], link) {
...
void wake_lock_init(struct wake_lock *lock, int type, const char *name)
{
	...
	//initialize command name, pid and tgid of the current wakelock
	lock->comm = current->comm;
	...
	struct wake_lock *lock, long timeout, int has_timeout)
{
	...
	if(!strcmp("rpc-interface",lock->name))
	{
