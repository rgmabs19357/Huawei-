...
/*max bytes print to console one time,almost cost 1.2s when 115200 bps*/
#define MAX_CONSOLE_PRINT_BYTES  ((115200/10)*12/10)
static unsigned int g_statistic_print_bytes = 0;
static unsigned int g_over_print_flag = 0;
/*
...
#ifdef CONFIG_SRECORDER
static char srecorder_log_buf[CONFIG_SRECORDER_LOG_BUF_LEN + __LOG_BUF_LEN] __attribute__((__section__(".data")));
static int srecorder_log_buf_len __attribute__((__section__(".data"))) = CONFIG_SRECORDER_LOG_BUF_LEN + __LOG_BUF_LEN;
...
static void __call_console_drivers(unsigned start, unsigned end)
{
	...
	for_each_console(con) {
		...
		if ((con->flags & CON_ENABLED) && con->write &&
			    ...
			    g_statistic_print_bytes +=(unsigned int)(end - start);
		    }
	}
}
...
#ifdef CONFIG_K3_LOG
static void emit_log_char_to_rbuf(char c)
{
	int i;
	int need_init = 0;
	int start = 0;
	...
	if (!phyaddr_mapped) {
		if (!log_buf_info)
			log_buf_info = ioremap(K3_LOG_INFO_BASE, LOG_INFO_BUF_LEN);

		if (!res_log_buf)
			res_log_buf = ioremap(K3_KERNEL_LOG_BASE, KERNEL_LOG_BUF_LEN);

		if (!log_buf_info || !res_log_buf)
			return;  /* map failed */

		if (strncmp("kdumplog", (const char *)(log_buf_info+6), 8) == 0) {
			if (((log_buf_info+0)->waddr >= KERNEL_LOG_BUF_LEN) || ((log_buf_info+0)->raddr >= KERNEL_LOG_BUF_LEN)
				...
				|| ((log_buf_info+5)->waddr >= POWER_LOG_BUF_LEN) || ((log_buf_info+5)->raddr >= POWER_LOG_BUF_LEN)) {
		...
		if (need_init) {
			...
			strncpy((char *)(log_buf_info+6), kdumplog, sizeof(kdumplog));
		}
	...
	if (k3_early_log) {
		while (start != log_end) {
			...
			if (__LOG_BUF_LEN == log_buf_info->waddr)
				log_buf_info->waddr = 0;

			res_log_buf[log_buf_info->waddr] = log_buf[log_idx_mask];
			log_buf_info->waddr++;
	...
	if (log_buf_info->waddr < log_buf_info->raddr) {
		if ((log_buf_info->waddr + KERNEL_LOG_BUF_LEN - log_buf_info->raddr)
			>= (KERNEL_LOG_BUF_LEN >> 1)) {
			up(&k3log_sema);
		}
	} else {
		if ((log_buf_info->waddr - log_buf_info->raddr) >= (KERNEL_LOG_BUF_LEN >> 1)) {
			up(&k3log_sema);
		}
	}
...
asmlinkage int printk(const char *fmt, ...)
{
	...
	if((g_statistic_print_bytes > MAX_CONSOLE_PRINT_BYTES)&&(g_over_print_flag == 0)){
		/*bug_on only one time*/
		g_over_print_flag = 1;
		//BUG_ON(1); remark it for uart debug
	}
	...
	for (; *p; p++) {
		if (new_text_line) {
			...
			if (printk_time) {
                ...
                tlen += sprintf(tbuf + tlen, "[%lu:%.2d:%.2d %.2d:%.2d:%.2d] ",
					1900 + tm_rtc.tm_year, tm_rtc.tm_mon + 1, tm_rtc.tm_mday,
					tm_rtc.tm_hour, tm_rtc.tm_min, tm_rtc.tm_sec);

				tlen += sprintf(tbuf + tlen, "[%5lu.%06lu] ",
						(unsigned long) t,
						nanosec_rem / 1000);
				if(unlikely(in_irq())){
					tlen += sprintf(tbuf + tlen, "[%d.%d in_irq %s]", current->pid, smp_processor_id(), current->comm);
				}else{
					tlen += sprintf(tbuf + tlen, "[%d.%d %s]", current->pid, smp_processor_id(), current->comm);
				}
