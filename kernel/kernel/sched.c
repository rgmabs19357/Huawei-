...
typedef void (*exchTaskSwitchHookCB)(void *, void *);
exchTaskSwitchHookCB g_pTaskSwitchTable [LINUX_MAX_TASK_SWITCH_RTNS + 1]={NULL};
	       ...
	       struct task_struct *next)
{
    ...
    /* call switch hooks */
    for (idx = 0; (idx < LINUX_MAX_TASK_SWITCH_RTNS) && (g_pTaskSwitchTable[idx] != NULL); ++idx)
    {
        (* g_pTaskSwitchTable[idx]) ((void *)prev, (void *)next);
    }
...
#ifdef CONFIG_MIGRATION_RT_WALKAROUND
	unthrottle_task(rq);
#endif
	for ( ; ; ) {
		/*
...
(
    ...
    exchTaskSwitchHookCB table[],    /* table to which to add */
...
int task_switch_hook_add (exchTaskSwitchHookCB switchHook)
{
    int status;
...
void task_switch_hook_delete (void)
{
    int ix;

    preempt_disable();              /* disable task switching */

    for (ix = 0; ix < LINUX_MAX_TASK_SWITCH_RTNS; ++ix)
    {
        if (g_pTaskSwitchTable[ix] != NULL)
        {
            g_pTaskSwitchTable[ix] = NULL;
        }
    }
...
extern void print_tasks_in_task_group(void * arg);
int __print_tasks_in_task_group(struct task_struct *p, void * arg)
{
    struct task_group *tg;
    ...
    tg = task_group(p);
    if(tg == (struct task_group *)arg)
    {
        debug_print("task %s prio:%d pid:%d\n", p->comm, p->prio, task_pid_vnr(p));
        return 1;
    }
    return 0;
}

void print_task_group(struct task_group *tg)
{
    print_tasks_in_task_group((void *)tg);
}
#endif
