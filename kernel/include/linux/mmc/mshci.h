...
#define MSHCI_RST	0x78	/* RST setting */
...
#define MSHCI_UHSREGEXT   0x108  /* phase shift set register */
#define MSHCI_DDRREG	0x10C	/* Card Threshold Control Register */
#define MSHCI_ENABLESHIFT 0x110  /* phase shift enable register */
...
/* phase shift enable register MSHCI_ENABLESHIFT(offset 0x110) */
#define ENABLESHIFT_360  0x2
#define ENABLESHIFT_0      0x0 
...
#define MSHCI_QUIRK_EMMC_INIT_FINISH		    (1 << 6)
#define MSHCI_QUIRK_TUNING_ENABLE	        	(1 << 7)
#define MSHCI_QUIRK_HI6620_TUNING_FUNC      	(1 << 8)
#define MSHCI_QUIRK_CBP_DETECTION      	(1 << 9)
...
#define MSHCI_CONTROLLER_NEED_RESET	(1 << 10)	/* MSHCI controller need reset */
...
#define CLK_DISABLED            (0<<0)
	...
	unsigned int        ios_timing;   /* old timing  */
	unsigned int		clock_to_restore; /* Saved clock for dynamic clock gating (MHz) */
	unsigned int        clock_to_set;  /* Saved to set clock for host */
...
struct mshci_ops {
	unsigned int	(*set_clock)(struct mshci_host *host, unsigned int clock);
	...
	int    (*set_timing_state)(struct mshci_host *host, int timing);
	int		(*tuning_state_enable)(struct mshci_host *host, int enable);
};
...
extern volatile int cbp_clock_enabled;
extern volatile int cbp_host_waked;
extern volatile int cbp_in_sdio_tras;
