 ...
 */
 ...
 #ifndef SII_MHL_TX_BASE_DRV_API
 #define SII_MHL_TX_BASE_DRV_API
...
typedef struct {
	/* CBUS_IDLE, CBUS_PENDING*/
	uint8_t reqStatus;
	uint8_t retryCount;
	/* VS_CMD or RCP opcode*/
	uint8_t command;
	/* Offset of register on CBUS or RCP data*/
	uint8_t offsetData;
	/* Only applicable to write burst. ignored otherwise.*/
	uint8_t length;
	union {
		/* Pointer to message data area.*/
		uint8_t msgData[16];
		/* pointer for write burst or read many bytes*/
...
/*void    SiiMhlTxDrvPowBitChange( bool_tt powerOn );*/
...
***********************************************************************************/
extern void SiiMhlTxNotifyConnection(bool_tt mhlConnected);
